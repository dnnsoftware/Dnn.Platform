...
internal class SearchControllerImpl : ISearchController
{
    ...
    private Tuple<int, IList<SearchResult>> GetResults(SearchQuery searchQuery)
    {
        ...
        {
            var strippedKeyword = SearchHelper.Instance.StripTagsNoAttributes(Constants.KeywordsPrefixTag + kvp.Key, true);
            query.Add(new TermQuery(new Term(strippedKeyword, kvp.Value)), Occur.MUST);
        }
