...
function attachPhoto(fileId, path, isImage) {
  ...
  $(".filePreviewArea").append(
    "<div id='attachClose' class='miniclose'></div>",
  );
  $("#attachClose")
    .show()
    .click(function () {
...
function bindConfirm() {
  ...
  $(".journalrow .minidel, .journalrow .miniclose").each(function () {
    ...
    if (
      ...
      $this.attr("onclick").length > 0
    ) {
      var clickFunc = $this
        .attr("onclick")
        .substr(0, $this.attr("onclick").indexOf("("));
      $this.attr("onclick", "");
      ...
      if (typeof $._data($this[0], "events") != "undefined") {
        for (var i = 0; i < $._data($this[0], "events").click.length; i++) {
          ...
          var functionName =
            handler.name ||
            handler.toString().match(/^function\s*([^\s(]+)/)[1];
...
(function ($, window) {
  ...
  $.fn.journalTools = function (options) {
    ...
    $shareButton.click(function (e) {
      ...
      $.ajax({
        type: "POST",
        url:
          opts.servicesFramework.getServiceRoot("Journal") + "Services/Create",
        data: data,
    ...
    function getNewItem(data, callback) {
      $.ajax({
        type: "POST",
        url:
          opts.servicesFramework.getServiceRoot("Journal") +
          "Services/GetListForProfile",
        beforeSend: opts.servicesFramework.setModuleHeaders,
        data: data,
        success: function (data) {
          if (typeof callback != "undefined") {
  ...
  $.fn.previewify = function (options) {
    ...
    function prevImg() {
      ...
      $(opts.previewSelector + " #imgCount").text(
        currImage + 1 + " of " + images.length,
      );
      $previewArea.find("#image img").fadeOut(function () {
        $(this)
          .hide()
          .attr("src", "")
          .attr("src", images[currImage].URL)
          .fadeIn();
      });
