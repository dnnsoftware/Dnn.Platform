function MemberDirectory($, ko, settings, composeMessageSettings) {
  ...
  var baseServicepath =
    serviceFramework.getServiceRoot("MemberDirectory") + "MemberDirectory/";
  var pageIndex = 0;
  ...
  function Member(item) {
    ...
    self.getProfilePicture = function (w, h) {
      return profilePicHandler
        .replace("{0}", self.UserId())
        .replace("{1}", h)
        .replace("{2}", w);
    };
  ...
  function MemberDirectoryViewModel(initialData) {
    ...
    var initialMembers = $.map(initialData, function (item) {
      ...
      $.ajax({
        ...
        .done(function (members) {
          if (typeof members !== "undefined" && members != null) {
            var mappedMembers = $.map(members, function (member) {
    ...
    self.xhrAdvancedSearch = function () {
      $.ajax({
        ...
        .done(function (members) {
          if (typeof members !== "undefined" && members != null) {
            var mappedMembers = $.map(members, function (item) {
              return new Member(item);
            });
            if (pageIndex === 0) {
              self.Members(mappedMembers);
    ...
    $.ajax({
      ...
      .done(function (members) {
        if (typeof members !== "undefined" && members != null) {
          ...
          ko.applyBindings(
            viewModel,
            document.getElementById($(element).attr("id")),
          );
          ...
          $("a#mdAdvancedSearch").mouseleave(function () {
            timer = setTimeout(function () {
              if (
                $("div#mdAdvancedSearchForm").is(":visible") &&
                !cursorIsOnAdvancedSearchForm
              ) {
