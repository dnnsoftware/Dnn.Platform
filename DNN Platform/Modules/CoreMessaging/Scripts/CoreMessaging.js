function CoreMessaging($, ko, settings, composeMessageOptions) {
  ...
  var baseServicepath =
    serviceFramework.getServiceRoot("CoreMessaging") + "MessagingService/";
  ...
  function setDocumentTitle(title) {
    document.title =
      title + " - " + settings.currentUserText + " - " + settings.portalText;
  }
    ...
    History.Adapter.bind(window, "statechange", function () {
      ...
      if (state.data != null) {
        ...
        var context = ko.contextFor(
          document.getElementById($(containerElement).attr("id")),
        );
  ...
  function messageConversationView(data) {
    ...
    self.SenderAvatar = profilePicHandler
      .replace("{0}", self.SenderUserID)
      .replace("{1}", 64)
      .replace("{2}", 64);
    self.SenderProfileUrl = data.SenderProfileUrl;
    self.MessageAbstract =
      self.Body.length < 50 ? self.Body : self.Body.substr(0, 50) + "...";
  ...
  function messageThreadView(data) {
    ...
    self.SenderDisplayName =
      data.Conversation.SenderDisplayName || data.Conversation.From;
  ...
  function notificationViewModel(data) {
    ...
    self.Actions = $.map(data.Actions, function (action) {
      return new notificationActionViewModel(action, data.NotificationId);
    });
  }
  ...
  function coreMessagingViewModel() {
    ...
    self.hasElementsSelected = ko.computed(function () {
      var selectedElement = ko.utils.arrayFirst(
        self.messages(),
        function (message) {
    ...
    self.fetch_unix_timestamp = function () {
    ...
    self.sendThreadRequest = function (message) {
      History.pushState(
        ...
        "",
        "?view=messages&action=thread&t=" + self.fetch_unix_timestamp(),
      );
    ...
    self.checkReplyHasRecipients = function (conversationId) {
      ...
      if (returnValue == 0) {
        ...
        displayMessage(
          "#dnnCoreMessaging",
          settings.replyHasNoRecipientsText,
          "dnnFormWarning",
        );
      } else {
    ...
    self.sendThreadRequestHandler = function (conversationId, afterMessageId) {
      ...
      $.ajax({
        ...
        data: {
          ...
          numberOfRecords: repliesPageSize,
        },
        cache: false,
      })
        .done(function (messagethreadsView) {
          if (
            typeof messagethreadsView !== "undefined" &&
            messagethreadsView != null &&
            typeof messagethreadsView.Conversations !== "undefined"
          ) {
            var mappedMessages = $.map(
              messagethreadsView.Conversations,
              function (item) {
            ...
            //update totals
            if (
              typeof messagethreadsView.TotalThreads !== "undefined" &&
              $.type(messagethreadsView.TotalThreads) === "number"
            ) {
              self.TotalThreads(messagethreadsView.TotalThreads);
            }

            if (
              typeof messagethreadsView.TotalNewThreads !== "undefined" &&
              $.type(messagethreadsView.TotalNewThreads) === "number"
            ) {
              self.TotalNewThreads(messagethreadsView.TotalNewThreads);
            }

            if (
              typeof messagethreadsView.TotalArchivedThreads !== "undefined" &&
              $.type(messagethreadsView.TotalArchivedThreads) === "number"
            ) {
              self.TotalArchivedThreads(
                messagethreadsView.TotalArchivedThreads,
              );
            }
          } else {
            displayMessage(
              "#dnnCoreMessaging",
              settings.serverErrorText,
              "dnnFormWarning",
            );
          }
        })
        .fail(function (xhr, status) {
          displayMessage(
            "#dnnCoreMessaging",
            settings.serverErrorWithDescriptionText + status,
            "dnnFormWarning",
          );
        });
    };
    ...
    self.showPreviousReplies = function () {
      if (self.TotalThreads() > self.messagethreads().length) {
        self.sendThreadRequestHandler(
          self.messagethreads()[0].ConversationId,
          self.messagethreads()[0].MessageID,
        );
      }
    };
    ...
    self.updateThreadCount = function (messageconversationView) {
      ...
      if (messageconversationView.Read() === true) {
        ...
        self.TotalNewThreads(
          previousNewThreads + messageconversationView.NewThreadCount(),
        );
      } else {
        self.TotalNewThreads(
          previousNewThreads - messageconversationView.NewThreadCount(),
    ...
    self.loadMore = function (data, event) {
      $(event.target).html(
        '<img src="images/dnnanim.gif" style="vertical-align:middle" /> ' +
      ...
      var afterMessageId =
        self.messages().length == 0
          ? -1
          : self.messages()[self.messages().length - 1].MessageID;
      var url = inboxpath;
      ...
      $.ajax({
        ...
        .done(function (messageboxView) {
          if (
            typeof messageboxView !== "undefined" &&
            messageboxView != null &&
            typeof messageboxView.Conversations !== "undefined"
          ) {
            var mappedMessages = $.map(
              messageboxView.Conversations,
              function (item) {
                return new messageConversationView(item);
              },
            );
            for (var i = 0; i < mappedMessages.length; i++) {
            ...
            if (
              typeof messageboxView.TotalConversations !== "undefined" &&
              $.type(messageboxView.TotalConversations) === "number"
            ) {
              self.TotalConversations(messageboxView.TotalConversations);
            }
            if (
              typeof messageboxView.TotalNewThreads !== "undefined" &&
              $.type(messageboxView.TotalNewThreads) === "number"
            ) {
        ...
        .done(function (data) {
          if (data.Result === "success") {
            if (message.ThreadCount != undefined) {
              self.TotalNewThreads(
                self.TotalNewThreads() - message.NewThreadCount(),
              );
            }
    ...
    self.loadNotificationsTab = function () {
      ...
      History.pushState(
        { view: "notifications", action: "notifications" },
        "",
        "?view=notifications&action=notifications&t=" +
          self.fetch_unix_timestamp(),
      );
      ...
      $.ajax({
        ...
        .done(function (notificationsViewModel) {
          if (
            ...
            if (
              typeof notificationsViewModel.TotalNotifications !==
                "undefined" &&
              $.type(notificationsViewModel.TotalNotifications) === "number"
            ) {
              self.TotalNotifications(
                notificationsViewModel.TotalNotifications,
              );
            }
          } else {
            displayMessage(
    ...
    self.loadMoreNotifications = function () {
      var afterNotificationId =
        self.notifications().length == 0
          ? -1
          : self.notifications()[self.notifications().length - 1]
              .NotificationId;
      ...
      $.ajax({
        ...
        .fail(function () {
          displayMessage(
            "#dnnCoreNotification",
            settings.serverErrorWithDescriptionText,
            "dnnFormWarning",
          );
        });
    };
    ...
    self.performNotificationAction = function (action) {
      if (action.Confirm.length > 0) {
        var confirmDialog = $("<div class='dnnDialog'></div>")
          .html(action.Confirm)
          .dialog({
            autoOpen: false,
            resizable: false,
            ...
            open: function () {
              $(".ui-dialog-buttonpane")
                .find(
                  'button:contains("' +
                    settings.notificationConfirmNoText +
                    '")',
                )
    ...
    self.dismissAllNotifications = function () {
      var opts = {
        ...
        text: String.format(
          settings.dismissAllConfirmText,
          self.TotalNotifications(),
        ),
        yesText: settings.yesText,
        noText: settings.noText,
    ...
    self.apiCallRequest = function (action) {
      $.ajax({
        ...
        .done(function (data) {
          if (data.Result === "success") {
            var notificationToRemove = ko.utils.arrayFirst(
              self.notifications(),
              function (notification) {
            ...
            var notifications = $("#dnn_dnnUser_notificationLink").children(
              "span",
            );
            ...
            if (
              ...
              displayMessage(
                "#dnnCoreNotification",
                settings.actionNotPerformedText,
                "dnnFormWarning",
              );
            }
          }
    ...
    self.hideNotification = function (elem) {
      if (elem.nodeType === 1) {
        $(elem).fadeOut(self.notificationFadeTime(), function () {
          $(elem).remove();
        });
    ...
    self.loadBox = function (boxPath) {
      ...
      $.ajax({
        ...
        .fail(function (xhr, status) {
          displayMessage(
            "#dnnCoreMessaging",
            settings.serverErrorWithDescription + status,
            "dnnFormWarning",
          );
        })
        .always(function () {
    ...
    self.myinbox = function () {
      History.pushState(
        { view: "messages", action: "inbox" },
        "",
        "?view=messages&action=inbox&t=" + self.fetch_unix_timestamp(),
      );
      setDocumentTitle(settings.messageTitleText);
    };
    ...
    self.mysentbox = function () {
      History.pushState(
        { view: "messages", action: "sentbox" },
        "",
        "?view=messages&action=sentbox&t=" + self.fetch_unix_timestamp(),
      );
    ...
    self.myarchivebox = function () {
      History.pushState(
        { view: "messages", action: "archivebox" },
        "",
        "?view=messages&action=archivebox&t=" + self.fetch_unix_timestamp(),
      );
      ...
      $.ajax({
        ...
        .done(function (data) {
          if (
            ...
            if (
              typeof data.TotalThreads !== "undefined" &&
              $.type(data.TotalThreads) === "number"
            ) {
              self.TotalThreads(data.TotalThreads);
            }
            if (
              typeof data.TotalNewThreads !== "undefined" &&
              $.type(data.TotalNewThreads) === "number"
            ) {
              self.TotalNewThreads(data.TotalNewThreads);
            }
            if (
              typeof data.TotalArchivedThreads !== "undefined" &&
              $.type(data.TotalArchivedThreads) === "number"
            ) {
        ...
        .fail(function (xhr, status) {
          displayMessage(
            "#dnnCoreMessaging",
            settings.serverErrorWithDescription +
    ...
    self.getTotals = function (refresh) {
      $.ajax({
        ...
        if (typeof totalsViewModel !== "undefined" && totalsViewModel != null) {
          ...
          if (
            typeof totalsViewModel.TotalUnreadMessages !== "undefined" &&
            $.type(totalsViewModel.TotalUnreadMessages) === "number"
          ) {
            var oldTotalNewThreads = self.TotalNewThreads();
            self.TotalNewThreads(totalsViewModel.TotalUnreadMessages);
            if (
              refresh &&
              oldTotalNewThreads !== totalsViewModel.TotalUnreadMessages
            ) {
              if (
                state.data == null ||
                !state.data.view ||
                (state.data.view == "messages" && state.data.action == "inbox")
              ) {
                self.loadBox(inboxpath);
              }
            }
          ...
          if (
            typeof totalsViewModel.TotalNotifications !== "undefined" &&
            $.type(totalsViewModel.TotalNotifications) === "number"
          ) {
            var oldNotifications = self.TotalNotifications();
            self.TotalNotifications(totalsViewModel.TotalNotifications);
            if (
              refresh &&
              oldNotifications !== totalsViewModel.TotalNotifications
            ) {
              if (
                state.data == null ||
                !state.data.view ||
                (state.data.view == "notifications" &&
