...
export class IpSafelistClient
{
  ...
  {
    const response = await fetch(
      `${this.requestUrl}Create?name=${encodeURIComponent(name)}&ip=${ipAddress}`,
      {
        method: 'POST',
        headers: this.sf.getModuleHeaders(),
      });
  ...
  {
    const response = await fetch(
      `${this.requestUrl}GetIpSafelistConfiguration`,
      {
        method: 'GET',
        headers: this.sf.getModuleHeaders(),
      });
  ...
  {
    await fetch(
      `${this.requestUrl}SaveIpSafelistConfiguration?enabled=${enabled}`,
      {
        method: 'POST',
        headers: this.sf.getModuleHeaders(),
      });
