...
export class DnnResourceManager {
  ...
  @Watch("foldersExpanded")
  foldersExpandedChanged(expanded: boolean){
    const lastWidth = parseFloat(localStorage.getItem(localStorageSplitWidthKey)) || 30;
    if (expanded){
      void this.splitView.setSplitWidthPercentage(lastWidth);
      return;
    }
    
    void this.splitView.setSplitWidthPercentage(0);
  }
  
  async componentWillLoad() {
    try {
      state.localization = await this.localizationClient.getResources();
      state.sortField = sortField.itemName;
      state.settings = await this.itemsClient.getSettings();
      state.canManageFolderTypes = await this.itemsClient.canManageFolderTypes();
    } catch (error) {
      alert(error);
    }
  }
      ...
      <Host>
        <div class="container">
          ...
          <dnn-vertical-splitview
            ...
            onWidthChanged={e => this.handleSplitWidthChanged(e)}
          >
            <div class="splitter">
              <button
                type="button"
                ...
                title={this.foldersExpanded ? state?.localization?.CollapseFolders : state?.localization?.ExpandFolders}
              >
            ...
            {state.canManageFolderTypes &&
              <button
                ...
                onClick={() => void this.folderMappingsModal.show()}>
