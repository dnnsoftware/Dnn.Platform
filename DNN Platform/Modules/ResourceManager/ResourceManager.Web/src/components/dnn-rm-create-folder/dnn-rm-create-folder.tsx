...
export class DnnRmCreateFolder {
  ...
  componentWillLoad() {
    this.itemsClient
      .getFolderMappings()
      .then((data) => {
        this.folderMappings = data.sort(
          (a, b) => a.FolderMappingID - b.FolderMappingID,
        );
  ...
  private handleSave(e: Event): void {
    e.preventDefault();
    this.itemsClient
      .createNewFolder(this.newFolderRequest)
      .then(() => {
  ...
  private canChooseFolderProvider() {
    return (
      this.folderMappings &&
      this.folderMappings.find(
        (f) => f.FolderMappingID == state.currentItems.folder.folderMappingId,
      ).IsDefault
    );
  }
        ...
        <form onSubmit={(e) => this.handleSave(e)}>
          <div class="form">
            ...
            {this.canChooseFolderProvider() &&
              this.folderMappings.find(
                (m) =>
                  m.FolderMappingID == this.newFolderRequest.FolderMappingId,
              ).IsDefault == false && [
                <dnn-input
                  type="text"
          ...
          <div class="controls">
            ...
            <dnn-button appearance="primary" formButtonType="submit">
