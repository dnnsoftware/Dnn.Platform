...
export class DnnRmFolderListItem {
  ...
  componentWillLoad() {
    this.itemsClient
      .getFolderIconUrl(Number.parseInt(this.folder.data.key))
  ...
  private handleUserExpanded() {
    const children = Array.from(
      this.el.shadowRoot.querySelectorAll("dnn-rm-folder-list-item"),
    );
    children.forEach((element) => {
      element.shadowRoot
        .querySelectorAll("dnn-treeview-item")
        .forEach((t) => t.removeAttribute("expanded"));
    });
    this.internalServicesClient
      .getFolderDescendants(this.folder.data.key)
      .then((data) => {
        this.folder = {
          ...this.folder,
  ...
  private handleContextMenu(e: MouseEvent): void {
    ...
    this.itemsClient
      .getFolderItem(Number.parseInt(this.folder.data.key))
      .then((item) => {
      ...
      <Host class={this.getItemClasses()}>
        ...
        >
          ...
          >
            ...
            <span class="item-name">{this.folder.data.value}</span>
          </button>
