...
dnn.xml.documentObject.prototype = {
  getXml: function () {
    ...
    else return this._doc.getXml(); //wish other browsers supported getters/setters
  },
...
dnn.xml.XmlNode.prototype = {
  ...
  appendXml: function (sXml) {
    ...
    for (
      var i = 0;
      i < oDoc.childNodes(0).childNodeCount();
      i++ //appending child actually removes node from document, so get references then do append
    )
      aNodes[aNodes.length] = oDoc.childNodes(0).childNodes(i).node; //use real underlying node object
    for (var i = 0; i < aNodes.length; i++) this.node.appendChild(aNodes[i]); //surprised I don't need importNode
