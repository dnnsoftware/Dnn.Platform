...
dnn.extend(dnn.util, {
  tableReorderMove: function (ctl, bUp, sKey) {
    ...
    if (oTR != null) {
      ...
      var iNextIdx = bUp
        ? this.tableReorderGetPrev(oCtr, iIdx - 1)
        : this.tableReorderGetNext(oCtr, iIdx + 1);
      if (iNextIdx > -1) {
        ...
        } else oCtr.appendChild(oTR);
        this.setInputValues(oTR, aryValues);
        if (oSwapNode) this.setInputValues(oSwapNode, aryValues2);
  ...
  getInputValues: function (oCtl) {
    ...
    for (var i = 0; i < aryInputs.length; i++) {
      if (aryInputs[i].type == "checkbox") aryValues[i] = aryInputs[i].checked;
    }
    return aryValues;
  },
  ...
  setInputValues: function (oCtl, aryValues) {
    var aryInputs = dnn.dom.getByTagName("input", oCtl);
    for (var i = 0; i < aryInputs.length; i++) {
      if (aryInputs[i].type == "checkbox") aryInputs[i].checked = aryValues[i];
    }
  },
  ...
  tableReorderGetNext: function (oParent, iStartIdx) {
    for (var i = iStartIdx; i < oParent.childNodes.length; i++) {
      ...
      if (dnn.dom.getAttr(oCtl, "origidx", "") != "") return i;
    }
    return -1;
  },
  ...
  tableReorderGetNewRowOrder: function (oParent) {
    ...
    for (var i = 0; i < oParent.childNodes.length; i++) {
      ...
      if (sIdx != "") sRet += (sRet.length > 0 ? "," : "") + sIdx;
    }
    return sRet;
  },
  ...
  checkallChecked: function (oCtl, iCellIndex) {
    ...
    for (
      var i = 0;
      i < iCellIndex;
      i++ //firefox has text nodes
    ) {
      if (oTR.childNodes[i].tagName == null) iOffset++;
    }
    ...
    for (var i = 0; i < oCtr.childNodes.length; i++) {
      ...
      if (oTD != null) {
        ...
        if (oChk.length > 0) oChk[0].checked = bChecked;
      }
    }
    //return true;
  },
