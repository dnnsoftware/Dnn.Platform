...
//------------------------------------------------------//
var m_oSolpartMenu;
if (m_oSolpartMenu == null) m_oSolpartMenu = new Array(); //stores all menu objects (SolpartMenu) in array
var m_spm_sBrowser;
var m_spm_sVersion;
...
function SolpartMenu(o) {
  ...
  this.menuTransitionLength = spm_getAttr(o, "MenuTransitionLength", 0.3);
  ...
  if (spm_browserType() == "safari")
    //safari has issues with mouseoutdelay...
    this.moutDelay = 5000;
  ...
  this.useIFrames =
    spm_getAttr(o, "useIFrames", "1") != "0" && spm_supportsIFrameTrick();

  this.delaySubmenuLoad =
    spm_getAttr(o, "delaySubmenuLoad", "0") != "0" && spm_needsSubMenuDelay();
...
SolpartMenu.prototype.GenerateMenuHTML = function (oXML) {
  ...
  if (oXML == null) {
    if (this._m_oDOM == null) {
      ...
      if (this.xml.length) oXML.loadXML(this.xml);
  ...
  } else this._m_oDOM = oXML;

  if (this.display == "vertical") {
    sHTML +=
      '<table ID="tbl' +
      this._m_sNSpace +
      "MenuBar\" CELLPADDING='0' CELLSPACING='0' BORDER=\"0\" CLASS=\"" +
      spm_fixCSSForMac(this.getIntCSSName("spmbctr") + this.cssMenuContainer) +
      '" HEIGHT="100%" STYLE="vertical-align: center;">\n'; //removed position: relative;  for IE and display: block; for Opera
    sHTML += MyIIf(
      this.MBLeftHTML.length,
      "<tr>\n       <td>" + this.MBLeftHTML + "</td>\n</tr>\n",
      "",
    );
    sHTML += MyIIf(
      Number(this.moveable),
      '<tr>\n       <td ID="td' +
        this._m_sNSpace +
        "MenuMove\" height='3px' style='cursor: move; " +
        spm_getMenuBorderStyle(this) +
        "'>" +
        spm_getSpacer(this) +
        "</td>\n</tr>\n",
      "",
    ...
    sHTML +=
      ...
      "MenuBar\" CELLPADDING='0' CELLSPACING='0' BORDER=\"0\" CLASS=\"" +
      spm_fixCSSForMac(this.getIntCSSName("spmbctr") + this.cssMenuContainer) +
      '" WIDTH="100%" STYLE="vertical-align: center; ">\n'; //removed position: relative;  for IE and display: block; for Opera
    sHTML += "	<tr>\n";
    sHTML += MyIIf(
      this.MBLeftHTML.length,
      "<td>" + this.MBLeftHTML + "</td>\n",
    ...
    sHTML += MyIIf(
      ...
      '       <td ID="td' +
        this._m_sNSpace +
        "MenuMove\" width='3px' style='cursor: move; " +
        spm_getMenuBorderStyle(this) +
        "'>" +
        spm_getSpacer(this) +
        "</td>\n",
  ...
  spm_getTags("BODY")[0].onclick = spm_appendFunction(
    spm_getTags("BODY")[0].onclick,
    'm_oSolpartMenu["' + this._m_sNSpace + '"].bodyclick();',
  ); //document.body.onclick = this.bodyclick;
...
function spm_getMenuBarEvents(sCtl) {
  return (
    "onmouseover=\"m_oSolpartMenu['" +
    sCtl +
    "'].onMBMO(this);\" onmouseout=\"m_oSolpartMenu['" +
    sCtl +
    "'].onMBMOUT(this);\" onclick=\"m_oSolpartMenu['" +
    sCtl +
    "'].onMBC(this, event);\" onmousedown=\"m_oSolpartMenu['" +
    sCtl +
    "'].onMBMD(this);\" onmouseup=\"m_oSolpartMenu['" +
    sCtl +
    "'].onMBMU(this);\""
  );
}
...
function spm_getMenuItemEvents(sCtl) {
  return (
    "onmouseover=\"m_oSolpartMenu['" +
    sCtl +
    "'].onMBIMO(this);\" onmouseout=\"m_oSolpartMenu['" +
    sCtl +
    "'].onMBIMOUT(this);\" onclick=\"m_oSolpartMenu['" +
    sCtl +
    "'].onMBIC(this, event);\""
  );
}
...
SolpartMenu.prototype.GetMenuItems = function (oParent) {
  ...
  for (var i = 0; i < oParent.childNodes.length; i++) {
    ...
    if (oNode.nodeType != 3 && oNode.nodeType != 8) {
      //exclude nodeType of Text (Netscape/Mozilla) issue!
      //'determine if root level item and set parent id accordingly
      if (oNode.parentNode.nodeName != "menuitem") sParentID = "-1";
      else sParentID = oNode.parentNode.getAttribute("id");

      if (oNode.nodeName == "menuitem") sID = oNode.getAttribute("id");
      else sID = "";
      ...
      if (sParentID == "-1") {
        ...
        if (oNode.nodeName == "menubreak") {
          ...
          sHTML +=
            ...
            sID +
            '" ' +
            spm_getMenuBarEvents(this._m_sNSpace) +
            '  class="' +
            spm_fixCSSForMac(
              this.getIntCSSName("spmbar spmitm") +
                this.cssMenuBar +
                " " +
                this.cssMenuItem +
                " " +
                spm_getMenuItemCSS(oNode),
            ) +
            '" savecss="' +
            spm_getMenuItemCSS(oNode) +
            '" saveselcss="' +
            spm_getMenuItemSelCSS(oNode) +
            '" menuclick="' +
            sClickAction +
            '" style="' +
            spm_getMenuItemStyle("item", oNode) +
            '">\n';
          var sAlign =
          ...
          sHTML +=
            ...
            sAlign +
            ' TITLE="' +
            spm_getAttr(oNode, "tooltip", "") +
            '">' +
            spm_getImage(oNode, this) +
            spm_getItemHTML(oNode, "left", "&nbsp;") +
            spm_getAttr(oNode, "title", "") +
            spm_getItemHTML(oNode, "right") +
            MyIIf(
              Number(this.rootArrow) && spm_nodeHasChildren(oNode),
              '</td>\n<td align="right" class="' +
                spm_fixCSSForMac(
                  this.getIntCSSName("spmrarw") + this.cssMenuRootArrow,
                ) +
                '">' +
        ...
        if (this.display == "vertical") sHTML += "</tr>\n";
      } //'submenu - not top level menu item
      else {
        switch (oNode.nodeName) {
          case "menuitem": {
            sHTML +=
              ...
              sID +
              '" ' +
              spm_getMenuItemEvents(this._m_sNSpace) +
              ' parentID="' +
              sParentID +
              '" class="' +
              spm_fixCSSForMac(
                this.getIntCSSName("spmitm") +
                  this.cssMenuItem +
                  " " +
                  spm_getMenuItemCSS(oNode),
              ) +
            ...
            sHTML +=
              ...
              '" style="' +
              spm_getMenuItemStyle("image", oNode) +
              "; " +
              spm_getMenuItemStyle("item", oNode) +
              '">' +
              spm_getImage(oNode, this) +
            ...
            sHTML +=
              ...
              '">' +
              MyIIf(
                spm_nodeHasChildren(oNode),
                spm_getArrow(this.arrowImage, this),
                spm_getSpacer(this),
              ) +
              "</td>\n";
            sHTML += "   </tr>\n";
...
SolpartMenu.prototype.onMBC = function (e, evt) {
  ...
  if (oMenu != null) {
    ...
    if (oMenu.style.display == "") {
      this.hideAllMenus();
      if (this.useIFrames) spm_iFrameIndex(oMenu, false, this.systemImagesPath);
      else spm_showElement("SELECT|OBJECT");
    } else {
      ...
      if (this.useIFrames) spm_iFrameIndex(oMenu, true, this.systemImagesPath);
      else spm_hideElement("SELECT|OBJECT", oMenu);
    }
  }
...
SolpartMenu.prototype.onMBMO = function (e, bBypassDelay) {
  ...
  if (this._m_aOpenMenuID.length || this.moExpand != "0") {
    if (this.minDelay != 0 && bBypassDelay != true) {
      if (this.minDelayTimer != null) window.clearTimeout(this.minDelayTimer);
      this.minDelayType = "root";
      this.minDelayObj = e;
      this.minDelayTimer = setTimeout(
        'm_oSolpartMenu["' + this._m_sNSpace + '"].mouseInDelayHandler()',
        this.minDelay,
      );
    } else {
  ...
  oCell.className = spm_fixCSSForMac(
    this.getIntCSSName("spmitmsel spmbar") +
      this.cssMenuBar +
      " " +
      this.cssMenuItemSel +
      " " +
      spm_getAttr(oCell, "saveselcss", "") +
      " " +
...
SolpartMenu.prototype.onMBMOUT = function (e) {
  ...
  if (this.moutDelay != 0)
    setTimeout(
      'm_oSolpartMenu["' + this._m_sNSpace + '"].hideMenuTime()',
      this.moutDelay,
    );
...
SolpartMenu.prototype.onMBIC = function (e, evt) {
  ...
  if (spm_itemHasChildren(sID, this._m_sNSpace) == false) this.hideAllMenus();
...
SolpartMenu.prototype.handleNewItemSelect = function (sID) {
  ...
  for (i = 0; i < this._m_aOpenMenuID.length; i++) {
    if (bDeleteRest) {
      ...
      if (this.useIFrames)
        spm_iFrameIndex(
          spm_getById("tbl" + this._m_aOpenMenuID[i]),
          false,
  ...
  if (iNewLength != -1) this._m_aOpenMenuID.length = iNewLength + 1;
};
...
SolpartMenu.prototype.hideAllMenus = function () {
  ...
  if (this.useIFrames != true) spm_showElement("SELECT|OBJECT");
...
SolpartMenu.prototype.doTransition = function (oMenu) {
  if (this.menuTransition == "None" || this.supportsTransitions == "0") return;
  ...
  switch (this.menuTransition) {
    ...
    case "RandomBars": {
      oMenu
        .filters("DXImageTransform.Microsoft." + this.menuTransition)
        .apply();
      oMenu.filters(
        "DXImageTransform.Microsoft." + this.menuTransition,
      ).duration = this.menuTransitionLength;
...
SolpartMenu.prototype.doFilter = function (bStop) {
  if (this.SolpartMenuTransitionObject.id == null) return;
  ...
  switch (this.menuTransition) {
    case "AlphaFade": {
      if (this.SolpartMenuTransitionObject.stop) {
        ...
        o.filters("DXImageTransform.Microsoft.Alpha").opacity =
          this.SolpartMenuTransitionObject.opacity;
        if (this.SolpartMenuTransitionObject.opacity < 100) {
          setTimeout(
            'm_oSolpartMenu["' + this._m_sNSpace + '"].doFilter()',
            50,
          );
          this.SolpartMenuTransitionObject.opacity +=
            (100 / 20) * this.menuTransitionLength;
        }
      }
      break;
    }
...
SolpartMenu.prototype.handlembi_mo = function (oRow, bBypassDelay) {
  ...
  spm_getById("icon" + sID).className = spm_fixCSSForMac(
    this.getIntCSSName("spmitmsel spmicn") +
      this.cssMenuIcon +
      " " +
      this.cssMenuItemSel +
      " " +
  ...
  spm_getById("td" + sID).className = spm_fixCSSForMac(
    this.getIntCSSName("spmitmsel") +
      this.cssMenuItemSel +
      " " +
      spm_getAttr(oRow, "saveselcss", ""),
  );
  spm_getById("arrow" + sID).className = spm_fixCSSForMac(
    this.getIntCSSName("spmitmsel spmarw") +
      this.cssMenuItemSel +
      " " +
      this.cssMenuArrow +
      " " +
  ...
  if (
    this._m_aOpenMenuID[this._m_aOpenMenuID.length - 1] !=
    oRow.id.replace("tr", "")
  ) {
    ...
    if (spm_getById("tbl" + sID) != null) {
      ...
      if (
        oMDims.t - spm_getBodyScrollTop() + oMDims.h >
        spm_getViewPortHeight()
      ) {
        if (oMDims.h < spm_getViewPortHeight())
          oMenu.style.top = spm_getCoord(
            spm_getViewPortHeight() + spm_getBodyScrollTop() - oMDims.h,
          );
        else {
      ...
      if (this.direction == "rtl")
        oMenu.style.left = spm_getCoord(
          oPDims.l - oMDims.w - spm_getBodyScrollLeft(),
        );
      else
        oMenu.style.left = spm_getCoord(
          oPDims.l + oPDims.w - spm_getBodyScrollLeft(),
        );
      ...
      if (this.direction == "rtl") {
        ...
        if (
          oPDims.l - spm_getBodyScrollLeft() + oPDims.w + oMDims.w >
...
SolpartMenu.prototype.hideMenuTime = function () {
  if (this._m_dHideTimer != null && this.moutDelay > 0) {
    if (new Date() - this._m_dHideTimer >= this.moutDelay) {
      this.hideAllMenus();
      this._m_dHideTimer = null;
    } else setTimeout(this.hideMenuTime, this.moutDelay);
  }
};

SolpartMenu.prototype.mouseInDelayHandler = function () {
  if (this.minDelayType == "root") this.onMBMO(this.minDelayObj, true);
...
SolpartMenu.prototype.scrollMenu = function () {
  if (this._m_oScrollingMenu != null) {
    ...
    else this._m_oScrollingMenu = null;
  }
};

//global
function spm_iFrameIndex(eMenu, bShow, sysImgPath) {
  if (spm_browserType() == "op") return; //not needed

  if (document.readyState != "complete") return; //avoid operation aborted
  ...
  if (bShow) {
    ...
    if (iIndex == null || iIndex == 0) eMenu.style.zIndex = 1;
...
function spm_positionMenu(me, oMenu, oCell) {
  ...
  if (me.display == "vertical") {
    ...
    if (
      ...
      if (oMDims.h < spm_getViewPortHeight())
        oMenu.style.top = spm_getCoord(
          spm_getViewPortHeight() - oMDims.h + spm_getBodyScrollTop(),
        );
      else spm_handleScrollMenu(me, oMenu);
    }

    var oOrigMDims;

    if (spm_browserType() != "ie")
      //since mozilla doesn't set width greater than window size we need to store it here
    ...
    if (oOrigMDims == null) oOrigMDims = oMDims;

    if (
      oMDims.l - spm_getBodyScrollLeft(true) + oOrigMDims.w >
      spm_getViewPortWidth()
    ) {
      if (spm_getViewPortWidth() - oOrigMDims.w > 0)
        //only do this if it fits
        oMenu.style.left = spm_getCoord(
          oPDims.l - oOrigMDims.w - spm_getBodyScrollLeft(true),
        );
    }
    ...
    if (me.direction == "rtl") {
      ...
      oMenu.style.left = spm_getCoord(
        oPDims.l + oPDims.w - oMDims2.w - spm_getBodyScrollLeft(),
      );
    } else oMenu.style.left = spm_getCoord(oPDims.l - spm_getBodyScrollLeft());

    oMenu.style.top = spm_getCoord(oPDims.t + oPDims.h);
    var oMDims = new spm_elementDims(oMenu);

    if (
      oMDims.l - spm_getBodyScrollLeft(true) + oMDims.w >
      spm_getViewPortWidth()
    ) {
      if (spm_getViewPortWidth() - oMDims.w > 0)
        //only do this if it fits
        oMenu.style.left = spm_getCoord(
          spm_getViewPortWidth() - oMDims.w + spm_getBodyScrollLeft(true),
        );
    }
...
SolpartMenu.prototype.applyBorder = function (
  oCell,
  iSize,
  sTopLeftColor,
  sBottomRightColor,
  ...
  if (this.moDisplay == "HighLight") {
    if (sTopLeftColor == this.backColor) {
      ...
      oCell.className = spm_fixCSSForMac(
        this.getIntCSSName("spmbar spmitmsel") +
          " " +
          this.cssMenuItemSel +
          " " +
          spm_getAttr(oCell, "saveselcss", ""),
...
function spm_applyRowBorder(oRow, iSize, sColor, bSelected, sStyle) {
  ...
  if (sColor == "" && bSelected == false) {
    ...
    if (sColor2 == null || sColor2 == "") sColor2 = "transparent";
  }
...
function spm_getCellBackColor(o) {
  ...
  if (spm_browserType() == "ie") {
    ...
    while (sColor == "transparent") {
      ...
      o = o.parentElement;
      if (o.id.indexOf("divOuterTables") != -1)
        //if we are outside the realm of the menu then use transparency
        break;
    }
  }
  return sColor;
...
function spm_handleScrollMenu(me, oMenu) {
  ...
  if (oMenu.scrollPos == null) {
    ...
    oCell.innerHTML =
      '<div id="dn' +
      oMenu.id.substr(3) +
      '" onclick="return spm_ScrollMenuClick(this, event);" onmouseover="spm_ScrollMenuMO(this, m_oSolpartMenu[\'' +
      me._m_sNSpace +
      "']);\" onmouseout=\"spm_ScrollMenuMOUT(m_oSolpartMenu['" +
      me._m_sNSpace +
      '\']);" class="' +
      spm_fixCSSForMac(me.getIntCSSName("spmitmscr")) +
      " " +
      me.cssMenuScrollItem +
      '" style="width: 100%; font-size: 6pt;">...</div>';
  ...
  if (oMenu.ScrollRowHeight == null) {
    ...
    oMenu.ScrollItemHeight =
      spm_getElementHeight(spm_getTags("tr", oTbl)[0]) * 2;
    spm_getTags("tr", oTbl)[0].style.display = "none";

    oMenu.ScrollRowHeight = spm_getElementHeight(spm_getTags("tr", oTbl)[1]);
  }

  oMenu.scrollItems = parseInt(
    (spm_getViewPortHeight() -
      spm_elementTop(oMenu) +
      spm_getBodyScrollTop() -
      oMenu.ScrollItemHeight) /
...
function spm_ScrollMenuMOUT(me) {
  ...
  if (me.moutDelay != 0)
    setTimeout(
      'm_oSolpartMenu["' + me._m_sNSpace + '"].hideMenuTime()',
      me.moutDelay,
    );
}
...
function spm_ScrollMenuClick(e, evt) {
  if (e != null) {
    ...
    if (oCell.id == "up" + oMenu.id.substring(3)) {
      if (oMenu.scrollPos > 1) oMenu.scrollPos--;
      else return true;
    } else {
    ...
    spm_showScrolledItems(oMenu);
    if (evt != null) spm_stopEventBubbling(evt);
  }
  return false;
}
...
function spm_insertTableRow(tbl, iPos) {
  ...
  if (tbl.getElementsByTagName("TBODY").length == 0) {
    ...
    tbl.appendChild(oTB);
  } else oTB = tbl.getElementsByTagName("TBODY")[0];

  if (iPos == null) oTB.appendChild(oRow);
  else oTB.insertBefore(oRow, tbl.rows[iPos]);
  return oRow;
}
...
function spm_getElementWidth(o) {
  if (o.offsetWidth == null || o.offsetWidth == 0) {
    ...
    } else return o.offsetParent.offsetWidth;
...
function spm_elementTop(eSrc, includeBody) {
  ...
  if (
    includeBody != null &&
    eParent != null &&
    (spm_browserType() == "safari" || spm_browserType() == "kq")
  )
    iTop += eParent.offsetTop;
...
function spm_itemHasChildren(sID, ns) {
  ...
  if (objTable != null) {
    if (objTable.rows != null) {
      if (objTable.rows.length > 0) return true;
      else return false;
    }
  }
}
  ...
  if (sURL.length) {
    if (sURL.toLowerCase().substr(0, "javascript:".length) == "javascript:")
      ...
      if (me.target.length > 0 && document.frames[me.target] != null)
        return (
          "document.frames['" + me.target + "'].location.href='" + sURL + "';"
        );
      else return "document.location.href='" + sURL + "';";
    }
  }
  return "";
}
...
function spm_getMenuSpacingImage(sPos, me) {
  ...
  if (
    (sPos == "left" && sAlign == "right") ||
    (sPos == "right" && sAlign == "left")
  )
...
function spm_getImage(oAttr, me) {
  ...
  if (sImage.length) {
    return spm_getHTMLImage(
      sImage,
      spm_getAttr(oAttr, "imagepath", me.iconImagesPath),
      null,
      spm_getAttr(oAttr, "title", ""),
    );
  } else return spm_getMenuImage("spacer.gif", me, null, " ");
}
...
function spm_getHTMLImage(sImage, sPath, bForce, sAlt) {
  //'generates html for image using the SystemImagesPath property
  if (
    spm_browserNeedsSpacer() == false &&
    sImage == "spacer.gif" &&
    bForce != true
  )
    return "&nbsp;";
  else return '<IMG SRC="' + sPath + sImage + '" ' + spm_getAlt(sAlt) + ">";
}

function spm_getAlt(sAlt) {
  if (sAlt != null && sAlt.rtrim().length > 0) return ' ALT="' + sAlt + '" ';
  else return "";
}

function spm_browserNeedsSpacer() {
  if (spm_browserType() == "ie") return false;
  else return true;
}

function MyIIf(bFlag, sTrue, sFalse) {
  if (bFlag) return sTrue;
  else return sFalse;
}

function spm_getArrow(sImg, me) {
  //FIX
  if (sImg.length) return spm_getMenuImage(sImg, me, null, ">");
  else {
    if (me.direction == "rtl") return "3";
    else return "4"; //'defaults to using wingdings font (4 = arrow)
  }
}
...
function spm_getMenuBorderStyle(me, shColor, hlColor, width) {
  ...
  return (
    getBorderStyle("border-bottom", shColor, width) +
...
function spm_getById(sID) {
  if (document.all == null) return document.getElementById(sID);
  else return document.all(sID);
}

function spm_getTags(sTag, oCtl) {
  if (oCtl == null) oCtl = document;

  if (spm_browserType() == "ie") return oCtl.all.tags(sTag);
  else return oCtl.getElementsByTagName(sTag);
}
...
function spm_browserType() {
  if (m_spm_sBrowser == null) {
    ...
    if (agt.toLowerCase().indexOf("konqueror") != -1) m_spm_sBrowser = "kq";
    else if (agt.toLowerCase().indexOf("opera") != -1) m_spm_sBrowser = "op";
    else if (agt.toLowerCase().indexOf("netscape") != -1) m_spm_sBrowser = "ns";
    else if (agt.toLowerCase().indexOf("msie") != -1) m_spm_sBrowser = "ie";
    else if (agt.toLowerCase().indexOf("safari") != -1)
      m_spm_sBrowser = "safari";

    if (m_spm_sBrowser == null) m_spm_sBrowser = "mo";
  }
...
function spm_needsSubMenuDelay() {
  if (spm_browserType() == "ie") return true;
  else return false;
}
...
function spm_isMac(sType) {
  ...
  if (agt.indexOf("mac") != -1) {
    if (sType == null || spm_browserType() == sType) return true;
  } else return false;
}
...
function spm_getAttr(o, sAttr, sDef) {
  ...
  if (s != null && s.length > 0) return o.getAttribute(sAttr);
  else return sDef;
}

function spm_setAttr(o, sAttr, sVal) {
  if (sVal.length > 0) o.setAttribute(sAttr, sVal);
  else o.removeAttribute(sAttr);
}

function spm_fixUnit(s) {
  if (s.length && isNaN(s) == false) return s + "px";
}

function spm_nodeHasChildren(node) {
  if (typeof node.selectSingleNode != "undefined")
    //(node.selectSingleNode != null) //(spm_browserType() == 'ie')
    ...
    if (node.childNodes.length > 0) {
      ...
      for (var i = 0; i < node.childNodes.length; i++) {
        if (node.childNodes[i].nodeName == "menuitem") return true;
      }
    }
  }
  return false;
...
function spm_findNode(oParent, sID) {
  for (var i = 0; i < oParent.childNodes.length; i++) {
    ...
    if (oNode.nodeType != 3) {
      //exclude nodeType of Text (Netscape/Mozilla) issue!
      if (
        (oNode.nodeName == "menuitem" || oNode.nodeName == "menubreak") &&
        oNode.getAttribute("id") == sID
      )
        return oNode;
...
function spm_stopEventBubbling(e) {
  if (spm_browserType() == "ie") window.event.cancelBubble = true;
  else e.stopPropagation();
}

//--- if you have a better solution send me an email - jhenning@solpart.com ---//
function spm_appendFunction(from_func, to_func) {
  if (from_func == null) return new Function(to_func);
  return new Function(
    spm_parseFunctionContents(from_func) +
      "\n" +
...
function spm_parseFunctionContents(fnc) {
  ...
  if (s.indexOf("{") > -1) s = s.substring(s.indexOf("{") + 1, s.length - 1);
  return s;
}

//--- For JS DOM ---//
function SPJSXMLNode(
  sNodeName,
  sID,
  oParent,
  sTitle,
  ...
  if (oParent != null) {
    ...
    if (oParent.documentElement == null) this.documentElement = oParent;
    else this.documentElement = oParent.documentElement;
  } else this.documentElement = this;
...
function __db(s) {
  if (spm_browserType() != "ie" || m_bSPDebug == false) return;
  ...
  if (document.forms.length > 0 && m_oSPDebugCtl == null) {
    document
      ...
      .insertAdjacentHTML(
        "afterEnd",
        '<br><TEXTAREA ID="my__Debug" STYLE="WIDTH: 100%; HEIGHT: 100px"></TEXTAREA>',
      );
...
if (window.__smartNav != null) window.setTimeout(spm_fixSmartNav, 1000);
function spm_fixSmartNav() {
  if (window.__smartNav != null) {
    if (document.readyState == "complete") {
      ...
      if (o != null) {
        if (o.length == null) {
          if (o.xml != null) spm_initMyMenu(o, o.parentElement);
        } else {
          for (var i = 0; i < o.length; i++) {
            if (o[i].xml != null) spm_initMyMenu(o[i], o.parentElement);
          }
        }
      }
    } else window.setTimeout(spm_fixSmartNav, 1000);
  }
}
...
function spm_elementDims(o, bIncludeBody, me) {
  ...
  if (bHidden) o.style.display = "none";
}
