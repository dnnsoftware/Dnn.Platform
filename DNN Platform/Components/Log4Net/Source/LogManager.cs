...
namespace log4net
{
	...
	{
		private readonly static WrapperMap s_wrapperMap;

		static LogManager()
		{
			LogManager.s_wrapperMap = new WrapperMap(new WrapperCreationHandler(LogManager.WrapperCreationHandler));
		}
		...
		{
			ILog[] logArray = new ILog[(int)loggers.Length];
			for (int i = 0; i < (int)loggers.Length; i++)
			{
				logArray[i] = LogManager.WrapLogger(loggers[i]);
			}
			return logArray;
		}
