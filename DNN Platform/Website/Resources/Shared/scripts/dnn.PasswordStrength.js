...
(function ($, window, document, undefined) {
  ...
  PasswordStrength.prototype = {
    ...
    init: function () {
      ...
      this._$meter = $("<div/>")
        .addClass(this.options.meterCss)
      ...
      this._$meterValue = $(
        '<div><span class=""></span><span class=""></span><span class=""></span><span class="last"></span></div>',
      ).appendTo(this._$meter);
      ...
      this.$element.on(
        "change keyup paste input propertychange",
    ...
    _initializeTooltip: function () {
      this._$tooltipContent = $("<div/>")
        .addClass("password-strength-tooltip-content")
        .appendTo(this._$tooltipContainer);
      this._$tooltipContent.width(this.$element.outerWidth() - 20);
      $("<h2 />")
        .html(this.options.passwordRulesHeadText)
        .appendTo(this._$tooltipContent);

      var list = $("<ul/>").appendTo(this._$tooltipContent);
      this._$labelAtLeastNChars = $("<label/>")
        .text(this.options.criteriaAtLeastNChars)
        .appendTo($("<li/>").appendTo(list));
      this._$labelAtLeastNSpecialChars = $("<label/>")
        .text(this.options.criteriaAtLeastNSpecialChars)
        .appendTo($("<li/>").appendTo(list));
      this._$labelValidationExpression = $("<label/>")
        .text(this.options.criteriaValidationExpression)
        .appendTo($("<li/>").appendTo(list));
    ...
    _updateTooltipState: function (strength) {
      ...
      this._updateCriteriaLabel(
        this._$labelAtLeastNChars,
        strength.hasLengthOfNChars,
      );
      this._updateCriteriaLabel(
        this._$labelAtLeastNSpecialChars,
        strength.hasMinNumberOfSpecialChars,
      );
      this._updateCriteriaLabel(
        this._$labelValidationExpression,
        strength.matchValidationExpression,
      );
    },

    _updateTooltipVisible: function () {
      var password = this.$element.val();
      var strength = (this._strength = this._getStrength(
        password,
        this.options,
      ));
    ...
    _matchedPasswordPolicy: function (strength) {
      return (
        strength.hasLengthOfNChars &&
        (this.options.minNumberOfSpecialChars === 0 ||
          strength.hasMinNumberOfSpecialChars) &&
        (this.options.validationExpression === "" ||
          strength.matchValidationExpression)
      );
    },
    ...
    _getStrength: function (password, options) {
      ...
      if (password.length > 0) {
        ...
        if (validationExpression) {
          matchValidationExpression = new RegExp(
  ...
  $.fn[pluginName] = function (option) {
    ...
    return this.each(function () {
      ...
      if (!instance) {
        ...
        $this.data(
          pluginName,
          (instance = new PasswordStrength(this, options)),
        );
      }
