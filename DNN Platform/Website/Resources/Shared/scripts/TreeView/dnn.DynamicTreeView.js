...
(function ($, window, document, undefined) {
  ...
  // DynamicTreeView
  //
  var DynamicTreeView = (this.DynamicTreeView = function (
    element,
    options,
    controller,
  ) {
  ...
  DynamicTreeView.prototype = {
    ...
    _onCreateNodeElement: function (eventObject, nodeElement, nodeContext) {
      ...
      var textNode = $("<a>" + $.htmlEncode(data.name) + "</a>").addClass(
        this.options.textCss,
      );
      if (data.selectable) {
        textNode
          .prop("href", "javascript:void(0);")
          .on("click", this._onNodeTextClickHandler);
      } else {
        textNode.addClass(this.options.unselectableCss);
      }
      $nodeElement
        .append(
          $("<a href='javascript:;' aria-label=\"Node\"/>")
            .addClass(this.options.iconCss)
            .on("click", this._onNodeIconClickHandler),
        )
        .append(textNode);
    ...
    _onNodeIconClick: function (eventObject) {
      ...
      if ($clickedElement.hasClass(this.options.collapsedCss)) {
        // expand
        $clickedElement
          .removeClass(this.options.collapsedCss)
          .addClass(this.options.expandingCss);
        ...
        $clickedElement
          .removeClass(this.options.expandedCss)
          .addClass(this.options.collapsedCss);
    ...
    showChildren: function (nodeContext, children) {
      ...
      this._initializeNodeIcon(
        nodeContext,
        this._tree.getNodeElement(nodeContext),
      );
    ...
    _onCollapseNode: function (nodeContext) {
      ...
      if (
        this._selectedNode !== nodeContext &&
        nodeContext.hasNode(this._selectedNode)
      ) {
    ...
    _selectNodeElement: function (nodeContext, selected) {
      if (this._tree) {
        this._tree.addNodeClass(
          nodeContext,
          selected,
          this.options.selectedCss,
        );
      }
    },
    ...
    scrollToSelectedNode: function () {
      ...
      if (this.options.scroll && node) {
        ...
        var offset =
          $nodeElement.position().top -
          $nodeElement
            .parentsUntil(
              "div",
              'ul[class*="' +
                this._tree.options.nodeListCss +
                '"][class*="' +
                this._tree.options.rootCss +
                '"]',
            )
            .position().top;
    ...
    _showTree: function (rootNode) {
      ...
      this._numberOfNodes =
        this._rootNode.getNumberOfChildren(true) - (this._firstNode ? 1 : 0);
    },

    _onChangeNode: function (node) {
      var nodeComparer = function (one, another) {
  ...
  // SortableTreeView
  //
  var SortableTreeView = (this.SortableTreeView = function (
    element,
    options,
    controller,
  ) {
  ...
  SortableTreeView.prototype = {
    ...
    init: function () {
      ...
      this._$itemListHeaderElement = this.$element.find(
        "." + this.options.headerCss,
      );
      this._$clearButton = this._$itemListHeaderElement.find(
        "." + this.options.clearButtonCss,
      );
      this._$clearButton.on(
        "click.drop-down-list",
        $.proxy(this._onClearClick, this),
      );
      this._$searchButton = this._$itemListHeaderElement.find(
        "." + this.options.searchButtonCss,
      );
      this._$searchButton.on(
        "click.drop-down-list",
        $.proxy(this._onSearchClick, this),
      );
      var onSortHandler = $.proxy(this._onSortClick, this);
      var onSearchHandler = $.proxy(this._onSearchClick, this);
      this._$searchContainer = this._$itemListHeaderElement.find(
        "." + this.options.searchContainerCss,
      );
      this._$searchInput = this._$itemListHeaderElement
        .find("." + this.options.searchInputCss)
        .onEnter(onSearchHandler)
        .on("keyup.drop-down-list", $.proxy(this._displayClearButton, this));
      this._displayClearButton();
      this._$sortButton = this._$itemListHeaderElement.find(
        "." + this.options.sortButtonCss,
      );
      this._$sortButton.on("click.drop-down-list", onSortHandler);

      this._$itemListFooterElement = this.$element.find(
        "." + this.options.footerCss,
      );
      this._$resultValue = this._$itemListFooterElement
        .find("." + this.options.resultElementCss)
        .children(":first");

      this._$itemListContentElement = this.$element.find(
        "." + this.options.contentCss,
      );

      this._onLoadTreeHandler = $.proxy(this._onLoadTree, this);

      this._firstNode = this.options.firstItem
        ? TreeNodeConverter.createNodeFromModel(this.options.firstItem)
        : null;
      this.options.firstNode = this._firstNode;

      this._dynamicTree = new DynamicTreeView(
        this._$itemListContentElement[0],
        this.options,
      );
    ...
    _createLayout: function () {
      var layout = $("<div class='" + this.options.containerCss + "'/>")
        .append(
          $("<div class='" + this.options.headerCss + "'/>")
            .append(
              $(
                "<a href='javascript:void(0);' class='" +
                  this.options.sortButtonCss +
                  "'><span>" +
                  this.options.sortAscendingButtonTitle +
                  "</span></a>",
              ),
            )
            .append(
              $("<div class='" + this.options.searchContainerCss + "'/>")
                .append(
                  $(
                    "<div class='" +
                      this.options.searchInputContainerCss +
                      "'/>",
                  ).append(
                    $(
                      "<input type='text' placeholder='" +
                        this.options.searchInputPlaceHolder +
                        "' maxlength='200' autocomplete='off' class='" +
                        this.options.searchInputCss +
                        '\' aria-label="Search">',
                    ),
                  ),
                )
                .append(
                  $(
                    "<a href='javascript:void(0);' title='" +
                      this.options.clearButtonTooltip +
                ...
                .append(
                  $(
                    "<a href='javascript:void(0);' title='" +
                      this.options.searchButtonTooltip +
                      "' class='" +
                      this.options.searchButtonCss +
                      '\' aria-label="Search"/>',
                  ),
        ...
        .append(
          $("<div class='" + this.options.contentCss + "'/>").append(
            $("<div class='" + this.options.treeContainerCss + "'/>"),
          ),
        )
        .append(
          $("<div class='" + this.options.footerCss + "'/>")
            .append(
              $(
                "<span class='" +
                  this.options.resultElementCss +
                  "'><b></b> " +
                  this.options.resultsText +
                  "</span>",
              ),
            )
    ...
    _onExpandNode: function (eventObject, nodeContext) {
      var onLoadChildNodesHandler = $.proxy(
        this._onLoadChildren,
        this,
        nodeContext,
      );
    ...
    _onResize: function (eventObject, containerSize) {
      this._$itemListContentElement.height(
        containerSize.height -
          this._itemListHeaderHeight -
          this._itemListFooterHeight,
      );
    ...
    show: function () {
      ...
      this._itemListHeaderHeight =
        this._$itemListHeaderElement.outerHeight(true);
      this._itemListFooterHeight =
        this._$itemListFooterElement.outerHeight(true);
      this._$itemListContentElement.height(
        this.$element.height() -
          this._itemListHeaderHeight -
          this._itemListFooterHeight,
      ...
      if (!this._resizer) {
        var $resizerElement = this._$itemListFooterElement.find(
          "." + this.options.resizerElementCss,
        );
        this._resizer = new dnn.Resizer($resizerElement[0], {
    ...
    _showTree: function (selectedItemId) {
      ...
      if (
        selectedItemId &&
        (!this._firstNode ||
          (this._firstNode && this._firstNode.data.id !== selectedItemId))
      ) {
        this._searchInput("");
        this.controller.getTreeWithItem(
          selectedItemId,
          this._sortOrder(),
          this._onLoadTreeHandler,
        );
      } else {
    ...
    _sortOrder: function (order) {
      ...
      if (order === dnn.SortOrder.ascending) {
        this._$sortButton
          .removeClass("desc")
          .addClass("asc")
          .prop("title", this.options.sortDescendingButtonTooltip);
      } else if (order === dnn.SortOrder.descending) {
        this._$sortButton
          .removeClass("asc")
          .addClass("desc")
          .prop("title", this.options.unsortedOrderButtonTooltip);
      } else {
        // order === dnn.SortOrder.unspecified
        this._$sortButton
          .removeClass("desc")
          .removeClass("asc")
          .prop("title", this.options.sortAscendingButtonTooltip);
      }
    ...
    _sort: function () {
      ...
      this.controller.sortTree(
        this._sortOrder(),
        this._dynamicTree.rootNode(),
        searchText,
        this._onLoadTreeHandler,
      );
    },
  ...
  TreeNodeConverter.castModelData = function (model) {
    return model
      ? {
          ...
          id: model.key,
          selectable:
            typeof model.selectable === "undefined" || model.selectable,
        }
      : null;
  };
  ...
  var DynamicTreeViewController = (this.DynamicTreeViewController = function (
    options,
  ) {
    this.options = options;
    this.init();
  ...
  DynamicTreeViewController.prototype = {
    ...
    init: function () {
      this.options = $.extend(
        {},
        DynamicTreeViewController.defaults(),
        this.options,
      );
      this._serviceUrl = $.dnnSF(this.options.moduleId).getServiceRoot(
    ...
    getTreeWithItem: function (itemId, sortOrder, onGetTreeCallback) {
      ...
      this._callGet(
        { itemId: itemId, sortOrder: sortOrder },
        onGetTreeHandler,
        this.options.getTreeWithNodeMethod,
      );
    },
    ...
    _onGetTree: function (onGetTreeCallback, data, textStatus, jqXhr) {
      ...
      if (
        ...
        if (rootNode.children[0].data.id === this.options.rootNodeId) {
          // Check if first node is the configured root node. Search or Sort could change that
    ...
    getChildren: function (
      ...
      sortOrder,
      searchText,
      onGetChildrenCallback,
    ) {
      var onGetChildrenHandler = $.proxy(
        this._onGetChildren,
        this,
        onGetChildrenCallback,
      );
      ...
      this._callGet(
        callGetParams,
        onGetChildrenHandler,
        this.options.getNodeDescendantsMethod,
      );
    },
    ...
    search: function (searchText, sortOrder, onSearchCallback) {
      ...
      this._callGet(
        callGetParams,
        onSearchHandler,
        this.options.searchTreeMethod,
      );
    },

    getTree: function (sortOrder, onGetFirstLevelItemsCallback) {
      var onGetFirstLevelItemsHandler = $.proxy(
        this._onGetTree,
        this,
        onGetFirstLevelItemsCallback,
      );
      ...
      this._callGet(
        callGetParams,
        onGetFirstLevelItemsHandler,
        this.options.getTreeMethod,
      );
    },

    sortTree: function (sortOrder, rootNode, searchText, onSortTreeCallback) {
      var onSortTreeHandler = $.proxy(
        this._onGetTree,
        this,
        onSortTreeCallback,
      );
      this._callGet(
        {
          treeAsJson: JSON.stringify(TreeNodeConverter.toTreeOfIds(rootNode)),
          sortOrder: sortOrder,
          searchText: searchText,
        },
        onSortTreeHandler,
        this.options.sortTreeMethod,
      );
    },
  };
