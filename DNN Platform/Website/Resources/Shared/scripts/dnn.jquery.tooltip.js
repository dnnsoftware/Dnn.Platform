...
(function ($, window, document, undefined) {
  ...
  Tooltip.prototype = {
    ...
    _onMouseEnter: function (eventObject) {
      ...
      var self = this;
      var show = function () {
        if (self._hoverState === "mouse-enter") {
          self._show();
        }
      };
    ...
    _onMouseLeave: function (closeOnMouseLeave, eventObject) {
      ...
      if (closeOnMouseLeave) {
        ...
        var self = this;
        var hide = function () {
          if (self._hoverState === "mouse-leave") {
            self._hide();
          }
        };
    ...
    _createTooltip: function () {
      var $tooltip;
      $tooltip = $(this.options.template || "<div>")
      ...
      var tooltipMarkup = this.options.getTooltipMarkup.call(
        this,
        this.element,
      );
      if (tooltipMarkup.nodeType || this._isjQuery(tooltipMarkup)) {
        ...
        if (this.options.closeButtonSelector) {
          $(tooltipMarkup)
            .find(this.options.closeButtonSelector)
            .on("click", this._onCloseButtonClickHandler);
        }
      } else {
    ...
    _onDocumentClick: function (eventObject) {
      ...
      if (
        this._$tooltip &&
        !this._inContainer(this._$tooltip, clickedElement)
      ) {
        this._hide();
      }
    },
    ...
    _setTooltipOffset: function () {
      this._$tooltip
        .detach()
        .removeCssProperties("height", "width", "left", "top")
        .css({ display: "block" });
      ...
      var placement =
        typeof this.options.placement == "function"
          ? this.options.placement.call(this, this._$tooltip[0], this.element)
      ...
      switch (placement) {
        case "top":
          tooltipPosition = {
            top: elementPosition.top - tooltipHeight,
            left:
              elementPosition.left +
              10 /*+ (elementPosition.width - tooltipWidth) / 2 */,
          };
          break;
        case "bottom":
          tooltipPosition = {
        ...
        case "left":
          tooltipPosition = {
            top:
              elementPosition.top +
              (elementPosition.height - tooltipHeight) / 2,
            left: elementPosition.left - tooltipWidth,
          };
          break;
        case "right":
          tooltipPosition = {
        ...
        default:
          ...
          tooltipPosition = {
            top: elementPosition.top + this.options.top,
            left: elementPosition.left + this.options.left,
          };
      }
      ...
      var offScreenLength =
        tooltipPosition.left + tooltipWidth - viewPort.right();
      if (offScreenLength > 0) {
        tooltipPosition.left = Math.max(
          viewPort.left,
          tooltipPosition.left - offScreenLength,
        );
      }

      offScreenLength = tooltipPosition.top + tooltipHeight - viewPort.bottom();
      if (offScreenLength > 0) {
        tooltipPosition.top = Math.max(
          viewPort.top,
          tooltipPosition.top - offScreenLength,
        );
      }

      this._$tooltip
        .offset(tooltipPosition)
        .fadeIn(this.options.fadeInDuration); // show();
      if (this.options.animateOnOpen) {
        var originalSize = {
          height: this._$tooltip.height(),
          width: this._$tooltip.width(),
        };
        this._$tooltip
          .css({ height: 0, width: 0 })
          .animate(originalSize, this.options.fadeInDuration);
      }
    },
