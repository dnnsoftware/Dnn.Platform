(function ($, window, document, undefined) {
  ...
  WebResourceUrl.prototype = {
    ...
    toPathAndQuery: function () {
      var pathAndQuery = this.path().append(
        WebResourceUrl.toQueryString(this.parameters()),
        "?",
      );
      return pathAndQuery;
    },
        ...
        if (parts.length > 0) {
          for (var i = 0, size = parts.length; i < size; i++) {
            ...
            this._parameters.set(
              keyValuePair[0],
              WebResourceUrl.decodeParameterValue(keyValuePair[1]),
            );
          }
        }
      }
    ...
    for (var i = 0, size = keys.length; i < size; i++) {
      ...
      if (!Object.isNullOrUndefined(value)) {
        queryString = queryString.append(
          key + "=" + WebResourceUrl.encodeParameterValue(value),
          "&",
        );
      }
    }
