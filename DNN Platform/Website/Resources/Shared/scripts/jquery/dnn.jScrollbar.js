...
(function ($, window, document, undefined) {
  ...
  Scrollbar.prototype = {
    ...
    init: function () {
      ...
      this._$scrollbarX = $("<div class='ps-scrollbar-x'></div>").appendTo(
        this.$element,
      );
      this._$scrollbarX.on(
        "mousedown.perfect-scroll",
        $.proxy(this._onMouseDownX, this),
      );

      this._$scrollbarY = $("<div class='ps-scrollbar-y'></div>").appendTo(
        this.$element,
      );
      this._$scrollbarY.on(
        "mousedown.perfect-scroll",
    ...
    _scrollY: function () {
      var scrollTop = parseInt(
        (this._scrollbarYTop * this._contentHeight) / this._containerHeight,
        10,
      );
    ...
    _scrollX: function () {
      var scrollLeft = parseInt(
        (this._scrollbarXLeft * this._contentWidth) / this._containerWidth,
        10,
      );
    ...
    update: function () {
      ...
      if (this._containerWidth < this._contentWidth) {
        this._scrollbarXWidth = parseInt(
          (this._containerWidth * this._containerWidth) / this._contentWidth,
          10,
        );
        this._scrollbarXLeft = parseInt(
          (this.$element.scrollLeft() * this._containerWidth) /
            this._contentWidth,
          10,
        );
      } else {
      ...
      if (this._containerHeight < this._contentHeight) {
        this._scrollbarYHeight = parseInt(
          (this._containerHeight * this._containerHeight) / this._contentHeight,
          10,
        );
        this._scrollbarYTop = parseInt(
          (this.$element.scrollTop() * this._containerHeight) /
            this._contentHeight,
          10,
        );
      } else {
      ...
      if (
        ...
        this._containerWidth - this._scrollbarXWidth
      ) {
        this._scrollbarXLeft = this._containerWidth - this._scrollbarXWidth;
      }

      this._$scrollbarX.css({
        left: this._scrollbarXLeft + this.$element.scrollLeft(),
        bottom: this._scrollbarXBottom - this.$element.scrollTop(),
        width: this._scrollbarXWidth,
      });
      this._$scrollbarY.css({
        top: this._scrollbarYTop + this.$element.scrollTop(),
        right: this._scrollbarYRight - this.$element.scrollLeft(),
    ...
    _onMouseDownX: function (e) {
      ...
      $(this._getDocument()).on(
        "mousemove.perfect-scroll",
        this._onMouseMoveXHandler,
      );
      $(this._getDocument()).on(
        "mouseup.perfect-scroll",
        this._onMouseUpXHandler,
      );
      e.stopPropagation();
      e.preventDefault();
    },
    ...
    _onMouseDownY: function (e) {
      ...
      $(this._getDocument()).on(
        "mousemove.perfect-scroll",
        this._onMouseMoveYHandler,
      );
      $(this._getDocument()).on(
        "mouseup.perfect-scroll",
        this._onMouseUpYHandler,
      );
      e.stopPropagation();
      e.preventDefault();
    },
    ...
    _onMouseWheel: function (e, delta, deltaX, deltaY) {
      this.$element.scrollTop(
        this.$element.scrollTop() - deltaY * this.options.wheelSpeed,
      );
      this.$element.scrollLeft(
        this.$element.scrollLeft() + deltaX * this.options.wheelSpeed,
      );
    ...
    _shouldPreventDefault: function (deltaX, deltaY) {
      ...
      if (scrollTop === 0 && deltaY > 0 && deltaX === 0) {
        ...
        scrollTop >= this._contentHeight - this._containerHeight &&
        deltaY < 0 &&
        deltaX === 0
      ) {
        return !this.options.wheelPropagation;
      ...
      if (scrollLeft === 0 && deltaX < 0 && deltaY === 0) {
        ...
        scrollLeft >= this._contentWidth - this._containerWidth &&
        deltaX > 0 &&
        deltaY === 0
      ) {
        return !this.options.wheelPropagation;
