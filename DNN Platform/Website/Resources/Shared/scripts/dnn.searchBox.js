  ...
  $.dnnSearchBox.prototype = {
    ...
    _generateTemplate: function () {
      ...
      var advancedEnabled =
        ...
        this.options.advancedId &&
        $("#" + this.options.advancedId).length;
      if (advancedEnabled) {
        markup +=
          '<span class="dnnSearchBox_advanced_query"></span><a class="dnnSearchBoxClearAdvanced"></a>';
      }
      markup +=
        '<input id="' +
        this.options.id +
        '_input" type="text" autocomplete="off" aria-label="Search" />' +
      ...
      if (advancedEnabled) {
        markup +=
          ...
          '<a class="dnnSearchBox_advanced_label">' +
          this.options.advancedText +
          "</a>" +
          '<div class="dnnSearchBox_advanced_dropdown">';
        markup += "</div></div>";
      }

      markup += '<div class="dnnClear"></div>' + "</div>";
    ...
    _makeUrl: function (param, service) {
      var url = this.options.previewUrl
        ...
        : service
          ? service.getServiceRoot("internalservices") + "searchService/preview"
          : null;
      if (!url) return null;

      var params = {};
      params[this.options.previewQueryParamName] = param.replace(
        /^\s+|\s+$/g,
        "",
      );
      var urlAppend = [];
      ...
      if (data && data.length) {
        for (var i = 0; i < data.length; i++) {
          ...
          if (group.Results && group.Results.length) {
            ...
            markup +=
              '<li class="dnnSearchBox_preview_group">' + groupTitle + "</li>";
            for (var j = 0; j < group.Results.length; j++) {
              ...
              markup +=
                '<li data-url="' + itemUrl + '"><span>' + itemTitle + "</span>";
              if (itemSnippet) markup += "<p>" + itemSnippet + "</p></li>";
              else markup += "</li>";
            } // end for group items
          }
        } // end for group
        markup +=
          '<li><a href="javascript:void(0)" class="dnnSearchBox_preview_more">' +
          this.options.seemoreText +
          "</a></li>";
        markup += "</ul>";
      } else {
    ...
    _search: function (val) {
      if (
        ...
        typeof this.options.searchFunction == "function"
      ) {
        var params = {};
          ...
          .on("blur", function () {
            ...
            if (!val)
              realInput
                ...
                .addClass("dnnPlaceholder");
            else realInput.removeClass("dnnPlaceholder");
          })
          .blur();
      }
      ...
      $(".dnnSearchButton", this.$wrap).on("click", function () {
        ...
        if (self.options.refreshSearchResult) {
          if (
            ...
            typeof self.options.beforeRefreshSearchResult == "function"
          )
            self.options.beforeRefreshSearchResult();
