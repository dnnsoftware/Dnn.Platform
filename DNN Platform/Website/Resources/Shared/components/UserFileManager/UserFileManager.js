...
(function ($, ko) {
  ...
  $.fn.userFileManager = function (options) {
    var opts = $.extend({}, $.fn.userFileManager.defaultOptions, options),
      $wrap = $(this),
      templateUrl =
        opts.templatePath + opts.templateName + opts.templateExtension;
    ...
    $(opts.openTriggerScope).delegate(
      opts.openTriggerSelector,
      "click",
      function (e) {
        e.preventDefault();
        ...
        getItems(function (result) {
          if ($wrap.data("userFileManager.bound") === true) {
            ...
            ko.applyBindings(
              new fileManagerModel(result),
              document.getElementById($wrap.attr("id")),
            );
