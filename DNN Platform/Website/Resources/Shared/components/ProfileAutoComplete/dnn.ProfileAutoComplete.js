...
function ProfileAutoCompleteService($) {
  var baseServicepath =
    $.dnnSF(-1).getServiceRoot("InternalServices") + "ProfileService/";

  this.searchProfilePropertyValues = function (
    portalId,
    propName,
    searchString,
    success,
...
function initProfileAutoCompleteControls() {
  $('input[data-editor="AutoCompleteControl"]').autocomplete({
    ...
    source: function (request, response) {
      ...
      dnnProfileAutoCompleteService.searchProfilePropertyValues(
        this.element.attr("data-pid"),
