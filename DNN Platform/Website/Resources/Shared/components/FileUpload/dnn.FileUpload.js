...
(function ($, window, document, undefined) {
  ...
  FileUploadPanel.prototype = {
    ...
    init: function () {
      ...
      this._uploadMethods = new dnn.Enum([
    ...
    setUploadPath: function (path) {
      ...
      var folder =
        path == ""
          ? this.options.folderPicker.initialState.selectedItem.value
          : path.split("/").slice(-2)[0];
      this.$this[0].$element
        .find("div.fu-folder-picker-container > span")
        .text(folder);
    },
    ...
    _createLayout: function () {
      ...
      this._folderPicker = new dnn.DropDownList(
        null,
        this.options.folderPicker,
      );
      ...
      var dialog = $element("div", {
        ...
        $element("div", { class: "fu-dialog-content" }).append(
          $element("div", { class: "fu-dialog-content-header" }).append(
            $element("div", { class: "dnnLeft" }).append(
              $element("ul", { class: "dnnButtonGroup" }).append(
                $element("li").append(
                  $element("a", {
                    ...
                    class: "upload-file active",
                  })
                    .text(this.options.resources.uploadFileMethod)
                    .on(
                      "click",
                      $.proxy(
                        this._selectUpload,
                        this,
                        this._uploadMethods.local,
                      ),
                    ),
                ),
              ),
              (this._$decompressOption = $element("span").append(
                ...
                $element("label", {
                  for: checkBoxId,
                  class: "fu-decompress-label",
                }).text(this.options.resources.decompressLabel),
              )),
            ),
            $element("div", {
              class: "fu-folder-picker-container dnnRight",
            }).append(
              $element("label").text(
                this.options.resources.uploadToFolderLabel,
              ),
              !this.options.folderPicker.disabled
          ...
          $element("div", {
            class: "fu-dialog-content-fileupload-local",
          }).append(
            $element("div", { class: "fu-dialog-drag-and-drop-area" }).append(
              $element("div", {
                class: "fu-dialog-drag-and-drop-area-message",
              }),
            ),
          ),
          $element("div", {
            style: "display: none",
            class: "fu-fileupload-statuses-container",
          }).append($element("ul", { class: "fu-fileupload-statuses" })),
        ),
      );
    ...
    _isValidExtension: function (fileName, acceptableExtensions) {
      if (
        typeof acceptableExtensions === "undefined" ||
        acceptableExtensions === null ||
      ...
      for (var i = 0; i < acceptableExtensions.length; i++) {
        if (
          fileName.toLowerCase().endsWith(acceptableExtensions[i].toLowerCase())
        ) {
          return true;
        }
      }
    ...
    _createFileUploadStatusElement: function (status) {
      ...
      var cancelUpload = status.data
        ...
        : function () {};
      var $status = $element("li")
        .append(
          $element("a", {
            href: "javascript:void(0);",
            class: "fu-fileupload-thumbnail",
          }).append(
            $element("div").append(
          ...
          $element("div", { class: "fu-fileupload-filename-container" }).append(
            $element("span", {
              class: "fu-fileupload-filename",
              title: fileName,
            }).text(path + fileName),
          ),
          $element("div", {
            class: "fu-fileupload-progressbar-container",
          }).append(
            $element("div", {
              class: "fu-fileupload-progressbar ui-progressbar",
            }).append(
    ...
    _showFileUploadStatus: function ($fileUploadStatus, state) {
      $fileUploadStatus
        .find(
          ".fu-fileupload-filename-container .fu-file-already-exists-prompt",
        )
        .remove();
      ...
      $fileUploadStatus
        .find(".fu-fileupload-filename-container")
        .append($prompt);
      ...
      if (state.alreadyExists) {
        ...
        var self = this;
        var $keepButton = $element("a", {
          href: "javascript:void(0);",
          class: "fu-file-already-exists-prompt-button-keep",
        })
          .text(this.options.resources.keepButtonText)
          .click(function () {
            $fileUploadStatus
              .removeClass()
              .addClass(self.options.statusCancelledCss);
            self._showFileUploadStatus($fileUploadStatus, {
              message: self.options.resources.uploadStopped,
            });
          });

        var $replaceButton = $element("a", {
          href: "javascript:void(0);",
          class: "fu-file-already-exists-prompt-button-replace",
        })
      ...
      $fileUploadStatus
        .find(".fu-fileupload-progressbar-container")
        .attr("title", message);
    },
    ...
    _add: function (e, data) {
      ...
      if (
        !(
          typeof this.options.maxFiles === "undefined" ||
      ...
      if (data.originalFiles.slice(-1)[0] === data.files[0]) {
        ...
        for (var i = 0; i < count; i++) {
          if (
            !this._isValidExtension(
              data.originalFiles[i].name,
              this.options.extensions,
            )
          ) {
            $.dnnAlert({
              title: this.options.resources.errorDialogTitle || "Error",
              text: this.options.resources.invalidFileExtensions,
            });
            break;
          }
        }
      ...
      if (
        !this._isValidExtension(data.files[0].name, this.options.extensions)
      ) {
        return;
      }
    ...
    _submit: function (e, data) {
      var statusData = this._getFileUploadStatusElement(
        data.files[0].name,
      ).data("status");
      data.formData = {
    ...
    _progress: function (e, data) {
      var $fileUploadStatus = this._getFileUploadStatusElement(
        data.files[0].name,
      );
      if (data.formData.extract) {
        if (
          $fileUploadStatus.find(".fu-dialog-fileupload-extracting").length ===
          0
        ) {
          $fileUploadStatus
            .find(".fu-fileupload-filename")
            .append(
              $element("span", {
                class: "fu-dialog-fileupload-extracting",
              }).text(" - " + this.options.resources.decompressingFile),
            );
        }
        return;
      }
    ...
    _showThumbnail: function ($fileUploadStatus, result) {
      ...
      if (
        this._isValidExtension(result.fileName, [
          ".bmp",
          ".gif",
          ".png",
          ".jpg",
      ...
      path
        ? $link
            .attr({ target: path, href: path })
            .removeClass("fu-fileupload-thumbnail-inactive")
        : $link
            .attr("href", "javascript:void(0);")
            .removeAttr("target")
            .addClass("fu-fileupload-thumbnail-inactive");
    },
    ...
    _getFileUploadStatusElement: function (fileName) {
      ...
      return this._$fileUploadStatuses.children().filter(function (index) {
        return (
          $(this).data("status").fileName == fileName &&
          $(this).data("status").path == path
        );
      });
    },
    ...
    _setProgress: function ($fileUploadStatus, percent) {
      $fileUploadStatus
        .find(".fu-fileupload-progressbar > div")
        .css("width", percent + "%");

      if (percent === 0) {
        $fileUploadStatus
          .find(".fu-fileupload-progressbar")
          .addClass("indeterminate-progress");
      }

      if (percent === 100) {
        $fileUploadStatus
          .find(".fu-fileupload-progressbar")
          .removeClass("indeterminate-progress");
    ...
    _initPanel: function () {
      ...
      this._$buttonGroup = this.$element.find(
        ".fu-dialog-content-header ul.dnnButtonGroup",
      );
      this._$fileUploadStatusesContainer = this.$element
        .find(".fu-fileupload-statuses-container")
        .hide()
        .jScrollbar();
      this._$fileUploadStatuses = this._$fileUploadStatusesContainer
        .find(".fu-fileupload-statuses")
        .empty();
      this._$dragAndDropArea = this.$element.find(
        ".fu-dialog-drag-and-drop-area",
      );
      this._$inputFileControl = $element("input", {
        type: "file",
        name: "postfile",
        multiple: isMultiple,
        "data-text": this.options.resources.dragAndDropAreaTitle,
      });
      this._$inputFileControl.attr("aria-label", "File");
      this._$extract = this.$element
        .find("." + "fu-dialog-content-header")
        .find("input");

      this._$inputFileControl
        .appendTo(
          this._$dragAndDropArea.find(".fu-dialog-drag-and-drop-area-message"),
        )
        .dnnFileInput({
    ...
    _uploadFromLocalUrl: function () {
      var serviceUrl = $.dnnSF(this.options.moduleId).getServiceRoot(
      ...
      if (!suportAjaxUpload()) {
        ...
        webResource
          .parameters()
          .set("__RequestVerificationToken", antiForgeryToken);
      }
