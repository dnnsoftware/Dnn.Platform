...
(function ($, window, document, undefined) {
  ...
  // DropDownList
  //
  var DropDownList = (this.DropDownList = function (
    element,
    options,
    controller,
  ) {
  ...
  DropDownList.prototype = {
    ...
    init: function () {
      ...
      this._$selectedItemContainer = this.$element.find(
        "." + this.options.selectedItemCss,
      );
      this._$selectedItemCaption = this._$selectedItemContainer.find(
        "." + this.options.selectedValueCss,
      );

      this._onOpenItemListHandler = $.proxy(this._onOpenItemList, this);
      this._onCloseItemListHandler = $.proxy(this._onCloseItemList, this);
      this._onSelectedItemCaptionClickHandler = $.proxy(
        this._onSelectedItemCaptionClick,
        this,
      );

      this._onDocumentClickHandler = $.proxy(this._onDocumentClick, this);

      this._$selectedItemCaption.on(
        "click",
        this._onSelectedItemCaptionClickHandler,
      );

      this._state = new StateField(
        this.options.initialState,
        this.options.internalStateFieldId,
      );
    ...
    _createLayout: function () {
      var layout = $(
        "<div id='" +
          (this.options.containerId || dnn.uid()) +
          "' class='" +
          this.options.containerCss +
          "'/>",
      ).append(
        $("<div class='" + this.options.selectedItemCss + "'/>").append(
          $(
    ...
    disabled: function (disable) {
      if (typeof disable !== "undefined") {
        if (disable) {
          ...
          this._$selectedItemCaption
            .prop("href", "javascript:void(0);")
            .prop("title", this._selectedItemTitle || "");
        }
    ...
    _setSelectedItemCaption: function (item) {
      ...
      this._$selectedItemCaption.text(
        caption || this.options.selectItemDefaultText,
      );
    },
    ...
    _openItemList: function () {
      ...
      this._$treeViewLayout.slideDown(
        this.options.openItemListSpeed,
        this._onOpenItemListHandler,
      );
    },
    ...
    _closeItemList: function () {
      this._$treeViewLayout.slideUp(
        this.options.closeItemListSpeed,
        this._onCloseItemListHandler,
      );
    },
    ...
    _onDocumentClick: function (eventObject) {
      ...
      if (
        !$.inContainer(this.$element, clickedElement) &&
        this._isItemListOpen()
      ) {
        this._closeItemList();
      }
    },
    ...
    _createTreeView: function () {
      var controller = this._controller
        ? this._controller
        : new dnn.DynamicTreeViewController(this.options.services);
      var treeView = new dnn.SortableTreeView(
        null,
      ...
      $(treeView)
        .on("onchangenode", onChangeNodeHandler)
    ...
    _onTreeLoaded: function () {
      ...
      if (expandPath && expandPath.length > 0) {
        ...
        if (node) {
          var expandIcon = dynamicTree._tree
            .getNodeElement(node)
            .children(":first");
    ...
    _onChangeNode: function (eventObject, node) {
      ...
      if (
        ...
        this.options.onSelectionChanged.length
      ) {
        for (
          var i = 0, size = this.options.onSelectionChanged.length;
          i < size;
          i++
        ) {
          dnn.executeFunctionByName(
            this.options.onSelectionChanged[i],
            window,
            item,
    ...
    val: function (stateObject) {
      if (typeof stateObject === "undefined") {
        ...
        if (typeof this._state === "undefined") {
          try {
            this._state =
              this._stateElement && this._stateElement.value
                ? JSON.parse(this._stateElement.value)
                : null;
          } catch (ex) {}
        }
        return this._state;
