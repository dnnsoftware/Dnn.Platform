...
function CountryRegionService($) {
  var baseServicepath =
    $.dnnSF(-1).getServiceRoot("InternalServices") + "CountryRegion/";
...
function setupRegionLists() {
  $('div[data-list="Region"]').each(function (index, value) {
    var stringValue = $(value)
      .children('input[data-editor="DNNRegionEditControl_Hidden"]')
      .val();
    var $row = $(value);
    var $label = $row.prev().find("label");
    var $textControl = $row.children(
      'input[data-editor="DNNRegionEditControl_Text"]',
    );
    ...
    if ($selectControl.children("option").length < 2) {
      ...
      $textControl.hide();
      $selectControl
        .children('option[value="' + stringValue + '"]')
        .prop("selected", true);
...
function loadRegionList(category, country) {
  var selector = category
    ? 'div[data-list="Region"][data-category="' + category + '"]'
    : 'div[data-list="Region"]';
  $(selector).each(function (index, value) {
    var dd = $(value).children("select");
    $(dd).children().not(":first").remove();
    if (country != "") {
      if (dnnCountryRegionService == undefined) {
        dnnCountryRegionService = new CountryRegionService($);
      }
      dnnCountryRegionService.listRegions(country, function (data) {
        $.each(data, function (index, value) {
          $(dd).append(
            $("<option>").text(value.Text).attr("value", value.Value),
          );
        });
        setupRegionLists();
      });
