(function ($) {
  $.fn.dnnControlPanel = function (options) {
    ...
    return $wrap.each(function () {
      ...
      if ($('[id$="CommonTasksPanel"]').length > 0)
        $('[id$="CommonTasksPanel"]')
          .detach()
          .appendTo("#dnnCommonTasks .megaborder");
      else $("#dnnCommonTasks").remove();
      $('[id$="CurrentPagePanel"]')
        .detach()
        .appendTo("#dnnCurrentPage .megaborder");
      if ($('[id$="AdminPanel"]').length > 0)
      ...
      $controlPanel
        .find(opts.menuBorderSelector)
        .find(opts.primaryActionSelector)
        .css({ opacity: 1 }); //Compact IE7
      ...
      $controlPanel.on(
        "mouseover",
        opts.persistOnMouseoverSelector,
        function () {
          canHide = false;
        },
      );
      ...
      $controlPanel.on(
        "click",
        opts.persistOnWaitForChangeSelector,
        function () {
          forceShow = true;
          canHide = false;
  ...
  $.fn.dnnControlPanel.defaultOptions = {
    wrappingHtml:
      '<div id="dnnCPWrap"><div id="dnnCPWrapLeft"><div id="dnnCPWrapRight"></div></div></div>',
