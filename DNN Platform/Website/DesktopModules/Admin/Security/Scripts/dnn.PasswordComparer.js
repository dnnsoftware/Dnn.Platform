if (typeof dnn === "undefined" || dnn === null) {
  dnn = {};
} //var dnn = dnn || {}; IE 8 doesn't respect var
...
(function ($, window, document, undefined) {
  ...
  InputComparer.prototype = {
    ...
    init: function () {
      ...
      this._$firstElement = this.$element.find(
        this.options.firstElementSelector,
      );
      if (this._$firstElement.length === 0) {
        return false;
      }
      this._$secondElement = this.$element.find(
        this.options.secondElementSelector,
      );
      if (this._$secondElement.length === 0) {
        return false;
      }
      this._focusOutHandler = $.proxy(this._onFocusOut, this);
      this._$firstElement.on(
        "focusout change keyup paste input propertychange",
        this._focusOutHandler,
      );
      this._$secondElement.on(
        "focusout change keyup paste input propertychange",
        this._focusOutHandler,
      );
    ...
    _onFocusOut: function (eventObject) {
      if (
        eventObject.type === "propertychange" &&
        eventObject.originalEvent.propertyName.toLowerCase() !== "value"
      ) {
        return;
      }
  ...
  PasswordComparer.prototype = {
    ...
    init: function () {
      ...
      this._$confirmElement = this._$container.find(
        this.options.secondElementSelector,
      );
    ...
    _getTooltipMarkup: function (element) {
      if (!this._$tooltipContent) {
        this._$tooltipContent = $("<label/>").addClass(
    ...
    _updateTooltipState: function (isMatched) {
      ...
      var text = isMatched
        ? this.options.confirmPasswordMatchedText
        : this.options.confirmPasswordUnmatchedText;
...
dnn.initializePasswordComparer = function (options) {
  $(document).ready(function () {
    var instance = new dnn.PasswordComparer(options);
  });
};
