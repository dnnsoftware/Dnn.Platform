(function ($, dnn) {
  ...
  dnn.searchResult.queryUrl = function () {
    ...
    if (!dnn.searchResult.service) return null;
  ...
  dnn.searchResult.renderResult = function (data, renderUrl) {
    ...
    if (showDescription && data.Description) {
      ...
      var maxDescriptionLength =
        dnn.searchResult.defaultSettings.maxDescriptionLength;
    ...
    if (showSource) {
      markup +=
        "&nbsp;&nbsp;&nbsp;<span>" +
        dnn.searchResult.defaultSettings.sourceText +
        " </span>";
      markup +=
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        if (result.Results.length > 1) {
          ...
          markup +=
            ...
            dnn.searchResult.renderResult(result.Results[0], true) +
            "</div>";
        }
      }
    } else {
  ...
  dnn.searchResult.addLoading = function () {
    ...
    var h = resultContainer.outerHeight(),
  ...
  dnn.searchResult.init = function (settings) {
    ...
    $("#dnnSearchResultAdvancedSearch").on("click", function (e, isTrigger) {
      ...
      if (scopeList.length === scopeCtrl.get_options().length) scopeList = [];
    ...
    setTimeout(function () {
      $("#dnnSearchResultAdvancedSearch").trigger("click", [true]);
    }, 0);
  };
})(jQuery, dnn);
