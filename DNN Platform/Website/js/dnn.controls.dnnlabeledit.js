...
dnn.extend(dnn.controls, {
  initLabelEdit: function (a) {
    if (a && dnn.controls.controls[a.id] == null) {
      var b = new dnn.controls.DNNLabelEdit(a);
      b.initialize();
      return b;
    }
  },
...
dnn.controls.DNNLabelEdit = function (o) {
  ...
  if (
    dnn.dom.browser.isType(dnn.dom.browser.Safari) ||
    dnn.dom.browser.isType(dnn.dom.browser.Opera)
  ) {
    this.supportsCE = false;
  }
  this.supportsRichText =
    this.supportsCE ||
    (dnn.dom.browser.isType(dnn.dom.browser.Mozilla) &&
      navigator.productSub >= "20050111");
  if (this.eventName != "none") {
    this.addHandlers(
      o,
      this.getDynamicEventObject(
        this._getEventName(this.eventName),
        this.performEdit,
      ),
      this,
    );
  ...
  this.addHandlers(
    o,
    { mousemove: this.mouseMove, mouseout: this.mouseOut },
    this,
  );
...
dnn.controls.DNNLabelEdit.prototype = {
  ...
  initToolbar: function () {
    if (this.toolbar == null) {
      var a = dnn.dom.scriptStatus("dnn.controls.dnntoolbar.js");
      if (a == "complete") {
        this.toolbar = new dnn.controls.DNNToolBar(this.ns);
        this.toolbar.loadDefinition(
          this.toolbarId,
          this.nsPrefix,
          this.container,
          this.container.parentNode,
          this.container,
          this._toolbarActionDelegate,
        );
        this.handleToolbarDisplay();
      } else {
        if (a == "") {
          dnn.dom.loadScript(
            dnn.dom.getScriptPath() + "dnn.controls.dnntoolbar.js",
            "",
            this._initToolbarDelegate,
          );
        }
      }
    }
  },
  toolbarAction: function (a, d) {
    var c = a.clickAction;
    if (c == "edit") {
      this.performEdit();
    } else {
        ...
        if (c == "cancel") {
          ...
          if (this.isFormatButton(c)) {
            if (this.editWrapper) {
              ...
              this.editWrapper.execCommand(c, null, b);
            }
          }
        }
      }
  ...
  performEdit: function () {
    ...
    if (this.editContainer != null) {
      ...
      this.invoke_handler("beforeEdit", a);
      if (a.get_cancel()) {
        this.cancelEdit();
        return;
      }
  ...
  initEditWrapper: function () {
    if (this.editWrapper == null) {
      var e = this.richTextEnabled && this.supportsRichText;
      var c = e
        ? "dnn.controls.dnnrichtext.js"
        : "dnn.controls.dnninputtext.js";
      var b = dnn.dom.scriptStatus(c);
      if (b == "complete") {
        var a;
        if (this.richTextEnabled && this.supportsRichText) {
          var d = dnn.dom.getObjMethRef(this, "initEditControl");
          a = new dnn.controls.DNNRichText(d);
        } else {
          a = new dnn.controls.DNNInputText(this.multiLineEnabled);
        }
        this.editWrapper = a;
        ...
        if (this.richTextEnabled && this.supportsCE) {
          this.initEditControl();
        }
      } else {
        if (b == "") {
          dnn.dom.loadScript(
            dnn.dom.getScriptPath() + c,
            "",
            this._performEditDelegate,
          );
        }
      }
    }
  ...
  initEditControl: function () {
    if (this.editWrapper.initialized) {
      ...
      this.editControl.className =
        this.container.className + " " + this.cssEdit;
      this.editWrapper.focus();
      if (this.editWrapper.loaded == false) {
        ...
        if (this.onblurSave) {
          a.blur = this.persistEdit;
        }
        if (
          this.editWrapper.supportsCE ||
          this.editWrapper.isRichText == false
        ) {
          this.addHandlers(this.editControl, a, this);
        } else {
          this.addHandlers(this.editContainer.contentWindow.document, a, this);
        }
  ...
  persistEdit: function () {
    if (this.editWrapper.getText() != this.prevText) {
      if (this.invoke_compatHandler("beforeSave", null, this)) {
        this.editControl.className =
          this.container.className + " " + this.cssWork;
        eval(
          this.callBack.replace(
            "[TEXT]",
            dnn.escapeForEval(this.editWrapper.getText()),
          ),
        );
      }
    } else {
  ...
  callBackStatus: function (a, b, c) {
    var d = b;
    d.invoke_compatHandler("callBackStatus", a, b, c);
  },
  callBackSuccess: function (a, b, c) {
    b.callBackStatus(a, b);
    b.invoke_handler(
      "callBackSuccess",
      new dnn.controls.DNNCallbackEventArgs(a, b, c),
    );
    b.showLabel();
    b.flashLabel();
  },
  ...
  handleToolbarDisplay: function () {
    if (this.toolbar) {
      var c = this.isEditMode();
      var b;
      for (var a in this.toolbar.buttons) {
        b = this.toolbar.buttons[a];
        ...
        if (a == "edit") {
          ...
          if (this.isFormatButton(a)) {
            b.visible = c && this.editWrapper && this.editWrapper.isRichText;
          } else {
            b.visible = c;
          }
        }
  ...
  isFormatButton: function (a) {
    return (
      "~bold~italic~underline~justifyleft~justifycenter~justifyright~insertorderedlist~insertunorderedlist~outdent~indent~createlink~".indexOf(
        "~" + a + "~",
      ) > -1
    );
  },
  ...
  flashLabel: function () {
    ...
    setTimeout(function () {
      a.container.style.backgroundColor = "#fffff0";
      setTimeout(function () {
        a.container.style.backgroundColor = "transparent";
      }, 300);
    }, 2500);
  },
  callBackFail: function (a, b, c) {
    b.invoke_handler(
      "callBackFail",
      new dnn.controls.DNNCallbackEventArgs(a, b, c),
    );
  ...
  handleKeyPress: function (a) {
    if (
      a.charCode == KEY_RETURN &&
      this.editWrapper.supportsMultiLine == false
    ) {
      if (this.saveOnEnter) {
        this.persistEdit();
      }
...
dnn.controls.DNNLabelEdit.registerClass(
  "dnn.controls.DNNLabelEdit",
