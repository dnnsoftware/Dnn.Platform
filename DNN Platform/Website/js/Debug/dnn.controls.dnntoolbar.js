...
dnn.controls.DNNToolBar.prototype = {
  loadDefinition: function (
    ...
    if (def == null)
      //try and load with xhtmlconformance off
      def = dnn.controls.toolbars[nsPrefix + "$" + toolbarID];
    if (def == null)
      //try and load global toolbar
  ...
  addButton: function (
    ...
    if (key == null) key = this.buttons.length;
    if (this.cssButton) css = this.cssButton + " " + css;
    if (this.cssButtonHover) hoverCss = this.cssButtonHover + " " + hoverCss;

    this.buttons[key] = new dnn.controls.DNNToolBarButton(
      key,
      clickAct,
      css,
      hoverCss,
...
dnn.controls.DNNToolBarButton = function (
  key,
  clickAct,
  css,
  hoverCss,
...
dnn.controls.DNNToolBarButton.prototype = {
  render: function () {
    if (!this.ctl) {
      ...
      this.ctl.className = this.css;
      if (this.tooltip) this.ctl.title = this.tooltip;
      ...
      bar.addHandlers(
        this.ctl,
        bar.getDynamicEventObject(clickEvent, bar.buttonClick),
        this,
      );
      bar.addHandlers(
        this.ctl,
        {
          mouseover: bar.buttonMouseOver,
          mouseout: bar.buttonMouseOut,
        },
        this,
      );
    }
  ...
  hover: function () {
    if (this.hoverCss) this.ctl.className = this.css + " " + this.hoverCss;
  },

  unhover: function () {
    if (this.hoverCss) this.ctl.className = this.css;
  },

  click: function () {
    if (this.clickAction == "js") eval(this.js);
    else if (this.clickAction == "navigate") dnn.dom.navigate(this.url);
