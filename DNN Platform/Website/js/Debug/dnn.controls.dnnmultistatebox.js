...
dnn.controls.DNNMultiStateBox = function (o) {
  ...
  this.addHandlers(
    this._img,
    { click: this.click, keypress: this.keypress },
    this,
  );
};
...
dnn.controls.DNNMultiStateBox.prototype = {
  ...
  set_Value: function (value) {
    ...
    if (this.enabled) this._img.src = this.imgPath + state.ImageUrl;
    else this._img.src = this.imgPath + state.DisabledImageUrl;
    this._img.alt = state.ToolTip;
    if (this._label) this._label.innerHTML = state.Text;
  ...
  click: function (sender, args) {
    if (this.enabled) {
      ...
      if (index > this.states.length - 1) index = 0;
...
dnn.controls.DNNMultiStateBox.registerClass(
  "dnn.controls.DNNMultiStateBox",
