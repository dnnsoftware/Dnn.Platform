...
dnn.controls.DNNMenu.prototype = {
  ...
  renderNode: function (node, ctr) {
    ...
    if (menuBuilder.alreadyRendered == false) {
      ...
      if (this.useTables == false || mNode.level == 0)
        ...
        menuBuilder.subcont.className =
          "icn " + (mNode.cssIcon.length > 0 ? mNode.cssIcon : this.cssIcon); //assign style to container of icon
    ...
    if (mNode.hasNodes || mNode.hasPendingNodes) {
      ...
      ); //todo: preload spacer so it works on first try    -don't show first time
    ...
    if (mNode.hasNodes || mNode.hasPendingNodes) {
      ...
      if (imgSrc.trim().length > 0) {
        if (this.useTables && mNode.level > 0) {
          //do not require tables to need special padding to properly show arrow, place a real image there and have browser space it appropriately
  ...
  selectNode: function (mNode) {
    ...
    if (mNode.hasNodes || mNode.hasPendingNodes) this.expandNode(mNode, true); //force display
    ...
    if (mNode.selected) {
      ...
      if (sJS.length > 0) {
        ...
        if (eval(sJS) == false) return; //don't do postback if returns false
      }
  ...
  __getNextNode: function (mNode, iDir) {
    ...
    if (nodeIndex + iDir < 0)
      //if first node was selected and going left, select last node
    ...
    else node = parentNode.nodes[nodeIndex + iDir]; //node = parentNode.childNodes(nodeIndex + iDir);
...
dnn.controls.DNNMenuBuilder.prototype = {
  ...
  newCont: function () {
    ...
    else this.row = this.menu.createChildControl("span", this.mNode.id, "ctr"); //container for Node
