(function (window, $) {
  ...
  window.dnnModal = {
      ...
      try {
        if (parent.location.href !== undefined) {
          ...
          if (typeof parentTop.$find != "undefined") {
            if (
              location.href.indexOf("popUp") == -1 ||
              windowTop.location.href.indexOf("popUp") > -1
            ) {
              ...
              windowTop
                .jQuery("#iPopUp")
                .dialog({ autoOpen: false, title: document.title });
            }
          }
        }
        return false;
    ...
    show: function (url, showReturn, height, width, refresh, closingUrl) {
      ...
      var dialogOpened = function () {
        $modal.on("load", function () {
          ...
          var isSameDomain =
            currentHost === iframeHost && currentPort === iframePort;
      ...
      if (!isMobile) {
        ...
        if (
          $modal.parent().find(".ui-dialog-title").next("a.dnnModalCtrl")
            .length === 0
        ) {
          ...
          $dnnToggleMax.click(function (e) {
            ...
            if ($("button.ui-dialog-titlebar-close").length) {
              closeButtonHeightCorrection = $("button.ui-dialog-titlebar-close")
                .parent(".dnnModalCtrl")
                .height();
              closeButtonWidthCorrection = $("button.ui-dialog-titlebar-close")
                .parent(".dnnModalCtrl")
                .width();
            }
            ...
            if ($modal.data("isMaximized")) {
              var appliedHeight =
                $modal.data("height") + JQUERY_UI_HEIGHT_SHRINK_OFFSET;
              ...
              $modal
                .data("height", $modal.dialog("option", "minHeight"))
              ...
              newWidth =
                $window.outerWidth() -
                personaBarIFrameWidth -
                closeButtonWidthCorrection / 7.5 -
                40;
              ...
              horizontalPosition =
                "right-" + closeButtonWidthCorrection / 5.5 + " center";
              verticalPosition =
                "right center-" + closeButtonHeightCorrection / 11;
        ...
        $modal.dialog({
          ...
          close: function () {
            $("#Form").show();
            if (originalHeightCss) $("body").css("height", originalHeightCss);
            window.scrollTo(0, 0);
            window.dnnModal.closePopUp(refresh, closingUrl);
          },
        });
    ...
    refreshPopup: function (options) {
      ...
      if (
        windowTop.location.href !== windowTopTop.location.href &&
        windowTop.location.href !== options.url
      ) {
        windowTopTop.dnnModal.show(
          options.url,
          options.showReturn,
          options.height,
          options.width,
