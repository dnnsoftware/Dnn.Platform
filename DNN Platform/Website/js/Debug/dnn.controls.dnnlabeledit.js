...
dnn.controls.DNNLabelEdit = function (o) {
  ...
  this.supportsRichText =
    ...
    (dnn.dom.browser.isType(dnn.dom.browser.Mozilla) &&
      navigator.productSub >= "20050111"); //i belive firefox only works well with 1.5 or later, need a better way to detect this!
...
dnn.controls.DNNLabelEdit.prototype = {
  ...
  toolbarAction: function (btn, src) {
    ...
    if (action == "edit") this.performEdit();
    else if (action == "save") {
      this.persistEdit();
      this.toolbar.hide();
      ...
      if (this.editWrapper) {
        ...
        if (
          action == "createlink" &&
          dnn.dom.browser.isType(dnn.dom.browser.InternetExplorer) == false
        )
  ...
  performEdit: function () {
    ...
    if (this.editContainer != null) {
      ...
      if (
        ...
        //safari gets strange chars... use innerText
  ...
  showToolBar: function () {
    this.initToolbar();
    if (this.toolbar) this.toolbar.show(false);
  },
  ...
  mouseOut: function () {
    if (this.toolbar) this.toolbar.beginHide();
  ...
  initEditWrapper: function () {
    if (this.editWrapper == null) {
      ...
      if (status == "complete") {
        ...
        } else txt = new dnn.controls.DNNInputText(this.multiLineEnabled);
        ...
        if (this.richTextEnabled && this.supportsCE)
          //control is instantly available (not an iframe)
          this.initEditControl();
      } else if (status == "")
        //not loaded
        dnn.dom.loadScript(
          dnn.dom.getScriptPath() + script,
          "",
  ...
  initEditControl: function () {
    if (this.editWrapper.initialized) {
      ...
      if (this.editWrapper.loaded == false) {
        ...
        if (this.onblurSave) eventObj.blur = this.persistEdit;

        if (this.editWrapper.supportsCE || this.editWrapper.isRichText == false)
          //if browser supports contentEditable or is a simple INPUT control
          this.addHandlers(this.editControl, eventObj, this); //IFRAME event handlers
        else
          this.addHandlers(
            this.editContainer.contentWindow.document,
            eventObj,
            this,
          );
    ...
    if (this.toolbar) {
      ...
      for (var key in this.toolbar.buttons) {
        ...
        if (typeof btn == "function")
          //safeguard against other js frameworks
          continue;
        ...
        if (key == "edit") {
          ...
          btn.visible =
            inEdit && this.editWrapper && this.editWrapper.isRichText;
        else btn.visible = inEdit;
      }
  ...
  handleKeyPress: function (e) {
    if (
      e.charCode == KEY_RETURN &&
      this.editWrapper.supportsMultiLine == false
    ) {
      if (this.saveOnEnter) this.persistEdit();
      e.preventDefault();
