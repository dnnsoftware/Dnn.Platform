...
dnn.extend(dnn, {
  ...
  getVars: function () {
    ...
    if (this.vars == null) {
      ...
      if (ctl != null && ctl.value.length > 0)
        this.vars = Sys.Serialization.JavaScriptSerializer.deserialize(
          ctl.value,
        );
      else this.vars = [];
    }
  ...
  setVar: function (key, val) {
    ...
    else dnn._delayedSet = { key: key, val: val }; //doesn't matter how many times this gets overwritten, we just want one value to set after load so serialize is called
    return true;
  },
  ...
  doDelay: function (key, time, ptr, ctx) {
    ...
    if (this.delay[key] == null) {
      ...
      this.delay[key].num = window.setTimeout(
        dnn.createDelegate(this.delay[key], this.delay[key].complete),
        time,
      );
    }
  },
  ...
  encode: function (arg, doubleEncode) {
    ...
    /// <returns type="String" />
    var ret = arg;
    if (encodeURIComponent) ret = encodeURIComponent(ret);
    else ret = escape(ret);

    if (doubleEncode == false) return ret;
  ...
  _onload: function () {
    dnn.isLoaded = true;
    if (dnn._delayedSet) dnn.setVar(dnn._delayedSet.key, dnn._delayedSet.val);
  },

  addIframeMask: function (ele) {
    //add an iframe behind the element, so that element will not mask by some special objects.
    if (
      dnn.dom.browser.isType("ie") &&
      (ele.previousSibling == null ||
        ele.previousSibling.nodeName.toLowerCase() != "iframe")
    ) {
...
dnn.ScriptRequest = function (src, text, fCallBack) {
  ...
  if (src != null && src.length > 0) {
    ...
    if (embedSrc.length > 0) this.src = embedSrc;
    else this.src = src;
  }
...
dnn.ScriptRequest.prototype = {
  load: function () {
    ...
    } else alert("Cannot load dynamic script, no HEAD tag present.");

    if (this.src == null || dnn.dom.browser.isType(dnn.dom.browser.Opera))
      //opera loads script synchronously
      this.complete();
  ...
  complete: function () {
    ...
    this.status = "complete";
    if (typeof this.callBack != "undefined") this.callBack(this);
    this.dispose();
  },
  ...
  dispose: function () {
    ...
    if (this.ctl) {
      ...
      else if (this.ctl.onload) this.ctl.onload = null;
      this.ctl = null;
    }
    this.xmlhttp = null;
...
dnn.extend(dnn.dom, {
  ...
  attachEvent: function (ctl, type, fHandler) {
    ...
    if (ctl.addEventListener) {
      ...
      ctl.addEventListener(
        name,
        function (evt) {
          dnn.dom.event = new dnn.dom.eventObject(evt, evt.target);
          return fHandler();
        },
        false,
      );
    } else
      ctl.attachEvent(type, function () {
  ...
  cursorPos: function (ctl) {
    ...
    if (ctl.value.length == 0) return 0;
    ...
    if (ctl.selectionStart)
      ...
      if (
        range == null ||
        sel == null ||
        (sel.text != "" && range.inRange(sel) == false)
      )
        return -1;

      if (sel.text == "") {
        if (range.boundingLeft == sel.boundingLeft) pos = 0;
        else {
          ...
          if (tagName == "input") {
            ...
            while (i < text.length) {
              ...
              if (range.boundingLeft == sel.boundingLeft) break;
          ...
          // Handle text areas.
            ...
            while (
              oCaret.parentElement() == ctl &&
              oCaret.move("character", 1) == 1
            )
              --i;

            if (i == ctl.value.length + 1) i = -1;
          }
          pos = i;
        }
      } else pos = range.text.indexOf(sel.text);
    }
    return pos;
  },
  ...
  disableTextSelect: function (ctl) {
    if (typeof ctl.onselectstart != "undefined")
      ...
      //ff
      ctl.style.MozUserSelect = "none"; //others
    else
      ctl.onmousedown = function () {
        return false;
      };
  },
  ...
  getAttr: function (node, attr, def) {
    ...
    /// </param>
    /// <returns type="String" />
    if (node.getAttribute == null) return def;
  ...
  getByTagName: function (tag, ctl) {
    ...
    /// </param>
    /// <returns type="Array" />
    if (ctl == null) ctl = document;
    if (ctl.getElementsByTagName) return ctl.getElementsByTagName(tag);
    else if (ctl.all && ctl.all.tags) return ctl.all.tags(tag);
    else return null;
  },
  ...
  getParentByTagName: function (ctl, tag) {
    ...
    tag = tag.toLowerCase();
    while (parent != null) {
      if (parent.tagName && parent.tagName.toLowerCase() == tag) return parent;
  ...
  getNonTextNode: function (node) {
    ...
    /// </param>
    /// <returns type="Object" />
    if (this.isNonTextNode(node)) return node;
  ...
  addSafeHandler: function (ctl, evt, obj, method) {
    ...
    if (dnn.dom.browser.isType(dnn.dom.browser.InternetExplorer)) {
      ...
      this.__leakEvts[this.__leakEvts.length] = new dnn.dom.leakEvt(
        evt,
        ctl,
        ctl[evt],
      );
  ...
  loadScript: function (src, text, callBack) {
    ...
    if (sFile) this.scripts[sFile] = oSR;
    oSR.load();
    return oSR;
  },
  ...
  loadScripts: function (aSrc, aText, callBack) {
    if (dnn.scripts == null) {
      var oRef = function (aSrc, aText, callBack) {
        //closure to invoke self with same params when done
        return function () {
          dnn.dom.loadScripts(aSrc, aText, callBack);
        };
      };
      dnn.dom.loadScript(
  ...
  scriptStatus: function (src) {
    ...
    if (this.scripts[sFile]) return this.scripts[sFile].status; //dynamic load

    var oScript = this.getScript(src);
    if (oScript != null)
      //not a dynamic load, must be complete if found
      return "complete";
    else return "";
  },

  setScriptLoaded: function (src) {
    //called by pages js that is dynamically loaded.  Needed since Safari doesn't support onload for script elements
    var sFile = this.scriptFile(src);
  ...
  navigate: function (sURL, sTarget) {
    if (sTarget != null && sTarget.length > 0) {
      if (sTarget == "_blank" || sTarget == "_new")
        //todo: handle more
        window.open(sURL);
      else document.frames[sTarget].location.href = sURL;
    } else {
  ...
  getElementPostString: function (ctl) {
    var tagName;
    if (ctl.tagName) tagName = ctl.tagName.toLowerCase();
    ...
    if (tagName == "input") {
      ...
      if (
        ...
        type == "password" ||
        type == "hidden" ||
        ((type == "checkbox" || type == "radio") && ctl.checked)
      )
...
dnn.dom.browserObject = function () {
  ...
  if (agt.indexOf("konqueror") != -1) type = this.Konqueror;
  ...
  else if (Sys.Browser.agent === Sys.Browser.Safari) type = this.Safari;
  else if (Sys.Browser.agent === Sys.Browser.Opera) type = this.Opera;
  else type = this.Mozilla;
...
dnn.dom.browserObject.prototype = {
  ...
  isType: function () {
    for (var i = 0; i < arguments.length; i++) {
      if (dnn.dom.browser.type == arguments[i]) return true;
    }
    return false;
  },
};
