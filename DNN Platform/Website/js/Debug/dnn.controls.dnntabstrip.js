...
dnn.controls.DNNTabStrip = function (o) {
  ...
  for (var i = 0; i < this.tabData.length; i++) {
    ...
    tab.selected =
      tab.container != null && tab.container.style.display != "none"; //initial assignment of selected if container exists, otherwise showTab handles this property
...
*/
  ...
  tabClick: function (evt, element) {
    ...
    if (tab.enabled) {
      ...
      if (arg.get_cancel() == false) ts.showTab(tab.tabId);
    }
  },
    ...
    if (this.needsLookup(tabId)) {
      if (this.pendingLookupId == null) {
        ...
        eval(
          this.callBack
            .replace("[TABID]", tabId)
            .replace("'[POST]'", tab.postMode)
            .replace("[CBTYPE]", tab.callbackType),
        );
      }
    } else {
      for (var i = 0; i < this.tabIds.length; i++) {
        ...
        if (tab.tabId == tabId) {
          ...
          if (tab.container != null) tab.container.style.display = "none";
          tab.selected = false;
          tab.assignCss();
        }
      }
  ...
  callBackSuccess: function (result, ctx, req) {
    ...
    for (var key in vars) dnn.setVar(key, vars[key]);
    ...
    if (ret.scripts != null) {
      ...
      for (var s in scripts) {
        ...
        } else inlineScripts.push(scripts[s].__text);
      }
      dnn.dom.loadScripts(
        refScripts,
        inlineScripts,
        dnn.createDelegate(ts, ts.callBackScriptComplete),
      );
    } else ts.callBackScriptComplete();
  },
  ...
  _getLabelData: function (index, prop, def) {
    ...
    if (data[prop] != null && data[prop].length > 0) return data[prop];
    else return def;
  },
...
dnn.controls.DNNTabStrip.registerClass(
  "dnn.controls.DNNTabStrip",
...
dnn.controls.DNNTab = function (strip, index, id, clientId) {
  ...
  if (tab != null) {
    ...
    this.cssHover = this.strip._getLabelData(
      index,
      "csshover",
      strip.lblCssHover,
    );
    this.cssDisabled = this.strip._getLabelData(
      index,
      "cssdisabled",
      strip.lblCssDisabled,
    );
    this.postMode = this.strip._getTabData(index, "postmode", null);
    this.enabled = this.strip._getTabData(index, "enabled", "1") == "1";
    this.callbackType = this.strip._getTabData(
      index,
      "cbtype",
      strip.callbackType,
    );
    if (this.postMode) this.postMode = "'" + this.postMode + "'";
  }
};
...
dnn.controls.DNNTab.prototype = {
  ...
  showWork: function (show) {
    if (this.work != null) {
      if (show) {
        ...
        if (this.icon != null) this.icon.style.display = "none";
      } else {
        this.work.style.display = "none";
        if (this.icon != null) this.icon.style.display = "";
      }
    }
  },
  enable: function () {
  ...
  assignCss: function () {
    ...
    if (this.enabled == false && this.cssDisabled.length > 0)
      ...
      if (this.hovered && this.cssHover.length > 0) sCss = this.cssHover;
      else sCss = this.selected ? this.cssSel : this.css;
    }
