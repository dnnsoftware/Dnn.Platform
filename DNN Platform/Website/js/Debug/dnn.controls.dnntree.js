...
dnn.controls.DNNTree = function (o) {
  ...
  if (this.indentWidth == 0) this.indentWidth = 10;
  ...
  this.addHandlers(
    kbCtl,
    { keydown: this.keydownHandler, focus: this.focusHandler },
    this,
  );
...
dnn.controls.DNNTree.prototype = {
  ...
  keydownHandler: function (e) {
    ...
    if (dir != 0) {
      ...
      if (axis == "x") {
        if (dir == -1) {
          if (tNode.hasNodes && tNode.expanded) this.collapseNode(tNode);
          else node = tNode.node.parentNode();
        }

        if (dir == 1) {
          if (tNode.hasNodes || tNode.hasPendingNodes) {
            if (tNode.expanded != true) this.expandNode(tNode);
            else node = tNode.node.childNodes(0);
          }
        }
      } else if (axis == "y") {
        var iNodeIndex = tNode.node.getNodeIndex("id");
        var parentNode = tNode.node.parentNode();
        if (tNode.hasNodes && tNode.expanded && dir > 0)
          //if has expanded nodes and going down, select first child
          node = tNode.node.childNodes(0);
        else if (iNodeIndex + dir < 0)
          //if first node was selected and going up, select parent
          node = parentNode;
        else if (iNodeIndex + dir < parentNode.childNodeCount()) {
          //if navigated index less than number of nodes contained in parent
          node = parentNode.childNodes(iNodeIndex + dir); //navigate there
          if (dir == -1) {
            //if going up... look for expanded sibling above (recursively)
            var tNode2 = new dnn.controls.DNNTreeNode(node);
            while (tNode2.expanded) {
              //determine if parent node is expanded, if so find its last child node
              if (tNode2.node.childNodeCount() == 0) break;
          ...
          //logic for last node in collection
          var iNodeIndex = parentNode.getNodeIndex("id");
          var tempParent = parentNode;
          if (dir == 1) {
            //if going down... verify that parent node has sibling available to select, if not recursively look for one
            while (
              tempParent.nodeName() != "root" &&
              iNodeIndex + dir >= tempParent.parentNode().childNodeCount()
            ) {
              //while index greater than node count
  ...
  expandNode: function (tNode) {
    ...
    if (tNode.hasPendingNodes) {
      ...
      if (this.workImg != null) {
        ...
        if (icon) icon.src = this.sysImgPath + this.workImg;
      }
      if (this.callBack.indexOf("[NODEXML]") > -1)
        eval(this.callBack.replace("[NODEXML]", dnn.escapeForEval(sXml)));
      else eval(this.callBack.replace("[NODEID]", tNode.id));
  ...
  selectNode: function (tNode) {
    ...
    if (chk != null) chk.checked = tNode.selected;

    if (tNode.selected) {
      var js = "";
      if (this.defaultJS.length > 0) js = this.defaultJS;
      if (tNode.js.length > 0) js = tNode.js;

      if (js.length > 0) {
        if (eval(js) == false) return; //don't do postback if returns false
      }

      if (
        tNode.clickAction == null ||
        tNode.clickAction == dnn.controls.action.postback
      )
        eval(this.postBack.replace("[NODEID]", tNode.id));
      else if (tNode.clickAction == dnn.controls.action.nav)
        dnn.dom.navigate(
          tNode.url,
          tNode.target.length > 0 ? tNode.target : this.target,
        );
      else if (tNode.clickAction == dnn.controls.action.expand) {
        if (tNode.hasNodes || tNode.hasPendingNodes) {
          if (tNode.expanded) this.collapseNode(tNode);
          else this.expandNode(tNode);
        }
      }
    }
  ...
  selectAllChildren: function (tNode) {
    ...
    for (var i = 0; i < tNode.childNodeCount(); i++) {
      ...
      if (childTNode.selected != tNode.selected) this.selectNode(childTNode);
  ...
  assignCss: function (tNode) {
    ...
    if (tNode.level > 0 && this.cssChild.length > 0) sNodeCss = this.cssChild;

    if (tNode.css.length > 0) sNodeCss = tNode.css;

    //tNode.hoverCss;

    if (tNode.hover)
      sNodeCss +=
        " " + (tNode.cssHover.length > 0 ? tNode.cssHover : this.cssHover);
    if (tNode.selected)
  ...
  callBackSuccess: function (result, ctx, req) {
    ...
    if (tree.workImg != null) {
      ...
      if (tNode.image != "") icon.src = tNode.image;
  ...
  nodeCheck: function (evt) {
    ...
    if (node != null) {
      ...
      if (this.checkBoxMode == 1) this.selectAllChildren(tNode);
    }
  },
  ...
  nodeTextMOver: function (evt) {
    var node = this._findEventNode(evt);
    if (node != null) this.hoverNode(new dnn.controls.DNNTreeNode(node));
  },

  nodeTextMOut: function (evt) {
    var node = this._findEventNode(evt);
    if (node != null) this.assignCss(new dnn.controls.DNNTreeNode(node));
  },
  ...
  renderNode: function (node, oCont, bExists) {
    ...
    if (bExists != true) {
      if (node != null) {
        ...
        if (document.getElementById(oNewContainer.id) == null) {
          oNewContainer.appendChild(
            this.renderSpacer(
              this.indentWidth * tNode.level +
                (tNode.hasNodes || tNode.hasPendingNodes
                  ? 0
                  : this.expImgWidth),
            ),
          ); //indent node
          if (tNode.hasNodes || tNode.hasPendingNodes)
            //if node has children then render expand/collapse icon
          ...
          if (tNode.imageIndex > -1 || tNode.image != "") {
            //if node has image
      ...
      } else node = this.rootNode;

      if (tNode != null && (tNode.hasNodes || tNode.hasPendingNodes)) {
        //if node has children render container and hide if necessary
        oChildCont = this.createChildControl("DIV", tNode.id, "pctr"); //Not using SPAN due to FireFox bug...
        if (tNode.expanded != true) oChildCont.style.display = "none";
    ...
    for (
      var i = 0;
      i < node.childNodeCount();
      i++ //recursively call child rendering
    )
  ...
  renderExpCol: function (tNode) {
    ...
    if ((tNode.hasNodes || tNode.hasPendingNodes) && this.expandImg.length) {
      if (tNode.expanded) img.src = this.expandImg;
      else img.src = this.collapseImg;
    }
  ...
  renderIconCont: function (tNode) {
    ...
    if (tNode.cssIcon.length > 0) span.className = tNode.cssIcon;
    else if (this.cssIcon.length > 0) span.className = this.cssIcon;
  ...
  renderIcon: function (tNode) {
    ...
    if (tNode.image != "") img.src = tNode.image;
    else if (tNode.imageIndex > -1) img.src = this.imageList[tNode.imageIndex];
  ...
  renderText: function (tNode) {
    ...
    if (tNode.toolTip.length > 0) span.title = tNode.toolTip;
    ...
    if (tNode.enabled || tNode.clickAction == dnn.controls.action.expand) {
      ...
      else this.addHandlers(span, { click: this.nodeTextClick }, this);

      if (this.cssHover.length > 0) {
        //only do this if necessary
        //span.onmouseover = dnn.dom.getObjMethRef(this, 'nodeTextMOver');
        //span.onmouseout = dnn.dom.getObjMethRef(this, 'nodeTextMOut');
        this.addHandlers(
          span,
          { mouseover: this.nodeTextMOver, mouseout: this.nodeTextMOut },
          this,
        );
      }
    }
...
dnn.controls.DNNTree.registerClass(
...
dnn.controls.DNNTreeNode.registerClass(
  "dnn.controls.DNNTreeNode",
