...
else window.attachEvent("onload", __dnn_Page_OnLoad);
...
function __dnn_setScrollTop(iTop) {
  if (__dnn_ClientAPIEnabled()) {
    if (iTop == null) iTop = document.forms[0].ScrollTop.value;
...
function __dnn_SetInitialFocus(sID) {
  var oCtl = dnn.dom.getById(sID);
  if (oCtl != null && __dnn_CanReceiveFocus(oCtl)) oCtl.focus();
}
...
function __dnn_ContainerMaxMin_OnClick(oLnk, sContentID) {
  ...
  if (oContent != null) {
    ...
    else sMinIcon = dnn.getVar("min_icon");

    if (dnn.getVar("max_icon_" + sContainerID))
      sMaxIcon = dnn.getVar("max_icon_" + sContainerID);
    else sMaxIcon = dnn.getVar("max_icon");
    ...
    if (animf != null) iNum = new Number(animf);
    ...
    if (sCurrentFile == sMaxFile) {
      ...
      if (sCookieID != null) {
        ...
        else dnn.dom.deleteCookie(sCookieID);
      } else dnn.setVar("__dnn_" + sContainerID + "_Visible", "true");
    } else {
      oBtn.src = sMaxIcon;
      ...
      if (sCookieID != null) {
        ...
        else dnn.dom.setCookie(sCookieID, "false", 365);
      } else dnn.setVar("__dnn_" + sContainerID + "_Visible", "false");
    }
...
function __dnn_SectionMaxMin(oBtn, sContentID) {
  ...
  if (oContent != null) {
    ...
    if (oContent.style.display == "none") {
      ...
      else dnn.setVar(oBtn.id + ":exp", 1);
    } else {
      oBtn.src = sMaxIcon;
      oContent.style.display = "none";
      if (bCallback) sVal = "False";
      else dnn.setVar(oBtn.id + ":exp", 0);
    }
    if (bCallback)
      dnncore.setUserProp(
        oBtn.getAttribute("userctr"),
        oBtn.getAttribute("userkey"),
        sVal,
        null,
      );
    return true; //cancel postback
...
function __dnn_dragOver() {
  ...
  if (__dnn_dragEventCount % 75 != 0)
    //only calculate position every 75 events
    return;
  ...
  if (oPane != null) {
    ...
    if (oNextCtl != null) {
      ...
      dnn.dom.getNonTextNode(oNextCtl.control).style.borderTop =
        "5px groove " + DNN_HIGHLIGHT_COLOR;
    } else if (oPrevCtl != null) {
      __dnn_oPrevSelModule = oPrevCtl;
      dnn.dom.getNonTextNode(oPrevCtl.control).style.borderBottom =
        "5px groove " + DNN_HIGHLIGHT_COLOR;
    }
  }
}
...
function __dnn_dragComplete() {
  ...
  if (oPane == null) {
    ...
    for (var i = 0; i < oPanes.length; i++) {
      if (oPanes[i].id == oCtl.parentNode.id) oPane = oPanes[i];
    }
  }
  if (oPane != null) {
...
function __dnn_Modules(sModuleID) {
  if (__dnn_m_aryModules == null) __dnn_RefreshPanes();
...
function __dnn_getPaneControlIndex(oContent, oPane) {
  ...
  for (var i = 0; i < oPane.controls.length; i++) {
    ...
    if (oCDims.t < oIDims.t) return oCtl.index;
  }
  if (oCtl != null) return oCtl.index + 1;
  else return 0;
}
...
function __dnn_Pane(ctl, sPaneName) {
  ...
  for (var i = 0; i < ctl.childNodes.length; i++) {
    ...
    if (dnn.dom.isNonTextNode(oNode)) {
      if (__dnn_m_aryModules == null) __dnn_m_aryModules = new Array();
      ...
      if (sModuleID != null && sModuleID.length > 0) {
        ...
        this.controls[this.controls.length] = new __dnn_PaneControl(
          oNode,
          iIndex,
        );
...
__dnncore.prototype = {
  getUserProp: function (sNameCtr, sKey, pFunc) {
    this._doUserCallBack(
      ...
      sKey,
      null,
      new dnncore.UserPropArgs(sNameCtr, sKey, pFunc),
    );
  },
