...
dnn.extend(dnn.diagnostics, {
  ...
  displayDebug: function (sText) {
    if (this.debugCtl == null) {
      if (dnn.dom.browser.type == dnn.dom.browser.InternetExplorer) {
        ...
        if (this.debugWait && oBody.readyState != "complete") {
          ...
          if (
            oBody.onload == null ||
            oBody.onload.toString().indexOf("__dnn_documentLoaded") == -1
          )
            oBody.onload = dnn.dom.appendFunction(
              oBody.onload,
              "__dnn_documentLoaded()",
            );
          return;
        }
      }
    ...
    if (dnn.diagnostics.debugCtl == null) alert(sText);
    else dnn.diagnostics.debugCtl.value += sText + "\n";

    return true;
  },

  assertCheck: function (sCom, bVal, sMsg) {
    if (!bVal) this.displayDebug(sCom + " - FAILED (" + sMsg + ")");
    else if (this.verbose) this.displayDebug(sCom + " - PASSED");
  },
  ...
  assertEquals: function (sCom, sVal1, sVal2) {
    this.assertCheck(
      ...
      sVal1 == sVal2,
      "Testing Equals: " +
        this._safeString(sVal1) +
        " (" +
        typeof sVal1 +
        ") != " +
        this._safeString(sVal2) +
        " (" +
        typeof sVal2 +
        ")",
    );
  ...
  assertStringLength: function (sCom, sVal1) {
    this.assertCheck(
      sCom,
      sVal1 == null ? false : sVal1.length > 0,
      "Testing for string length: " +
        this._safeString(sVal1) +
        " (" +
        (sVal1 == null ? "null" : sVal1.length) +
        ")",
    );
  },
...
function dnn_diagnosticTests(oParent) {
  if (oParent.ns == "dnn") dnn.diagnostics.clearDebug();
  if (typeof oParent.UnitTests == "function") {
    dnn.diagnostics.displayDebug(
      "------- Starting " +
        oParent.pns +
        "." +
        oParent.ns +
        " tests (v." +
        (oParent.apiversion ? oParent.apiversion : dnn.apiversion) +
        ") " +
        new Date().toString() +
        " -------",
    );
  ...
  for (var obj in oParent) {
    if (
      oParent[obj] != null &&
      typeof oParent[obj] == "object" &&
      __dnn_m_aryHandled[obj] == null
    ) {
      //if (obj != 'debugCtl')	//what is this IE object???
      if (oParent[obj].pns != null) dnn_diagnosticTests(oParent[obj]);
    }
...
function __dnn_documentLoaded() {
  ...
  dnn.diagnostics.displayDebug(
    "document loaded... avoiding Operation Aborted IE bug",
  );
