...
dnn.controls.DNNRichText.prototype = {
  focus: function () {
    if (this.supportsCE) this.control.focus();
    else this.container.contentWindow.focus();
  },
  ...
  setText: function (s) {
    if (this.initialized) this.control.innerHTML = s;
    else this.text = s;
  },
  ...
  initDocument: function () {
    if (this.container.contentDocument != null) {
      if (this.document == null) {
        //iframe loaded, now write some HTML, thus causing it to not be loaded again
        ...
        this.control == null &&
        this.document.getElementById("__dnn_body") != null
      ) {
        //iframe loaded, now check if body is loaded
    ...
    if (this.initialized == false)
      //iframe and body not loaded, call ourselves until it is
