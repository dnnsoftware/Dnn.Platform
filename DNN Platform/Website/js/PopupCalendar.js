...
function popupCal() {
  ...
  if (popCalDstFld != "") popCalDstFld = document.getElementById(popCalDstFld);

  //default localized short date format if not provided
  if (popCalDstFmt == "") popCalDstFmt = "m/d/yyyy";
  ...
  //default localized months string if not provided
  if (popCalDays == "") popCalDays = "Sun,Mon,Tue,Wed,Thu,Fri,Sat";
  ...
  if (tmpString == "") popCalDateVal = new Date();
  else {
    ...
    if (tmpString.length - tmpNum == 3) {
      tmpString =
        tmpString.substring(0, tmpNum + 1) +
        "20" +
        tmpString.substr(tmpNum + 1);
  ...
  var dateString =
    String(popCalDateVal.getMonth() + 1) +
    "/" +
    String(popCalDateVal.getDate()) +
    "/" +
    String(popCalDateVal.getFullYear());
...
function reloadCalPopup() {
  //[0]dateString, [1]dstWindowName
  //Set the window's features here

  var windowFeatures =
    "toolbar=no, location=no, status=no, menubar=no, scrollbars=no, resizable=no, height=270, width=270, top=" +
    ((screen.height - 270) / 2).toString() +
    ",left=" +
    ((screen.width - 270) / 2).toString();
...
function calPopupSetData(firstDay, dstWindowName) {
  ...
  if (this.popCalMonths.split) {
    // javascript 1.1 defensive code
    var monthNames = this.popCalMonths.split(",");
    var dayNames = this.popCalDays.split(",");
  } // Need to build a js 1.0 split algorithm, default English for now
  else {
  ...
  var styles =
    "<style><!-- body{font-family:Arial,Helvetica,sans-serif;font-size:9pt}; td {  font-family: Arial, Helvetica, sans-serif; font-size: 9pt; color: #666666}; A { text-decoration: none; };TD.day { border-bottom: solid black; border-width: 0px; }--></style>";
  var cellAttribs =
    'align="center" class="day" BGCOLOR="#F1F1F1"onMouseOver="temp=this.style.backgroundColor;this.style.backgroundColor=\'#CCCCCC\';" onMouseOut="this.style.backgroundColor=temp;"';
  var cellAttribs2 =
    'align="center" BGCOLOR="#F1F1F1" onMouseOver="temp=this.style.backgroundColor;this.style.backgroundColor=\'#CCCCCC\';" onMouseOut="this.style.backgroundColor=temp;"';
  var htmlHead =
    "<HTML><HEAD><TITLE>" +
    popCalTitle +
    "</TITLE>" +
    styles +
    '</HEAD><BODY BGCOLOR="#F1F1F1" TEXT="#000000" LINK="#364180" ALINK="#FF8100" VLINK="#424282">';
  var htmlTail = "</BODY></HTML>";
  var closeAnchor =
    '<CENTER><input type=button value="' +
    popCalClose +
    '" onClick="javascript:window.opener.closeCalPopup()"></CENTER>';
  var todayAnchor =
    "<A HREF=\"javascript:window.opener.reloadCalPopup('" +
    sToday +
    "','" +
    dstWindowName +
    "');\">" +
  ...
  popCalData +=
    '<TABLE BORDER="0" cellspacing="0" callpadding="0" width="250"><TR><TD width="45">&nbsp</TD>';
  ...
  popCalData += " >";
  popCalData +=
    fnt[0] + nextYear1Anchor + '</FONT></TD><TD width="45">&nbsp</TD>';
  popCalData += "</TR></TABLE>";
  popCalData +=
  ...
  popCalData +=
    '<TABLE BORDER="0" cellspacing="2" cellpadding="1"  width="245">';
  popCalData += "";
  popCalData += "<TR>";
  ...
  for (xday = 0; xday < 7; xday++) {
    popCalData +=
      ...
      fnt[1] +
      '<FONT COLOR="#000000">' +
      dayNames[(xday + popCalFirstDayWeek) % 7] +
      "</FONT></TD>";
  }
  popCalData += "</TR>";
  ...
  do {
    ...
    for (calDay = 0; calDay < 7; calDay++) {
      if (
        ...
        anchorVal =
          "<A HREF=\"javascript:window.opener.calPopupSetDate(window.opener.popCalDstFld,'" +
          constructDate(monthDate, thisMonth + 1, thisYear) +
          "');window.opener.closeCalPopup()\">";
        jsVal =
          "javascript:window.opener.calPopupSetDate(window.opener.popCalDstFld,'" +
          constructDate(monthDate, thisMonth + 1, thisYear) +
          "');window.opener.closeCalPopup()";

        popCalData +=
          '<TD width="35" ' + cellAttribs + ' onClick="' + jsVal + '">';
        ...
        else popCalData += anchorVal + fnt[1] + monthDate + "</A></FONT></TD>";
