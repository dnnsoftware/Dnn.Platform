...
dnn.extend(dnn.scripts, { pns: "dnn", ns: "scripts" });
...
dnn.scripts.ScriptBatchRequest.prototype = {
  load: function () {
    ...
    for (var i = 0; i < this.ref.length; i++) {
      if (dnn.dom.scriptStatus(this.ref[i]) == "") ary.push(this.ref[i]);
    }
    ...
    for (var i = 0; i < ary.length; i++)
      this.requests.push(
        dnn.dom.loadScript(
          ary[i],
          null,
          dnn.createDelegate(this, this.asyncLoaded),
        ),
      );
    if (this.numToComplete == 0) this.asyncComplete();
  },
  asyncLoaded: function (sr) {
    if (sr.status == "complete") this.numComplete += 1;
    if (this.numComplete == this.numToComplete) this.asyncComplete();
  },
  ...
  disposeRequests: function () {
    for (var i = 0; i < this.requests.length; i++) this.requests[i].dispose();
  },
};
