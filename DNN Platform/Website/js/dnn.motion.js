dnn.extend(dnn.dom, {
  ...
  animate: function (
    ...
    var anims = [];
    var vertical =
      dir == dnn.motion.animationDir.Down || dir == dnn.motion.animationDir.Up;
    var expand =
      dir == dnn.motion.animationDir.Down ||
      dir == dnn.motion.animationDir.Right;
    var coord = vertical ? "t" : "l";
    var size = coord == "t" ? "h" : "w";
    var prop = coord == "t" ? "top" : "left";
    var easingFunc =
      dnn.getEnumByValue(dnn.motion.easingDir, easingDir) +
      dnn.getEnumByValue(dnn.motion.easingType, easingType);
    if (type == dnn.motion.animationType.Slide) {
      anims.push({ prop: "clip" + prop, begin: dims[size], finish: 0 });
      anims.push({
        prop: prop,
        begin: dims[coord] - dims[size],
        finish: dims[coord],
      });
    } else if (type == dnn.motion.animationType.Expand) {
      var newDir = vertical ? "bottom" : "right";
      anims.push({ prop: "clip" + newDir, begin: 0, finish: dims[size] });
  ...
  doTween: function (
    ...
    if (suffix) tween.set_Suffix(suffix);
    if (interval) tween.set_Interval(interval);
    tween.addEventHandler("onMotionStarted", dnn.dom.onMotionStarted);
    if (onStartFunc) tween.addEventHandler("onMotionStarted", onStartFunc);
    tween.addEventHandler("onMotionFinished", dnn.dom.onMotionFinished);
    if (onFinishFunc) tween.addEventHandler("onMotionFinished", onFinishFunc);
    tween.start();
    return tween;
  },
...
dnn.motion.prototype = {
  ...
  set_Position: function (val) {
    ...
    if (this._prop.indexOf("clip") == 0) {
      ...
      for (var i = 0; i < clips.length; i++) clip[clips[i]] = newVal;
      this._obj.clip =
        "rect(" +
        clip.cliptop +
        "," +
        ...
        clip.clipleft +
        ")";
    } else this._obj[this._prop] = newVal;
    this._pos = val;
  ...
  set_Looping: function (val) {
    this._looping = val;
  },
  get_Looping: function () {
    return this._looping;
  },
  set_Object: function (val) {
    this._obj = val;
  },
  get_Object: function () {
    return this._obj;
  },
  set_Property: function (val) {
    this._prop = val;
  },
  ...
  _onEnterFrame: function () {
    if (this.isPlaying) {
      ...
      dnn.doDelay(
        this.id,
        this._interval,
        dnn.createDelegate(this, this._onEnterFrame),
      );
    }
  },
  _update: function () {
  ...
  _getClips: function (prop) {
    var o = [];
    if (prop.indexOf("top") > -1) o.push("cliptop");
    if (prop.indexOf("left") > -1) o.push("clipleft");
    if (prop.indexOf("bottom") > -1) o.push("clipbottom");
    if (prop.indexOf("right") > -1) o.push("clipright");
    return o;
  },
};
...
dnn.motion.animationDir.prototype = { Up: 0, Down: 1, Left: 2, Right: 3 };
...
dnn.motion.easingDir.prototype = { easeIn: 0, easeOut: 1, easeInOut: 2 };
...
dnn.extend(dnn.easing, {
  ...
  easeInOutBounce: function (t, b, c, d) {
    if (t < d / 2) return dnn.easing.easeInBounce(t * 2, 0, c, d) * 0.5 + b;
    return dnn.easing.easeOutBounce(t * 2 - d, 0, c, d) * 0.5 + c * 0.5 + b;
  },
});
