...
dnn.controls.DNNTree.prototype = {
  ...
  keydownHandler: function (e) {
    ...
    if (dir != 0) {
      ...
      if (axis == "x") {
        ...
        else if (iNodeIndex + dir < 0) node = parentNode;
        else if (iNodeIndex + dir < parentNode.childNodeCount()) {
          node = parentNode.childNodes(iNodeIndex + dir);
          if (dir == -1) {
            var tNode2 = new dnn.controls.DNNTreeNode(node);
            while (tNode2.expanded) {
              if (tNode2.node.childNodeCount() == 0) break;
              node = tNode2.node.childNodes(tNode2.node.childNodeCount() - 1);
              tNode2 = new dnn.controls.DNNTreeNode(node);
            }
          }
  ...
  assignCss: function (tNode) {
    var oText = this.getChildControl(tNode.id, "t");
  ...
  update: function (force) {
    if (force) {
      if (this.selTreeNode)
        dnn.setVar(this.ns + ":selected", this.selTreeNode.id);
  ...
  nodeCheck: function (evt) {
    ...
    if (node != null) {
      ...
      if (this.checkBoxMode == 2 && this.selTreeNode != null)
        this.selectNode(this.selTreeNode);
  ...
  renderNode: function (node, oCont, bExists) {
    ...
    if (bExists != true) {
      if (node != null) {
        ...
        var oNewContainer;
        oNewContainer = this.createChildControl("DIV", tNode.id, "ctr");
