(function ($) {
  $.fn.dnnModuleActions = function (options) {
    ...
    function completeMove(targetPane, moduleOrder) {
      ...
      var service = $.dnnSF();
      var serviceUrl =
        $.dnnSF().getServiceRoot("InternalServices") + "ModuleService/";
      $.ajax({
    ...
    function buildMoveMenu(root, rootText, rootClass, rootIcon) {
      ...
      if (moduleIndex > 0) {
        ...
        var $upItem = $("<li></li>", {
          ...
          addClass: "common",
          text: opts.upText,
          click: () => moveUp(paneName, moduleIndex),
        });
      ...
      if (moduleIndex < moduleCount - 1) {
        var $downItem = $("<li></li>", {
          ...
          addClass: "common",
          text: opts.downText,
          click: () => moveDown(paneName, moduleIndex),
        });
        parent.append($downItem);

        var $bottomItem = $("<li></li>", {
          id: id + "-bottom",
          addClass: "common",
          text: opts.bottomText,
          click: () => moveBottom(paneName),
        });
      ...
      for (i = 0; i < panes.length; i++) {
        ...
        if (paneName !== pane) {
          id = pane + moduleId;
          htmlStringContainer +=
            '<li id="' + id + '">' + opts.movePaneText.replace("{0}", pane);
        }
      }
    ...
    function buildMenuLabel(root, rootText, rootClass) {
      ...
      root.append(
        $("<li></li>", { addClass: rootClass }).append(
          $("<div></div>", { text: moduleId + ":" + rootText }),
        ),
      );
    }
    ...
    if (count > 0 || supportsMove) {
      ...
      if ($form.find("div#moduleActions-" + moduleId).length === 0) {
        ...
        if (isShared) {
          menuLabel =
            menuLabel && menuLabel.length > 0
              ? sharedText + ": " + menuLabel
              : sharedText;
        }
    ...
    $(
      "#moduleActions-" + moduleId + " .dnn_mact > li.actionMenuMove > ul",
    ).jScrollPane();
    ...
    $("#moduleActions-" + moduleId + " .dnn_mact > li").hoverIntent({
      ...
      out: function () {
        if (
          !(
            $(this).hasClass("actionQuickSettings") &&
            $(this).data("displayQuickSettings")
          )
        ) {
