(function ($, window) {
  $.fn.dnnActionMenu = function (options) {
    var opts = $.extend({}, $.fn.dnnActionMenu.defaultOptions, options),
      $moduleWrap = this;
    ...
    $moduleWrap.each(function () {
      ...
      function hoverOut($m, opacity, effectMenu) {
        $m.removeAttr("style");
        $m.children("." + opts.borderClassName)
          .stop()
          .fadeTo(opts.fadeSpeed, 0);
        if (effectMenu) {
      ...
      function setMenuPosition($menuContainer) {
        var $menuBody = $module.find(opts.menuSelector).show(),
          ...
          bodyTop = isNaN(parseInt($(document.body).css("margin-top")))
            ? 0
        ...
        availableRoom.top =
          menuTop - $(window).scrollTop() - bodyTop - menuHeight > 0;
        availableRoom.bottom =
          windowHeight -
            (menuTop - $(window).scrollTop() + $menuContainer.height()) -
            menuHeight >
          0;
        availableRoom.left =
          menuLeft -
          ...
          0;
        availableRoom.right =
          windowWidth - (menuLeft - $(window).scrollLeft()) - menuWidth > 0;
        ...
        $menuBody.position({
          my: myPosition.x + " " + myPosition.y,
          at: targetPosition.x + " " + targetPosition.y,
          of: $menuContainer,
          collision: "none",
        });
      }
      ...
      if ($module.find(opts.menuSelector).length > 0) {
        $module.hoverIntent({
          ...
          out: function () {
            hoverOut(
              $(this).data("intentExpressed", false),
              opts.defaultOpacity,
              true,
            );
          },
        ...
        $module
          .find(opts.menuActionSelector)
          .css({ opacity: opts.defaultOpacity });
        ...
        $module.find(opts.menuWrapSelector).hoverIntent({
          ...
          over: function () {
            ...
            if (
              !$.support.cssFloat &&
              menuSelector.prev("iframe").length == 0
            ) {
          ...
          out: function () {
            if (!$.support.cssFloat)
              $module.find(opts.menuSelector).prev("iframe").remove();
            $module
              .find(opts.menuSelector)
              .stop()
              .fadeTo(opts.fadeSpeed, 0)
              .hide();
          },
