...
define([
  "jquery",
  "knockout",
  "templatePath/scripts/config",
  "templatePath/scripts/PersonaBarDialog",
], function ($, ko, cf) {
  var helper, bindViewModel, rootFolder, utility, appId, appSecret, container;
  ...
  var init = function (koObject, connectionHelper, pluginFolder, util) {
    ...
    if (typeof dnn === "undefined") {
      ...
      window.String.format = !window.String.format
        ? window.top.String.format
        : window.String.format;
    }
  ...
  var processData = function (conn, id) {
    ...
    conn.connected(
      !(typeof conn.container() === "undefined" || conn.container() === ""),
    );
    if (conn.id && conn.foldersData().length === 0) loadFoldersData(conn);
  };
  ...
  var loadFoldersData = function (conn) {
    ...
    utility.sf.get(
      "GetAllContainers?id=" + conn.id,
      {},
      foldersLoaded.bind(this, conn),
  ...
  var foldersLoaded = function (conn, data) {
    ...
    $("#connector-Azure")
      .find("select")
      .easyDropDown({ wrapperClass: "pb-dropdown", cutOff: 10, inFocus: true });
  };
  ...
  var onSaveComplete = function (conn, id) {
    ...
    processData(conn, id);
    utility.notify(
      (utility.resx.Connectors || utility.resx.PersonaBar).txt_Saved,
      true,
    );
  };
  ...
  var saveConnection = function (conn, success) {
    ...
    if (
      ...
      !postData.configurations[1].value
    ) {
