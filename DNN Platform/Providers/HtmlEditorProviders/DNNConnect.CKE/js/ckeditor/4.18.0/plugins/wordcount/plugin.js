  ...
  init: function (editor) {
    ...
    if (config.showParagraphs) {
      if (config.maxParagraphs > -1) {
        if (config.showRemaining) {
          defaultFormat +=
            "%paragraphsCount% " + editor.lang.wordcount.ParagraphsRemaining;
        } else {
          defaultFormat +=
            editor.lang.wordcount.Paragraphs + " %paragraphsCount%";
    ...
    if (config.showWordCount) {
      if (config.maxWordCount > -1) {
        if (config.showRemaining) {
          defaultFormat +=
            "%wordCount% " + editor.lang.wordcount.WordCountRemaining;
        } else {
    ...
    if (config.showCharCount) {
      if (config.maxCharCount > -1) {
        if (config.showRemaining) {
          ...
          defaultFormat +=
            editor.lang.wordcount[
              config.countHTML ? "CharCountWithHTML" : "CharCount"
            ] + " %charCount%";
    ...
    function limitReached(editorInstance, notify) {
      ...
      if (!notify) {
        counterElement(editorInstance).className =
          "cke_path_item cke_wordcountLimitReached";
        editorInstance.fire(
          "limitReached",
    ...
    function updateCounter(editorInstance) {
      ...
      editorInstance.fire(
        "getData",
        { dataValue: text, firedBy: "wordCount.updateCounter" },
        editor,
      );
      ...
      if (config.showRemaining) {
        ...
        if (config.maxParagraphs >= 0) {
          html = html.replace(
            "%paragraphsCount%",
            config.maxParagraphs - paragraphs,
          );
        } else {
      ...
      (
        editorInstance.config.wordcount ||
        (editorInstance.config.wordcount = {})
      ).wordCount = wordCount;
      (
        editorInstance.config.wordcount ||
      ...
      if (
        ...
        if (
          ...
          paragraphs > config.maxParagraphs
        ) {
          editorInstance.fire("saveSnapshot");
        }
        return true;
    ...
    function isCloseToLimits() {
      ...
      if (
        config.maxCharCount > -1 &&
        config.maxCharCount - lastCharCount < 20
      ) {
        return true;
      }
      ...
      if (
        config.maxParagraphs > -1 &&
        config.maxParagraphs - lastParagraphs < 1
      ) {
        return true;
      }
    ...
    editor.on(
      ...
      function (event) {
        if (editor.mode === "source") {
          ...
          timeoutId = setTimeout(updateCounter.bind(this, event.editor), 250);
        }
      },
      editor,
      null,
    ...
    editor.on(
      ...
      function (event) {
        ...
        clearTimeout(timeoutId);
        timeoutId = setTimeout(updateCounter.bind(this, event.editor), ms);
      },
      editor,
      null,
      100,
    ...
    editor.on(
      ...
      function (event) {
        if (
          !config.warnOnLimitOnly &&
          (config.maxWordCount > 0 ||
            config.maxCharCount > 0 ||
            config.maxParagraphs > 0)
        ) {
          ...
          event.editor.fire(
            "getData",
            { dataValue: text, firedBy: "wordCount.onPaste" },
            event.editor,
          );
