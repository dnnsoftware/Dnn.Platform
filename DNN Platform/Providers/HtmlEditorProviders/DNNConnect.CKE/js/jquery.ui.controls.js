$.widget("ui.form", {
  _init: function () {
    ...
    var inputs = form
      .find(
        "#lblCurrentDir, input:text, input:checkbox, input:radio , textarea",
      )
      .not(
        "#CKEditor_Options_rBlSetMode input:radio,#BrowserMode input:radio,.groupEdit",
      );
  ...
  buttons: function (element) {
    if ($(element).is(":submit")) {
      $(element).addClass(
        "ui-priority-primary ui-corner-all ui-state-disabled hover",
      );
  ...
  checkboxes: function (element) {
    ...
    $(parent)
      .children("span")
      .first()
      .html($(element).children("option:selected").html());
    ...
    parent.wrap(
      "<span class='ui-state-default ui-corner-all' style='display:inline-block;width:16px;height:16px;margin-right:5px;'/>",
    );
  ...
  radio: function (element) {
    ...
    if ($(element).prop) {
      if ($(element).prop("disabled")) {
        parent.wrap(
          "<span class='ui-state-disabled ui-corner-all' style='display:inline-block;width:16px;height:16px;margin-right:5px;'/>",
        );
      } else {
        parent.wrap(
          "<span class='ui-state-default ui-corner-all' style='display:inline-block;width:16px;height:16px;margin-right:5px;'/>",
  ...
  selector: function (element) {
    ...
    $(element).addClass("ui-helper-hidden");
    parent.append(
      "<span style='float:left;'></span><span style='float:right;display:inline-block' class='ui-icon ui-icon-triangle-1-s' ></span>",
    );
    parent.after(
      "<ul class=' ui-helper-reset ui-widget-content ui-helper-hidden' style='position:absolute;z-index:50;width:140px;' ></ul>",
    );
    $(parent)
      .children("span")
      .first()
    ...
    $.each($(element).find("option"), function () {
      $(parent)
        .next("ul")
        .append(
          "<li class='hover' style='cursor: pointer'>" +
            $(this).html() +
            "</li>",
        );
    });
...
(function ($, undefined) {
  $.widget("ui.selectgroup", {
    ...
    _create: function () {
      ...
      this.placeholder = $(
        '<a href="#" id="' +
          ...
          '<span class="' +
          this.widgetBaseClass +
          '-icon ui-icon ui-icon-triangle-1-s"></span></a>',
      );
      ...
      if (this.options.style === "popup") {
        ...
        this.placeholder
          .find("." + this.widgetBaseClass + "-icon")
      ...
      $(document).bind("click.selectmenu", function (event) {
        if (
          that.isOpen &&
          !(
            $(event.target).closest(".ui-selectgroup").length ||
            $(event.target).closest(".ui-selectgroup-group").length
          )
        ) {
    ...
    _renderGroup: function () {
      ...
      this.group = $(
        '<ul class="' +
          this.widgetBaseClass +
          '-list" role="listbox" aria-hidden="true"></ul>',
      );
      if (this.options.autoWidth) {
        if (this.options.style === "dropdown") {
          $.ui.selectgroup.group.width(this.placeholder.width());
        } else {
          $.ui.selectgroup.group.width(
            this.placeholder.width() - this.options.handleWidth,
          );
        }
      }
      if (this.options.style === "popup") {
    ...
    _renderOption: function () {
      ...
      $.each(this.selectors, function (index) {
        ...
        var list = $(
          '<li role="presentation"><a role="option" href="#">' +
            this.text +
            "</a></li>",
        )
          .bind("click.selectgroup", function (event) {
            event.preventDefault();
            if (
              !(self.disabled === "disabled" || self.optDisabled === "disabled")
            ) {
        ...
        if (this.optgroup.length) {
          var name =
            that.widgetBaseClass +
            "-optgroup-" +
            that.element.find("optgroup").index(this.optgroup);
          ...
          // Else we need to create a representation of an option group and then append the option list item.
            ...
            if (
              typeof this.optDisabled !== "undefined" &&
    ...
    _position: function () {
      ...
      if (this.options.style === "popup" && !this.options.positioning.offset) {
        var adjust =
          "0 -" + ($(instance).outerHeight() + $(instance).offset().top);
      }
    ...
    _traverse: function (value, record) {
      ...
      if (
        this.selectors[position].disabled === "disabled" ||
        this.selectors[position].optDisabled === "disabled"
      ) {
        if (value > 0) {
          ++value;
        } else {
    ...
    _typeahead: function (character) {
      ...
      function focusOption(index) {
        ...
        $(instance).addClass("ui-state-hover");
        that.placeholder
          .find(".ui-selectgroup-copy")
          .text(that.selectors[index].text);
        that.element.find("option:selected").removeAttr("selected");
        $(that.selectors[index].element).attr("selected", "selected");
        found = true;
        that.search[3] = index;
      ...
      if (this.search[0] === this.search[1][0]) {
        ...
        if (this.search[1].length < 2) {
          $.each(this.selectors, function (index) {
            if (!found) {
              if (
                $.trim(that.selectors[index].text)
                  .toLowerCase()
                  .indexOf(that.search[1][0]) === 0
              ) {
        ...
        $.each(this.selectors, function (index) {
          if (!found) {
            if (
              that.search[1] ===
              $.trim(that.selectors[index].text)
                .substring(0, that.search[1].length)
                .toLowerCase()
            ) {
              that.search[2] = index;
              focusOption(index);
  ...
  $.ui.selectgroup.group = $(
    '<div class="ui-selectgroup-group ui-widget ui-widget-content ui-corner-all"></div>',
  );
