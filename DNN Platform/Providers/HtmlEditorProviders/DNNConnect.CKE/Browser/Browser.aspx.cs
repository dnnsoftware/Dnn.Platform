...
public partial class Browser : PageBase
{
    ...
    private void FillFolderTree(IFolderInfo currentFolderInfo)
    {
        ...
        var readableFolders =
            FolderManager.Instance.GetFolders(UserController.Instance.GetCurrentUserInfo())
                .GroupBy(folder => folder.ParentID)
                .ToDictionary(
                    key => key.Key,
                    value => value?.Select(folder => folder) ?? Array.Empty<IFolderInfo>());
    ...
    private void InitializeComponent()
    {
        ...
        this.ClientResourceLoader.PreRender += (_, _) => JavaScript.Register(this.Page);
    }
    ...
    private void UploadFile(HttpPostedFile file, string command)
    {
        ...
        if (bAllowUpl)
        {
            ...
            try
            {
                ...
                FileManager.Instance.AddFile(currentFolderInfo, fileName, fileContents, false, true, FileContentTypeManager.Instance.GetContentType(Path.GetExtension(fileName)));
            }
            finally
            {
                if (disposeStream)
    ...
    private void ResizeNow_Click(object sender, EventArgs e)
    {
        ...
        using (var fileStream = FileManager.Instance.GetFileContent(file))
        {
            ...
            FileManager.Instance.AddFile(this.GetCurrentFolder(), resizedFileName, stream, false, true, FileContentTypeManager.Instance.GetContentType(Path.GetExtension(resizedFileName)));
        }
    ...
    private void GetDiskSpaceUsed()
    {
        ...
        var spaceUsed = PortalController.Instance.GetPortalSpaceUsedBytes(this.portalSettings.PortalId);
