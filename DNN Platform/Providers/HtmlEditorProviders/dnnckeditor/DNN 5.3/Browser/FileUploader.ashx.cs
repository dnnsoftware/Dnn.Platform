...
namespace WatchersNET.CKEditor.Browser
{
    ...
    public class FileUploader : IHttpHandler
    {
        ...
        private FolderInfo StorageFolder
        {
            get
            {
                return new FolderController().GetFolderInfo(
                    PortalSettings.PortalId,
                    Convert.ToInt32(HttpContext.Current.Request["storageFolderID"]));
            }
        }
        ...
        private void UploadWholeFile(HttpContext context, List<FilesUploadStatus> statuses)
        {
            for (int i = 0; i < context.Request.Files.Count; i++)
            {
                ...
                FileSystemUtils.UploadFile(this.StorageFolder.FolderPath, file, fileName);
