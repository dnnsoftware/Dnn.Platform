export class PagePickerDataManager {
    ...
    _flatten(){
        ...
        for( let tab in storage){
            ...
            const Right = () => {
              ...
              if(hasParent){
                this._store(Tab);
                const parent = this.getBy({TabId:Tab.ParentTabId})[0];
              }
            };
    ...
    _updateParentChildTabs(){
      ...
      children
        ...
        .forEach((id)=>{
          ...
          const parentTab = this.getBy({TabId:tab.ParentTabId})[0];
          mem_cache.push(id);
          const Left = () => {
    ...
    _mapRootTabs(TabId, fn=(root)=>root){
      ...
      const loop = (TabId) => {
        ...
        let parent = this.getBy({TabId: parentTabId})[0];
    ...
    _setParentTabCheckedStates({selection}){
    ...
    hasParent(id){
      const parent = this.getBy({TabId:id})[0];
    ...
    replace(obj=false) {
      ...
      const Right = () =>  { throw(`ppdm.replace requires an object input, got ${typeof obj}`); };
      (typeof obj == 'object') ? Left() : Right();
    }

    update({id}, updateObj) {
      const Right = () => { throw('Both a unique TabId must exist and Update Object is required'); };
      const Left = () => {
        Object.keys(updateObj).forEach((key) => this.storage[id][key] = updateObj[key]);
        this._setParentTabCheckedStates({selection:id});
      };
