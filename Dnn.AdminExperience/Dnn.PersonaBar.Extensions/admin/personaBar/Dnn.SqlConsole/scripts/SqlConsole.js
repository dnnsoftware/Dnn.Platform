...
define([
  ...
  var initUpload = function () {
    ...
    $uploadControl.on("change", function (e) {
      ...
      if (
        ...
        file.name.toLowerCase().split(".").pop() === "sql"
      ) {
  ...
  var changeTab = function (data) {
    ...
    var $tableContainer = $panel
      .find(".tables-container > div#result" + index)
      .find("> div.table-container");
  ...
  var scrollPaneInitialised = function (table, e) {
    var $table = $panel
      .find(".tables-container > div#result" + table.index)
      .find("table");
  ...
  var createDataTable = function (data) {
    ...
    table.changePageSize = function () {
      ...
      var $currentTable = $(".tables-container > div:visible").find(
        "div.table-container",
      );
      $currentTable.data("jsp").destroy();

      $currentTable = $(".tables-container > div:visible").find(
        "div.table-container",
      );
    ...
    table.statistics = ko.computed(function () {
      var args = [
        table.startIndex() + 1,
        table.endIndex(),
        table.filterData().length,
      ];
      return viewModel.resx.PageInfo.replace(/\{(\d+)\}/gi, function (i) {
    ...
    var exportExcel = function () {
      ...
      exportFile(
        table.title + ".xls",
        exportData.excel($("#result" + table.index + " table")[0]),
      );
    ...
    var exportCsv = function () {
      ...
      exportFile(
        table.title + ".csv",
        exportData.csv($("#result" + table.index + " table")[0]),
      );
      ...
      html2canvas($table[0], {
        onrendered: function (canvas) {
          ...
          if (typeof jsPdf === "undefined") {
            require(["main/../modules/dnn.sqlconsole/scripts/jspdf"], function (
              pdf,
            ) {
              jsPdf = pdf;
              generatePdf();
      ...
      function (data) {
        if (data.Data) {
          ...
          if ($animateControl.height() > 100) {
            $animateControl
              .animate({ height: "-=250" }, "fast")
              .on("click.sqlconsole", function () {
  ...
  var initSqlConsole = function () {
    ...
    monacoEditorLoaderScript.src =
      siteRoot + "Resources/Shared/components/MonacoEditor/loader.js" + version;
    document.body.appendChild(monacoEditorLoaderScript);

    require.config({
      paths: { vs: siteRoot + "Resources/Shared/components/MonacoEditor" },
    });
  ...
  var initMonacoEditor = function () {
    ...
    var monacoEditor = sqlConsole.create(
      document.getElementById("monaco-editor-sql"),
      {
        model: sqlContent,
        language: "sql",
