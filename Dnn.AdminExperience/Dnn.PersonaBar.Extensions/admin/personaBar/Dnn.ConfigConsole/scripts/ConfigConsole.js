...
define([
  ...
  var utility,
    $panel,
    viewModel,
    monacoConfigFilesEditor,
    configFilesEditor,
    configFilesModel,
    configFilesContent,
    monacoMergeScriptsEditor,
    mergeScriptsEditor,
    mergeScriptsModel,
    mergeScriptsContent,
    curConfigName;
  ...
  var getConfigs = function () {
    requestService(
      ...
      function (data) {
        ...
        $(".configConsolePanel select").easyDropDown({
          wrapperClass: "pb-dropdown",
          cutOff: 10,
          inFocus: true,
        });
      },
  ...
  var confirmValidationErrors = function (validationErrors, configFile) {
    ...
    var question = utility.resx.ConfigConsole.ValidationErrorsConfirm.replace(
      "{0}",
      BR,
    );
    ...
    utility.confirm(
      ...
      function () {
        requestService(
          "post",
          "MergeConfigFile",
          { fileName: "", fileContent: mergeScriptsContent.getValue() },
          function (data) {
            utility.notify(utility.resx.ConfigConsole.Success);
          },
          function () {
  ...
  var initUpload = function () {
    ...
    $uploadControl.on("change", function (e) {
      ...
      if (
        ...
        file.name.toLowerCase().split(".").pop() === "config"
      ) {
  ...
  var init = function (wrapper, util, params, callback) {
    ...
    $(".configConsolePanel .body").dnnTabs({
      selected: 0,
      activate: initTabsClickEvent(),
    });
    setEditorHeight();
  ...
  var setEditorHeight = function () {
    setTimeout(() => {
      ...
      if (panelHeight > 400) {
        if (
          document.querySelector("#configConsole-files").style.display !==
          "none"
        ) {
        ...
        if (
          document.querySelector("#configConsole-merge").style.display !==
          "none"
        ) {
  ...
  var initMonacoEditors = function () {
    ...
    monacoConfigFilesEditor = configFilesEditor.create(
      document.getElementById("monaco-editor-config-files"),
      {
    ...
    monacoMergeScriptsEditor = mergeScriptsEditor.create(
      document.getElementById("monaco-editor-merge-scripts"),
      {
