...
define([
  ...
  var wrapConnection = function (conn) {
    ...
    var secureSettings = secureSettingsValue
      ? secureSettingsValue.split(";")
      : new Array();
    ...
    var koConnectionObject = {
      ...
      edit: function (item, e) {
        ...
        var collapsedEditRow = $(
          "#connectionstbl tr.edit-row > td > div.edit-form:visible",
        );
      ...
      cancel: function (item, e) {
        ...
        $.each(item.configurations, function (i, v) {
          ...
          if (
            ...
            typeof item[name].reset === "function"
          ) {
            item[name].reset();
          }
        });
        var connected =
          item.previousConnectionState() &&
          (item.supportsMultiple ? !!item.id : true);
        ...
        if (e) {
          $(e.target)
            .parents("div.edit-form")
            .slideUp(200, "linear", function () {
              viewModel.connections().forEach(function (v) {
      ...
      loadLocalizedStrings(conn.name, function (data) {
        ...
        if (
          !ko.components.isRegistered(
            koConnectionObject.uniqueNumber + "component",
          )
        ) {
          ...
          (function (connection) {
            setTimeout(function () {
              ...
              $root.find("input, select").each(function () {
                ...
                if (
                  !$this.attr("id") ||
                  !$root.find('label[for="' + $this.attr("id") + '"]').length
                ) {
  ...
  var isConnectionConnected = function (item) {
    if (
      item.name === "Mailchimp" ||
      item.name === "GoogleTagManager" ||
      item.name === "GoogleAnalytics"
    ) {
      return !!item.configurations[0].value();
    }
    if (item.name === "Zendesk") {
      return (
        item.configurations[0].value() &&
        item.configurations[1].value() &&
        item.configurations[2].value()
      );
    }
    return item.configurations.every(function (config) {
      return (
        !!config.value() || config.name == "Id" || config.name == "Connected"
      );
    });
  };
    ...
    var isUnique = viewModel.connections().every(function (v) {
      return v.connections().every(function (t) {
        return !(
          ...
          t.uniqueNumber != conn.uniqueNumber
        );
      });
    });
    return isUnique;
  ...
  var saveConnection = function (conn, success, forceSave) {
    ...
    if (
      conn.supportsMultiple &&
      !postData.configurations[0].value &&
      (!postData.configurations[1] || !postData.configurations[1].value)
    ) {
      return onDelete(conn);
    }
    utility.sf.post(
  ...
  var onAddNew = function (item, e) {
    ...
    var newConnector = connections.find(function (conn) {
      return !conn.id;
    });
    ...
    var configurationKeys = Object.keys(
      connections[0].initialObject.configurations,
    );
    var newConfigurations = {};
    configurationKeys.forEach(function (key) {
      if (key === "AuthPage") {
        newConfigurations[key] =
          connections[0].initialObject.configurations[key];
      } else {
        newConfigurations[key] = "";
      }
    });
    ...
    closeAllSubConnectors();
    var newConnection = Object.assign(connections[0].initialObject, {
      id: null,
  ...
  var getNewName = function (item) {
    ...
    while (
      connections.some(function (conn) {
        return conn.displayName() === newName;
      })
    ) {
  ...
  var loadLocalizedStrings = function (name, cb) {
    ...
    utility.sf.controller = "Connectors";
    utility.sf.get(
      "GetConnectionLocalizedString",
      { name: name, culture: config.culture },
      function (data) {
        if (allowLocalStorage) {
          try {
