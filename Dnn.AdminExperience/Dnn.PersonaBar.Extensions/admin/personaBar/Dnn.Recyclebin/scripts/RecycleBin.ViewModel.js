...
define([
  ...
  "main/koBindingHandlers/setWidthFromParentScrollPaneWhen",
], function ($, ko) {
  ...
  var PageInfo = function (requestData) {
    ...
    var calculateDepthOfNode,
      calculateNumberOfAllChildrenTree,
      selectAllChildren,
      isItsRootElementEven,
      setOddEvenInChildren,
    ...
    calculateNumberOfAllChildrenTree = function (children) {
      ...
      children.forEach(function (child) {
        numberOfNestedChildren += calculateNumberOfAllChildrenTree(
          child.children(),
        );
      });
  ...
  DnnPageRecycleBin = (function () {
    ...
    var getSelectedElements,
      markAllElementsAs,
      markAllModulesAsIfPossible,
      getTreesOfPages,
      setOddEvenOrderInRoots,
      calculateDepthOfPages,
      changeElementSelectedStatus,
      restoreSelectedPagesHandler,
      removeSelectedPagesHandler,
      pageRestoreRevomeOperationsCallback,
      canBeDeleted,
      restoreSelectedModulesHandler,
      removeSelectedModulesHandler,
      restorePageHandler,
      removePageHandler,
      restoreModuleHandler,
      removeModuleHandler,
      emptyRecycleBinHandler,
      restoreUserHandler,
      removeUserHandler,
      restoreSelectedUsersHandler,
      removeSelectedUsersHandler,
      userRestoreRevomeOperationsCallback,
      Paginate,
      getDeletedPageList,
    ...
    function DnnPageRecycleBin(
      ...
      _settings = {
        canViewPages:
          settings.isHost ||
          settings.isAdmin ||
          settings.permissions.RECYCLEBIN_PAGES_VIEW,
        canViewModules:
          settings.isHost ||
          settings.isAdmin ||
          settings.permissions.RECYCLEBIN_MODULES_VIEW,
        canViewUsers:
          settings.isHost ||
          settings.isAdmin ||
          settings.permissions.RECYCLEBIN_USERS_VIEW,
        canManagePages:
          settings.isHost ||
          settings.isAdmin ||
          settings.permissions.RECYCLEBIN_PAGES_EDIT,
        canManageModules:
          settings.isHost ||
          settings.isAdmin ||
          settings.permissions.RECYCLEBIN_MODULES_EDIT,
        canManageUsers:
          settings.isHost ||
          settings.isAdmin ||
          settings.permissions.RECYCLEBIN_USERS_EDIT,
        canEmptyRecycleBin:
          settings.isHost ||
          settings.isAdmin ||
          (settings.permissions.RECYCLEBIN_PAGES_VIEW &&
      ...
      collection.forEach(function (element) {
        ...
        if (element.children) {
          selectedElements = selectedElements.concat(
            getSelectedElements(element.children()),
          );
        }
      });
    ...
    restoreSelectedPagesHandler = function () {
      if (_settings.canViewPages && _settings.canManagePages) {
        ...
        if (pagesList.length > 0) {
          ...
          confirmText =
            pagesList.length > 1
              ? _resx.recyclebin_RestorePagesConfirm
              : _resx.recyclebin_RestorePageConfirm;
    ...
    canBeDeleted = function (selectedPages) {
      ...
      selectedPages.forEach(function (page) {
        if (page.hasUnselectedChildren()) {
          result.value = false;
          result.errors += _resx.Service_RemoveTabError.replace(
            /\{0\}/g,
            page.name,
          );
        }
    ...
    removeSelectedPagesHandler = function () {
      if (_settings.canViewPages && _settings.canManagePages) {
        ...
        if (pagesList.length > 0) {
          ...
          confirmText =
            pagesList.length > 1
              ? _resx.recyclebin_RemovePagesConfirm
              : _resx.recyclebin_RemovePageConfirm;
          yesText = _resx.recyclebin_YesConfirm;
          noText = _resx.recyclebin_NoConfirm;
          ...
          _utility.confirm(confirmText, yesText, noText, function () {
            ...
            if (!result.value) {
              _utility.notifyError(
                _resx.Service_RemoveTabErrorHeader + result.errors,
              );
              return;
            }
            getService().post(
        ...
        if (modulesList.length > 0) {
          ...
          confirmText =
            modulesList.length > 1
              ? _resx.recyclebin_RestoreModulesConfirm
              : _resx.recyclebin_RestoreModuleConfirm;
          yesText = _resx.recyclebin_YesConfirm;
          noText = _resx.recyclebin_NoConfirm;
    ...
    removeSelectedModulesHandler = function () {
      if (_settings.canViewModules && _settings.canManageModules) {
        ...
        if (modulesList.length > 0) {
          ...
          confirmText =
            modulesList.length > 1
              ? _resx.recyclebin_RemoveModulesConfirm
              : _resx.recyclebin_RemoveModuleConfirm;
          yesText = _resx.recyclebin_YesConfirm;
          noText = _resx.recyclebin_NoConfirm;
    ...
    restoreSelectedUsersHandler = function () {
      if (_settings.canViewUsers && _settings.canManageUsers) {
        ...
        if (usersList.length > 0) {
          ...
          confirmText =
            usersList.length > 1
              ? _resx.recyclebin_RestoreUsersConfirm
              : _resx.recyclebin_RestoreUserConfirm;
          yesText = _resx.recyclebin_YesConfirm;
          noText = _resx.recyclebin_NoConfirm;
    ...
    removeSelectedUsersHandler = function () {
      if (_settings.canViewUsers && _settings.canManageUsers) {
        ...
        if (usersList.length > 0) {
          ...
          confirmText =
            usersList.length > 1
              ? _resx.recyclebin_RemoveUsersConfirm
              : _resx.recyclebin_RemoveUserConfirm;
          yesText = _resx.recyclebin_YesConfirm;
          noText = _resx.recyclebin_NoConfirm;
      ...
      if (_settings.canViewPages && _settings.canManagePages) {
        ...
        _utility.confirm(confirmText, yesText, noText, function () {
          if (pageData.children().length > 0) {
            _utility.notifyError(
              _resx.Service_RemoveTabParentTabError.replace(
                /\{0\}/g,
                pageData.name,
              ),
            );
    ...
    Paginate = function (API_METHOD, viewModelProp, elementId) {
      ...
      TotalResults[viewModelProp] = compare
        ? TotalResults[viewModelProp]
        : { modelName: viewModelProp, paginationRequestCount: 1 };
          ...
          if (!timeout) {
            if (
              ...
              TotalResults[viewModelProp].paginationRequestCount =
                TotalResults[viewModelProp].paginationRequestCount || 1;

              getService().get(
                API_METHOD,
                {
                  pageIndex: TotalResults[viewModelProp]
                ...
                function (data) {
                  ...
                  var conditional =
                    TotalResults[viewModelProp].remainingPages -
                      results.length >
                    0;
                  var temp = [];
                  TotalResults[viewModelProp].remainingPages = conditional
                    ? TotalResults[viewModelProp].remainingPages -
                      results.length
                    : 0;
    ...
    var initPagination = function () {
      var tabs = [
        {
          apiMethod: "GetDeletedPageList",
          viewModelProp: "deletedpagesList",
          elementId: "#pageList",
        },
        {
          apiMethod: "GetDeletedUserList",
          viewModelProp: "deletedusersList",
          elementId: "#userList",
        },
        {
          apiMethod: "GetDeletedModuleList",
          viewModelProp: "deletedmodulesList",
          elementId: "#moduleList",
        },
        {
          apiMethod: "GetDeletedTemplates",
          viewModelProp: "deletedtemplatesList",
          elementId: "#templateList",
        },
      ];
    ...
    getDeletedModuleList = function (isInitCall) {
      if (_settings.canViewModules) {
        var viewModel = getViewModel();
        if (
          (typeof isInitCall !== "boolean" ||
            (typeof isInitCall === "boolean" && !isInitCall)) &&
          !viewModel.deletedModulesReady()
        ) {
          return;
        }
        ...
        getService().get(
          ...
          function (data) {
            ...
            TotalResults[viewModelPropName].remainingPages =
              data.TotalResults - pageSize;
