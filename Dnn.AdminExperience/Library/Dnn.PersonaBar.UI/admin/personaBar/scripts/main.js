(function () {
  ...
  requirejs.config({
    ...
    urlArgs:
      (cdv ? "cdv=" + cdv : "") + (debugMode ? "&t=" + Math.random() : ""),
    shim: {
    ...
    map: {
      "*": {
        ...
        "dnn.jquery.extensions": [
          "../../../../../Resources/Shared/Scripts/dnn.jquery.extensions",
        ],
        "dnn.extensions": [
          "../../../../../Resources/Shared/scripts/dnn.extensions",
        ],
        "jquery.tokeninput": [
          "../../../../../Resources/Shared/components/Tokeninput/jquery.tokeninput",
        ],
        "dnn.jScrollBar": [
          "../../../../../Resources/Shared/scripts/jquery/dnn.jScrollBar",
        ],
        "dnn.servicesframework": ["../../../../../js/dnn.servicesframework"],
        "dnn.DataStructures": [
          "../../../../../Resources/Shared/scripts/dnn.DataStructures",
        ],
        "jquery.mousewheel": [
          "../../../../../Resources/Shared/scripts/jquery/jquery.mousewheel",
        ],
        "dnn.TreeView": [
          "../../../../../Resources/Shared/scripts/TreeView/dnn.TreeView",
        ],
        "dnn.DynamicTreeView": [
          "../../../../../Resources/Shared/scripts/TreeView/dnn.DynamicTreeView",
        ],
        "dnn.DropDownList": [
...
require([
  ...
  var customModules = [];
  var disableEditBar =
    window.parent["personaBarSettings"]["disableEditBar"] === true;
  ...
  var util = {
    ...
    setConfirmationDialogPosition: function () {
      ...
      var personaBarPage = document.getElementsByClassName(
        "dnn-persona-bar-page",
      )[0];
    ...
    closePersonaBar: function handleClosePersonarBar(callback, keepSelection) {
      ...
      if (keepSelection) {
        $(".btn_panel.selected, .hovermenu > ul > li.selected")
    ...
    loadPanel: function handleLoadPanel(identifier, params) {
      ...
      if (!params.settings) {
        ...
        params.settings = $.extend(
          {},
          util.findMenuSettings(params.identifier),
          params.settings,
        );
      }
      ...
      var hoverMenuItemSelector =
        ".hovermenu > ul > li[id='" + identifier + "']";
      if (moduleName) {
      ...
      if ($mask.css("display") === "none") {
        ...
        if (activePath !== path) {
          ...
          $activePanel.fadeOut("fast", function handleHideCurrentPanel() {
            $panel
              .css({ left: personaBarMenuWidth })
              .fadeIn("fast", function handleShowSelectedPanel() {
                activePath = path;
                activemodule = moduleName;
                inAnimation = false;
                self.loadTemplate(
        ...
        for (var i = 0; i < config.customModules.length; i++) {
          (function (index) {
            ...
            require([path], function (module) {
              ...
              if (
                index === config.customModules.length - 1 &&
    ...
    savePanelTabView: function (panelId) {
      var $panel = $("#" + panelId);
      var $primaryTabs = $panel
        .find(".dnn-tabs.primary, .ui-tabs")
        .eq(0)
        .find("> ul > li");
      if (!$primaryTabs.length) {
        $primaryTabs = $panel
          .find(".dnn-tabs.secondary")
          .eq(0)
          .find("> ul > li");
      }
      var $primarySelected = $primaryTabs
        .parent()
        .find('>li[aria-selected="true"],>li[class*="selected"]');
      if ($primarySelected.length) {
        ...
        if ($primaryPanel.length) {
          var $secondaryTabs = $primaryPanel
            .find(".dnn-tabs.secondary")
            .eq(0)
            .find("> ul > li");
          var $secondarySelected = $secondaryTabs
            .parent()
            .find('>li[aria-selected="true"],>li[class*="selected"]');
    ...
    updatePanelTabView: function (panelId) {
      ...
      if (
        $primaryTab.attr("aria-selected") !== "true" &&
        $primaryTab.attr("class").indexOf("selected") === -1
      ) {
  ...
  function handleDisabledEditBar($btnEdit) {
    ...
    var $tooltip = $(
      '<div class="editmode-tooltip"><span class="tooltip-title"></span><span class="tooltip-message"></span></div>',
    );
  ...
  util.asyncParallel(
    [
          ...
          var viewModel = {
            ...
            logOff: function () {
              function onLogOffSuccess() {
                if (
                  typeof window.top.dnn != "undefined" &&
                  typeof window.top.dnn.PersonaBar != "undefined"
                ) {
          ...
          setTimeout(function () {
            ...
            if (isTouch) {
              ...
              $(document).on("touchstart", function (event) {
                if (
                  !$(event.target).closest("#topLevelMenu .hovermenu").length
                ) {
                  $("#topLevelMenu .hovermenu").hide();
                }
              });
            ...
            if (config.visible) {
              (function setupMenu() {
                ...
                $(".btn_panel, .hovermenu > ul > li").click(
                  function handleClickOnHoverMenuItem(evt) {
                    evt.preventDefault();
                    evt.stopPropagation();
                    ...
                    if ($this.hasClass("selected")) {
                      var panelId = utility.getPanelIdFromPath(
                        $this.data("path"),
                      );
                      var panelAlreadyOpened = $(
                        "#" + panelId + ":not(visible)",
                      );
                ...
                $showSiteButton.click(
                  function handleShowSite(e, keepSelection) {
                    e.preventDefault();
              ...
              $(".personabarnav > li.btn_panel, .hovermenu > ul > li").addClass(
                "disabled",
              );
            }
            ...
            (function setupEditButton() {
              ...
              eventEmitter.addPanelCloseEventListener(
                function handleClosingPersonaBar() {
                  $btnEdit.show();
                },
              );
              eventEmitter.addPanelOpenEventListener(
                function handleOpeningPersonaBar() {
                  $btnEdit.hide();
                },
              );
            ...
            if (
              !$iframe.attr("style") ||
              $iframe.attr("style").indexOf("width") === -1
            ) {
              $iframe.width(personaBarMenuWidth);
            }
          }, 0);
      ...
      function showPersonaBar(callback) {
        ...
        if ($parentBody.hasClass("dnnEditState")) {
          ...
          $parentBody.animate(
            { marginLeft: personaBarMenuWidth },
            1,
            "linear",
            onShownPersonaBar,
          );
    ...
    function loadPanelFromPersistedSetting() {
      ...
      if (
        pageUrl.indexOf("skinsrc=") > -1 ||
        pageUrl.indexOf("containersrc=") > -1 ||
        pageUrl.indexOf("dnnprintmode=") > -1
      ) {
        return;
      }
