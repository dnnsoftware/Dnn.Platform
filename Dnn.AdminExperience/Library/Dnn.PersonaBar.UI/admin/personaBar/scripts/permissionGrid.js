...
if (typeof dnn.controls === "undefined" || dnn.controls === null) {
  dnn.controls = {};
}

(function ($) {
  var permissionGrid = (dnn.controls.PermissionGrid = function (
    parent,
    data,
    options,
  ) {
  ...
  dnn.controls.PermissionGrid.prototype = {
    ...
    getPermission: function (permissionObj) {
      return this.data.permissionDefinitions.find(function (permission) {
        return (
          permission.permissionKey == permissionObj.permissionKey &&
    ...
    _buildGrid: function () {
      var gridContainer = (this._gridContainer = $(
        '<div class="dnnGrid dnnPermissionsGrid dnnForm"></div>',
      ));
      var rolesTable = $(
        '<table class="dnnPermissionsGrid rolesGrid" cellspacing="0" cellpadding="2" style="border-collapse:collapse;"><tbody></tbody></table>',
      );

      this._buildGridHeader(
        rolesTable.find("tbody"),
        this.data.permissionDefinitions,
        "roles",
      );
      ...
      var usersTable = $(
        '<table class="dnnPermissionsGrid usersGrid" cellspacing="0" cellpadding="2" style="border-collapse:collapse;"><tbody></tbody></table>',
      );
      ...
      if (this.data.userPermissions.length === 0) {
        //hide users table if there is no users defined.
        usersTable.hide();
      }
    ...
    _buildGridHeader: function (table, definitions, type) {
      ...
      header.append(
        '<td class="permissionHeader">' +
          this._localizedString(type, "Type") +
          "</td>",
      );
      for (var i = 0; i < definitions.length; i++) {
        var def = definitions[i];
        var col = $(
          "<td>" + this._localizedString(def.permissionName) + "</td>",
        ).appendTo(header);
      ...
      $(
        '<td class="permissionGridActions">' +
          this._localizedString("Actions") +
          "</td>",
      )
        .appendTo(header)
        .prev()
    ...
    _buildGridRow: function (table, data, type) {
      ...
      row.append(
        '<td class="permissionHeader">' +
          (type == "users" ? data.displayName : data.roleName) +
          "</td>",
      );
      for (var i = 0; i < cols.length; i++) {
        ...
        if (permissionId) {
          ...
          if (permission || data.locked) {
            //permission has set to grant/deny.
            this._setPermission(
              col,
              permission ? (permission.allowAccess ? 1 : 2) : 0,
              data.locked,
            );
          } else {
          ...
          //add actions buttons if not default data(admin/registered users/all users)
          var actionCol = $('<td class="permissionGridActions"></td>').appendTo(
            row,
          );
          if (!data.default) {
            actionCol.append(
              '<a href="#" class="btn-delete" aria-label="Delete"></a>',
            );
          }
    ...
    _buildRoleSelector: function (rolesData) {
      ...
      rolesContainer.html(
        '<div class="leftGroup">' +
          '<label for="roleGroupSelector">' +
          this._localizedString("Filter By Group") +
          "</label>" +
          '<select id="roleGroupSelector"></select>' +
          "</div>" +
          '<div class="rightGroup">' +
          '<label for="roleSelector">' +
          this._localizedString("Select Role") +
          "</label>" +
          '<select id="roleSelector"></select>' +
          '<a class="simple-button btn-addrole" href="#">' +
          this._localizedString("Add") +
          "</a>" +
          "</div>",
      );
      ...
      for (var i = 0; i < rolesData.Groups.length; i++) {
        ...
        var option = new Option(
          this._localizedString(group.Name),
          group.GroupId,
        );
    ...
    _buildUserSelector: function () {
      ...
      userContainer.html(
        '<label for="permissionGrid_txtUser">' +
          this._localizedString("Add User") +
          "</label>" +
          ...
          '<a class="simple-button btn-adduser" href="#">' +
          this._localizedString("Add") +
          "</a>",
      );
    ...
    _handlerEvents: function () {
      this._gridContainer.on(
        "permissionChanged",
        "td",
        $.proxy(this._permissionChanged, this),
      );
      this._gridContainer.on(
        "click",
        "td span",
        $.proxy(this._changePermissionState, this),
      );
      this._gridContainer.on(
        "click",
        "td.permissionGridActions a.btn-delete",
        $.proxy(this._deletePermissionRow, this),
      );
      this._gridContainer.on(
        "change",
        "#roleGroupSelector",
        $.proxy(this._roleGroupChanged, this),
      );
      this._gridContainer.on(
        "click",
        ".roles-container a.btn-addrole",
        $.proxy(this._addRoleToGrid, this),
      );
      this._gridContainer.on(
        "click",
        ".users-container a.btn-adduser",
        $.proxy(this._addUserToGrid, this),
      );
    },
      ...
      if (isFullControl) {
        col
          ...
          .each(function () {
            var column = $(this);
            if (
              column.data("permissionId") &&
              column.data("permissionId") != permissionId
            ) {
        ...
        if ($fullControl.length > 0) {
          ...
          $notFullControl.each(function () {
            ...
            if (
              column.data("permissionId") &&
              column.find("span").data("permission-code") != currentState
            ) {
          ...
          handler._setPermission(
            $fullControl,
            setFullControl ? currentState : 0,
          );
        }
      }
    ...
    _roleGroupChanged: function (e) {
      ...
      for (var i = 0; i < rolesData.Roles.length; i++) {
        ...
        if (
          (groupId == -2 || role.GroupId == groupId) &&
          !this._roleExistsInGrid(role.RoleId)
        ) {
          var option = new Option(
            this._localizedString(role.Name),
            role.RoleId,
          );
    ...
    _setPermission: function (permissionCol, state, locked) {
      ...
      permissionCol
        .find("span")
        .attr("class", permissionState.css)
        .data("permission-code", state);
    },
    ...
    _localizedString: function (name, prefix, suffix) {
      var key =
        "permissiongrid" +
        "." +
        (prefix && prefix.length > 0 ? prefix + "-" + name : name) +
