...
const vocabularyTermActions = {
  getVocabularyTerms(vocabularyId) {
    return (dispatch) => {
      VocabularyTermService.getVocabularyTerms(
        vocabularyId,
        (data) => {
          dispatch({
  ...
  addVocabularyTerm(term, totalCount, callback) {
    return (dispatch) => {
      VocabularyTermService.addVocabularyTerm(
        term,
        (data) => {
  ...
  updateVocabularyTerm(payload, index, callback) {
    return (dispatch) => {
      VocabularyTermService.updateVocabularyTerm(
        payload,
        () => {
          dispatch({
  ...
  deleteVocabularyTerm(termId, vocabularyTerms, totalCount) {
    return (dispatch) => {
      VocabularyTermService.deleteVocabularyTerm(
        termId,
        () => {
          dispatch({
