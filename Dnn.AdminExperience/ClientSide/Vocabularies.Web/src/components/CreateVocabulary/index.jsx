...
class CreateVocabulary extends Component {
  ...
  onTermValueChange(key, event) {
    const value =
      typeof event === "object" ? event.target.value : parseInt(event); //event is value
  ...
  onAddNewVocabulary(event) {
    ...
    props.dispatch(
      VocabularyActions.addVocabulary(state.term, ++totalCount, () => {
        this.onCloseVocabulary();
        props.dispatch(
          PaginationActions.loadTab(
            this.getNextPage(0, 10000, state.term.ScopeTypeId),
          ),
  ...
  render() {
    ...
    return (
      ...
      <PersonaBarPageBody
        ...
        className={styles.createVocabulary}
          ...
          <GridCell className="create-box">
            <InputGroup>
              <SingleLineInputWithError
                ...
                errorMessage={LocalizedResources.get(
                  "TermValidationError.Message",
                )}
            ...
            <GridCell className="action-buttons">
              <Button
                ...
                onClick={this.onCloseVocabulary.bind(this)}
              >
                {LocalizedResources.get("cancelCreate")}
              </Button>
              <Button
                type="primary"
                onClick={this.onAddNewVocabulary.bind(this)}
              >
                {LocalizedResources.get("CreateVocabulary")}
              </Button>
              <span className="required-help-text">
                * {LocalizedResources.get("RequiredField")}
              </span>
            </GridCell>
          </GridCell>
        )}
