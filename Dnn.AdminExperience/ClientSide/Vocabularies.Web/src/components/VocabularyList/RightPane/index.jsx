...
class RightPane extends Component {
  ...
  openAddTerm(editMode, type, event) {
    ...
    termBeingEdited = {
      ...
      ParentTermId:
        type === "Hierarchy"
          ? (props.vocabularyTerms.length > 0 &&
              props.vocabularyTerms[0].TermId) ||
            -1
          : -1,
    };
    if (!editMode) {
    ...
    let parentTerm = JSON.parse(JSON.stringify(props.vocabularyTerms)).find(
      (term) => {
  ...
  getChildTerms(term, clickFunction, isEditable) {
    ...
    if (term.ChildTerms) {
      children = term.ChildTerms.map((child) => {
        ...
        return (
          <ul className="term-ul" key={"ul-" + term.TermId}>
            <Term
              term={child}
  ...
  deleteTerm() {
    ...
    util.utilities.confirm(
      LocalizedResources.get("ConfirmDeletion_Term").replace(
        "{0}",
        state.termBeingEdited.Name,
      ), //confirm message
  ...
  render() {
    ...
    let parentTermTree = getVocabularyTermList(
      removeChildTerms(_vocabularyTerms).filter((term) => {
        return term.TermId !== state.termBeingEdited.TermId;
      }),
      props.type,
      state.termBeingEdited.TermId,
    );

    parentTermTree = parentTermTree.map((term) => {
      let children = this.getChildTerms(
        term,
        this.onSelectParent.bind(this),
        false,
      );
      return (
        <Term
        ...
        <GridCell className="term-list">
          {state.editBoxOpened && (
            <AddTermBox
              ...
              termTreeVisible={
                ...
                (!state.editMode || state.termBeingEdited.ParentTermId > 0)
              }
          ...
          {!state._editBoxOpened && (
            <GridCell
              className={
                "term-list-content " +
                (!this.state.editBoxOpened ? "open" : "closed")
              }
            >
              <span className="term-list-label">
                {LocalizedResources.get("Terms") +
                  " (" +
                  props.totalTermCount +
                  ")"}
              </span>
              {this.canEdit() && (
                <div
