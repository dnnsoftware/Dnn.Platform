...
const AddTermBox = ({
  ...
  <Collapsible isOpened={isOpened}>
    <div className={styles.addTermBox}>
      <p className="add-term-title">
        {editMode
          ? LocalizedResources.get("CurrentTerm")
      ...
      {termTreeVisible && (
        <InputGroup style={{ marginBottom: 32 }}>
          ...
          <p className="parent-display" onClick={toggleParentTree}>
            {(parentDisplay && parentDisplay.Name) || ""}
          </p>
          <Collapsible
            isOpened={parentTreeOpened}
            theme={{
              collapse: "parent-term-collapse ReactCollapse--collapse",
      ...
      <div className="add-term-footer">
        {editMode && (
          <Button type="secondary" onClick={deleteTerm}>
            {LocalizedResources.get("DeleteTerm")}
          </Button>
        )}
        <Button type="secondary" onClick={closeAddTerm}>
          {LocalizedResources.get("cancelCreate")}
        </Button>
        <Button type="primary" onClick={onUpdateTerm}>
          {LocalizedResources.get("SaveTerm")}
        </Button>
      </div>
    </div>
  </Collapsible>
