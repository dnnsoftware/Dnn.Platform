...
class Term extends Component {
  ...
  render() {
    ...
    const className =
      styles.termLi +
      (props.children.length > 0 ? " has-children" : "") +
      (state.isOpened ? " opened" : "") +
      (props.term.selected ? " selected" : "");

    return (
      <li className={className}>
        <div>
          {props.children.length > 0 && !state.isOpened && (
            <div className="edit-svg" onClick={this.toggleTerm.bind(this)}>
          ...
          <div onClick={this.onLiClick.bind(this)}>
            <span className="term-name">
              <Html html={props.term.Name} />
            </span>
            {props.isEditable && util.canEdit() && (
              <div className="edit-button" onClick={this.onClick.bind(this)}>
                <SvgIcons.EditIcon />
              </div>
            )}
          </div>
