...
class VocabularyListComponent extends Component {
  ...
  componentDidMount() {
    const { props } = this;
    props.dispatch(
      VocabularyActions.getVocabularyList(
        this.getNextPage(
          props.pagination.pageIndex,
          props.pagination.pageSize,
          props.pagination.scopeTypeId,
        ),
      ),
    );
  }
  ...
  getVocabularyTerms(vocabularyId, index) {
    const { props } = this;
    props.dispatch(
      VocabularyTermListActions.getVocabularyTerms(vocabularyId, index),
    );
  }
  ...
  onUpdateTerm(termBeingEdited, editMode, callback) {
    ...
    if (editMode) {
      ...
      props.dispatch(
        VocabularyTermListActions.updateVocabularyTerm(
          getTermUpdates(termBeingEdited),
          index,
          callback,
        ),
      );
      ...
      props.dispatch(
        VocabularyTermListActions.addVocabularyTerm(
          getTermUpdates(termBeingEdited),
          ++totalTermCount,
          callback,
        ),
  ...
  handleSelect(index /*,last*/) {
    const { props } = this;
    props.dispatch(
      PaginationActions.loadTab(
        this.getNextPage(0, props.pagination.pageSize, index),
      ),
    ); //index acts as scopeTypeId
  }
  onDeleteVocabulary(vocabulary, index, callback) {
    const { props } = this;

    util.utilities.confirm(
      LocalizedResources.get("ConfirmDeletion_Vocabulary").replace(
        "{0}",
        vocabulary.Name,
      ...
      () => {
        ...
        props.dispatch(
          VocabularyActions.deleteVocabulary(
            ...
            () => {
              if (props.vocabularyList.length < props.totalCount) {
                let { pageIndex } = props.pagination; //copy page index
                let currentPage = this.getNextPage(
                  pageIndex,
                  props.pagination.pageSize,
                  props.pagination.scopeTypeId,
                );
  ...
  onDeleteTerm(termId) {
    ...
    props.dispatch(
      VocabularyTermListActions.deleteVocabularyTerm(
        termId,
        vocabularyTerms,
      ...
      <div className="vocabulary-list">
        <PersonaBarPageBody>
          <Tabs
            ...
            tabHeaders={[
              LocalizedResources.get("All"),
              LocalizedResources.get("Application"),
              LocalizedResources.get("Portal"),
            ]}
            type="secondary"
          >
            {(renderedVocabularyList.length > 0 && renderedVocabularyList) || (
              <p className="vocabulary-error">
                {LocalizedResources.get("NoVocabularyTerms.Error")}
              </p>
            )}
            {(renderedVocabularyList.length > 0 && renderedVocabularyList) || (
              <p className="vocabulary-error">
        ...
        {loadMoreEnabled && (
          ...
          >
            {LocalizedResources.get("LoadMore")}
          </Button>
        )}
      </div>
    );
...
VocabularyListComponent.propTypes = {
  ...
  scopeTypeId: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
};
