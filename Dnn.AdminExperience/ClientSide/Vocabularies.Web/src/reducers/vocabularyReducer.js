...
export default function vocabularyList(
  ...
  switch (action.type) {
    case PaginationActionTypes.LOAD_MORE:
      return {
        ...state,
        vocabularyList: insertRecords(
          JSON.parse(JSON.stringify(state.vocabularyList)),
          action.data.vocabularyList,
        ),
    ...
    case ActionTypes.UPDATED_VOCABULARY_TERM:
      return {
        ...state,
        vocabularyTerms: [
          ...state.vocabularyTerms.slice(0, action.payload.index),
          action.payload.updatedTerm,
          ...state.vocabularyTerms.slice(action.payload.index + 1),
        ],
      };
    ...
    case ActionTypes.ADDED_VOCABULARY:
      return {
        vocabularyAddedIsValid: true,
        vocabularyList: state.vocabularyList.concat(
          action.payload.addedVocabulary,
        ),
    ...
    case ActionTypes.FAILED_TO_ADD_VOCABULARY:
      return { ...state, vocabularyAddedIsValid: false };
    default:
      return { ...state };
  }
}
