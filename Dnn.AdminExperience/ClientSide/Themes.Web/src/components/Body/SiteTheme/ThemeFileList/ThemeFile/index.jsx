...
class ThemeFile extends Component {
  ...
  selectedAsSite() {
    ...
    if (themeFile.type === 0) {
      return (
        currentTheme.SiteLayout.path.toLowerCase() ===
        themeFile.path.toLowerCase()
      );
    } else {
      return (
        currentTheme.SiteContainer.path.toLowerCase() ===
        themeFile.path.toLowerCase()
      );
    }
  }
  ...
  selectedAsEdit() {
    ...
    if (themeFile.type === 0) {
      return (
        currentTheme.EditLayout.path.toLowerCase() ===
        themeFile.path.toLowerCase()
      );
    } else {
      return (
        currentTheme.EditContainer.path.toLowerCase() ===
        themeFile.path.toLowerCase()
      );
    }
  }
  ...
  getClassName() {
    ...
    let themeFile = props.themeFile;
    let className =
      themeFile.type === 0 ? "theme-file-skin" : "theme-file-container";
    ...
    return (
      <span className="actions">
        {!this.selectedAsSite() ? (
          ...
          >
            {type == 0
              ? Localization.get("SetSiteLayout")
              : Localization.get("SetSiteContainer")}
          </a>
        ) : null}
        {!this.selectedAsEdit() ? (
          <a
          ...
          >
            {type == 0
              ? Localization.get("SetEditLayout")
              : Localization.get("SetEditContainer")}
          </a>
        ) : null}
      </span>
    );
  ...
  renderThumbnail() {
    ...
    return (
      <span className={className}>
        {themeFile.thumbnail ? (
          <img src={themeFile.thumbnail} alt={themeFile.name} />
        ) : (
          <SvgIcon name="EmptyThumbnail" />
        )}
