...
class ThemeService {
  ...
  getCurrentTheme(callback, errorCallback) {
    ...
    sf.get(
      "GetCurrentTheme?language=" + utils.utilities.getCulture(),
      {},
      callback,
      errorCallback,
    );
  ...
  getThemeFiles(themeName, themeType, themeLevel, callback, errorCallback) {
    ...
    sf.get(
      "GetThemeFiles",
      { themeName: themeName, type: themeType, level: themeLevel },
      callback,
      errorCallback,
    );
  }
  ...
  applyTheme(themeFile, scope, callback, errorCallback) {
    const sf = this.getServiceFramework("Themes");
    sf.post(
      "ApplyTheme?language=" + utils.utilities.getCulture(),
      { themeFile: themeFile, scope: scope },
      callback,
      errorCallback,
    );
  }
  ...
  updateTheme(path, token, setting, value, callback, errorCallback) {
    ...
    sf.post(
      "UpdateTheme",
      { path: path, token: token, setting: setting, value: value },
      callback,
      errorCallback,
    );
  }
  ...
  parseTheme(themeName, parseType, callback, errorCallback) {
    ...
    sf.post(
      "ParseTheme",
      { themeName: themeName, parseType: parseType },
      callback,
      errorCallback,
    );
  }
  ...
  restoreTheme(callback, errorCallback) {
    const sf = this.getServiceFramework("Themes");
    sf.post(
      "RestoreTheme?language=" + utils.utilities.getCulture(),
      {},
      callback,
      errorCallback,
    );
  ...
  applyDefaultTheme(themeName, level, callback, errorCallback) {
    const sf = this.getServiceFramework("Themes");
    sf.post(
      "ApplyDefaultTheme?language=" + utils.utilities.getCulture(),
      { themeName: themeName, level: level },
      callback,
      errorCallback,
    );
  }
