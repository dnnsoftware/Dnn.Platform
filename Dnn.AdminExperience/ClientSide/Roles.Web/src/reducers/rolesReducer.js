import {
  roles as ActionTypes,
  roleUsers as RoleUserActionTypes,
  ...
  switch (action.type) {
    case ActionTypes.RETRIEVED_GROUPS_LIST: {
      let roleGroups = [
        {
          id: -2,
          name: resx.get("AllGroups"),
          description: resx.get("AllGroups"),
        },
        {
          id: -1,
          name: resx.get("GlobalRolesGroup"),
          description: resx.get("GlobalRolesGroup"),
        },
      ].concat(action.data.roleGroups);
      return { ...state, roleGroups: roleGroups };
    }
    case ActionTypes.RETRIEVED_ROLES_LIST:
      return {
        ...state,
        rolesList: action.reload
          ? action.data.rolesList
          : state.rolesList.concat(action.data.rolesList),
        loadMore: action.loadMore,
        rsvpLink: action.rsvpLink,
      };
    case ActionTypes.UPDATE_ROLEGROUP: {
      let roleGroups = Object.assign(
        [],
        JSON.parse(JSON.stringify(state.roleGroups)),
      );
      return {
        ...state,
    ...
    case ActionTypes.UPDATE_ROLE: {
      let rolesList = Object.assign(
        [],
        JSON.parse(JSON.stringify(state.rolesList)),
      );
      if (
        action.data.currentGroupId === action.data.roleDetails.groupId ||
        action.data.currentGroupId === -2
      ) {
        return {
          ...state,
    ...
    case RoleUserActionTypes.ADD_USER_INTO_ROLE: {
      if (action.data.isAdd) {
        ...
        return {
          ...state,
          rolesList: incrementUsersCountFromRoleList(
            rolesList,
            action.data.roleUserDetails.roleId,
          ),
        };
      }
      return { ...state };
    ...
    case RoleUserActionTypes.REMOVE_USER: {
      ...
      return {
        ...state,
        rolesList: decrementUsersCountFromRoleList(
          rolesList,
          action.data.RoleId,
        ),
      };
