...
export default function roleUsersReducer(
  state = {
    matchedUsers: [],
    roleUsers: [],
    totalRecords: 0,
  ...
  switch (action.type) {
    case ActionTypes.RETRIEVED_SUGGEST_LIST:
      return { ...state, matchedUsers: action.data.matchedUsers };
    ...
    case ActionTypes.ADD_USER_INTO_ROLE: {
      let roleUsers = Object.assign(
        [],
        JSON.parse(JSON.stringify(state.roleUsers)),
      );
      return {
        ...state,
