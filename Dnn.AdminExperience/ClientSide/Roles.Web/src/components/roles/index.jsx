...
import { createRoleGroupOptions } from "./helpers/roles";
...
class RolesPanel extends Component {
  ...
  componentDidMount() {
    ...
    deleteAllowed =
      props.rolesList !== undefined && props.rolesList.length === 0;
  ...
  refreshRolesList() {
    ...
    props.dispatch(
      RolesActions.getRolesList(state.parameter, (data) => {
  ...
  renderHeader() {
    ...
    return (
      <div id="users-header-row" className="users-header-row">
        {tableHeaders}
      </div>
    );
  }
  renderedRolesList(roleGroupOptions, securityModeOptions, statusOptions) {
    if (this.props.rolesList.length > 0) {
      let validRolesList = this.props.rolesList.filter(
