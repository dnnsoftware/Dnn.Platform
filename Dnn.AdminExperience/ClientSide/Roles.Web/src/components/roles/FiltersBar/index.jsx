...
class FiltersBar extends Component {
  ...
  onDeleteGroup(e) {
    ...
    util.utilities.confirm(
      resx.get("DeleteRoleGroup.Confirm"),
      resx.get("Delete"),
      resx.get("Cancel"),
      () => {
        props.dispatch(
          RolesActions.deleteRoleGroup(this.getCurrentGroup(), () => {
  ...
  getCurrentLabel() {
    ...
    if (
      props.roleGroups.some(
        (group) => group.id === this.state.selectedGroup.value,
      )
    ) {
      group = Object.assign(
        {},
        props.roleGroups.filter((group) => group.id === value)[0],
      );
    }
    if (value > -1) {
      if (canEdit) {
