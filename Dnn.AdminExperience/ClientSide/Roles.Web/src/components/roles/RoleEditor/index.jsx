...
import { roles as RolesActions } from "../../../actions";
...
class RolesEditor extends Component {
  ...
  getValue(selectKey) {
    ...
    switch (selectKey) {
      case "RoleGroup":
        return state.roleDetails.groupId !== undefined
          ? state.roleDetails.groupId
          : -1;
      case "SecurityMode":
        return state.roleDetails.securityMode !== undefined
          ? state.roleDetails.securityMode
          : 0;
      case "Status":
        return state.roleDetails.status !== undefined
          ? state.roleDetails.status
          : 1;
      case "AutoAssignment":
        return state.roleDetails.autoAssign !== undefined
          ? state.roleDetails.autoAssign
          : false;
      case "Public":
        return state.roleDetails.isPublic !== undefined
          ? state.roleDetails.isPublic
          : false;
      default:
        break;
    }
  ...
  deleteRole(event) {
    ...
    if (props.roleId > 0) {
      util.utilities.confirm(
        resx.get("DeleteRole.Confirm"),
        resx.get("Delete"),
        resx.get("Cancel"),
        () => {
          props.dispatch(
            RolesActions.deleteRole(roleDetails, () => {
  ...
  getRoleGroupOptions() {
    ...
    groupOptions = [
      {
        label: <span className="do-not-close">{resx.get("lblNewGroup")}</span>,
        value: -3,
      },
    ].concat(groupOptions);
    return groupOptions;
    ...
    const columnOne = (
      <div key="editor-container-columnOne" className="editor-container">
        <div className="editor-row divider" style={{ marginTop: "-3rem" }}>
          <Label label={resx.get("RoleId") + ": " + state.roleDetails.id} />
        </div>
    ...
    const columnTwo = (
      ...
      >
        ...
        <div className="editor-row divider">
          <SingleLineInputWithError
            value={
              state.roleDetails.rsvpCode && state.roleDetails.rsvpCode !== ""
    ...
    return (
      <div className="role-details-editor">
        ...
        <div className="buttons-box">
          ...
          <Button type="primary" onClick={this.addUpdateRoleDetails.bind(this)}>
            {resx.get("Save")}
          </Button>
        </div>
      </div>
