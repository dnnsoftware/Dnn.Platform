...
class UsersInRole extends Component {
  ...
  getUsers() {
    ...
    props.dispatch(
      RoleUsersActions.getRoleUsers(parameter, () => {
  ...
  getSuggestUsers() {
    const { props, state } = this;
    let keyword =
      state.userSelectState.userId > 0 ? "" : state.userSelectState.keyword;
    props.dispatch(
      RoleUsersActions.getSuggestUsers({
        keyword: keyword,
        roleId: -1,
        count: 10,
      }),
  ...
  onUserSelectorSelected(item) {
    ...
    this.setState(
      { userSelectState: { userId: item.userId, keyword: item.displayName } },
      function () {
        this.getSuggestUsers();
      },
    );
  ...
  saveUser(isAdd, userId, startTime, expiresTime) {
    ...
    props.dispatch(
      RoleUsersActions.addUserToRole(
        parameter,
        ...
        >
          {field.name !== "" ? (
            ...
            <div className="search-container">
              <SearchBox
                placeholder={resx.get("Search")}
                onSearch={this.onUsersKeywordChanged.bind(this)}
                maxLength={50}
              />
              <div className="clear" />
            </div>
  ...
  render() {
    ...
    return (
      <div className={className}>
          ...
          <div className="add-box">
            ...
            <GridCell columnSize={50}>
              <span>
                ...
                >
                  <IconButton type="add" width={17} height={15} />{" "}
                  {resx.get("Add")}
                </div>
              </span>
            </GridCell>
        ...
        <div className="role-users-list-paging">{this.renderPaging()}</div>
      </div>
    );
  }
}
