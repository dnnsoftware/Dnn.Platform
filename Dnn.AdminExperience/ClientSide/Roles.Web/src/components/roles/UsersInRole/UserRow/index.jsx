...
class UserRow extends Component {
  ...
  onChange(userRole, command, FirstDate) {
    ...
    let expiresTime =
      command === "expiresTime" ? FirstDate : userRole.expiresTime;
  ...
  getBoundDate(userRole, command) {
    if (command === "startTime") {
      ...
      if (!this.isEmptyDate(userRole.expiresTime)) {
        maxValue = new Date(
          new Date().setTime(
            new Date(userRole.expiresTime).getTime() - 1 * 86400000,
          ),
        );
      }
      return maxValue;
      ...
      if (!this.isEmptyDate(userRole.startTime)) {
        minValue = new Date(
          new Date().setTime(
            new Date(userRole.startTime).getTime() + 1 * 86400000,
          ),
        );
      }
      return minValue;
  ...
  createUserActions() {
    ...
    let startTimeAction = props.userDetails.allowExpired ? (
      <span>
        <DatePicker
          ...
          mode={"start"}
          applyButtonText={resx.get("Apply")}
          showIcon={true}
          showInput={false}
    ...
    let expiresTimeAction = props.userDetails.allowExpired ? (
      <span>
        <DatePicker
    ...
    let deleteAction = props.userDetails.allowDelete ? (
      <IconButton
        customIcon={XIcon}
        width={17}
    ...
    return (
      <div className={state.editIndex === props.index ? "edit-row" : null}>
        {deleteAction}
        {expiresTimeAction}
        {startTimeAction}
      </div>
    ...
    return (
      <div className="role-user-row">
        ...
        <GridCell columnSize={35}>
          <div className="actions">{this.createUserActions()}</div>
        </GridCell>
      </div>
    );
  }
