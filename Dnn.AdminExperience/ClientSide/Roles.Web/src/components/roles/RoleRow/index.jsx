...
class RoleRow extends Component {
  constructor() {
    ...
    canEdit =
      ...
      util.settings.permissions.EDIT;
  }
  componentDidMount() {
    let opened =
        ...
        >
          {!props.addIsClosed && (
            <div className={"collapsible-header1 " + !opened}>
              ...
              <GridCell columnSize={20}>{props.groupName}</GridCell>
              <GridCell columnSize={15}>{props.userCount}</GridCell>
              <GridCell columnSize={15}>
                {props.auto ? (
                  <IconButton className="icon-flat" customIcon={CheckIcon} />
                ) : (
                  <div>&nbsp; </div>
                )}{" "}
              </GridCell>
                ...
                <GridCell columnSize={10}>
                  {props.id !== "add" && props.roleIsApproved && (
                    <IconButton
                      type="user"
                      className={
                        "edit-icon " + !(opened && props.currentIndex === 0)
                      }
                  ...
                  <IconButton
                    type="edit"
                    className={
                      "edit-icon " + !(opened && props.currentIndex === 1)
                    }
