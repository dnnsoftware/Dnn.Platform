...
class SecurityBulletinsPanelBody extends Component {
  ...
  UNSAFE_componentWillMount() {
    ...
    props.dispatch(
      SecurityActions.getSecurityBulletins(
        () => {},
        (error) => {
  ...
  renderHeader() {
    const tableFields = [{ name: resx.get("Bulletins"), id: "Bulletins" }];
        ...
        return (
          ...
          >
            <div className="bulletin-detail">
              <div style={{ fontWeight: "bolder", margin: "0 0 15px 0" }}>
                {resx.get("BulletinDescription")}
              </div>
              <div className="bulletin-detail-desc">{term.Description}</div>
              <div style={{ fontWeight: "bolder", margin: "15px 0 15px 0" }}>
                {resx.get("BulletinLink")}
              </div>
              <div className="bulletin-detail-link">
                <a target="_blank" href={term.Link} rel="noopener noreferrer">
                  {term.Link}
                </a>
              </div>
      ...
      if (props.securityBulletins && props.securityBulletins.length > 0) {
        ...
        return (
          <div style={{ margin: "25px" }}>
            {resx
              .get("BulletinsDoNotExist")
              .replace("{0}", props.platformVersion)}
          </div>
        );
      }
    }
