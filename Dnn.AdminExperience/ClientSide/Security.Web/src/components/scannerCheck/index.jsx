...
class ScannerCheckPanelBody extends Component {
  ...
  onKeywordChanged(keyword) {
    ...
    if (keyword === props.scannerCheckKeyword) {
      props.dispatch(
        SecurityActions.updatefileSystemAndDatabaseActiveTab("search"),
      );
      return;
    }
    ...
    if (keyword && keyword !== "") {
      props.dispatch(
        SecurityActions.searchFileSystemAndDatabase(
          { term: keyword },
          () => {},
        ),
      );
    ...
    props.dispatch(
      SecurityActions.updatefileSystemAndDatabaseSearchKeyword(keyword),
    );
  }
  ...
  getModifiedSettings() {
    ...
    if (props.modifiedSettings) {
      props.dispatch(
        SecurityActions.updatefileSystemAndDatabaseActiveTab("settings"),
      );
      return;
    }
  ...
  getModifiedFiles() {
    ...
    if (props.modifiedFiles) {
      props.dispatch(
        SecurityActions.updatefileSystemAndDatabaseActiveTab("files"),
      );
      return;
    }
    ...
    return (
      <div className={styles.scannerCheckResults}>
        <div className="scannercheck-topbar">
          <div
            className={
              props.scannerCheckActiveTab === "search"
          ...
          >
            <SearchBox
              ...
              initialValue={
                props.scannerCheckKeyword &&
                props.scannerCheckKeyword.length > 0
                  ? props.scannerCheckKeyword
                  : null
              }
          ...
          >
            <TextOverflowWrapper
              text={resx.get("ModifiedSettings")}
              maxWidth={150}
            />
          </div>
          <div
            className={
              this.showModifiedFiles() ? "files-filter-active" : "files-filter"
            }
            onClick={this.getModifiedFiles.bind(this)}
          >
            <TextOverflowWrapper
              text={resx.get("ModifiedFiles")}
              maxWidth={150}
            />
          </div>
        </div>
        {this.showSearchResults() && props.searchResults.FoundInFiles && (
          <div>
            <div className="scannerCheckItems-title">
              {resx
                .get("SearchFileSystemResult")
                .replace("{0}", props.searchResults.FoundInFiles.length)}
            </div>
        ...
        {this.showSearchResults() && props.searchResults.FoundInDatabase && (
          <div>
            <div className="scannerCheckItems-title">
              {resx
                .get("SearchDatabaseResult")
                .replace("{0}", props.searchResults.FoundInDatabase.length)}
            </div>
