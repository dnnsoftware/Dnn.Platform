...
class AuditCheckPanelBody extends Component {
  ...
  getResultDisplay(result) {
    ...
    switch (severity) {
      case 0:
        return (
          <div className="label-result-severity">
            <div className="label-result-severity-pass">{resx.get("Pass")}</div>
            <div>{successText}</div>
          </div>
        );
      case 1:
      ...
      case 2:
        return (
          <div className="label-result-severity">
            <div className="label-result-severity-fail">{resx.get("Fail")}</div>
            <div>
              <Html html={failureText} />
            </div>
          </div>
        );
      default:
  ...
  onAuditCheck(result) {
    ...
    this.setState({ checking: true }, () => {
      props.dispatch(
        SecurityActions.getAuditCheckResult(result.CheckName, () => {
  ...
  render() {
    const { props } = this;
    let contentShouldShow =
      props.auditCheckResults && props.auditCheckResults.length > 0
        ? true
        : false;
    return (
      <ContentLoadWrapper
