...
export class Body extends Component {
  constructor() {
    ...
    canViewBasicLoginSettings =
      isHost || isAdmin || util.settings.permissions.BASIC_LOGIN_SETTINGS_VIEW;
    canViewRegistrationSettings =
      isHost || isAdmin || util.settings.permissions.REGISTRATION_SETTINGS_VIEW;
  }
  ...
  renderTabs() {
    ...
    if (isHost || canViewRegistrationSettings) {
      ...
      if (canViewRegistrationSettings) {
        memberAccountsTabHeaders.push(
          <div style={{ marginLeft: isHost ? 35 : 0 }}>
            <div
              style={{
        ...
        memberAccountsTabs.push(
          <RegistrationSettings key="registrationSettings" />,
        );
      }
    }
    if (isAdmin) {
    ...
    if (isHost) {
      securityTabs.push(
        <Tabs
          ...
          onSelect={this.handleSelect.bind(this)}
          tabHeaders={[
            resx.get("TabAuditChecks"),
            resx.get("TabScannerCheck"),
            resx.get("TabSuperuserActivity"),
          ]}
          type="secondary"
        >
          <AuditCheck />
  ...
  render() {
    return <PersonaBarPageBody>{this.renderTabs()}</PersonaBarPageBody>;
  }
}
