...
class IpFilterEditor extends Component {
        ...
        SecurityActions.getIpFilter(
          ...
          (data) => {
            ...
            this.setState({
              ...
              ruleSpecificity:
                ipFilter.SubnetMask === "" ? SINGLE_IP : IP_RANGE,
              ipFilter,
            });
          },
        ),
  ...
  validateIPAddressContainsError() {
    ...
    if (state.ruleSpecificity === IP_RANGE) {
      state.error["mask"] = !this.isValidIpAddress(
        state.ipFilter["SubnetMask"],
      );
    } else {
  ...
  render() {
    ...
    return (
      <div className="ip-filter-setting-editor">
        <GridSystem numberOfColumns={1}>{children}</GridSystem>
