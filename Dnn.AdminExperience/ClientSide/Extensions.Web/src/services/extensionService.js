...
function mapPackageInformation(extensionBeingUpdated, addMode) {
  let _extensionBeingUpdated = {
    ...
    License:
      extensionBeingUpdated.releaseNotes && extensionBeingUpdated.license.value,
    ReleaseNotes:
      extensionBeingUpdated.releaseNotes &&
      extensionBeingUpdated.releaseNotes.value,
  };
  if (addMode) {
    delete _extensionBeingUpdated.License;
    delete _extensionBeingUpdated.ReleaseNotes;
    _extensionBeingUpdated.PackageType =
      extensionBeingUpdated.packageType.value;
  }
...
class ExtensionService {
  ...
  deployAvailablePackage(cultureCode, callback, errorCallback) {
    const sf = this.getServiceFramework("Extensions");
    sf.post(
      "ParseLanguagePackage?cultureCode=" + cultureCode,
      {},
      callback,
      errorCallback,
    );
  ...
  getPackageSettings(packageId, callback, errorCallback) {
    ...
    sf.get(
      "GetPackageSettings?" + serializeQueryStringParameters(parameters),
      {},
      callback,
      errorCallback,
    );
  ...
  getPackageUsage(portalId, packageId, callback) {
    ...
    sf.get(
      "GetPackageUsage?" + serializeQueryStringParameters(parameters),
      {},
      callback,
    );
  }
