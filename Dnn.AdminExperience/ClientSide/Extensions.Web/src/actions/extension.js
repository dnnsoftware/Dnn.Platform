import {
  extension as ActionTypes,
  installation as InstallationActionTypes,
...
function errorCallback(error) {
  let message = utilities.utilities.getResx(
    "SharedResources",
...
const extensionActions = {
  getInstalledPackages(type, callback) {
    return (dispatch) => {
      ExtensionService.getInstalledPackages(
        type,
        (payload) => {
          dispatch({
  ...
  getAvailablePackages(type, callback) {
    return (dispatch) => {
      ExtensionService.getAvailablePackages(
        type,
        (payload) => {
          dispatch({
            ...
            payload: {
              Results:
                payload.Results &&
                payload.Results[0] &&
                payload.Results[0].ValidPackages,
      ...
      ExtensionService.updateExtension(
        ...
        (data) => {
          ...
          utilities.utilities.notify(
            Localization.get("EditExtension_Notify.Success"),
          );
          if (callback) {
  ...
  installAvailablePackage(packageType, packageName, newExtension, callback) {
    return (dispatch) => {
      ExtensionService.installAvailablePackage(
        packageType,
        packageName,
        (data) => {
          if (data.success) {
  ...
  parseAvailablePackage(packageType, fileName, callback) {
    return (dispatch) => {
      ExtensionService.parseAvailablePackage(
        packageType,
        fileName,
        (data) => {
          dispatch({
  ...
  deletePackage(payload, index, callback) {
    return (dispatch) => {
      ExtensionService.deletePackage(
        payload,
        (data) => {
          dispatch({
  ...
  createNewModule(payload, shouldAppend, callback) {
    return (dispatch) => {
      ExtensionService.createNewModule(
        payload,
        (data) => {
          if (shouldAppend) {
            dispatch({
  ...
  createNewExtension(extensionBeingAdded, editorActions, index, callback) {
    return (dispatch) => {
      ExtensionService.createNewExtension(
        extensionBeingAdded,
        editorActions,
        (data) => {
          let _extensionBeingAdded = JSON.parse(
            JSON.stringify(extensionBeingAdded),
          );
  ...
  editExtension(parameters, extensionBeingEditedIndex, callback) {
    return (dispatch) => {
      ExtensionService.getPackageSettings(
        parameters,
        (data) => {
          dispatch({
            type: ActionTypes.EDIT_EXTENSION,
  ...
  deployAvailablePackage(_package, updatedPackageIndex, callback) {
    return () => {
      ExtensionService.deployAvailablePackage(
  ...
  getPackageUsage(portalId, packageId, callback) {
    return (dispatch) => {
      ExtensionService.getPackageUsage(
        portalId,
        packageId,
        (data) => {
          dispatch({
