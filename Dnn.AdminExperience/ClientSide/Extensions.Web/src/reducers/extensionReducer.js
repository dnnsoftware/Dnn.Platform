  ...
  switch (action.type) {
    case ActionTypes.RETRIEVED_INSTALLED_PACKAGES:
      return {
        ...
        selectedInstalledPackageType:
          action.payload.selectedInstalledPackageType,
      };
    case ActionTypes.RETRIEVED_AVAILABLE_PACKAGES:
      return {
        ...
        selectedAvailablePackageType:
          action.payload.selectedAvailablePackageType,
      };
    case ActionTypes.UPDATED_EXTENSION:
      return {
        ...state,
        installedPackages: [
          ...state.installedPackages.slice(0, action.payload.index),
          action.payload.updatedExtension,
          ...state.installedPackages.slice(action.payload.index + 1),
        ],
      };
    case ActionTypes.DELETED_EXTENSION:
      return {
        ...state,
        installedPackages: removeRecordFromArray(
        ...
        tabsWithError:
          action.payload.action === "remove"
            ? removeRecordFromArray(
                state.tabsWithError,
                state.tabsWithError.indexOf(action.payload.tabIndex),
              )
            : state.tabsWithError.indexOf(action.payload.tabIndex) < 0
              ? state.tabsWithError.concat(action.payload.tabIndex)
              : state.tabsWithError,
      };
