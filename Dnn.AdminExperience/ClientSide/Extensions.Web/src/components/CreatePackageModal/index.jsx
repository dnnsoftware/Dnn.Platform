...
class CreatePackage extends Component {
  ...
  UNSAFE_componentWillMount() {
    ...
    _packagePayload.archiveName =
      props.packageManifest.name +
      "_" +
      props.packageManifest.version +
      "_Install.zip";
  ...
  onChange(key, event) {
    ...
    if (key === "useExistingManifest") {
      if (!_packagePayload.selectedManifest && value) {
        const selectedManifestKey = Object.keys(
          props.packageManifest.manifests,
        )[0];
        _packagePayload.selectedManifest =
          props.packageManifest.manifests[selectedManifestKey];
  ...
  goToStepFour() {
    ...
    if (props.packagePayload.reviewManifest) {
      props.dispatch(
        CreatePackageActions.generateManifestPreview(
          mapToManifestPayload(
            deepCopy(props.packagePayload),
            deepCopy(props.packageManifest),
          ),
          () => {
            this.goToStep(3);
          },
  ...
  createPackage() {
    ...
    if (props.packagePayload.createManifest) {
      props.dispatch(
        CreatePackageActions.createManifest(
          mapToManifestPayload(
            deepCopy(props.packagePayload),
            deepCopy(props.packageManifest),
          ),
    ...
    if (props.packagePayload.createPackage) {
      props.dispatch(
        CreatePackageActions.createPackage(
          mapToPackagePayload(
            deepCopy(props.packagePayload),
            deepCopy(props.packageManifest),
          ),
  ...
  onRefresh() {
    ...
    props.dispatch(
      CreatePackageActions.refreshPackageFiles(
        mapToFileRequestPayload(deepCopy(props.packageManifest)),
        () => {},
        (data) => {
          utilities.utilities.notifyError(
            JSON.parse(data.responseText).Message,
  ...
  getCurrentWizardUI(step) {
    ...
    const version = packageManifest.version
      ? packageManifest.version.split(".")
      : [0, 0, 0];
    const manifestDropdown = this.getManifestDropdown(
      props.packageManifest.manifests,
    );
    switch (step) {
      case 0:
      ...
      default:
        return (
          <p>
            ...
            <a href="javascript:void(0)" onClick={props.onCancel.bind(this)}>
              {" "}
              here{" "}
            </a>{" "}
            to go back
  ...
  render() {
    ...
    return (
      <GridCell className={styles.createPackage}>
        ...
        <PersonaBarPageBody
          backToLinkProps={{
            text: Localization.get("BackToEditExtension").replace(
              "{0}",
              packageManifest.friendlyName,
            ),
