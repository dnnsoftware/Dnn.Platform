...
class DeleteExtension extends Component {
  ...
  render() {
    ...
    const version = extensionBeingDeleted.version
      ? extensionBeingDeleted.version.split(".")
      : [0, 0, 0];
    return (
      <GridCell className={styles.DeleteExtension}>
        <PersonaBarPageHeader
          title={Localization.get("DeleteExtension.Action").replace(
            "{0}",
            extensionBeingDeleted.friendlyName,
          )}
        />
        <PersonaBarPageBody
          backToLinkProps={{
        ...
        >
          <GridCell className="delete-extension-box extension-form">
            ...
            <GridCell className="modal-footer">
              ...
              <Button type="primary" onClick={this.onDelete.bind(this)}>
