...
class Controls extends Component {
    ...
    props.dispatch(
      ExtensionActions.updateExtension(
        ...
        () => {
          ...
          props.onControlSave();
          props.dispatch(
            ModuleDefinitionActions.setControlFormDirt(false, () => {
              this.exitEditMode();
            }),
          );
        },
  ...
  onEdit(controlBeingEdited, controlBeingEditedIndex) {
    this.confirmAction(() => {
      const { props } = this;
      const sourceFolder =
        getSourceFolder(controlBeingEdited.source) || "Admin/Skins/";
      props.dispatch(ModuleDefinitionActions.getSourceFolders());
      props.dispatch(
        ModuleDefinitionActions.getSourceFiles(sourceFolder, () => {
          props.dispatch(
            ModuleDefinitionActions.getControlIcons(
              controlBeingEdited.source,
              () => {
                this.setState({
                  editMode: true,
                  controlBeingEdited,
  ...
  onSelectSourceFolder(option) {
    ...
    props.dispatch(
      ModuleDefinitionActions.getSourceFiles(option.value, () => {
        if (props.sourceFolders.length > 0) {
          props.dispatch(
            ModuleDefinitionActions.getControlIcons(option.value),
            () => {
              this.setState({
                selectedSourceFolder: option.value,
              });
  ...
  render() {
    ...
    const moduleControls =
      props.moduleControls &&
      props.moduleControls.map((moduleControl, index) => {
        return (
          <ControlRow
            ...
            onEdit={this.onEdit.bind(
              this,
              Object.assign({}, moduleControl),
              index,
            )}
            key={index}
          />
        ...
        <GridCell className="module-controls-table">
          ...
          >
            <ControlFields
              ...
              onSelectSourceFolder={this.onSelectSourceFolder.bind(this)}
              selectedSourceFolder={state.selectedSourceFolder}
              onCancel={this.exitEditMode.bind(
                this,
