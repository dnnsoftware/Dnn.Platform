...
class ModuleDefinitions extends Component {
  ...
  confirmAction(callback) {
    ...
    if (props.formIsDirty) {
      utilities.utilities.confirm(
        Localization.get("UnsavedChanges.HelpText"),
        Localization.get("UnsavedChanges.Confirm"),
        Localization.get("UnsavedChanges.Cancel"),
        () => {
          callback();
  ...
  onEditModuleDefinition(
    moduleDefinitionBeingEdited,
    moduleDefinitionBeingEditedIndex,
  ) {
    this.confirmAction(() => {
      this._onEditModuleDefinition(
        moduleDefinitionBeingEdited,
        moduleDefinitionBeingEditedIndex,
      );
    });
  ...
  onSave() {
    ...
    let extensionBeingUpdated = JSON.parse(
      JSON.stringify(props.extensionBeingEdited),
    );
    let moduleDefinitions = extensionBeingUpdated.moduleDefinitions.value;
    if (state.moduleDefinitionBeingEditedIndex > -1) {
      moduleDefinitions[state.moduleDefinitionBeingEditedIndex] =
        state.moduleDefinitionBeingEdited;
    } else {
      moduleDefinitions.push(state.moduleDefinitionBeingEdited);
    }

    state.moduleDefinitionBeingEdited.desktopModuleId =
      props.extensionBeingEdited.desktopModuleId.value;
    let actions = {
      savedefinition: JSON.stringify(state.moduleDefinitionBeingEdited),
    };

    props.dispatch(
      ExtensionActions.updateExtension(
        extensionBeingUpdated,
        actions,
        props.extensionBeingEditedIndex,
        () => {
          props.dispatch(
            ModuleDefinitionActions.setFormDirt(false, () => {
              this.exitEditMode();
            }),
          );
        },
  ...
  onDelete(definitionId) {
    utilities.utilities.confirm(
      "Are you sure you want to delete this module definition?",
      "Yes",
      "No",
      () => {
        const { props } = this;
  ...
  render() {
    ...
    const moduleDefinitions =
      props.extensionBeingEdited.moduleDefinitions.value.map(
        (moduleDefinition, index) => {
          return (
            <ModuleDefinitionRow
              ...
              onEdit={this.onEditModuleDefinition.bind(
                this,
                Object.assign({}, moduleDefinition),
                index,
              )}
              key={index}
            />
    ...
    const isAddMode =
      state.editMode && state.moduleDefinitionBeingEditedIndex === -1;
      ...
      <GridCell className="module-definitions">
        <GridCell className="header-container">
          ...
          <a
            ...
            onClick={this.onEditModuleDefinition.bind(
              this,
              this.getNewModuleDefinition(),
              -1,
            )}
          >
            <span className={isAddMode ? "svg-active" : ""}>
              <SvgIcons.AddIcon />
            </span>{" "}
        ...
        <GridCell className="module-definitions-table">
          <Collapsible
            isOpened={isAddMode}
            className="add-module-definition-wrapper"
          >
            <GridCell
              className={
                "add-module-definition-box" + (isAddMode ? " row-opened" : "")
              }
            >
              <DefinitionFields
                onChange={this.onChange.bind(this)}
              ...
              <GridCell className="modal-footer">
                <Button type="secondary" onClick={this.exitEditMode.bind(this)}>
