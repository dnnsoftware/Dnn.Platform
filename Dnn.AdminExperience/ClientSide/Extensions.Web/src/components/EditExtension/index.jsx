...
class EditExtension extends Component {
  ...
  UNSAFE_componentWillMount() {
    ...
    if (
      (!props.moduleCategories || props.moduleCategories.length === 0) &&
      this.isHost
    ) {
  ...
  updateExtensionBeingEdited(extensionBeingEdited, callback) {
    const { props } = this;
    props.dispatch(
      ExtensionActions.updateExtensionBeingEdited(
        extensionBeingEdited,
        callback,
      ),
    );
  ...
  _getTabHeaders() {
    let tabHeaders = [
      Localization.get("EditExtension_PackageInformation.TabHeader"),
    ...
    if (
      !this.isHost ||
      !this.getExtensionSettingTabVisible(
        this.props.extensionBeingEdited.packageType.value,
      )
    ) {
  ...
  startCreatePackageWizard() {
    ...
    props.dispatch(
      CreatePackageActions.getPackageManifest(
        props.extensionBeingEdited.packageId.value,
        () => {
          this.selectPanel(5);
        },
      ),
  ...
  getTabUI() {
    ...
    let allTabs = [
      <GridCell key="first" className="package-information-box extension-form">
        <PackageInformation
          ...
          updateExtensionBeingEdited={this.updateExtensionBeingEdited.bind(
            this,
          )}
    ...
    if (
      !this.getSiteSettingTabVisible(extensionBeingEdited.packageType.value)
    ) {
      ...
      <GridCell className={styles.editExtension}>
        <PersonaBarPageHeader
          title={
            ...
            " " +
            Localization.get("Extension.Header")
          }
        >
          {this.isHost && (
            <Button
