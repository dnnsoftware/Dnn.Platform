...
import {
  ModuleCustomSettings,
  CoreLanguagePack,
  ExtensionLanguagePack,
} from "./packageCustomSettings";
import "./style.less";
...
class Body extends Component {
  ...
  selectPanel(panel, event) {
    ...
    const { props } = this;
    props.dispatch(
      InstallationActions.setIsPortalPackage(false, () => {
  ...
  onEditExtension(extensionBeingEditedIndex, packageId) {
    const { props } = this;
    props.dispatch(
      ExtensionActions.editExtension(
        packageId,
  ...
  createExtension() {
    ...
    _newExtension = Object.assign(newExtension, {
      packages: props.localePackages,
    });
    props.dispatch(
      ExtensionActions.addExtension(
        validationMapExtensionBeingEdited(_newExtension),
        () => {
          this.selectPanel(2);
        },
      ),
        ...
        <PersonaBarPageBody>
          {this.isHost && (
            <Tabs
              ...
              tabHeaders={[
                Localization.get("InstalledExtensions"),
                Localization.get("AvailableExtensions"),
              ]}
            >
              <InstalledExtensions
                isHost={this.isHost}
