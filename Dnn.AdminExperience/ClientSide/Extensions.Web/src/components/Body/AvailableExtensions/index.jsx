...
class AvailableExtensions extends Component {
  ...
  checkIfAvailablePackageTypesEmpty(props) {
    return (
      !props.availablePackageTypes || props.availablePackageTypes.length === 0
    );
  }
  ...
  UNSAFE_componentWillMount() {
    ...
    if (
      !this.checkIfAvailablePackageTypesEmpty(props) &&
      this.checkIfAvailablePackagesEmpty(props) &&
      props.selectedAvailablePackageType === ""
    ) {
      this.setState({ loading: true }, () => {
        props.dispatch(
          ExtensionActions.getAvailablePackages(
            props.availablePackageTypes[0].Type,
            () => {
  ...
  onFilterSelect(option) {
    ...
    this.setState({ loading: true }, () => {
      props.dispatch(
        ExtensionActions.getAvailablePackages(option.value, () => {
          this.setState({ loading: false });
        }),
      );
    });
  ...
  onInstall(name, event) {
    ...
    props.dispatch(
      ExtensionActions.parseAvailablePackage(
        name,
        props.selectedAvailablePackageType,
        () => {
          this.setDoingOperation(false);
          props.dispatch(
            InstallationActions.setInstallingAvailablePackage(
              name,
              props.selectedAvailablePackageType,
              () => {
                props.dispatch(
                  InstallationActions.navigateWizard(1, () => {
  ...
  onDeploy(index, _package, event) {
    ...
    this.setDoingOperation(true);
    props.dispatch(
      ExtensionActions.deployAvailablePackage(_package, index, (data) => {
        props.dispatch(
          ExtensionActions.getAvailablePackages(
            props.selectedAvailablePackageType,
            () => {
              let packageToInstall = this.props.availablePackages.find(
                (_package) => {
    ...
    return (
      <GridCell className="extension-list">
        <GridCell className="collapse-section filter-section">
          <DropdownWithError
            ...
            options={
              ...
              props.availablePackageTypes.map((_package) => {
                return {
        ...
        {props.availablePackages &&
          props.availablePackages.length > 0 &&
          !state.loading && (
            <ExtensionList
