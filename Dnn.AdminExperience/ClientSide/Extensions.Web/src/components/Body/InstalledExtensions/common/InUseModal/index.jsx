...
class InUseModal extends Component {
  ...
  UNSAFE_componentWillMount() {
    ...
    if (props.usageFilter) {
      ...
      if (state.portalId === -1) {
        let portalId = props.usageFilter.find(
          (p) => p.IsCurrentPortal === true,
        ).PortalID;
        this.setState({
          usageFilter: props.usageFilter,
          portalId: portalId,
        ...
        props.dispatch(
          ExtensionActions.getPackageUsage(portalId, props.packageId),
        );
      }
      return;
    }
  ...
  onSelectPortal(event) {
    ...
    props.dispatch(
      ExtensionActions.getPackageUsage(event.value, props.packageId),
    );
  }
  ...
  getCurrentUsageFilter() {
    ...
    if (props.usageFilter && state.portalId !== -1) {
      return props.usageFilter.find((p) => p.PortalID === state.portalId)
        .PortalName;
    } else return null;
  }
  ...
  getUsageDetailSubject() {
    ...
    if (props.tabUrls) {
      if (props.tabUrls.length > 0) {
        return resx
          .get("ModuleUsageDetail")
          .replace("{0}", props.tabUrls.length)
          .replace("{1}", this.getCurrentUsageFilter());
      } else
        return resx
          .get("NoModuleUsageDetail")
          .replace("{0}", this.getCurrentUsageFilter());
    }
  }
  ...
  renderUsageDetail() {
    ...
    if (props.tabUrls) {
      return props.tabUrls.map((item, i) => {
        return (
          <div key={i} className="usage-detail-taburl">
            <Html html={item.TabUrl} />
          </div>
        );
      });
    }
  ...
  render() {
    const { props, state } = this;
    return (
      <Modal isOpen={props.isOpened} style={modalStyles}>
        <div
          className="modepanel-content-wrapper"
          style={{ height: "calc(100% - 100px)" }}
        >
          <div className="modepanel-content-title">
          ...
          <div className="modepanel-content-detail-wrapper">
            <Label label={this.getUsageDetailSubject()} />
            <div className="usage-detail">{this.renderUsageDetail()}</div>
          </div>
          <div className="button-box">
            <Button type="secondary" onClick={this.onClose.bind(this)}>
              {resx.get("Close")}
            </Button>
          </div>
        </div>
