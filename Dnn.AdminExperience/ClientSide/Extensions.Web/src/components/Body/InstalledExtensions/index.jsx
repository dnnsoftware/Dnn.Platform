...
class InstalledExtensions extends Component {
  ...
  checkIfPackageTypesEmpty(props) {
    return (
      !props.installedPackageTypes || props.installedPackageTypes.length === 0
    );
  }
  ...
  UNSAFE_componentWillReceiveProps(props) {
    if (
      !this.checkIfPackageTypesEmpty(props) &&
      this.checkIfInstalledPackagesEmpty(props) &&
      props.selectedInstalledPackageType === ""
    ) {
      this.setState({ loading: true }, () => {
        props.dispatch(
          ExtensionActions.getInstalledPackages(
            props.installedPackageTypes[0].Type,
            () => {
  ...
  onFilterSelect(option) {
    ...
    this.setState({ loading: true }, () => {
      props.dispatch(
        ExtensionActions.getInstalledPackages(option.value, () => {
          this.setState({ loading: false });
        }),
      );
    });
  ...
  onDelete(_package, index) {
    const { props } = this;
    props.dispatch(
      ExtensionActions.setPackageBeingDeleted(
        Object.assign(
      ...
      <GridCell className="extension-list">
        <GridCell className="collapse-section filter-section">
          <DropdownWithError
            ...
            options={
              props.installedPackageTypes &&
              props.installedPackageTypes.map((_package) => {
                return {
        ...
        {props.installedPackages &&
          props.installedPackages.length > 0 &&
          !state.loading && (
            <ExtensionList
