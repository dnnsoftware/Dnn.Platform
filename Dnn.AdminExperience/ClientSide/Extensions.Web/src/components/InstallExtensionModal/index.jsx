...
class InstallExtensionModal extends Component {
  ...
  parsePackage(file, callback, errorCallback) {
    if (!file) {
      utilities.utilities.notifyError(
        Localization.get("InstallExtension_EmptyPackage.Error"),
      );
      return;
    }
    const { props } = this;
    this.setState(
      ...
      () => {
        props.dispatch(
          InstallationActions.parsePackage(
            file,
            (data) => {
  ...
  installPackage() {
    ...
    if (!props.installingAvailablePackage) {
      props.dispatch(
        InstallationActions.installExtension(
          this.state.package,
          props.parsedInstallationPackage,
          this.state.selectedLegacyType,
          this.state.isPortalPackage,
          () => {
            this.goToStep(4);
            this.setState({
      ...
      props.dispatch(
        ExtensionActions.installAvailablePackage(
          props.availablePackage.PackageType,
          props.availablePackage.FileName,
          props.parsedInstallationPackage,
          () => {
            this.setState({
              installingPackage: false,
  ...
  getPackageInformationStep() {
    const { props } = this;
    const parsedInstallationPackageCopy = utilities.utilities.getObjectCopy(
      props.parsedInstallationPackage,
    );
    let parsedInstallationPackage = this.state.selectedLegacyType
      ? Object.assign(parsedInstallationPackageCopy, {
          ...
          friendlyName:
            this.state.package && this.state.package.name.replace(".zip", ""),
          name:
            this.state.package && this.state.package.name.replace(".zip", ""),
          version: "1.0.0",
        })
  ...
  endInstallation() {
    ...
    if (props.installingAvailablePackage) {
      props.dispatch(
        PaginationActions.loadTab(0, () => {
          const _packageType =
            props.availablePackage && props.availablePackage.PackageType
              ? props.availablePackage.PackageType
              : "Module";
      ...
      if (
        props.parsedInstallationPackage.packageType &&
        props.parsedInstallationPackage.packageType !==
          props.selectedInstalledPackageType
      ) {
        const _packageType =
          props.parsedInstallationPackage &&
          props.parsedInstallationPackage.packageType
        ...
        props.dispatch(
          ExtensionActions.getInstalledPackages(this.state.selectedLegacyType),
        );
      } else if (props.selectedInstalledPackageType) {
        props.dispatch(
          ExtensionActions.getInstalledPackages(
            props.selectedInstalledPackageType,
          ),
        );
      }
  ...
  onToggleLicenseAccept() {
    this.props.dispatch(
      InstallationActions.toggleAcceptLicense(!this.props.licenseAccepted),
    );
  }
  toggleViewLog(value) {
  ...
  getSelectedLegacyTypeIsInstalled() {
    if (this.state.selectedLegacyType === "Skin") {
      return (
        this.props.parsedInstallationPackage &&
        this.props.parsedInstallationPackage.legacySkinInstalled
      );
    }
    if (this.state.selectedLegacyType === "Container") {
      return (
        this.props.parsedInstallationPackage &&
        this.props.parsedInstallationPackage.legacyContainerInstalled
      );
    }
  }
  render() {
    ...
    const { wizardStep } = props,
      legacyInstalled =
        props.parsedInstallationPackage &&
        (props.parsedInstallationPackage.legacySkinInstalled ||
          props.parsedInstallationPackage.legacyContainerInstalled),
    ...
    return (
      <GridCell className={styles.installExtensionModal}>
        ...
        <PersonaBarPageBody
          backToLinkProps={{
            text:
              props.backToReferrer && props.backToReferrerText
                ? props.backToReferrerText
                : Localization.get("BackToExtensions"),
          ...
          <GridCell className="install-extension-box extension-form">
            {wizardStep === 0 && (
              <GridCell>
                ...
                <p>
                  {Localization.get("InstallExtension_UploadPackage.HelpText")}
                </p>
                <GridCell className="upload-package-box">
                  <FileUpload
                    ...
                    clearParsedInstallationPackage={this.clearParsedInstallationPackage.bind(
                      this,
                    )}
                ...
                <GridCell className="modal-footer">
                  <Button
                    onClick={
                      !props.viewingLog
                        ? this.cancelInstallationOnUpload.bind(this)
                        : this.toggleViewLog.bind(this, false)
                    }
                  >
                  ...
                  <Button
                    onClick={
                      legacyInstalled
                        ? this.setAlreadyInstalled.bind(
                            this,
                            legacyTypeIsInstalled,
                            true,
                          )
                        : this.goToStep.bind(this, 1)
                    }
                    type="primary"
                    disabled={
                      !props.parsedInstallationPackage ||
                      !props.parsedInstallationPackage.success
                    }
                  >
            ...
            {wizardStep === 3 && (
              <License
                ...
                primaryButtonDisabled={
                  !props.licenseAccepted || this.state.installingPackage
                }
