...
class EditExtension extends Component {
  render() {
    const { props } = this;
    let errorCount = props.logs.filter((log) => {
    ...
    return (
      <GridCell
        style={{ padding: 50 }}
        className={
          "extension-install-logs" + (errorCount > 0 ? " with-error" : "")
        }
      >
        ...
        >
          <div className="package-installation-report">
            {props.logs.map((log, i) => {
              return (
                <p className={log.Type.toLowerCase()} key={i}>
        ...
        <GridCell columnSize={100} className="modal-footer">
          <Button type="primary" onClick={props.onDone.bind(this)}>
