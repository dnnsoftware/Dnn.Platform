...
class TranslatePageContent extends Component {
  ...
  getPageList() {
    ...
    props.dispatch(
      LanguagesActions.getPageList({ cultureCode, portalId }, (data) => {
  ...
  getBasicSettings() {
    const { props } = this;
    props.dispatch(
      SiteInfoActions.getPortalSettings(
        props.portalId,
        props.cultureCode,
        (data) => {
          this.setState({
  ...
  getProgressData() {
    const { props } = this;
    props.dispatch(
      LanguagesActions.getLocalizationProgress((data) => {
        this.setState(data);
        this.getPageList();
  ...
  doneProgress() {
    ...
    this.getPageList();
    props.dispatch(
      LanguagesActions.getLanguages(props.portalId, (data) => {
        const languages = data.Languages;
        if (!languages.length) {
          return;
        }
        const language = languages.find(
          (l) => l.Code === state.languageBeingEdited.Code,
        );
        this.props.dispatch(
          LanguageEditorActions.setLanguageBeingEdited(language),
        );
      }),
    );
  }
  ...
  renderPageList() {
    ...
    return pageList.map((page, i) => {
      return (
        <div className="page-list-item" key={i}>
          <span>{page.PageName}</span>
          <a
            className="float-right"
            onClick={this.goToPageSettings.bind(this, page.PageId)}
          >
            {resx.get("EditPageSettings")}
          </a>
          <a
            className="float-right"
            target="_blank"
  ...
  onMarkAllPagesAsTranslated(cultureCode) {
    ...
    this.props.dispatch(
      LanguagesActions.markAllPagesAsTranslated(
  ...
  onEraseAllLocalizedPages() {
    ...
    utils.utilities.confirm(
      resx.get("EraseTranslatedPagesWarning").replace("{0}", cultureCode),
      resx.get("Yes"),
      resx.get("No"),
      () => {
        props.dispatch(
          LanguagesActions.deleteLanguagePages(
  ...
  onPublishTranslatedPages(enable = true) {
    ...
    const portalId = props.portalId;
    props.dispatch(
      LanguagesActions.publishAllPages(
        { portalId, cultureCode, enable },
        () => {
          const message = enable
            ? resx.get("PublishedAllTranslatedPages")
            : resx.get("UnPublishedAllTranslatedPages");
  ...
  addPages() {
    ...
    const portalId = props.portalId;
    props.dispatch(
      LanguagesActions.localizeContent({ cultureCode, portalId }, () => {
        this.getProgressData();
      }),
    );
  }
  ...
  toggleActivateLanguage(languageBeingEdited) {
    this.props.dispatch(
      LanguagesActions.activateLanguage(
        {
        ...
        () => {
          this.props.dispatch(
            LanguagesActions.getLanguages(this.props.portalId),
          );
        },
      ),
    );
  ...
  onToggleActive(active) {
    if (!active) {
      ...
      const languageName =
        this.props.languageDisplayMode.toLowerCase() === "native"
          ? languageBeingEdited.NativeName
          : languageBeingEdited.EnglishName;
      utils.utilities.confirm(
        resx.get("DeactivateLanguageWarning").replace("{0}", languageName),
        resx.get("Yes"),
        resx.get("No"),
        () => {
      ...
      >
        <div className="language-settings-page-list">
            ...
            <div className="top-block">
              <div className="language-block">
                <img
                  ...
                  src={languageBeingEdited.Icon}
                  alt={languageBeingEdited.NativeName}
                />
                <span>{languageBeingEdited.NativeName}</span>
                {state.basicSettings && (
                  <span className="float-right">
                    {state.basicSettings.PortalName}
                  </span>
                )}
              </div>
              <InputGroup>
              ...
              <div className="button-block">
                <Button
                  ...
                  onClick={this.onMarkAllPagesAsTranslated.bind(
                    this,
                    languageBeingEdited.Code,
                  )}
                >
                  <TextOverflowWrapper
                    text={resx.get("MarkAllPagesAsTranslated")}
                    maxWidth={150}
                  />
                </Button>
                <Button
                ...
                >
                  <TextOverflowWrapper
                    text={resx.get("EraseAllLocalizedPages")}
                    maxWidth={150}
                  />
                </Button>
                <Button
                  disabled={
                    !languageBeingEdited.Active ||
                    !isEnabled ||
                    !TranslatedPages
                  }
                  type="primary"
                ...
                >
                  <TextOverflowWrapper
                    text={resx.get("PublishTranslatedPages")}
                    maxWidth={150}
                  />
                </Button>
                <Button
                ...
                >
                  <TextOverflowWrapper
                    text={resx.get("UnpublishTranslatedPages")}
                    maxWidth={150}
                  />
                </Button>
              </div>
          ...
          <div className="list-header">
            <span>
              {resx.get("PagesToTranslate")} <span>{pagesNumber}</span>
            </span>
            <span className="float-right" onClick={this.addPages.bind(this)}>
              <em>+</em>
              {resx.get("AddAllUnlocalizedPages")}
            </span>
          </div>
          ...
          {!!state.pageList && !!state.pageList.length && (
            <div className="page-list">
              <Scrollbars
                className="scrollArea content-vertical"
                autoHeight
                autoHeightMin={0}
                autoHeightMax={500}
              >
...
function mapStateToProps(state) {
  return {
    ...
    languageDisplayMode:
      (state.languages.languageSettings &&
        state.languages.languageSettings.LanguageDisplayMode) ||
      "NATIVE",
  };
}
