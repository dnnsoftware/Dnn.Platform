...
class SiteLanguageSelector extends Component {
  ...
  componentDidMount() {
    ...
    document.addEventListener(
      ...
      (event) => {
        props.dispatch(
          SiteInfoActions.getPortalSettings(event.portalId, event.cultureCode),
        );
      },
      false,
    );
  ...
  onSiteChange(event) {
    ...
    props.dispatch(
      SearchActions.getCultureList(event.value, () => {
      ...
      options = props.languages.map((item, i) => {
        return (
          <div
            className={
              "language-flag" +
              (item.Code === this.state.cultureCode ? " selected" : "")
            }
            onClick={this.onLanguageChange.bind(this, { value: item.Code })}
            key={i}
          >
  ...
  renderLanguagesList() {
    ...
    if (props.languages) {
      return (
        <div className="language-selection">{this.getLanguageOptions()}</div>
      );
    } else return <div />;
  }
