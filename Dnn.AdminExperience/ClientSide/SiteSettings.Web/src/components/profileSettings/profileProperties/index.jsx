...
class ProfilePropertiesPanel extends Component {
  ...
  componentDidUpdate() {
    ...
    if (props.profileProperties) {
      if (
        ...
        props.profileProperties.PortalId === props.portalId
      ) {
        return;
      } else {
        this.loadData();
  ...
  loadData() {
    const { props } = this;
    props.dispatch(
      SiteBehaviorActions.getProfileProperties(props.portalId, (data) => {
        this.setState({
    ...
    tableFields.push({
      name: resx.get("DefaultVisibility.Header"),
  ...
  renderHeader() {
    let tableHeaders = tableFields.map((field) => {
      ...
      return (
        <div className={className} key={"header-" + field.id}>
  ...
  onDeleteProperty(propertyId) {
    ...
    if (props.profilePropertyClientModified) {
      ...
      util.utilities.confirm(
        resx.get("PropertyDefinitionDeletedWarning"),
        resx.get("Yes"),
        resx.get("No"),
        () => {
          const profileProperties = Object.assign({}, props.profileProperties);
          profileProperties.Properties = profileProperties.Properties.filter(
            (item) => item.PropertyDefinitionId !== propertyId,
          );
          props.dispatch(
            SiteBehaviorActions.deleteProfileProperty(
              propertyId,
              profileProperties,
              () => {
  ...
  onMovePropertyUp(propertyId) {
    ...
    if (props.profilePropertyClientModified) {
      ...
      if (index > 0) {
        ...
        props.dispatch(
          SiteBehaviorActions.updateProfilePropertyOrders(
            {
      ...
      return this.props.profileProperties.Properties.map((item, index) => {
        ...
        return (
          <ProfilePropertyRow
            ...
            onDelete={this.onDeleteProperty.bind(
              this,
              item.PropertyDefinitionId,
            )}
            onMoveUp={this.onMovePropertyUp.bind(
              this,
              item.PropertyDefinitionId,
            )}
            onMoveDown={this.onMovePropertyDown.bind(
              this,
              item.PropertyDefinitionId,
            )}
            id={id}
        ...
        <div className="property-items">
          ...
          <div className="property-items-grid">
            ...
            <Collapsible
              isOpened={opened}
              autoScroll={true}
              className="property-item-collapse"
            >
              <ProfilePropertyRow
                name={"-"}
                dataType={"-"}
