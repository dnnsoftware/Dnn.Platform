...
export class Body extends Component {
      ...
      return (
        ...
        >
          ...
          <MoreSettings
            portalId={props.portalId}
            cultureCode={props.cultureCode}
            openHtmlEditorManager={props.openHtmlEditorManager.bind(this)}
          />
        </DnnTabs>
      );
    } else {
  ...
  getSearchSecondaryTabs() {
    const SearchExtras =
      window.dnn.SiteSettings && window.dnn.SiteSettings.SearchExtras;

    let searchTabHeaders = [
      resx.get("TabBasicSettings"),
    ...
    if (SearchExtras && SearchExtras.length > 0) {
      SearchExtras.sort(function (a, b) {
        ...
        searchTabContent.push(
          <div key={"content-" + index}>{searchExtra.Component}</div>,
        );
      });
    }
  ...
  render() {
    let tabHeaders = [];
    if (canViewSiteInfo) tabHeaders.push([resx.get("TabSiteInfo")]);
    if (isAdmin) {
