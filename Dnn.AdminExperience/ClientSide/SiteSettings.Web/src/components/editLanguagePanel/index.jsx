...
function findParent(parentList, item) {
  ...
  parentList.forEach(function (listItem) {
    ...
    if (
      listItem.NewValue === compareValue ||
      (listItem.NewValue === "" &&
        compareValue === "_" &&
        !alreadyAChildTerm(listItem.ChildFolders, compareValue))
    ) {
...
function generateList(list) {
  ...
  utilities.utilities.getObjectCopy(list).forEach(function (item) {
    ...
    if (
      splitValue.length === 1 ||
      item.NewValue.toLowerCase() === "portals/_default"
    ) {
      _list.push(item);
    } else {
...
class EditLanguagePanel extends Component {
  ...
  refreshFileList() {
    this.props.dispatch(
      LanguageEditorActions.getRootResourcesFolder(
        this.props.portalId,
  ...
  getChildFolders(folder, callback) {
    ...
    this.props.dispatch(
      LanguageEditorActions.getSubRootResources(_folder, () => {
        if (callback) {
          callback();
        }
      }),
  ...
  getResxEntries(resxFile, callback) {
    const { props } = this;
    props.dispatch(
      LanguageEditorActions.getResxEntries(
        {
  ...
  onSaveTranslations() {
    ...
    props.dispatch(
      LanguageEditorActions.saveTranslations(payload, (data) => {
  ...
  render() {
    ...
    const { languageBeingEdited } = props,
      languageFolders = generateList(
        props.languageFolders.concat(props.languageFiles),
        this.state.selectedMode === "System",
      );
    return (
      <PersonaBarPageBody
      ...
      >
        <LanguageInfoView
          ...
          onHighlightPendingTranslations={this.onHighlightPendingTranslations.bind(
            this,
          )}
          highlightPendingTranslations={this.state.highlightPendingTranslations}
        />
        <ResourceList
          ...
          highlightPendingTranslations={this.state.highlightPendingTranslations}
          onResxChange={this.onResxChange.bind(this)}
        />
      </PersonaBarPageBody>
    );
  }
