...
class ResourceList extends Component {
  ...
  filterListFromSearchText(list) {
    return list.filter((key) => {
      return (
        key.toLowerCase().indexOf(this.state.searchText.toLowerCase()) > -1
      );
    });
  }
  onSearch(searchText) {
  ...
  render() {
    ...
    const searchedList = this.filterListFromSearchText(
      ...
      <GridCell className="resource-list">
        <GridCell className="resource-controls">
          <GridCell columnSize={33}>
            <p className="pending-translations">
              [Pending Translations: {pendingTranslations.length}]
            </p>
          </GridCell>
          <GridCell columnSize={33}></GridCell>
        ...
        <GridCell className="resource-rows">
          {searchedList.map((key, i) => {
            const shouldBeHighlighted =
              props.highlightPendingTranslations &&
              this.props.list[key].First === this.props.list[key].Second;

            return (
              <GridCell className="resource-row" key={i}>
