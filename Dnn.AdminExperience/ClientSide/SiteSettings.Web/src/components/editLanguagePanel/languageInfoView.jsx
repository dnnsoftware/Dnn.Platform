      ...
      <GridCell className="edit-language-info">
        <GridCell className="edit-language-top-bar">
          <GridCell columnSize={40} className="language-info">
            <div className="language-flag">
              <Flag
                culture={languageBeingEdited.Code}
                title={languageBeingEdited.NativeName}
              />
            </div>
          ...
          <GridCell columnSize={60} className="mode-container">
            {util.settings.isHost && (
              <RadioButtons
                tooltipMessage={resx.get("Mode.HelpText")}
                label={resx.get("Mode.Label")}
                buttonGroup="mode"
                options={ModeOptions}
                value={selectedMode}
        ...
        <GridCell className="edit-resource-files">
          ...
          <GridCell columnSize={50}>
            <Switch
              label={resx.get("HighlightPendingTranslations")}
              onText={resx.get("SwitchOn")}
              offText={resx.get("SwitchOff")}
              value={highlightPendingTranslations}
              onChange={onHighlightPendingTranslations}
            />
          </GridCell>
          <GridCell columnSize={50} className="translation-action-buttons">
            ...
            <Button
              ...
              disabled={!resxBeingEdited}
            >
              <TextOverflowWrapper
                text={resx.get("SaveTranslationsToFile")}
                maxWidth={100}
              />
            </Button>
          </GridCell>
