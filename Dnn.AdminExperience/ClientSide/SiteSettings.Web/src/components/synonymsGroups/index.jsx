...
class SynonymsGroupsPanel extends Component {
  ...
  loadData() {
    const { props } = this;
    props.dispatch(
      SearchActions.getSynonymsGroups(
        props.portalId,
        props.cultureCode,
        (data) => {
          this.setState({
  ...
  componentDidMount() {
    ...
    if (tableFields.length === 0) {
      tableFields.push({
        name: resx.get("SynonymsGroup.Header"),
        id: "Synonyms",
      });
    }
  ...
  onUpdateSynonymsGroup(group) {
    ...
    if (group.SynonymsGroupId) {
      synonymsGroups.SynonymsGroups = synonymsGroups.SynonymsGroups.map(
        (item) => {
      ...
      props.dispatch(
        SearchActions.updateSynonymsGroup(
          Object.assign({ PortalId: props.synonymsGroups.PortalId }, group),
          synonymsGroups,
          () => {
      ...
      props.dispatch(
        SearchActions.addSynonymsGroup(
          Object.assign({ PortalId: props.synonymsGroups.PortalId }, group),
          props.synonymsGroups,
          () => {
  ...
  onDeleteSynonymsGroup(group) {
    ...
    util.utilities.confirm(
      resx.get("SynonymsGroupDeletedWarning"),
      resx.get("Yes"),
      resx.get("No"),
      () => {
        const synonymsGroups = Object.assign({}, props.synonymsGroups);
        synonymsGroups.SynonymsGroups = synonymsGroups.SynonymsGroups.filter(
          (item) => item.SynonymsGroupId !== group.SynonymsGroupId,
        );
        props.dispatch(
          SearchActions.deleteSynonymsGroup(
            Object.assign({ PortalId: props.synonymsGroups.PortalId }, group),
            synonymsGroups,
            () => {
  ...
  onSelectCulture(event) {
    ...
    props.dispatch(
      SearchActions.getSynonymsGroups(props.portalId, event.value, (data) => {
        this.setState({
