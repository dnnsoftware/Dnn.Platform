...
class TranslationProgressBars extends Component {
  ...
  render() {
    ...
    const progressText = resx
      .get("Progress")
      .replace("[number]", props.SecondaryPercent);

    return (
      <div
        className={"translation-progress-bars" + (props.Error ? " error" : "")}
      >
        <div className="text">
          <span>
            {resx
              .get("TranslationProgressBarText")
              .replace("[number]", props.SecondaryTotal)}
          </span>
        </div>
        <ProgressBar
          text={totalProgressText}
          percentageValue={props.PrimaryPercent}
        />
        <ProgressBar
          text={progressText}
          rightText={props.CurrentOperationText}
          percentageValue={props.SecondaryPercent}
        />
