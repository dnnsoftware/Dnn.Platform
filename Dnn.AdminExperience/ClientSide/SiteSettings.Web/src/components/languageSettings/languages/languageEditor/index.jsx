...
class LanguageEditor extends Component {
  ...
  componentDidMount() {
    ...
    if (!props.languageDetail || props.code !== props.languageDetail.Code) {
      props.dispatch(
        LanguagesActions.getLanguage(props.portalId, props.languageId),
      );
  ...
  getLanguageOptions() {
    ...
    if (props.id === "add") {
      ...
      if (props.languageDisplayMode === "NATIVE") {
        options.unshift({
          label: state.languageDetail.NativeName,
          value: state.languageDetail.Code,
        });
      } else {
        options.unshift({
          label: state.languageDetail.EnglishName,
          value: state.languageDetail.Code,
        });
      }
    }
    ...
    if (!enabled) {
      ...
      const languageName =
        this.props.languageDisplayMode.toLowerCase() === "native"
          ? languageDetail.NativeName
          : languageDetail.EnglishName;
      util.utilities.confirm(
        resx.get("DisableLanguageWarning").replace("{0}", languageName),
        resx.get("Yes"),
        resx.get("No"),
        () => {
  ...
  renderNewForm() {
    let { state, props } = this;
    const columnOne = (
      <div key="left-column" className="left-column">
        <InputGroup>
          <Label
  ...
  renderEditForm() {
    let { state } = this;
    const columnOne = (
      <div className="left-column" key="editForm-columnOne">
        <InputGroup>
          <Label
            tooltipMessage={resx.get("fallBackLabel.Help")}
            label={resx.get("fallBackLabel")}
    ...
    const columnTwo = (
      <div className="right-column" key="editForm-columnTwo">
        <InputGroup>
          <div className="languageDetailSettings-row_switch">
            <Label labelType="inline" label={resx.get("enableLanguageLabel")} />
            <Switch
