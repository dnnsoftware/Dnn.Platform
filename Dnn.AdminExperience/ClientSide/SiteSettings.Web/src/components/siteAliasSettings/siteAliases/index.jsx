...
class SiteAliasesPanel extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (props !== prevProps) {
      ...
      if (tableFields.length === 0) {
        ...
        if (
          props.siteAliases !== undefined &&
          props.siteAliases.Languages.length > 1
        ) {
          tableFields.push({
            name: resx.get("Language.Header"),
            id: "Language",
          });
        }
      ...
      if (
        ...
        (props.portalId !== undefined &&
          props.siteAliases.PortalId !== props.portalId)
      ) {
        this.loadData();
      }
    }
  ...
  onUpdateSiteAlias(aliasDetail) {
    ...
    if (aliasDetail.PortalAliasID) {
      props.dispatch(
        SiteBehaviorActions.updateSiteAlias(
          aliasDetail,
          () => {
      ...
      props.dispatch(
        SiteBehaviorActions.addSiteAlias(
          alias,
          () => {
  ...
  onDeleteSiteAlias(aliasId) {
    ...
    util.utilities.confirm(
      resx.get("SiteAliasDeletedWarning"),
      resx.get("Yes"),
      resx.get("No"),
      () => {
        const siteAliases = Object.assign({}, props.siteAliases);
        siteAliases.PortalAliases = siteAliases.PortalAliases.filter(
          (item) => item.PortalAliasID !== aliasId,
        );
        props.dispatch(
          SiteBehaviorActions.deleteSiteAlias(
            aliasId,
            siteAliases,
            () => {
      ...
      return this.props.siteAliases.PortalAliases.map((item, index) => {
        ...
        return (
          <SiteAliasRow
            ...
            showLanguageColumn={
              this.props.siteAliases.Languages &&
