...
class SiteAliasEditor extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (props !== prevProps && props.aliasDetail) {
      if (
        props.aliasDetail["HTTPAlias"] === undefined ||
        props.aliasDetail["HTTPAlias"] === ""
      ) {
    ...
    if (this.state.aliasDetail !== undefined || this.props.id === "add") {
      const columnOne = (
        <div key="column-one" className="left-column">
          <InputGroup>
            <Label label={resx.get("SiteAlias")} />
            <SingleLineInputWithError
              inputStyle={{ margin: "0" }}
              withLabel={false}
      ...
      return (
        <div className="alias-editor">
          ...
          <div className="editor-buttons-box">
            ...
            <Button
              type="secondary"
              onClick={this.onChangePrimary.bind(
                this,
                this.state.aliasDetail.IsPrimary ? false : true,
              )}
            >
              {this.state.aliasDetail.IsPrimary
                ? resx.get("UnassignPrimary")
                : resx.get("SetPrimary")}
            </Button>
