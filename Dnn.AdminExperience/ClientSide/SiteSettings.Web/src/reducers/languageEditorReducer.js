...
function insertRecords(folders, newFolders, clearArray) {
  let newFolderList = !clearArray
    ? utilities.utilities.getObjectCopy(folders)
    : [];
...
export default function languageEditor(
  ...
  switch (action.type) {
    case ActionTypes.SET_LANGUAGE_BEING_EDITED:
      return { ...state, languageBeingEdited: action.payload };
    case ActionTypes.RETRIEVED_ROOT_RESOURCES_FOLDER:
      return {
        ...state,
        languageFolders: insertRecords(
          state.languageFolders,
