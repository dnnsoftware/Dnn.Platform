...
class App extends Component {
  ...
  componentDidMount() {
      ...
      (e) => {
        props.dispatch(
          ImportExportActions.siteSelected(
            e.importExportJob.portalId,
            e.importExportJob.portalName,
            () => {
              props.dispatch(
                ImportExportActions.getLastExportTime({
    ...
    if (util.settings.importExportJob) {
      props.dispatch(
        ImportExportActions.siteSelected(
          util.settings.importExportJob.portalId,
          util.settings.importExportJob.portalName,
          () => {
            props.dispatch(
              ImportExportActions.getLastExportTime({
                portal: util.settings.importExportJob.portalId,
                jobType: "Export",
              }),
            );
            this.selectPanel(
              util.settings.importExportJob.jobType === "Export" ? 1 : 2,
            );
          },
        ),
      );
  ...
  render() {
    ...
    return (
      <div>
        ...
        >
          <PersonaBarPageHeader
            title={Localization.get("SiteImportExport.Header")}
          ></PersonaBarPageHeader>
          <PersonaBarPageBody
            backToLinkProps={{
        ...
        <PersonaBarPage isOpen={props.selectedPage === 1}>
          ...
          <PersonaBarPageBody
            backToLinkProps={{
              text:
                (state.referrer && state.referrerText) ||
                Localization.get("BackToImportExport"),
              onClick:
                state.backToReferrerFunc || this.selectPanel.bind(this, 0),
            }}
          >
          ...
          <PersonaBarPageBody
            backToLinkProps={{
              text:
                ...
                Localization.get("BackToImportExport"),
              onClick:
                state.backToReferrerFunc ||
                this.selectPanelInternal.bind(this, 0),
            }}
          >
