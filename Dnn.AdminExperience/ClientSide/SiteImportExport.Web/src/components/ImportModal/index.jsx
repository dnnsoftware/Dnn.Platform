...
class ImportModal extends Component {
  ...
  componentDidMount() {
    ...
    importRequest.PackageId = props.selectedPackage
      ? props.selectedPackage.PackageId
      : -1;
    this.setState({
      importRequest,
    });
  ...
  goToStep(wizardStep) {
    ...
    props.dispatch(
      ImportExportActions.importWizardGoToSetp(0, () => {
  ...
  selectPackage(pkg) {
    ...
    if (
      ...
      props.selectedPackage.PackageId === pkg.PackageId
    ) {
      props.dispatch(ImportExportActions.selectPackage());
    } else {
      props.dispatch(
        ImportExportActions.selectPackage(pkg, () => {
  ...
  cancelImport() {
    ...
    if (props.wizardStep === 0) {
      ...
      util.utilities.confirm(
        Localization.get("CancelImportMessage"),
  ...
  onAnalyze() {
    ...
    if (props.selectedPackage) {
      props.dispatch(
        ImportExportActions.importWizardGoToSetp(1, () => {
          props.dispatch(
            ImportExportActions.verifyImportPackage(
              props.selectedPackage.PackageId,
              () => {},
              () => {
  ...
  onImport() {
    ...
    props.dispatch(
      ImportExportActions.importSite(
        state.importRequest,
        (data) => {
          this.setState({
            requestSubmitting: false,
        ...
        () => {
          ...
          util.utilities.notifyError(
            Localization.get("ImportRequestSubmit.ErrorMessage"),
          );
        },
      ),
    );
  ...
  onFilterChanged(filter) {
    ...
    this.setState(
      ...
      () => {
        props.dispatch(
          ImportExportActions.getImportPackages(this.getNextPage()),
        );
      },
    );
  }
    ...
    return (
      <div className={styles.importModal}>
        <div className="pageTitle">
          {Localization.get("SelectImportPackage")}
        </div>
        <div className="packages-wrapper">
          ...
          <div className="packages">
            ...
            {props.wizardStep === 1 &&
              !props.packageVerified &&
              this.renderPackageVerification()}
          </div>
          {props.wizardStep === 0 && this.renderPager()}
        ...
        <GridCell className="action-buttons">
          <Button type="secondary" onClick={this.cancelImport.bind(this)}>
