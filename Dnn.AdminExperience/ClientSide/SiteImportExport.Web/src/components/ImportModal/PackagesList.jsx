...
class PackagesList extends Component {
  ...
  render() {
    ...
    if (props.importPackages && props.importPackages.length > 0) {
      return (
        <div className="package-cards">
          {props.importPackages.map((pkg, i) => {
            return (
              <div className="package-card-wrapper" key={i}>
                <PackageCard
                  selectedPackage={pkg}
                  className={
                ...
                {pkg.Description && (
                  ...
                  >
                    <Html html={this.renderTooltipMessage(pkg.Description)} />
                  </div>
                )}
              </div>
            );
      ...
      return <div className="noPackages">{Localization.get("NoPackages")}</div>;
  }
}
