...
class ExportModal extends Component {
  ...
  onExport() {
    ...
    if (this.Validate()) {
      ...
      props.dispatch(
        ImportExportActions.exportSite(
          state.exportRequest,
          (data) => {
            this.setState({
              requestSubmitting: false,
            });
            utilities.utilities.notify(
              Localization.get("ExportRequestSubmitted"),
            );
            props.dispatch(
              ImportExportActions.getAllJobs(
                {
          ...
          () => {
            ...
            utilities.utilities.notifyError(
              Localization.get("ExportRequestSubmit.ErrorMessage"),
            );
          },
        ),
      );
  ...
  Validate() {
    ...
    if (success && !this.ValidateHasExportItem()) {
      utilities.utilities.notifyError(
        Localization.get("NoExportItem.ErrorMessage"),
      );
      success = false;
    }
    return success;
  ...
  getDescendantPortalTabs(PortalTabsParameters, ParentTabId, callback) {
    this.props.dispatch(
      ImportExportActions.getDescendantPortalTabs(
  ...
  ValidateHasExportItem() {
    ...
    const { exportRequest } = this.state;
    if (
      exportRequest.IncludeContent ||
      exportRequest.IncludeFiles ||
      exportRequest.IncludeUsers ||
      exportRequest.IncludeRoles ||
      exportRequest.IncludeVocabularies ||
      exportRequest.IncludeTemplates ||
      exportRequest.IncludeProperfileProperties ||
      exportRequest.IncludePermissions ||
      exportRequest.IncludeExtensions ||
      (exportRequest.pages && exportRequest.pages.length > 0) ||
  ...
  updatePagesToExport(selectedPages) {
    let { exportRequest } = this.state;
    let prevCount =
      exportRequest.pages !== null && exportRequest.pages !== undefined
        ? exportRequest.pages.length
        : 0;
  ...
  getExportModeOptions() {
    let options = [
      {
        label: Localization.get("ExportModeDifferential"),
        value: "Differential",
      },
      ...
      <div className={styles.exportModal}>
        ...
        <GridCell className="export-site-container">
          <div className="left-column">
            <GridCell>
              <Label labelType="block" label={Localization.get("Site")} />
              <Dropdown
                enabled={false}
          ...
          <GridSystem>
            <div className="export-switches">
              <div className="sectionTitle">
                {Localization.get("IncludeInExport")}
              </div>
              <InputGroup>
                <Label labelType="inline" label={Localization.get("Content")} />
                <Switch
              ...
              <InputGroup>
                <Label labelType="inline" label={Localization.get("Assets")} />
                <Switch
                  onText={Localization.get("SwitchOn")}
                  offText={Localization.get("SwitchOff")}
              ...
              {registeredItemsToExport.map((item, i) => (
                <InputGroup key={i}>
                  ...
                  <Switch
                    ...
                    offText={Localization.get("SwitchOff")}
                    value={
                      state.exportRequest.ItemsToExport.indexOf(
                        item.category,
                      ) !== -1
                    }
                    onChange={this.onChangeItemsToExport.bind(
                      this,
                      item.category,
                    )}
                  />
              ...
              <InputGroup>
                <Label labelType="inline" label={Localization.get("RunNow")} />
                <Switch
                  onText={Localization.get("SwitchOn")}
                  offText={Localization.get("SwitchOff")}
            ...
            <div className="export-pages">
              <div className="sectionTitle">
                {Localization.get("PagesInExport")}
              </div>
              <TreeControlInteractor
                ...
                getDescendantPortalTabs={this.getDescendantPortalTabs.bind(
                  this,
                )}
              />
                ...
                <RadioButtons
                  ...
                  value={
                    props.lastExportTime
                      ? state.exportRequest.ExportMode
                      : "Full"
                  }
                />
              </InputGroup>
              <InputGroup>
                ...
                />
                <div className="lastExport">
                  {props.lastExportTime || Localization.get("EmptyDateTime")}
                </div>
              </InputGroup>
            </div>
          </GridSystem>
          <GridCell className="action-buttons">
            <Button type="secondary" onClick={this.cancelExport.bind(this)}>
