...
import "./style.less";
import { importExport as ImportExportActions } from "../../../actions";
...
class JobDetails extends Component {
  ...
  getSummaryItem(category) {
    ...
    if (props.jobDetail.Summary) {
      let detail = props.jobDetail.Summary.SummaryItems.find(
      ...
      if (detail) {
        if (
          ...
          return detail.ProcessedItems > detail.TotalItems
            ? detail.TotalItemsString
            : detail.ProcessedItemsString + " / " + detail.TotalItemsString;
        } else {
          return (
            <div>
              ...
              <div style={{ float: "right" }}>
                {detail.ProcessedItemsString +
                  " / " +
                  detail.TotalItemsString +
                  " (" +
                  ((detail.ProcessedItems / detail.TotalItems) * 100).toFixed(
                    1,
                  ) +
                  "%)"}
              </div>
  ...
  getUsersSummary() {
    const { props, state } = this;
    if (props.jobDetail.Summary) {
      let users = props.jobDetail.Summary.SummaryItems.find(
        (c) => c.Category === "USERS",
      );
      if (users && props.jobDetail.JobType === "Site Import") {
        let usersData = props.jobDetail.Summary.SummaryItems.find(
          (c) => c.Category === "USERS_DATA",
        );
        if (users.TotalItems > 0 && !users.Completed && !state.cancelled) {
            ...
            <GridCell>
              ...
              <div className="import-summary-item users">
                <div>
                  ...
                  <div style={{ float: "right" }}>
                    {users.ProcessedItemsString +
                      " / " +
                      users.TotalItemsString +
                      " (" +
                      ((users.ProcessedItems / users.TotalItems) * 100).toFixed(
                        1,
                      ) +
                      "%)"}
                  </div>
          ...
          return (
            <GridCell>
              ...
              />
              <div className="import-summary-item users">
                {users.ProcessedItems > users.TotalItems
                  ? users.TotalItemsString
                  : users.ProcessedItemsString + " / " + users.TotalItemsString}
              </div>
            </GridCell>
          );
        } else if (
            ...
            <GridCell>
              ...
              <div className="import-summary-item users">
                <div>
                  ...
                  <div style={{ float: "right" }}>
                    {usersData.ProcessedItemsString +
                      " / " +
                      usersData.TotalItemsString +
                      " (" +
                      (
                        (usersData.ProcessedItems / usersData.TotalItems) *
                        100
                      ).toFixed(1) +
                      "%)"}
                  </div>
          ...
          return (
            <GridCell>
              <Label labelType="inline" label={Localization.get("Users")} />
              <div className="import-summary-item users">
                {users.ProcessedItems > users.TotalItems
                  ? users.TotalItemsString
                  : users.ProcessedItemsString + " / " + users.TotalItemsString}
  ...
  renderExportSummary() {
    const { props, state } = this;
    const registeredItemsToExport =
      itemsToExportService.getRegisteredItemsToExport();
    return (
          ...
          <div className="export-summary">
            <GridCell className="export-site-container">
              <div className="left-column">
                ...
                <GridCell>
                  <Label labelType="inline" label={Localization.get("Pages")} />
                  <div className="import-summary-item pagers">
                    {this.getSummaryItem("Pages")}
                  </div>
                </GridCell>
                <GridCell>
                  <Label labelType="inline" label={Localization.get("Roles")} />
                  <div className="import-summary-item">
                ...
                <GridCell>
                  ...
                  />
                  <div className="import-summary-item">
                    {props.jobDetail.Summary.IncludeContent
                ...
                <GridCell>
                  ...
                  />
                  <div className="import-summary-item">
                    {props.jobDetail.Summary.IncludeProfileProperties
                      ? Localization.get("Yes")
                      : Localization.get("No")}
                  </div>
                </GridCell>
                <GridCell>
                  ...
                  />
                  <div className="import-summary-item">
                    {props.jobDetail.Summary.IncludePermissions
                      ? Localization.get("Yes")
                      : Localization.get("No")}
                  </div>
                </GridCell>
                <GridCell>
                  ...
                  />
                  <div className="import-summary-item">
                    {props.jobDetail.Summary.IncludeExtensions
                      ? Localization.get("Yes")
                      : Localization.get("No")}
                  </div>
                </GridCell>
                <GridCell>
                  ...
                  />
                  <div className="import-summary-item">
                    {props.jobDetail.Summary.IncludeDeletions
                      ? Localization.get("Yes")
                      : Localization.get("No")}
                  </div>
                </GridCell>
              ...
              <div className="right-column">
                <GridCell>
                  <Label labelType="inline" label={Localization.get("Name")} />
                  <div className="import-summary-item">
                ...
                <GridCell>
                  ...
                  />
                  <div className="import-summary-item">
                    {this.getSummaryItem("Assets")}
                  </div>
                </GridCell>
                {registeredItemsToExport.map((item, i) => (
                  <GridCell key={i}>
                    <Label labelType="inline" label={item.name} />
                    <div className="import-summary-item">
                      {this.getSummaryItem(item.category)}
                    </div>
                  </GridCell>
                ))}
                <GridCell>
                  ...
                  />
                  <div className="import-summary-item">
                    {props.jobDetail.Summary.ExportFileInfo
                      ? props.jobDetail.Summary.ExportFileInfo.ExportSize
                      : "-"}
                  </div>
                </GridCell>
                <GridCell>
                  ...
                  />
                  <div className="import-summary-item">
                    {props.jobDetail.Summary.ExportMode === 1
                      ? Localization.get("ExportModeDifferential")
                      : Localization.get("ExportModeComplete")}
                  </div>
                </GridCell>
                <GridCell>
                  <div className="summary-note">
                    <div className="note-title">
                      {Localization.get("SummaryNoteTitle")}
                    </div>
                    <div className="note-description">
                      {Localization.get("SummaryNoteDescription")}
                    </div>
                  </div>
                </GridCell>
              </div>
              <GridCell className="action-buttons">
                {props.jobDetail.Status < 2 && !state.cancelled && (
                  ...
                  >
                    {props.jobDetail.JobType.indexOf("Export") >= 0
                      ? Localization.get("CancelExport")
                      : Localization.get("CancelImport")}
                  </Button>
                )}
      ...
      return (
        <div className="job-details">
          <div className="summary-title">
            {props.jobDetail.JobType.indexOf("Export") >= 0
              ? Localization.get("ExportSummary")
              : Localization.get("ImportSummary")}
          </div>
