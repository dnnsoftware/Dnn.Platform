...
class DashboardPanelBody extends Component {
  ...
  componentDidMount() {
    const { props } = this;
    props.dispatch(
      ImportExportActions.getPortals((data) => {
        if (data.TotalResults === 1) {
          props.dispatch(
            ImportExportActions.siteSelected(
              data.Results[0].PortalID,
              data.Results[0].PortalName,
              () => {
                props.dispatch(
                  ImportExportActions.getAllJobs(
                    this.getNextPage(data.Results[0].PortalID),
                    (data) => {
                      if (
                        data.Jobs &&
                        data.Jobs.find((j) => j.Status < 2 && !j.Cancelled)
                      ) {
          ...
          props.dispatch(
            ImportExportActions.getAllJobs(
              this.getNextPage(props.portalId),
              (data) => {
                if (
                  data.Jobs &&
                  data.Jobs.find((j) => j.Status < 2 && !j.Cancelled)
  ...
  addInterval(props) {
    ...
    this.jobListTimeout = setInterval(() => {
      if (
        persistedSettings.expandPersonaBar &&
        persistedSettings.activeIdentifier === "Dnn.SiteImportExport"
      ) {
        props.dispatch(
          ImportExportActions.getAllJobs(this.getNextPage(), (data) => {
            if (
              !data.Jobs ||
              !data.Jobs.find((j) => j.Status < 2 && !j.Cancelled)
            ) {
              ...
              if (this.state.currentJobId) {
                props.dispatch(
                  ImportExportActions.getJobDetails(this.state.currentJobId),
                );
              }
            } else if (
              ...
              data.Jobs.find(
                (j) =>
                  j.JobId === this.state.currentJobId &&
                  j.Status < 2 &&
                  !j.Cancelled,
              )
            ) {
  ...
  onPortalChange(option) {
    ...
    if (option.value !== props.portalId) {
      this.setState(
        ...
        () => {
          props.dispatch(
            ImportExportActions.siteSelected(option.value, option.label),
          );
          props.dispatch(
            ImportExportActions.getAllJobs(
              this.getNextPage(option.value),
              (data) => {
                if (
                  data.Jobs &&
                  data.Jobs.find((j) => j.Status < 2 && !j.Cancelled)
  ...
  onPageChange(currentPage, pageSize) {
    ...
    this.setState(
      ...
      () => {
        props.dispatch(
          ImportExportActions.jobSelected(null, () => {
            props.dispatch(
              ImportExportActions.getAllJobs(
                this.getNextPage(props.portalId),
                (data) => {
  ...
  cancelJob(jobId) {
    ...
    util.utilities.confirm(
      Localization.get("CancelJobMessage"),
          ...
          ImportExportActions.cancelJob(
            ...
            () => {
              util.utilities.notifyError(
                Localization.get("JobCancel.ErrorMessage"),
              );
            },
          ),
        );
  ...
  deleteJob(jobId) {
    const { props, state } = this;
    util.utilities.confirm(
      Localization.get("DeleteJobMessage"),
            ...
            () => {
              props.dispatch(
                ImportExportActions.deleteJob(
                  ...
                  () => {
                    util.utilities.notifyError(
                      Localization.get("JobDelete.ErrorMessage"),
                    );
                  },
                ),
              );
        ...
        <GridCell columnSize={100}>
          ...
          <div className="last-actions">
            <div className="action-labels">
              <Label labelType="block" label={Localization.get("LastImport")} />
              <Label labelType="block" label={Localization.get("LastExport")} />
            </div>
            <div className="action-dates">
              <div>
                {props.lastImportTime || Localization.get("EmptyDateTime")}
              </div>
              <div>
  ...
  renderedJobList() {
    ...
    if (props.portals.length > 0 && props.jobs && props.jobs.length > 0) {
      return props.jobs.map((job, index) => {
        return (
          <JobRow
            ...
            jobUser={job.User}
            jobPortal={
              props.portals.find((p) => p.PortalID === job.PortalId)
                ? props.portals.find((p) => p.PortalID === job.PortalId)
                    .PortalName
                : Localization.get("DeletedPortal")
            }
      ...
      return (
        <GridCell className="no-jobs">{Localization.get("NoJobs")}</GridCell>
      );
  }
  ...
  renderedLegend() {
    ...
    let legend = legendItems.map((item, i) => {
      return (
        <div className="logLegend-item" key={i}>
