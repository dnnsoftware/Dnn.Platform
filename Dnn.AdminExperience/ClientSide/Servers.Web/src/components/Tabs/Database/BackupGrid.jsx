...
export default class BackupGrid extends Component {
  getBackUpsGridRows() {
        ...
        return (
          <div className="row" key={i}>
            <GridCell columnSize={45}>
              <OverflowText text={field.name} maxWidth={290} />
            </GridCell>
            <GridCell columnSize={15}>
              {util.formatDateNoTime(field.startDate)}
            </GridCell>
            <GridCell columnSize={15}>
              {util.formatDateNoTime(field.finishDate)}
            </GridCell>
            <GridCell columnSize={10}>
              {util.formatNumeric(field.size)}
            </GridCell>
    ...
    return (
      <div className="row">
        <GridCell className="alert-message" columnSize={100}>
          {Localization.get("NoBackups")}
        </GridCell>
      </div>
    );
  }
    ...
    return (
      <div className="grid">
        <div className="row header">
          ...
          <GridCell columnSize={15}>
            {Localization.get("BackupStarted")}
          </GridCell>
          <GridCell columnSize={15}>
            {Localization.get("BackupFinished")}
          </GridCell>
