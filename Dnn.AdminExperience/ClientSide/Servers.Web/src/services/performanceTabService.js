...
const getPerformanceSettings = function () {
  return serviceFramework
    .get("ServerSettingsPerformance", "GetPerformanceSettings")
    .then((response) => {
      return {
        ...
        cachingProviderOptions: convertKeyValueToLabelValue(
          response.CachingProviderOptions,
        ),
        pageStatePersistenceOptions: convertKeyValueToLabelValue(
          response.PageStatePersistenceOptions,
        ),
        moduleCacheProviders: convertKeyValueToLabelValue(
          response.ModuleCacheProviders,
        ),
        pageCacheProviders: convertKeyValueToLabelValue(
          response.PageCacheProviders,
        ),
        cacheSettingOptions: convertKeyValueToLabelValue(
          response.CacheSettingOptions,
        ),
        authCacheabilityOptions: convertKeyValueToLabelValue(
          response.AuthCacheabilityOptions,
        ),
        unauthCacheabilityOptions: convertKeyValueToLabelValue(
          response.UnauthCacheabilityOptions,
        ),
      };
    });
};
...
const save = function (performanceSettings) {
  const request = {
    ...
    ClientResourcesManagementMode:
      performanceSettings.clientResourcesManagementMode,
  };

  if (performanceSettings.clientResourcesManagementMode === "h") {
    request.CurrentHostVersion = performanceSettings.currentHostVersion;
    request.HostEnableCompositeFiles =
      performanceSettings.hostEnableCompositeFiles;
    ...
    request.PortalEnableCompositeFiles =
      performanceSettings.portalEnableCompositeFiles;
...
const incrementVersion = function (version, isGlobalSetting) {
  if (isGlobalSetting) {
    return serviceFramework.post(
      "ServerSettingsPerformance",
      "IncrementHostVersion",
    );
  }

  return serviceFramework.post(
    "ServerSettingsPerformance",
    "IncrementPortalVersion",
  );
};
