...
class Body extends Component {
  ...
  componentDidMount() {
    this.props.dispatch(
      CommonUsersActions.getUserFilters((data) => {
  ...
  getWorkSpaceTray() {
    return (
      this.state.userFilters.length > 0 && (
  ...
  onRemoveDeletedUsers() {
    utilities.confirm(
      ...
      () => {
        this.props.dispatch(
          CommonUsersActions.removeDeletedUsers(() => {
            let { searchParameters } = this.state;
            this.props.dispatch(CommonUsersActions.getUsers(searchParameters));
  ...
  canAddUser() {
    return (
      ...
      appSettings.applicationSettings.settings.permissions.ADD_USER
    );
  }
  render() {
      ...
      <GridCell>
        <PersonaBarPageHeader title={Localization.get("nav_Users")}>
          {this.canAddUser() && (
            <Button
              ...
              title={Localization.get("btnCreateUser")}
            >
          ...
          {appSettings.applicationSettings.settings.isAdmin && (
            <Button
              ...
              size="large"
              onClick={() => {
                this.onRemoveDeletedUsers();
              }}
              title={Localization.get("RemoveDeleted.Btn")}
            >
        ...
        >
          <UserTable
            ...
            changeSortOrder={(c, a) => this.onSortChanged(c, a)}
