...
export const getPasswordStrength = (password, passwordStrengthOptions) => {
  if (
    passwordStrengthOptions === undefined ||
    passwordStrengthOptions === null
  ) {
    return null;
  }

  let pStrengthRating = passwordStrengthRating(
    password,
...
export const passwordStrengthRating = (password, options) => {
  ...
  if (password.length > 0) {
    ...
    if (validationExpression) {
      matchValidationExpression = new RegExp(validationExpression, "g").test(
        password,
      );
    }
  }
