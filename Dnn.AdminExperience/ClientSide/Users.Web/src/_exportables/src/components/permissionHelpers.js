export function canManageRoles(settings, user) {
  return (
    !user.isSuperUser && (settings.isAdmin || settings.permissions.MANAGE_ROLES)
  );
}
...
export function canDeleteUser(settings, userId) {
  return (
    (settings.isAdmin || settings.permissions.DELETE_USER) &&
...
export function canAuthorizeUnAuthorizeUser(settings, userId) {
  return (
    (settings.isAdmin || settings.permissions.AUTHORIZE_UNAUTHORIZE_USER) &&
    userId !== settings.userId
  );
}
