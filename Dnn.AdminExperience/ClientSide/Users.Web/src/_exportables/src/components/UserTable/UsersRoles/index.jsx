...
class UserRoles extends Component {
  ...
  getSuggestRoles() {
    const { props, state } = this;
    let keyword =
      state.roleSelectState.roleId >= 0 ? "" : state.roleSelectState.keyword;
    props.dispatch(
      CommonUsersActions.getSuggestRoles({ keyword: keyword, count: 10 }),
    );
  }
  ...
  onRoleSelectorSelected(item) {
    this.onRoleSelected(item.roleId, () => {
      this.setState(
        { roleSelectState: { roleId: item.roleId, keyword: item.roleName } },
        () => {
          this.getSuggestRoles();
        },
      );
  ...
  saveRole(roleId, startTime, expiresTime) {
    ...
    props.dispatch(
      CommonUsersActions.saveUserRole(
        parameter,
  ...
  onRoleSelected(roleId, callback) {
    if (
      this.props.matchedRoles !== undefined &&
      this.props.matchedRoles.length > 0 &&
      this.props.matchedRoles.some((r) => r.roleId === roleId)
    ) {
  ...
  render() {
    ...
    return (
      <div className="userroles-form-form">
            ...
            <GridCell columnSize={30}>
              <div className="send-email-box">
                <Checkbox
                  ...
                  label={Localization.get("SendEmail")}
                  labelPlace="right"
                />
                {this.state.allowOwner && (
                  <Checkbox
                    ...
                    label={Localization.get("IsOwner")}
                    labelPlace="right"
                  />
                )}
              </div>
        ...
        <div className="user-roles-list-paging">{this.renderPaging()}</div>
      </div>
    );
  }
}
