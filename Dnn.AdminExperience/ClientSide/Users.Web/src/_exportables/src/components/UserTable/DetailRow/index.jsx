...
class DetailsRow extends Component {
  ...
  handleClick(event) {
    ...
    if (
      ...
      !(event.target.id === "confirmbtn" || event.target.id === "cancelbtn") &&
      this.props.openId !== "add"
    ) {
  ...
  getUserActions(user, opened) {
    ...
    actionIcons = actionIcons.concat(
      (this.props.getUserTabsIcons && this.props.getUserTabsIcons(user)) || [],
    );
    ...
    if (
      canManageRoles(this.props.appSettings.applicationSettings.settings, user)
    ) {
      actionIcons = actionIcons.concat([
        {
          index: 5,
    ...
    let userActions = sort(actionIcons, "index", "desc").map((actionIcon) => {
      ...
      let element = (
        <div
          ...
          className={
            "extension-action " + !(opened && this.props.currentIndex === i)
          }
    ...
    return [
      <div
        key={`user_action_wrapper_${user.userId}`}
      ...
      >
        ...
        {this.state.showMenu && (
          <UserMenu
            ...
            getUserMenu={
              this.props.getUserMenu && this.props.getUserMenu.bind(this)
            }
            userMenuAction={
              this.props.userMenuAction && this.props.userMenuAction.bind(this)
            }
  ...
  getUserColumns(user, id, opened) {
    ...
    let extraColumns =
      this.props.getUserColumns && this.props.getUserColumns(user);
    let columnSizes =
      this.props.columnSizes !== undefined
      ...
      userColumns = [
        {
          ...
          content: (
            <GridCell
              key={`gc-userstatus-${user.userId}`}
              columnSize={columnSizes.find((x) => x.index === 3).size}
            >
              <span className={"user-status " + statusClass} title={hoverText}>
                <StatusIcon />
              </span>
            </GridCell>
          ),
      ...
      .concat([
        {
          ...
          content: (
            <GridCell
              key={`gc-username-${user.userId}`}
              columnSize={columnSizes.find((x) => x.index === 5).size}
            ...
            >
              <div className="user-name">
                ...
                <div>
                  ...
                  {user.displayName !== "-" && (
                    <p title={`${Localization.get("UserId")} ${user.userId}`}>
                      {user.userName}
                    </p>
                  )}
                </div>
        ...
        },
        {
          ...
          content: (
            <GridCell
              key={`gc-email-link-${user.userId}`}
              columnSize={columnSizes.find((x) => x.index === 10).size}
        ...
        },
        {
          ...
          content: (
            <GridCell
              key={`gc-createdon-${user.userId}`}
              columnSize={columnSizes.find((x) => x.index === 15).size}
              className={"user-joined" + (user.isDeleted ? " deleted" : "")}
            >
              {user.createdOnDate !== "-" && (
                <p>{formatDate(user.createdOnDate)}</p>
              )}
        ...
        },
        {
          ...
          content: id !== "add" && (
            <GridCell
              key={`gc-actions-${user.userId}`}
              columnSize={columnSizes.find((x) => x.index === 25).size}
              style={{ float: "right", textAlign: "right", paddingRight: 2 }}
            >
              {userActions}
            </GridCell>
          ),
    ...
    return (
      ...
      >
        <GridCell className={"collapsible-header-users " + !opened}>
          <GridCell
            className={styles.extensionDetailRow + " " + props.addIsOpened}
            columnSize={100}
          >
            {(!props.addIsOpened || props.addIsOpened === "add-opened") && (
              <GridCell>{userColumns}</GridCell>
            )}
            <Collapsible
              accordion={true}
              isOpened={opened}
