...
class UserSettings extends Component {
  ...
  componentWillMount() {
    ...
    if (
      ...
      props.userDetails.userId !== props.userId
    ) {
  ...
  componentWillReceiveProps(newProps) {
    if (
      newProps.userDetails === undefined &&
      newProps.userDetails.userId !== newProps.userId
    ) {
  ...
  getUserDetails(props, userId) {
    ...
    props.dispatch(
      CommonUsersActions.getUserDetails({ userId: userId }, (data) => {
  ...
  save() {
    if (this.validateForm()) {
      this.props.dispatch(
        CommonUsersActions.updateUserBasicInfo(
  ...
  validateForm() {
    ...
    if (
      startingValues.userName !== accountSettings.userName &&
      accountSettings.userName.length <
        this.props.appSettings.applicationSettings.settings.userNameMinLength
    ) {
      errors.userName = true;
      valid = false;
    }
  ...
  render() {
    let { state } = this;
    let agreedToTerms = this.props.appSettings.applicationSettings.settings
      .dataConsentActive ? (
      <GridSystem>
    ...
    let agreedToTermsOn = this.props.appSettings.applicationSettings.settings
      .dataConsentActive ? (
      <GridSystem>
        ...
        <GridCell>
          {formatDate(state.userDetails.hasAgreedToTermsOn, true) === "-"
    ...
    let userRequestedDeletion = state.userDetails.requestsRemoval ? (
      <span className="importantNote">
        {Localization.get("RequestedRemoval")}
      </span>
    ) : null;
    return (
      <GridCell className={styles.userSettings}>
        <GridCell>
          <GridCell className="outer-box" columnSize={50}>
            <ChangePassword
              ...
              userId={this.props.userId}
            />
            <div className="title">{Localization.get("AccountSettings")}</div>
            ...
            {canManagePassword(
              ...
              <GridCell className="no-padding">
                ...
                >
                  <div onClick={this.onChangePassword.bind(this)}>
                ...
                {!state.userDetails.needUpdatePassword && (
                  <GridCell
                    className={"link" + (this.state.loading ? " disabled" : "")}
                  >
                    <div onClick={this.onForcePasswordChange.bind(this)}>
                      [ {Localization.get("ForceChangePassword")} ]
                    </div>
                  </GridCell>
                )}
                <GridCell
                  className={"link" + (this.state.loading ? " disabled" : "")}
                >
                  <div onClick={this.onSendPasswordLink.bind(this)}>
                    [ {Localization.get("ResetPassword")} ]
                  </div>
                </GridCell>
              </GridCell>
            )}
          ...
          <GridCell className="outer-box right" columnSize={50}>
            <div className="title">{Localization.get("AccountData")}</div>
            <GridSystem className="first">
              <GridCell title={Localization.get("UserId.Help")}>
                {Localization.get("UserId")}
              </GridCell>
              <GridCell>{state.userDetails.userId}</GridCell>
            </GridSystem>
            <GridSystem>
            ...
            <GridSystem>
              ...
              <GridCell>
                {formatDate(state.userDetails.lastLockout, true) === "-"
                  ? Localization.get("Never")
                  : formatDate(state.userDetails.lastLockout, true)}
              </GridCell>
            </GridSystem>
            ...
            <GridSystem>
              ...
              <GridCell>{state.userDetails.userFolder}</GridCell>
            </GridSystem>
          </GridCell>
        </GridCell>
        {canEditSettings(
