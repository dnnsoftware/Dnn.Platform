...
class ChangePassword extends Component {
  ...
  save() {
    if (this.validateForm()) {
      this.props.dispatch(
        CommonUsersActions.changePassword(this.state.changePassword, () => {
          this.cancel();
  ...
  render() {
    ...
    return (
      this.props.visible && (
        <div className="dnn-user-change-password">
          <GridCell className="do-not-close">
            <GridCell>
              <div className="title">{Localization.get("ChangePassword")}</div>
              <SingleLineInputWithError
              ...
              />
              <SingleLineInputWithError
                label={Localization.get("NewConfirm")}
                error={
                ...
                errorMessage={
                  state.errors.confirmPassword
                    ? Localization.get("NewConfirm.Required")
                    : Localization.get("NewConfirmMismatch.ErrorMessage")
                }
                style={inputStyle}
                type="password"
                inputStyle={{ marginBottom: 15 }}
