...
class RoleRow extends Component {
  ...
  onDeleteClick(userRole) {
    ...
    utilities.confirm(
      formatString(
        Localization.get("DeleteRole.Confirm"),
        userRole.roleName,
  ...
  createRoleActions() {
    ...
    let startTimeAction = props.roleDetails.allowExpired ? (
      <span>
        <DatePicker
          ...
          mode={"start"}
          applyButtonText={Localization.get("btnApply")}
          showIcon={true}
          showInput={false}
    ...
    let expiresTimeAction = props.roleDetails.allowExpired ? (
      <span>
        <DatePicker
    ...
    return (
      <div className="user-role-row">
        ...
        <GridCell columnSize={35}>
          <div className="actions">{this.createRoleActions()}</div>
        </GridCell>
      </div>
    );
  }
