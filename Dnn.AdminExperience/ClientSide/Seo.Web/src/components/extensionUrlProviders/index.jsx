...
class ExtensionUrlProvidersPanelBody extends Component {
  ...
  onUpdateProviderStatus(providerId, isActive) {
    ...
    props.dispatch(
      SeoActions.updateExtensionUrlProviderStatus(
        { ProviderId: providerId, IsActive: isActive },
        () => {
  ...
  renderedProviders() {
    if (this.props.providers) {
      if (this.props.providers.length > 0) {
        ...
        return (
          <GridCell className="no-extension-url-providers">
            {resx.get("NoExtensionUrlProviders")}
          </GridCell>
        );
      }
    }
