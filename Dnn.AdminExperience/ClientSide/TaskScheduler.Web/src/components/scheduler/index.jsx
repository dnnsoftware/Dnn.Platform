...
class SchedulerPanel extends Component {
  ...
  componentDidMount() {
    ...
    if (
      props.serverList === null ||
      props.serverList === undefined ||
    ...
    tableFields.push({
      name: resx.get("RetryTimeLapse.Header"),
      id: "RetryTimeLapse",
    });
  ...
  onSelectServer(event) {
    ...
    if (server && serverId !== state.currentServerId) {
      ...
      this.setState(
        ...
        () => {
          props.dispatch(
            TaskActions.getSchedulerItemList({ serverName: server }),
          );
        },
      );
    }
  ...
  onUpdateSchedulerItem(scheduleItemDetail) {
    ...
    if (scheduleItemDetail.ScheduleID) {
      props.dispatch(
        TaskActions.updateScheduleItem(
          scheduleItemDetail,
          () => {
      ...
      props.dispatch(
        TaskActions.createScheduleItem(
          scheduleItemDetail,
          () => {
  ...
  onDeleteSchedulerItem(scheduleId) {
    ...
    util.utilities.confirm(
      resx.get("ScheduleItemDeletedWarning"),
      resx.get("Yes"),
      resx.get("No"),
      () => {
        const itemList = props.schedulerItemList.filter(
          (item) => item.ScheduleID !== scheduleId,
        );
        props.dispatch(
          TaskActions.deleteSchedule(
