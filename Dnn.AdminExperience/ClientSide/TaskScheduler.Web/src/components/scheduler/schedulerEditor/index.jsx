...
import { task as TaskActions } from "../../../actions";
...
class SchedulerEditor extends Component {
  ...
  componentDidMount() {
    ...
    if (props.scheduleId) {
      props.dispatch(
        TaskActions.getGetScheduleItem({
    ...
    if (timeLapseMeasurementOptions.length === 0) {
      timeLapseMeasurementOptions.push({
        value: "s",
        label: resx.get("Seconds"),
      });
      timeLapseMeasurementOptions.push({
        value: "m",
        label: resx.get("Minutes"),
      });
      timeLapseMeasurementOptions.push({
        value: "h",
        label: resx.get("Hours"),
      });
      timeLapseMeasurementOptions.push({ value: "d", label: resx.get("Days") });
      timeLapseMeasurementOptions.push({
        value: "w",
        label: resx.get("Weeks"),
      });
      timeLapseMeasurementOptions.push({
        value: "mo",
        label: resx.get("Months"),
      });
      timeLapseMeasurementOptions.push({
        value: "y",
        label: resx.get("Years"),
      });
    }
    ...
    if (catchUpEnabledOptions.length === 0) {
      catchUpEnabledOptions.push({
        value: "false",
        label: resx.get("Disabled"),
      });
      catchUpEnabledOptions.push({
        value: "true",
        label: resx.get("Enabled.Label"),
      });
    }
  ...
  getValue(selectKey) {
    ...
    switch (selectKey) {
      case "FriendlyName":
        return this.getDefaultIfNull(
          props.scheduleItemDetail,
          "FriendlyName",
          "",
        );
      case "TypeFullName":
  ...
  onSettingChange(key, event) {
    ...
    if (key === "ScheduleStartDate") {
      ...
      key === "RetainHistoryNum" ||
      key === "TimeLapseMeasurement" ||
      key === "RetryTimeLapseMeasurement" ||
      key === "AttachToEvent"
    ) {
      scheduleItemDetail[key] = event.value;
    } else {
      scheduleItemDetail[key] =
    ...
    if (
      (scheduleItemDetail[key] === "" || !re.test(scheduleItemDetail[key])) &&
      key === "TimeLapse"
    ) {
      state.error["frequency"] = true;
    } else if (
      scheduleItemDetail[key] !== "" &&
      re.test(scheduleItemDetail[key]) &&
      key === "TimeLapse"
    ) {
  ...
  render() {
    ...
    if (props.panelIndex === 1) {
      return (
        <History
          pageSize={5}
          scheduleId={props.scheduleId}
          title={resx.get("HistoryModalTitle")}
        />
      );
    }
    ...
    if (props.scheduleItemDetail !== undefined || props.id === "add") {
      const columnOne = (
        <div className="container" key="columnOne">
          ...
          <div className="editor-row divider">
            <Label
              label={resx.get("plRetainHistoryNum")}
      ...
      const columnTwo = (
        <div className="container right-column" key="columnTwo">
          <div className="editor-row divider">
            <Label
              label={resx.get("plScheduleStartDate")}
              style={{ margin: "0 0 5px 0" }}
            />
            <DatePicker
          ...
          <div className="editor-row divider">
            <SingleLineInputWithError
              ...
              value={
                this.getValue("RetryTimeLapse") === "-1"
                  ? ""
                  : this.getValue("RetryTimeLapse")
              }
          ...
          <div className="editor-row divider">
            <Label
              label={resx.get("plAttachToEvent")}
              style={{ margin: "0 0 5px 0" }}
            />
            <Dropdown
          ...
          <div className="editor-row divider">
            <Label
              label={resx.get("plCatchUpEnabled")}
              style={{ margin: "0 0 5px 0" }}
            />
            <Dropdown
      ...
      return (
        <div className="scheduler-setting-editor">
          <div className="scheduler-item-container">
            <div className="scheduler-item-column">{columnOne}</div>
            <div className="scheduler-item-column">{columnTwo}</div>
          </div>
          <div className="buttons-box">
            ...
            {props.scheduleId !== undefined &&
              props.scheduleItemDetail.Enabled &&
              props.enabled && (
                <Button type="secondary" onClick={this.runSchedule.bind(this)}>
                  {resx.get("cmdRun")}
                </Button>
              )}
            <Button
