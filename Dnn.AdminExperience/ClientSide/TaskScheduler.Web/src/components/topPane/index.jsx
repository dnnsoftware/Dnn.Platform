...
class TopPane extends Component {
  ...
  componentDidMount() {
    ...
    this.taskListTimeout = setInterval(() => {
      if (
        ...
        persistedSettings.activeIdentifier === "Dnn.TaskScheduler"
      ) {
        props.dispatch(TaskActions.getTaskStatusList());
      }
    }, 5000);
  ...
  getButtonDisplay() {
    if (this.isStopped() || this.props.schedulerMode === "2") {
      return (
        <TextOverflowWrapper text={resx.get("StartSchedule")} maxWidth={100} />
      );
    } else {
      return (
        <TextOverflowWrapper text={resx.get("StopSchedule")} maxWidth={100} />
      );
    }
  }
      ...
      if (props.schedulingEnabled === "True") {
        if (props.schedulerMode === "2") {
          ...
          return (
            <Button
              className={
                props.status !== "STOPPED" ? "topPane-button-start" : ""
              }
        ...
        <div className="topPane">
          ...
          <div className="topPane-middle">
            ...
            <div className="topPane-middle-common">
              <div
                ...
                style={{ maxWidth: "60%" }}
              >
                {resx.get("plSchedulerMode")}
              </div>
              <div>
                <div
                  className={
                    props.status === "SHUTTING_DOWN"
                      ? "editIconDisabled"
                      : "editIcon"
                  }
              ...
              <div className="topPane-middle-common-value">
                {this.getschedulerModeDisplay()}
              </div>
            </div>

            <div className="topPane-middle-common">
              <div className="topPane-middle-common-title">
                {resx.get("lblStartDelay")}
              </div>
              <div className="topPane-middle-common-value">
                {props.schedulerDelay}
              </div>
          ...
          <div className="topPane-right">
            <div className="topPane-right-common">
              <div className="topPane-right-common-title">
                {resx.get("lblMaxThreadsLabel")}
              </div>
              <div className="topPane-right-common-value">
                {props.maxThreads}
              </div>
            </div>
            <div className="topPane-right-common">
              <div className="topPane-right-common-title">
                {resx.get("lblActiveThreadsLabel")}
              </div>
              <div className="topPane-right-common-value">
                {props.activeThreads}
              </div>
          ...
          <div className="topPane-button">{this.renderButton()}</div>
        </div>
      </div>
    );
  }
