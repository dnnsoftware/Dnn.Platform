...
export default class Folders extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (props.folders !== prevProps.folders) {
      if (
        !props.folders ||
        !props.folders.children ||
        !props.folders.children[0] ||
        !props.folders.children[0].data ||
        !props.folders.children[0].data.key
      ) {
        return;
      }
  ...
  getFolders(folder) {
    ...
    const children = folder.children.map((child) => {
      ...
      return (
        <li className={className} key={child.data.key}>
  ...
  render() {
    ...
    return <div className="dnn-folders-component">{folders}</div>;
  }
}
