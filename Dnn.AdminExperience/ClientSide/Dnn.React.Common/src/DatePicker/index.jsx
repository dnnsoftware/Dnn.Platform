...
class DatePicker extends Component {
  constructor(props) {
    ...
    let secondDate =
      typeof props.secondDate === "string"
        ? new Date(props.secondDate)
        : props.secondDate;
  ...
  handleClick(e) {
    const isController =
      hasClass(e.target, DefaultControllerClassName) ||
      (this.props.controllerClassName &&
        hasClass(e.target, this.props.controllerClassName));
  ...
  firstDisableDates(day) {
    const maxDate = this.props.maxDate
      ? this.resetHours(new Date(this.props.maxDate))
      : null;
    const minDate = this.props.minDate
      ? this.resetHours(new Date(this.props.minDate))
      : null;
    const SecondDate = this.state.Date.SecondDate
      ? this.resetHours(new Date(this.state.Date.SecondDate))
      : null;
  ...
  secondDisableDates(day) {
    const FirstDate = this.state.Date.FirstDate
      ? this.resetHours(new Date(this.state.Date.FirstDate))
      : null;
    const minDate = this.props.minSecondDate
      ? this.resetHours(new Date(this.props.minSecondDate))
      : null;
    const maxDate = this.props.maxSecondDate
  ...
  updateDate(firstDate, secondDate, options = {}) {
    ...
    if (typeof this.props.date === "string") {
      if (FirstDate) {
        FirstDate =
          this.formatDate(FirstDate, "L") +
          " " +
          this.formatDate(FirstDate, "LT");
      }
      if (SecondDate) {
        SecondDate =
          this.formatDate(SecondDate, "L") +
          " " +
          this.formatDate(SecondDate, "LT");
      }
    }
    ...
    if (
      options.callUpdateDate ||
      (!this.props.isDateRange && !this.props.hasTimePicker)
    ) {
      this.callUpdateDate();
  ...
  updateFirstDate(date, preventUpdateTime = false) {
    ...
    if (validateDate(firstDate, minDate, maxDate)) {
      this.updateDate(firstDate, secondDate, {
        preventHide: true,
        callUpdateDate: true,
        preventUpdateTime: preventUpdateTime,
      });
    }
  }
  ...
  render() {
    ...
    if (this.props.hasTimePicker) {
      displayFirstDate += displayFirstDate
        ? " " + this.formatDate(firstDate, "LT")
        : "";
      displaySecondDate += displaySecondDate
        ? " " + this.formatDate(secondDate, "LT")
        : "";
    }
    ...
    const calendarClassName =
      "calendar-container " +
      this.getPositionCss() +
      (this.state.isCalendarVisible ? " visible" : " invisible");
    ...
    const style =
      this.props.isDateRange && this.props.hasTimePicker ? { width: 380 } : {};
    const buttonStyle = this.props.isDateRange
      ? {}
      : { margin: "10px auto", float: "none" };
    const inputClassName =
      "calendar-text" + (this.props.hasTimePicker ? " with-time-picker" : "");

    const showClearDates =
      !!this.props.isDateRange && this.props.showClearDates;
    const clearButtonStyle =
      this.state.Date.FirstDate || this.state.Date.SecondDate
        ? clearButtonStyleVisible
        : clearButtonStyleInvisible;

    return (
      <div className="dnn-day-picker" ref={this.dayPickerRef}>
        {showInput && (
          <div
          ...
          >
            ...
            {!this.props.isInputReadOnly && (
              <div style={{ float: "right" }}>
                <DateInput
                  date={firstDate}
                  onUpdateDate={this.updateFirstDate.bind(this)}
                  hasTimePicker={this.props.hasTimePicker || false}
                />
                {this.props.isDateRange && (
                  <div>
                    ...
                    <DateInput
                      date={secondDate}
                      onUpdateDate={this.updateSecondDate.bind(this)}
                      hasTimePicker={this.props.hasTimePicker || false}
                    />
                  </div>
                )}
        ...
        {showIcon && (
          <div
            className={
              "calendar-icon" + (this.state.isCalendarVisible ? " active" : "")
            }
        ...
        <div
          ...
          style={this.getStyle()}
          ref={(element) => (this.calendarContainer = element)}
        >
          <div>
            <DayPicker
          ...
          {this.props.isDateRange && (
            <div>
              ...
              {this.props.hasTimePicker && (
                <TimePicker
                  updateTime={this.updateSecondTime.bind(this)}
                  time={this.formatDate(this.secondDate, "LT")}
                />
              )}
            </div>
          )}
          {showButton && (
            ...
            >
              {this.props.applyButtonText || "Apply"}
            </button>
          )}
          {showClearDates && (
            <button
              ...
              onClick={this.onClearDatesPressed.bind(this)}
            >
              Clear
            </button>
          )}
