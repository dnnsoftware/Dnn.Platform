...
class Dropdown extends Component {
  ...
  render() {
    ...
    const layoutOptions = options.map((option) => {
      return (
        <div
          ...
          key={option.value}
          onClick={this.onUpdate.bind(this, option.value)}
        >
          {option.label}
        </div>
      );
    ...
    return (
      <div
        className={"dnn-time-picker " + (className || "")}
        ref={this.timePickerRef}
      >
        <div className="time-text" onClick={this.showTimeSelector.bind(this)}>
          <span className="time-text-label" title={label}>
            {label}
          </span>
          {this.state.isTimeSelectorVisible && (
            <div className={"time-selector " + this.state.className}>
              <div className="time-options">{layoutOptions}</div>
            </div>
          )}
        </div>
      </div>
