...
class Pager extends Component {
  ...
  formatCommaSeparate(number) {
    ...
    while (/(\d+)(\d{3})/.test(number.toString())) {
      number = number
        .toString()
        .replace(/(\d+)(\d{3})/, "$1" + numbersSeparatorByLocale + "$2");
    }
    return number;
  }
  ...
  calculateTotalPages(props) {
    ...
    if (state.pageSize >= props.totalRecords || props.totalRecords === 0) {
      ...
      if (state.endIndex === 1) {
        state.endIndex =
          props.numericCounters !== undefined ? props.numericCounters : 0;
      }
      ...
      if (props.numericCounters > 0) {
        if (currentPage >= totalPages - 1) {
          endIndex = totalPages;
          startIndex =
            endIndex - props.numericCounters > 0
              ? endIndex - props.numericCounters
              : 0;
        } else if (currentPage <= 0) {
          startIndex = 0;
          endIndex =
            startIndex + props.numericCounters > totalPages - 1
              ? totalPages
              : startIndex + props.numericCounters;
        }
  ...
  onPageChanged(step) {
    ...
    if (this.props.numericCounters > 0) {
      ...
      if (
        ...
        endIndex =
          endIndex + boxesHalf > state.totalPages
            ? state.totalPages
            : endIndex + boxesHalf;
        startIndex =
          endIndex < state.totalPages
            ? startIndex + boxesHalf
            : endIndex - this.props.numericCounters > 0
              ? endIndex - this.props.numericCounters
              : 0;
        ...
        startIndex = 0;
        endIndex =
          startIndex + this.props.numericCounters > state.totalPages - 1
            ? state.totalPages
            : startIndex + this.props.numericCounters;
      }
  ...
  getPageSummary() {
    if (this.props.showSummary) {
      ...
      return this.format(
        ...
        this.formatCommaSeparate(start),
        this.formatCommaSeparate(end),
        this.formatCommaSeparate(props.totalRecords),
      );
    }
    return "&nbsp;";
  }
  getPagingBoxes() {
    ...
    if (this.props.numericCounters > 1) {
      for (let i = startIndex; i < endIndex; i++) {
        let step = i + 1;
        if (i !== currentPage) {
          pagingBoxes = pagingBoxes.concat(
            <li
              key={"li" + i}
              className="pages do-not-close"
              onClick={this.onPageChanged.bind(this, i)}
            >
              {this.formatCommaSeparate(step)}
            </li>,
          );
        } else {
          pagingBoxes = pagingBoxes.concat(
            <li key={"li" + i} className="pages current do-not-close">
              {this.formatCommaSeparate(i + 1)}
            </li>,
          );
        }
      }
      ...
      pagingBoxes = pagingBoxes.concat(
        <li key="li-single" className="pages current do-not-close">
          {this.formatCommaSeparate(currentPage + 1)}
        </li>,
      );
    }
    return pagingBoxes;
  ...
  getPageSizeDropDown() {
    let pageSizeOptions = [];
    if (this.props.totalRecords >= 1)
      pageSizeOptions.push({
        value: 10,
        label: this.format(
          this.props.pageSizeOptionText,
          this.formatCommaSeparate(10),
        ),
      });
    if (this.props.totalRecords >= 10)
      pageSizeOptions.push({
        value: 25,
        label: this.format(
          this.props.pageSizeOptionText,
          this.formatCommaSeparate(25),
        ),
      });
    if (this.props.totalRecords >= 25)
      pageSizeOptions.push({
        value: 50,
        label: this.format(
          this.props.pageSizeOptionText,
          this.formatCommaSeparate(50),
        ),
      });
    if (this.props.totalRecords >= 50)
      pageSizeOptions.push({
        value: 100,
        label: this.format(
          this.props.pageSizeOptionText,
          this.formatCommaSeparate(100),
        ),
      });
    if (this.props.totalRecords >= 100)
      pageSizeOptions.push({
        value: 250,
        label: this.format(
          this.props.pageSizeOptionText,
          this.formatCommaSeparate(250),
        ),
      });

    if (
      !pageSizeOptions.some((option) => option.value === this.props.pageSize)
    ) {
      pageSizeOptions = pageSizeOptions.concat({
        value: this.props.pageSize,
        label: this.format(
          this.props.pageSizeOptionText,
          this.formatCommaSeparate(this.props.pageSize),
        ),
      });
  ...
  renderIcon(OnClick, Type, Disabled) {
    if (!Disabled) {
      return (
        <li className={"do-not-close pages prev"} onClick={OnClick.bind(this)}>
      ...
      return (
        <li className={"do-not-close pages prev disabled"}>
          <span className="icon-flat">
            <Type />
          </span>
        </li>
      );
          ...
          <div className="dnn-pager-control">
            ...
            {this.props.showPageInfo && !this.props.showPageSizeOptions && (
              <div className="dnn-pager-options-info-box">
                {this.format(
                  this.props.pageInfoText,
                  this.formatCommaSeparate(state.currentPage + 1),
                  this.formatCommaSeparate(state.totalPages),
                )}
              </div>
            )}
            {this.props.showPageSizeOptions && (
