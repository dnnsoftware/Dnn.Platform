...
class PagePicker extends Component {
  ...
  UNSAFE_componentWillReceiveProps(newProps) {
    ...
    const { props } = this;
    if (
      props.PortalTabsParameters.portalId !==
        newProps.PortalTabsParameters.portalId ||
      props.PortalTabsParameters.cultureCode !==
        newProps.PortalTabsParameters.cultureCode ||
      props.PortalTabsParameters.isMultiLanguage !==
        newProps.PortalTabsParameters.isMultiLanguage ||
      props.PortalTabsParameters.excludeAdminTabs !==
        newProps.PortalTabsParameters.excludeAdminTabs ||
    ...
    if (newProps.Reload && !newProps.IsInDropDown && this._isMounted) {
      ...
      this.setState(
        { selectedPages: [], dropDownOpen: false, portalTabs: [] },
        () => {
  ...
  getPortalTabs(cultureCode, callback) {
    ...
    let service = new Service(
      this.props.serviceFramework,
      this.props.moduleRoot,
      this.props.controller,
    );
    const portalTabsParameters = !props.IsMultiSelect
      ? Object.assign(props.PortalTabsParameters, {
  ...
  setPages(callback) {
    ...
    this.setState(
      ...
      () => {
        this.SetOpenNodesCount(
          this.state.portalTabs[0],
          this.state.portalTabs[0].TabId,
          0,
        );
  ...
  SetOpenNodesCount(root, lastId, count) {
    if (root.IsOpen && root.ChildTabs !== null && root.ChildTabs.length > 0) {
      root.ChildTabs.map((child) => {
        ...
        if (
          child.IsOpen &&
          child.ChildTabs !== null &&
          child.ChildTabs.length > 0
        ) {
  ...
  onPageSelect(page, keepOpen) {
    keepOpen =
      keepOpen === undefined || typeof keepOpen !== "boolean"
        ? false
        : keepOpen;
    const { props } = this;
    if (props.IsMultiSelect) {
      ...
      selectedPagesC = this.CheckUnCheckAllChildren(
        page,
        page.CheckedState,
        selectedPagesC,
      );
  ...
  getDescendants(page, callback, event) {
    ...
    if (
      parentToAddChildrenTo.ChildTabs &&
      parentToAddChildrenTo.ChildTabs.length > 0
    ) {
    ...
    const portalTabsDescendantsParameters = Object.assign(
      props.PortalTabsParameters,
      { parentId: page.TabId },
    );
    ...
    service.getTabsDescendants(portalTabsDescendantsParameters, (data) => {
      ...
      parentToAddChildrenTo.ChildTabs = data.Results.map((child) => {
        if (page.CheckedState === 0) {
          ...
          selectedPages = this.UpdateSelectedData(
            child,
            page.CheckedState !== undefined ? page.CheckedState : 1,
            selectedPages,
          );
        ...
        return {
          ...
          CheckedState: this.props.IsMultiSelect
            ? page.CheckedState !== undefined
              ? page.CheckedState
              : 1
            : 1,
  ...
  getChildItems(children, isCurrentOrDescendant) {
    ...
    return children.map((page) => {
      ...
      const textClass =
        props.IsMultiSelect && props.ShowIcon
          ? " text-with-page-icon"
          : !props.IsMultiSelect && !props.ShowIcon
            ? "no-icon"
            : "";
      const pageClass =
        props.IsMultiSelect && props.ShowIcon ? " page-icon" : "";
      const parentNotSelectable = isCurrentOrDescendant
        ? isCurrentOrDescendant
        : this.props.currentTabId === parseInt(page.TabId);
        ...
        >
          {!page.IsOpen && page.HasChildren && (
            <div
              ...
              onClick={this.getDescendants.bind(this, page, null)}
            >
              <ArrowRightIcon />
            </div>
          )}
          ...
          <div
            className={this.getSelected(page, parentNotSelectable)}
            onClick={
              page.Selectable && !parentNotSelectable
                ? this.onPageSelect.bind(this, page)
                : void 0
            }
          >
            {props.ShowIcon && (
    ...
    if (props.IsInDropDown && !props.IsMultiSelect) {
      ...
      if (portalTabs !== undefined && portalTabs.length > 0) {
        ...
        if (props.selectedTabId === -1) {
          ...
          page = this.findTabById(
            props.selectedTabId,
            portalTabs[0].ChildTabs,
            null,
          );
        }
  ...
  handleClick(event) {
    ...
    if (!this._isMounted || !props.closeOnBlur || !props.IsInDropDown) {
      return;
    }
    if (
      !this.dnnPagePickerRef.current.contains(event.target) &&
  ...
  onSearch(value) {
    ...
    if (value !== "") {
      const apiParameters = Object.assign(props.PortalTabsParameters, {
        searchText: value,
      });
      let service = new Service(
        this.props.serviceFramework,
  ...
  CheckUnCheckAllChildren(root, checked, selectedPages) {
    ...
    if (root.HasChildren && root.ChildTabs.length > 0) {
      root.ChildTabs.forEach((child) => {
        selectedPages = this.CheckUnCheckAllChildren(
          child,
          checked,
          selectedPages,
        );
  ...
  UpdateSelectedData(Tab, checked, selectedPages) {
    if (
      (checked === 0 || checked === 2) &&
      !selectedPages.some(
        (page) => page.TabId === Tab.TabId && page.CheckedState === checked,
      )
    ) {
      ...
      checked === 1 &&
      selectedPages.some((page) => page.TabId === Tab.TabId)
    ) {
      selectedPages = selectedPages.filter((page) => {
        return page.TabId !== Tab.TabId;
      });
  ...
  DoRecursiveSelect(page, mainKey, originalChecked, callback) {
    ...
    if (
      mainKey !== page.TabId &&
      page.HasChildren &&
      page.ChildTabs.length > 0
    ) {
      if (
        !page.ChildTabs.some(
          (child) =>
            child.CheckedState === undefined ||
            child.CheckedState === 1 ||
            child.CheckedState === 2,
        )
      )
        checked = 0;
      else if (
        ...
        page.ChildTabs.some(
          (child) =>
            child.CheckedState !== undefined &&
            (child.CheckedState === 2 || child.CheckedState === 0),
        )
      )
        checked = 2;
    }
  ...
  RecursiveFindParent(parentId, currentTree) {
    ...
    if (currentTree.some((child) => child.IsOpen && child.TabId === parentId)) {
      parent = currentTree.find(
        (child) => child.IsOpen && child.TabId === parentId,
      );
  ...
  render() {
    ...
    if (this.loaded || props.IsInDropDown) {
      ...
      if (state.portalTabs.length > 0 && state.portalTabs[0].Processed) {
        let children =
          state.portalTabs.length > 0 &&
          state.portalTabs[0].Processed &&
          this.getChildItems(state.portalTabs);
        picker = (
          <PagePickerScrollbar
            ...
            <div className="pages-container">
              <ul>
                {state.portalTabs.length > 0 &&
                  ...
                  children}
                {state.portalTabs.length <= 0 ||
                  (!state.portalTabs[0].Processed && props.noneSpecified)}
              </ul>
            </div>
          </PagePickerScrollbar>
        );
      ...
      return (
        ...
        >
          ...
          {props.IsInDropDown && (
            <div
              className={
                "collapsible-content" + (state.dropDownOpen ? " open" : "")
              }
              style={props.style}
            >
              <Collapse
                theme={{
                  collapse: "page-picker-content ReactCollapse--collapse",
