...
class GridSystem extends Component {
  ...
  getStyle() {
    ...
    return Object.assign(
      { width: (props.width || 100) + (props.type ? props.type : "%") },
      props.style,
    );
  }
  /*
  ...
  getColumns() {
    ...
    for (let i = 0; i < (props.numberOfColumns || props.children.length); i++) {
      children.push(
        <GridCell
          key={this.uniqueId + i}
          columnSize={100 / (props.numberOfColumns || props.children.length)}
          style={props.gridCellStyle}
        >
          {props.children[i]}
        </GridCell>,
      );
  ...
  render() {
    ...
    return (
      <div
        className={"dnn-grid-system " + props.className}
        style={this.getStyle()}
      >
        {this.getColumns()}
      </div>
