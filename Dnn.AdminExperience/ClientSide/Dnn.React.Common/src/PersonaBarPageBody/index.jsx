...
const PersonaBarPageBody = ({
  ...
  const _workSpaceTrayClassName =
    "dnn-workspace-tray" + getClassNameIfExists(workSpaceTrayClassName);
  const personaBarPageBodyClassName =
    "dnn-persona-bar-page-body" +
    getClassNameIfExists(className) +
    getClassNameIfExists(
      !backToLinkProps ? "" : backToLinkProps.text && "with-back-to-link",
    );
  return (
    <GridCell className={personaBarPageBodyClassName} style={style}>
      {backToLinkProps && backToLinkProps.text && (
        <BackToLink {...backToLinkProps} />
      )}
      {outsideTrayVisible && (
        <GridCell className={_workSpaceTrayClassName}>{workSpaceTray}</GridCell>
      )}
      <GridCell className="persona-bar-page-body">
        {insideTrayVisible && (
          <GridCell className={_workSpaceTrayClassName}>
            {workSpaceTray}
          </GridCell>
        )}
        {children}
