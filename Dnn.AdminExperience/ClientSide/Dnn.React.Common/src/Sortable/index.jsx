...
export default class Sortable extends Component {
  ...
  updateItemClass(sortableItems, dropY) {
    [].forEach.call(sortableItems, (item, index) => {
      if (
        dropY >
          item.offsetHeight * index + item.getBoundingClientRect().height / 2 &&
        dropY <
          item.offsetHeight * index +
  ...
  sortOnDrag(event, dropX, dropY) {
    ...
    if (this.dnnSortableRef.current) {
      const itemElement = this.dnnSortableRef.current.querySelectorAll(
        `[data-dnn-sortable-id="${id}"]`,
      )[0];
  ...
  onDrop(event, dropX, dropY) {
    ...
    if (this.dnnSortableRef.current) {
      ...
      const isSameIndex =
        currentIndex != null &&
        (newIndex == currentIndex || newIndex - 1 == currentIndex);
      if (isSameIndex) {
        return;
      }
  ...
  render() {
    ...
    return (
      <div className="dnn-sortable" ref={this.dnnSortableRef}>
        <SortableContainer
