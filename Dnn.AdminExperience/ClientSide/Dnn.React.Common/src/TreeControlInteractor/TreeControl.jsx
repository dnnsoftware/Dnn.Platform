...
export default class TreeControl extends Component {
      ...
      const left = () =>
        childTab.forEach((tab) => {
          Array.isArray(tab.ChildTabs)
            ? comparator(tab, this.props.tabs)
            : null;
  ...
  _mapToParentTabs(parent, fn) {
    ...
    const loop = () => {
      ...
      parent =
        parseInt(parent.TabId) !== -1 ? this.props.findParent(parent) : {};
  ...
  resetCheckedState(tab) {
    ...
    tab.HasChildren
      ? this._mapToChildTabs(tab, unselectChildren)
      : unselectIndividual();
    tab.CheckedState = this.props.unchecked;
    tab.ChildrenSelected = false;
    this.props.updateTree(tab);
  ...
  setParentCheckedState(parent) {
    ...
    const checkParent = () => {
      ...
      switch (true) {
        ...
        case checkedArray.indexOf(true) !== -1:
          parent.CheckedState =
            parent.CheckedState === this.props.fullyChecked
              ? this.props.individuallyChecked
              : parent.CheckedState;
          this.props.updateTree(parent);
          return;
      }
    ...
    ChildTabs.forEach((tab) =>
      tab.CheckedState ? checkedArray.push(true) : checkedArray.push(false),
    );
  ...
  render_ListBullet(tab, fn) {
    const bullet = (() => {
      return (
        <div className="arrow-bullet" onClick={() => fn()}>
    ...
    const checkbox = (() => {
      ...
      const anyChildrenSelected = (tab) => {
        ...
        const left = () => {
          ...
          const AreChildrenChecked = (t) => {
            ...
            condition
              ? trueCheckedState.push(true)
              : trueCheckedState.push(false);
          };
      ...
      const renderCheckBoxClassName = () => {
        switch (true) {
          ...
          case !!tab.HasChildren === true &&
            !!tab.CheckedState === false &&
            anyChildrenSelected(tab):
  ...
  render_tabName(tab) {
    const charLimit = 15;
    const name =
      tab.Name.length > charLimit
        ? `${tab.Name.substr(0, charLimit)}...`
        : tab.Name;
    const render = (() => {
  ...
  render_li(tabs) {
    const render = (() => {
      return tabs.map((tab) => {
        ...
        const bullet = this.render_ListBullet.call(
          this,
          tab,
          this.expandParent.bind(this, tab),
        );
