...
export default class TreeControlInteractor extends Component {
  ...
  requestDescendantTabs(ParentTabId, callback) {
    let descendantTabs = [];
    const captureDecendants = (tabs) =>
      (descendantTabs = tabs.map((tab) => {
    ...
    const appendDescendants = (parentTab) => {
      descendantTabs.forEach((tab) => {
        const condition =
          parseInt(tab.ParentTabId) === parseInt(parentTab.TabId);
        condition ? parentTab.ChildTabs.push(tab) : null;
      });
    };

    this.props.getDescendantPortalTabs(
      this.PortalTabsParameters,
      ParentTabId,
      (response) => {
          ...
          const left = () => {
            const select = () => {
              tab.ChildTabs = tab.ChildTabs.map((child) => {
                child.CheckedState = child.HasChildren
                  ? this.fullyChecked
                  : this.individuallyChecked;
                return child;
              });
            };
  ...
  traverse(comparator) {
    ...
    const loop = () => {
      ...
      const left = () =>
        childTab.forEach((tab) => {
          Array.isArray(tab.ChildTabs) ? comparator(tab, copy) : null;
          Array.isArray(tab.ChildTabs) && tab.ChildTabs.length
            ? cachedChildTabs.push(tab.ChildTabs)
            : null;
          condition ? loop() : exit();
        });
      const right = () => null;
  ...
  updateTree(tabData) {
    ...
    const find = (tab, copy) => {
      parseInt(tab.TabId) === parseInt(tabData.TabId)
        ? capture(tab, copy)
        : null;
    };
    this.traverse(find);
  ...
  reAlignTree() {
    let iterationsArray = [];
    const iterations = (t) =>
      t.ChildTabs.length ? iterationsArray.push(...t.ChildTabs) : null;
    ...
    const realign = (tab) => {
      ...
      const expect =
        tabsWithoutChildren.length +
        tabsWithChildren.length * this.fullyChecked;
  ...
  export() {
    ...
    const filterOnlyParents = (tabs) =>
      tabs.filter((tab) => tab.CheckedState === this.fullyChecked);
    const filterOnlyChildren = (tabs) =>
      tabs.filter((tab) => tab.CheckedState === this.individuallyChecked);
    const filterOutIfAllSelected = (arr1, arr2) =>
      arr1.filter((tab) => {
        let exists = false;
        arr2.forEach((t) =>
          t.TabId === tab.ParentTabId ? (exists = t) : null,
        );
        return exists === false;
      });

    const generateSelections = (tab) =>
      tabs.push(this.generateSelectionObject(tab));
