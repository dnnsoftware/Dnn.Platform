...
class DnnTabs extends Component {
  ...
  getTabPanels() {
    ...
    if (Object.prototype.toString.call(props.children) === "[object Array]") {
      for (let i = 0; i < props.children.length; i++) {
        children.push(
          <TabPanel key={this.uniqueId + "panel-" + i}>
            {props.children[i]}
          </TabPanel>,
        );
      }
    ...
    return (
      <TabPanel key={this.uniqueId + "panel-0"}>{props.children}</TabPanel>
    );
  }
  ...
  render() {
    ...
    return (
      <Tabs
        className={"dnn-tabs react-tabs" + className + (" " + props.type)}
      ...
      >
        <TabList>{this.getTabHeaders()}</TabList>
        {this.getTabPanels()}
      </Tabs>
    );
  }
