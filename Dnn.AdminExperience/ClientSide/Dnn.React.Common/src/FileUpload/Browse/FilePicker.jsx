...
export default class FilePicker extends Component {
  ...
  handleClick(e) {
    ...
    if (
      ...
      e.target.className === "clear-button"
    ) {
      return;
    }
    this.hide();
    ...
    if (isMatchSearch) {
      return (
        <li>
          <div
            className="icon"
            onClick={this.onFileNameClick.bind(this, child)}
          >
            <FileIcon />
          </div>
          <div
  ...
  getFiles() {
    ...
    return (
      <ul>
        <li>
          <div className="icon" onClick={this.onFileNameClick.bind(this)}>
            <FileIcon />
          </div>
          <div
            className="item-name none-specified"
            onClick={this.onFileNameClick.bind(this)}
          >
  ...
  render() {
    const selectedFileText = this.props.selectedFile
      ? this.props.selectedFile.value
      : this.props.notSpecifiedText;

    const files = this.getFiles();

    return (
      <div className="drop-down" ref={(node) => (this.node = node)}>
        <div className="selected-item" onClick={this.onFilesClick.bind(this)}>
          {selectedFileText}
        </div>
        <div
          className={
            "item-picker-container" + (this.state.showFilePicker ? " show" : "")
          }
        >
          <div className="inner-box">
            <div className="search">
              <input
                type="text"
                value={this.state.searchFileText}
                onChange={this.onChangeSearchFileText.bind(this)}
                placeholder={this.props.searchFilesPlaceHolderText}
                aria-label="Search"
              />
            ...
            <div className="items">
              <Scrollbars
                ...
                autoHeightMax={200}
              >
                <div className="item-picker">{files}</div>
              </Scrollbars>
            </div>
          </div>
        </div>
