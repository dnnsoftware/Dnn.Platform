...
export default class Browse extends Component {
  ...
  getFolders(searchText) {
    ...
    let portalIdParams =
      this.props.portalId && this.props.portalId !== -1
  ...
  getFiles() {
    const sf = this.getServiceFramework();
    let parentId = this.state.selectedFolder
    ...
    if (parentId) {
      sf.get(
        "GetFiles",
        { parentId, filter: extensions },
        this.setFiles.bind(this),
        this.handleError.bind(this),
      );
    } else if (this.state.selectedFolder) {
      sf.get(
        "SearchFolders",
        { searchText: this.state.selectedFolder.value },
        this.setFolderId.bind(this),
        this.handleError.bind(this),
      );
    } else {
      this.setFiles();
  ...
  setFolderId(result) {
    ...
    sf.get(
      "GetFiles",
      { parentId: selectedFolder.key, filter: extensions },
      this.setFiles.bind(this),
      this.handleError.bind(this),
    );
  }
  ...
  setFolders(result) {
    ...
    if (
      (this.state.selectedFile && !this.state.selectedFolder) ||
      (this.state.selectedFolder && this.state.selectedFolder.value === "0")
    ) {
      const selectedFolder = result.Tree.children[0].data;
