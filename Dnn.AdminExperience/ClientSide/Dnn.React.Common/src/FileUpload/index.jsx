...
export default class FileUpload extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (!props.selectedFile && props.selectedFile !== prevProps.selectedFile) {
      this.setState(
        { fileExist: null, selectedFile: null, selectedFolder: null },
        () => {},
      );
      return;
    }
    if (
      props.selectedFile &&
      props.selectedFile !== prevProps.selectedFile &&
      this.state.selectedFile
    ) {
      if (
        props.selectedFile.fileId !==
        (this.state.selectedFile.fileId || +this.state.selectedFile.key)
      ) {
  ...
  handleError(error) {
    const errorText =
      error && typeof error === "string" ? error : this.props.uploadFailedText;
  ...
  sendResult() {
    ...
    const fileId = selectedFile
      ? selectedFile.fileId || +selectedFile.key
      : null;
  ...
  getPreviewUrl() {
    const fileId = this.state.selectedFile
      ? this.state.selectedFile.fileId || +this.state.selectedFile.key
      : "";
    if (!fileId) {
      this.setState(
        { fileUrl: "", fileExist: false },
  ...
  compareDimensions(image) {
    ...
    if (
      ...
      image.width / image.height >
        componentDimension.width / componentDimension.height
    ) {
  ...
  postFile(file) {
    ...
    if (
      ...
      typeof this.props.validationCode === "string"
    ) {
      formData.append("validationCode", this.props.validationCode);
    }
    sf.postfile(
      `UploadFromLocal${this.props.portalId === -1 ? "" : "?portalId=" + this.props.portalId}`,
      formData,
  ...
  render() {
    ...
    const className =
      "overlay" +
      (src && state.fileExist ? " has-image" : "") +
      (state.draggedOver ? " hover" : "");

    return (
      <div className="dnn-file-upload" ref={(node) => (this.node = node)}>
        <div>
          <div
            id="dropzoneId"
            className={className}
          ...
          {state.selectedFile && (
            <div className="dnn-file-upload-file-name">
              <span>{state.selectedFile.value}</span>
            </div>
          )}
        </div>
        {state.uploading && (
