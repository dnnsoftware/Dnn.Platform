...
export default class Suggestions extends Component {
  ...
  getSuggestions() {
    let suggestions = this.props.suggestions.map((suggestion, index) => {
      let className =
        this.props.selectedIndex > -1 && index === this.props.selectedIndex
          ? "selected"
          : "";
      return (
        <div
          ref={(itemRef) => (this.itemRef = index === 0 ? itemRef : null)}
          className={`suggestion ${className}`}
          key={index}
          onClick={this.onSelectSuggestion.bind(this, suggestion.value)}
        >
          {suggestion.value}
        </div>
      );
      ...
      setTimeout(() => {
        if (selectedItemScroll < currentScroll) {
          ...
          scrollBars.scrollTop(
            selectedItemScroll + h + currentScroll + viewHeight,
          );
        }
      });
    }
