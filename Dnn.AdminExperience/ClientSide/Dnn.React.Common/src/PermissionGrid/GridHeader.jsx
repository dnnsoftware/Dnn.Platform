...
class GridHeader extends Component {
  ...
  renderHeader() {
    ...
    return (
      <GridCell className="grid-header">
        <GridCell columnSize={roleColumnWidth}>
          <span title={props.type}>{this.getLocaliztion(props.type)}</span>
        </GridCell>
        {props.definitions.map((def) => {
          return (
            <GridCell key={def.permissionName} columnSize={columnWidth}>
              <span title={def.permissionName}>
                {this.getLocaliztion(def.permissionName)}
              </span>
            </GridCell>
          );
        })}
