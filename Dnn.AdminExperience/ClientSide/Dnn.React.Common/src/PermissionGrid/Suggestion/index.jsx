...
class Suggestion extends Component {
  ...
  getSuggestions() {
    ...
    let keyword =
      state.selectedValue.value > -1 ? "" : state.selectedValue.label;
  ...
  onUserSelectorSelected(item) {
    this.setState({
      selectedValue: { value: item.value, label: item.label },
      suggestions: [],
    });
  }
      ...
      <div className="dnn-suggestion">
        <span>
          ...
          <div className="add-button" onClick={this.onAddSuggestion.bind(this)}>
            <IconButton type="add" width={17} height={17} />{" "}
            {props.localization.add}
          </div>
        </span>
      </div>
    );
