...
class GridRow extends Component {
  ...
  getHeaderColumnText() {
    ...
    return props.type === "role"
      ? props.permission.roleName
      : props.permission.displayName;
  }
  ...
  onStatusChanged(def, status) {
    ...
    permission.permissions = Object.assign(
      [],
      props.permission.permissions.filter((p) => {
    ...
    if (status > 0) {
      permission.permissions.push(
        Object.assign({}, def, { allowAccess: status === 1 }),
      );
    }
  ...
  fixPermissionsStatus(permission, def, status) {
    ...
    if (def.fullControl) {
      if (status === 0) {
        ...
        props.definitions.forEach(function (d) {
          ...
          if (p.length) {
            ...
            permission.permissions.push(
              Object.assign({}, d, { allowAccess: status === 1 }),
            );
          }
        });
      }
          ...
          if (permissionName !== "navigate" && permissionName !== "browse") {
            props.definitions.forEach(function (d) {
              ...
              if (p.length) {
                ...
                permission.permissions.push(
                  Object.assign({}, d, { allowAccess: true }),
                );
              }
            });
          }
      ...
      if (status > 0) {
        let samePermissionsSet =
          props.definitions.filter(function (d) {
            if (d.fullControl) {
              return false;
            }
            ...
            return (
              (!allowAccess && status === 1) || (allowAccess && status === 2)
            );
          }).length === 0;
        ...
        if (samePermissionsSet) {
          ...
          if (d.length) {
            ...
            if (p.length) {
              ...
              permission.permissions.push(
                Object.assign({}, d[0], { allowAccess: status === 1 }),
              );
            }
          }
        }
  ...
  renderRow() {
    ...
    return (
      <GridCell className="grid-row" ref={(node) => (this.node = node)}>
        <GridCell columnSize={roleColumnWidth}>
          <span title={this.getHeaderColumnText()}>
            {this.getHeaderColumnText()}
          </span>
        </GridCell>
        {props.definitions.map(function (def) {
          ...
          if (props.permission.locked) {
            ...
            status =
              permission.length > 0 ? (permission[0].allowAccess ? 1 : 2) : 0;
          }

          return (
            <GridCell
              style={{ height: self.columnHeight + "px" }}
              columnSize={columnWidth}
              key={def.permissionId}
            >
              <StatusSwitch
                permission={permission}
                status={status}
                onChange={self.onStatusChanged.bind(self, def)}
              />
            </GridCell>
          );
        })}
        <GridCell
          style={{ height: self.columnHeight + "px" }}
          columnSize={actionsWidth}
          className="col-actions"
        >
          {!props.permission.default && (
            <IconButton type="trash" onClick={this.onDelete.bind(this)} />
          )}
        </GridCell>
      </GridCell>
    );
