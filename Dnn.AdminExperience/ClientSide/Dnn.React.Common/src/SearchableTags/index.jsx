...
export default class SearchableTags extends Component {
  ...
  getSearchResults() {
    this.resizeInputField();
    const searchQuery =
      this.state.newTagText && typeof this.state.newTagText === "string"
    ...
    this.setTimeout = setTimeout(() => {
      ...
      return sf.get(
        "Search",
        { q: searchQuery },
        this.setSearchResults.bind(this),
  ...
  resizeInputField() {
    ...
    if (this.state.tags.length) {
      width = this.state.newTagText.length
        ? this.state.newTagText.length * 7 + 6
        : 13;
    }
  ...
  removeTagByName(tagId) {
    ...
    let tags = this.state.tags;
    tags = tags.filter((_tag) => {
      return _tag.id !== tagId;
    });
    this.updateTags(tags);
  }
  ...
  render() {
    const Tags = this.state.tags.map((tag, index) => {
      return (
        <Tag
          tag={tag.name}
          key={tag.id + index}
          onRemove={this.removeTagByName.bind(this, tag.id)}
        />
      );
    });
    ...
    const placeholderText =
      this.props.isDisabled || this.state.tags.length
        ? ""
        : "Begin typing to search tags";
    let className =
      "tags-field" +
    ...
    return (
      ...
      >
        ...
        {searchResults && (
          <div className="tag-search-results">{searchResults}</div>
        )}
