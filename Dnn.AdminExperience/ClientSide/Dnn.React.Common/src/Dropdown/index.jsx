...
class Dropdown extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (props.isDropDownOpen !== prevProps.isDropDownOpen) {
      this.setState({ dropDownOpen: !props.isDropDownOpen }, () =>
  ...
  getDropdownLabel() {
    ...
    return props.prependWith ? (
      <span className="dropdown-prepend">
        <strong>{props.prependWith}</strong> {label}
      </span>
    ) : (
      label
    );
  ...
  startWith(option) {
    ...
    const label = props.getLabelText
      ? props.getLabelText(option.label)
      : option.label;
  ...
  scrollToSelectedItem(eventKey) {
    const optionRef = this.selectedOptionElement
  ...
  getCurrentIndex() {
    ...
    const currentIndex =
      this.state.currentIndex !== undefined ? this.state.currentIndex : -1;
  ...
  onArrowUp(eventKey) {
    ...
    this.setState({
      currentIndex,
      selectedOption: this.getOption(currentIndex),
      closestValue: null,
      ignorePreselection: true,
    });
    this.scrollToSelectedItem(eventKey);
          ...
          <li
            ...
            ref={
              this.isSelectedItem(index)
                ? this.addOptionRef.bind(this)
                : (f) => f
            }
            className={this.getOptionClassName(option, index)}
          >
            {option.label}
          </li>
        );
  ...
  getOptionClassName(option, index) {
    ...
    const isPreselected =
      !this.state.ignorePreselection &&
      props.value !== null &&
      option.value === props.value &&
      state.closestValue === null &&
      currentIndex < 0;
    const isSearchResult =
      state.closestValue !== null && option.value === state.closestValue;
    const selected =
      currentIndex === -1 ? isPreselected : isCurrentIndex || isSearchResult;
  ...
  render() {
    ...
    return (
      ...
      >
        <div
          className={"collapsible-label" + this.getIsMultiLineLabel()}
        ...
        >
          <Collapse isOpened={state.dropDownOpen}>
            <div>
              <Scrollbars
              ...
              >
                <ul
                  className="dnn-dropdown-options"
                  ref={(ul) => (this.dropDownListElement = ul)}
                >
                  {this.initOptions()}
                </ul>
              </Scrollbars>
