...
const Help = ({
  ...
  const out = (
    <section key={DomKey.get("help")} className="dnn-prompt-inline-help">
      {anchorName}
      {headingName}
      {paragraphDescription}
      {options && options.length > 0 && (
        <h4>{Localization.get("Help_Options")}</h4>
      )}
      {options && options.length > 0 && (
        <div>
          <DataTable rows={options} columns={fields} cssClass="table" />
        </div>
      )}
      {resultHtml && (
        <div>
          <Html html={stripWhiteSpaces(resultHtml)} />
        </div>
      )}
    </section>
  );
