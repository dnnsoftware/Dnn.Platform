...
class Input extends Component {
  ...
  getTabId() {
    const dnnVariable = JSON.parse(
      window.parent.document.getElementsByName("__dnnVariable")[0].value,
    );
  ...
  readInput() {
    ...
    if (
      txt === "" &&
      props.nextPageCommand !== null &&
      props.nextPageCommand !== ""
    ) {
    ...
    if (cmd === Cmd.HELP) {
      if (txt.toUpperCase() === "HELP") {
        actions.getCommandList(
          { cmdLine: "list-commands", currentPage: self.tabId },
          () => {},
  ...
  changeUserMode(tokens) {
    ...
    if (mode && !this.isFlag(mode) && Mode[mode]) {
      ...
      actions.runLocalCommand(
        "ERROR",
        formatString(Localization.get("Prompt_FlagIsRequired"), "Mode"),
      );
    }
  }
  ...
  getFlag(flag, tokens) {
    ...
    return tokens.find(
      (token) =>
        this.isFlag(token) && flag.toUpperCase() === token.toUpperCase(),
    );
  }

  hasFlag(flag, tokens) {
    return tokens
      ? tokens.find((token) => token.toUpperCase === flag.toUpperCase())
      : false;
  }
  ...
  isPaging() {
    const { props } = this;
    const IS_PAGING =
      props.paging !== null && props.paging.pageNo <= props.paging.totalPages;
    return IS_PAGING;
  }
  ...
  render() {
    ...
    return (
      <div
        className={`dnn-prompt-input-wrapper ${IS_PAGING && !IS_LAST_PAGE ? "hidden-cursor" : ""}`}
      >
        <input
          className={`dnn-prompt-input ${IS_PAGING && !IS_LAST_PAGE ? "hidden-text" : ""}`}
          ref={(el) => {
