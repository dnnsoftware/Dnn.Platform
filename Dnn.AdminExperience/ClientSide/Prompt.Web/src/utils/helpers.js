...
export function renderObjectFieldCell(data, fieldName, key) {
  ...
  const value =
    data[cleanFieldName] !== undefined && data[cleanFieldName] !== null
      ? data[cleanFieldName]
      : "";
  const renderedValue = value.isHtml ? (
    <Html html={value.output} />
  ) : value.isHtml === false ? (
    value.output
  ) : (
    value.toString()
...
export function renderObject(data, fieldOrder) {
  ...
  return (
    <table className="dnn-prompt-tbl">
      <tbody>{rows}</tbody>
    </table>
  );
}
