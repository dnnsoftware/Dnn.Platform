...
class AdminLogPanelBody extends Component {
  ...
  UNSAFE_componentWillMount() {
    const { props } = this;
    props.dispatch(
      LogActions.getPortalList(util.settings.isHost, (dataPortal) => {
  ...
  getLogTypes() {
    const { props } = this;
    props.dispatch(
      LogActions.getLogTypeList((dataLog) => {
  ...
  onDeleteLogItems() {
    ...
    if (props.selectedRowIds.length > 0) {
      util.utilities.confirm(
        Localization.get("LogDeleteWarning"),
        Localization.get("yes"),
        Localization.get("no"),
        () => {
          if (props.excludedRowIds.length == 0 && this.isLastPage()) {
            this.setState(
              ...
              () => {
                let getNextPageParam = this.getNextPage();
                props.dispatch(
                  LogActions.deleteLogItems(props.selectedRowIds, () => {
                    props.dispatch(LogActions.getLogList(getNextPageParam));
                  }),
                );
              },
  ...
  renderLogListHeader() {
    ...
    const isDeselectState =
      (props.excludedRowIds.length == 0 && props.excludedRowIds.length) ||
      (!props.excludedRowIds.length == 0 && props.selectedRowIds.length);
    const checkboxClassName =
      "checkbox" + (isDeselectState ? " deselect-state" : "");
  ...
  renderedLogLegend() {
    ...
    let legend = legendItems.map((item) => {
      return (
        <div key={item.id} className="logLegend-item">
        ...
        <div className="toolbar">
          ...
          {(canEdit || util.settings.isAdmin) && (
            <div
              ...
              style={{ width: "15%", paddingRight: 0, textAlign: "right" }}
            >
          ...
          {isHost && (
            <div
              ...
              style={{ borderLeft: "none", width: "15%" }}
            >
              <TextOverflowWrapper
