...
class LogSettingsPanel extends Component {
  constructor() {
    ...
    canEdit =
      util.settings.isHost || util.settings.permissions.LOG_SETTINGS_EDIT;
  }
  ...
  UNSAFE_componentWillMount() {
    ...
    if (canEdit) {
      if (
        this.props.logTypeList === null ||
        this.props.logTypeList === undefined ||
        this.props.logTypeList.length === 0
      ) {
        props.dispatch(LogActions.getLogTypeList());
      }
      if (
        this.props.portalList === null ||
        this.props.portalList === undefined ||
        this.props.portalList.length === 0
      )
  ...
  renderedLogSettingList(logTypeOptions, portalOptions) {
    let validLogSettingList = this.props.logSettingList.filter(
    ...
    return validLogSettingList.map((logSetting, index) => {
      ...
      return (
        <SettingRow
          ...
          activeStatus={
            logSetting.LoggingIsActive
              ? Localization.get("True")
              : Localization.get("False")
          }
...
function mapStateToProps(state) {
  return {
    ...
    notificationTimeTypesOptions:
      state.logSettings.notificationTimeTypesOptions,
