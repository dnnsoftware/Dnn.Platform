export function updateLogSettingList(logSettingList, logSettingDetail) {
  if (
    logSettingList.some((logSetting) => logSetting.ID === logSettingDetail.ID)
  ) {
...
export function createOccurrenceOptions(actionOccurrenceData) {
  ...
  occurrenceOptions.thresholdsOptions = actionOccurrenceData.thresholds.map(
    (item) => {
      return { label: item.Key, value: item.Value };
    },
  );
  occurrenceOptions.notificationTimesOptions =
    actionOccurrenceData.notificationTimes.map((item) => {
      return { label: item.Key, value: item.Value };
    });
  occurrenceOptions.notificationTimeTypesOptions =
    actionOccurrenceData.notificationTimeTypes.map((item) => {
      return { label: item.Key, value: item.Value };
    });
