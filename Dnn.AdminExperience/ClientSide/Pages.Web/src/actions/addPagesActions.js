...
function processResponse(response) {
    ...
    const mainMessage =
    Localization.get("BulkPageResponseTotalMessage")
    ...
    const errors = response.pages
        ...
        .map(
            (p) => "<strong>" + p.pageName + "</strong>: " + p.errorMessage + "<br/>",
        );
...
function processValidateResponse(response) {
    const totalCount = response.pages.length;
    const mainMessage =
    Localization.get("BulkPageValidateResponseTotalMessage").replace(
        "[PAGES_TOTAL]",
        totalCount,
    ) + "<br/><br/>";
    const errors = response.pages
...
const addPagesActions = {
    ...
    addPages(callback) {
        return (dispatch, getState) => {
            ...
            PagesService.addPages(addPages.bulkPage)
                .then((response) => {
    ...
    validatePages(callback) {
        return (dispatch, getState) => {
            ...
            PagesService.validatePages(addPages.bulkPage)
                .then((response) => {
                    if (response.Status === responseStatus.ERROR) {
                        utils.notifyError(response.Message, 3000);
                        return;
                    ...
                    if (
                        response.Response.pages.filter(
                            (p) => p.errorMessage !== null && p.errorMessage !== "",
                        ).length > 0
                    ) {
                        utils.notifyError(processValidateResponse(response.Response), {
                            clickToClose: true,
                        });
                    } else {
