...
export default function pageSeoReducer(
    state = {
        newFormOpened: false,
        addingNewUrl: false,
        editingUrl: false,
    ...
    const changeField = function changeField(key, value) {
        ...
        if (key === "statusCode") {
            ...
            newEditedUrl[key].Value = statusCodes.filter(
            ...
            if (key === "path" && (!value || !value.startsWith("/"))) {
                value = "/" + value;
            } else if (key === "queryString" && (!value || !value.startsWith("?"))) {
                value = "?" + value;
            }
    ...
    switch (action.type) {
        ...
        case ActionTypes.SEO_CLOSE_NEW_FORM:
            return { ...state, newFormOpened: false, editedUrl: null };
        ...
        case ActionTypes.SEO_ADD_URL:
            return { ...state, addingNewUrl: true };
        ...
        case ActionTypes.ERROR_SEO_ADDING_URL:
            return { ...state, addingNewUrl: false };
        case ActionTypes.SEO_OPEN_EDIT_FORM:
            return { ...state, editedUrl: action.payload.url, newFormOpened: false };
        case ActionTypes.SEO_CLOSE_EDIT_FORM:
            return { ...state, editedUrl: null };
        case ActionTypes.SEO_SAVE_URL:
            return { ...state, editingUrl: true };
        case ActionTypes.SEO_SAVED_URL:
            return { ...state, editingUrl: false, editedUrl: null };
        case ActionTypes.ERROR_SEO_SAVING_URL:
            return { ...state, editingUrl: false };
