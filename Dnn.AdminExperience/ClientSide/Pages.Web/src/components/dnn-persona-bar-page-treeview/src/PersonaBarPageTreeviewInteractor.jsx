...
class PersonaBarPageTreeviewInteractor extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    let pageList = null;
    const { activePage, NoPermissionSelectionPageId } = props;
    const pageListCopy = cloneDeep(props.pageList);
    if (activePage || NoPermissionSelectionPageId) {
      const tabId =
        (activePage && activePage.tabId) || NoPermissionSelectionPageId;
      props._traverse((item, list) => {
        item.selected = false;
        if (item.id === tabId) {
          item.selected = true;
          item.includeInMenu =
            activePage && activePage.includeInMenu ? true : false;
        }
        pageList = list;
      }, pageListCopy);
    } else {
  ...
  getListItemTitle(item) {
    const element = document.getElementById(
      `list-item-title-${item.name}-${item.id}`,
    );
    return element;
  }
  ...
  setMouseCoordinates(e) {
    const capture = () => {
      this.setState({
        setMouseCoordDebounce: true,
        pageX: e.pageX,
        pageY: e.pageY,
      });
      setTimeout(
        () => this.setState({ setMouseCoordDebounce: false }),
  ...
  onDrag(e) {
    const move = () => {
      ...
      setTimeout(
        () => this.setState({ dragDebounce: false }),
        this.state.debounceAmount,
      );
    };
  ...
  onDragOver(e, item) {
    ...
    const action = () => {
      ...
      setTimeout(
        () => this.setState({ dragOverDebounce: false }),
        this.state.debounceAmount,
      );
    };
  ...
  onDrop(e, item) {
    ...
    const left = () => {
      ...
      this.getPageInfo(treeViewActivePage.id)
        .then(() => {
          let treeViewActivePage = Object.assign(
            {},
            this.state.treeViewActivePage,
          );
    ...
    const right = () => null;
    item.id !== this.state.draggedItem.id &&
    item.id !== this.state.draggedItem.parentId
      ? left()
      : right();
  }
  ...
  onMovePage({
    ...
    onMovePage({
      ...
      .then((response) => {
        ...
        if (
          RelatedPageParentId === -1 &&
          ParentId !== -1 &&
          utils.getCurrentPageId() === response.Page.id
        ) {
  ...
  reOrderPage({ Action, PageId, ParentId, RelatedPageId }) {
    return new Promise((resolve) => {
      ...
      const removeFromPageList = () =>
        new Promise((rez) => {
          this.props._traverse((item, list, updateStore) => {
            // remove item from pagelist and cache
            runUpdateStore = updateStore;
            switch (true) {
          ...
          this.setState(
            ...
            () => {
              this.getPageInfo(cachedItem.id).then(() => {
                ...
                if (pageList) runUpdateStore(pageList);
                rez();
              });
            },
          );
  ...
  loadAllChildListItems(id) {
    return new Promise((resolve) => {
      const getChildListItems = () => {
        this.props.getChildPageList(id).then((childListItems) => {
      ...
      this.props._traverse((item) =>
        item.childCount > 0 &&
        item.id === id &&
        !Object.prototype.hasOwnProperty.call(item, "childListItems")
          ? getChildListItems()
          : resolve(),
      );
    });
  ...
  render_treeview() {
    return (
      <span
        className="dnn-persona-bar-treeview-ul tree"
        onMouseOver={(e) =>
          this.props.enabled
      ...
      >
        {this.state.rootLoaded ? (
          <PersonaBarPageTreeview
            ...
            onSelection={
              this.props.enabled ? this.onSelection.bind(this) : () => {}
            }
            onNoPermissionSelection={
              this.props.enabled
                ? this.onNoPermissionSelection.bind(this)
                : () => {}
            }
            onDragEnter={
              this.props.enabled ? this.onDragEnter.bind(this) : () => {}
            }
            onDrag={this.props.enabled ? this.onDrag.bind(this) : () => {}}
            onDragStart={
              this.props.enabled ? this.onDragStart.bind(this) : () => {}
            }
            onDragOver={
              this.props.enabled ? this.onDragOver.bind(this) : () => {}
            }
            onDragLeave={
              this.props.enabled ? this.onDragLeave.bind(this) : () => {}
            }
            onDragEnd={
              this.props.enabled ? this.onDragEnd.bind(this) : () => {}
            }
            onDrop={this.props.enabled ? this.onDrop.bind(this) : () => {}}
            onMovePage={
              this.props.enabled ? this.onMovePage.bind(this) : () => {}
            }
      ...
      <span className="dnn-persona-bar-treeview-ul">
        {this.state.rootLoaded ? (
          <PersonaBarPageTreeMenu
            CallCustomAction={
              this.props.enabled ? this.props.CallCustomAction : () => {}
            }
            onAddPage={this.props.enabled ? this.props.onAddPage : () => {}}
            onViewPage={this.props.enabled ? this.props.onViewPage : () => {}}
            onViewEditPage={
              this.props.enabled ? this.props.onViewEditPage : () => {}
            }
            onDuplicatePage={
              this.props.enabled ? this.onDuplicatePage.bind(this) : () => {}
            }
            listItems={this.state.pageList}
  ...
  render_collapseExpand() {
    return (
      <div
        onClick={
          this.props.enabled ? this.toggleExpandAll.bind(this) : () => {}
        }
        className="collapse-expand"
      >
        [
        {this.state.isTreeviewExpanded
          ? Localization.get("lblCollapseAll").toUpperCase()
          : Localization.get("lblExpandAll").toUpperCase()}
        ]
      </div>
    );
  }
  ...
  setMouseOver(isMouseOver) {
    let hasChildren =
      this.state.pageList &&
      this.state.pageList.some((page) => page.childCount > 0);
      ...
      >
        ...
        >
          ...
          <GridCell columnSize={55} style={{ marginLeft: "-2px" }}>
            <ScrollBar
              contentStyle={{ width: this.calculateTreeContentArea() + "px" }}
            >
...
export const connectedPersonaBarPageTreeviewInteractor = connect(
  mapStateToProps,
