...
export class PersonaBarTreeInContextMenu extends Component {
  ...
  getAdditionalMenuItems() {
    if (this.props.pageInContextComponents && securityService.isSuperUser()) {
      ...
      const additionalMenus = cloneDeep(
        this.props.pageInContextComponents || [],
      );
      additionalMenus &&
        additionalMenus.forEach((value, index, arr) => {
        ...
        <Menu>
          {visibleMenus.map((menu) => {
            ...
            return (
              <MenuItem
                key={menu.key}
                onMenuAction={menu.onClick.bind(
                  this,
                  menu.key,
                  item,
                  menu.OnClickAction,
  ...
  render() {
    ...
    return <span>{this.renderActionable(item)}</span>;
  }
}
