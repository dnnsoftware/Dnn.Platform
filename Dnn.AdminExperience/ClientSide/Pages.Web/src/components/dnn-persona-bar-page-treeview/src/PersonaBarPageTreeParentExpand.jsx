...
export class PersonaBarPageTreeParentExpand extends Component {
  ...
  render_parentExpandIcon(item) {
    return <PersonaBarExpandCollapseIcon isOpen={item.isOpen} item={item} />;
  }
  ...
  render_parentExpandButton(item) {
    ...
    return (
      <div
        className="parent-expand-button"
        onClick={() => {
          getChildListItems(item.id);
        }}
      >
        {item.childCount > 0 ? (
          this.render_parentExpandIcon(item)
        ) : (
          <div className="parent-expand-icon"></div>
        )}
      </div>
    );
  }
        ...
        return (
          <li key={"list-item-menu-key-" + index} className="list-item-menu">
            ...
            {item.childListItems && item.isOpen
              ? this.render_tree(item.childListItems)
              : null}
          </li>
        );
      });
