...
export default class PersonaBarSelectionArrow extends Component {
  ...
  toggleInContext(li) {
    ...
    let updateReduxStore,
      pageList = null;
    _traverse((item, list, updateStore) => {
      item.showInContextMenu =
        item.id === li.id
          ? !item.showInContextMenu
  ...
  renderMoreActions() {
    return (
      <div
        className={this.state.showMenu ? "dots active" : "dots"}
      ...
      >
        ...
        {this.state.showMenu && (
          <PersonaBarTreeInContextMenu
            {...this.props}
            onClose={this.onMouseLeave.bind(this)}
          />
        )}
      </div>
    );
  ...
  render() {
    ...
    return (
      <div
        id={`menu-item-${item.name} ${item.selected}`}
