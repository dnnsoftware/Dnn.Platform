...
class ModuleEdit extends Component {
  ...
  onIFrameLoad() {
    ...
    if (location.toLowerCase().indexOf("popup") === -1) {
      ...
      const pageRequestManager =
        iframe.contentWindow.Sys.WebForms.PageRequestManager.getInstance();
  ...
  beginRequestHandler(pageRequestManager, beginRequestEventArgs) {
    ...
    this.closeOnEndRequest =
      postBackElement.id ===
      "dnn_ctr" + this.props.module.id + "_ModuleSettings_cmdUpdate";
    if (!this.closeOnEndRequest) {
      this.closeOnEndRequest =
        postBackElement.id.indexOf("dnn_ctr" + this.props.module.id) > -1 &&
