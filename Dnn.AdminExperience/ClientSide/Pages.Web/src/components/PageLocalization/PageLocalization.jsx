...
class PageLocalization extends Component {
  ...
  getLanguages() {
    ...
    props.dispatch(
      LanguagesActions.getLanguages(tabId, (data) => {
        this.setState(data);
      }),
    );
  }
  ...
  makePageTranslatable() {
    ...
    const { tabId } = props.page;
    props.dispatch(
      LanguagesActions.makePageTranslatable(tabId, () => {
        this.getLanguages();
      }),
    );
  }
  ...
  makePageNeutral() {
    utils.confirm(
      Localization.get("MakeNeutralWarning"),
      ...
      () => {
        ...
        const { tabId } = props.page;
        props.dispatch(
          LanguagesActions.makePageNeutral(tabId, () => {
            this.getLanguages();
          }),
        );
      },
  ...
  onAddMissingLanguages() {
    ...
    const { tabId } = props.page;
    props.dispatch(
      LanguagesActions.addMissingLanguages(tabId, (data) => {
        if (data && data.Success && !data.AllLanguagesAdded) {
          utils.notify(
            Localization.get("OnlyAddedSomeMissingLanguagesWarning"),
            { clickToClose: true },
          );
  ...
  onDeleteModule(tabModuleId) {
    const { props } = this;
    props.dispatch(
      LanguagesActions.deleteModule({ tabModuleId }, () => {
        this.getLanguages();
      }),
    );
  }
  ...
  onRestoreModule(tabModuleId) {
    const { props } = this;
    props.dispatch(
      LanguagesActions.restoreModule({ tabModuleId }, () => {
        this.getLanguages();
      }),
    );
  }
  ...
  onUpdateLocalization() {
    utils.confirm(
      Localization.get("UpdateLocalizationWarning"),
      ...
      () => {
        ...
        props.dispatch(
          LanguagesActions.updateTabLocalization(
            params,
            () => {
              this.getLanguages();
  ...
  onSendNotifyMessage() {
    ...
    props.dispatch(
      LanguagesActions.notifyTranslators(params, () => {
    ...
    return (
      <div className="page-localization">
        <div className="page-localization-container">
          ...
          <div className="languages-container">
            <Scrollbars
              ...
              autoHeightMax={9999}
            >
              <div style={containerStyle}>{Languages}</div>
            </Scrollbars>
          </div>
        </div>
        <div className="button-container">
          ...
          >
            <TextOverflowWrapper
              text={Localization.get("MakePageNeutral")}
              maxWidth={110}
            />
          </Button>
          <Button
          ...
          >
            <TextOverflowWrapper
              text={Localization.get("AddMissingLanguages")}
              maxWidth={110}
            />
          </Button>
          <Button
          ...
          >
            <TextOverflowWrapper
              text={Localization.get("UpdateLocalization")}
              maxWidth={110}
            />
          </Button>
          <Button
          ...
          >
            <TextOverflowWrapper
              text={Localization.get("NotifyTranslators")}
              maxWidth={110}
            />
          </Button>
        </div>
