...
class PageDetailsFooter extends Component {
    ...
    const additionalLeftComponents = this.props.components.filter(
      function (component) {
        return (
          ...
          component.leftSide &&
          (component.pageType === pageType || component.pageType === "all")
        );
      },
    );
  ...
  getRightColumnComponents(normalPage, pageType) {
    ...
    const defaultRightColumnComponents = !normalPage
      ? [
          <DisplayInMenu
            key={"displayInMenu" + page.tabId}
            includeInMenu={page.includeInMenu}
            onChangeIncludeInMenu={this.onChangeValue.bind(
              this,
              "includeInMenu",
            )}
          />,
          <EnableScheduling
            key={"enableScheduling" + page.tabId}
            schedulingEnabled={page.schedulingEnabled}
            onChangeSchedulingEnabled={this.onChangeValue.bind(
              this,
              "schedulingEnabled",
            )}
  ...
  render() {
    ...
    const componentsInNewSection = this.getNewSections(
      normalPage,
      page.pageType,
    );
