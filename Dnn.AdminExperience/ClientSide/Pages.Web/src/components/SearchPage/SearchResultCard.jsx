...
class SearchResultCard extends Component {
    ...
    this.props.clearSearch(() => {
      if (item.canManagePage) {
        this.props.onLoadPage(item.id, () => {
          this.props.buildTree(item.id);
        });
      } else {
        this.noPermissionSelectionPageId = item.id;
        this.props.buildBreadCrumbPath(item.id);
        this.props.setEmptyStateMessage(
          Localization.get("NoPermissionEditPage"),
        );
      }
    });
  }
  ...
  distinct(list) {
    ...
    list.map((item) => {
      if (
        item.trim() !== "" &&
        distinctList.indexOf(item.trim().toLowerCase()) === -1
      )
  ...
  handlePageType() {
    if (this.props.filterByPageType !== this.props.item.pageType) {
      this.props.updateFilterByPageTypeOptions({
        value: this.props.item.pageType,
      });
      this.props.onSearch();
    }
  }
  ...
  handlePagePublishStatus() {
    if (this.props.filterByPublishStatus !== this.props.item.publishStatus) {
      this.props.updateFilterByPageStatusOptions({
        value: this.props.item.publishStatus,
      });
      this.props.onSearch();
    }
  }
  ...
  handlePublishDate() {
    ...
    if (
      this.props.startDate.toString() !==
        new Date(this.props.item.publishDate.split(" ")[0]).toString() ||
  ...
  handleWorkflow() {
    if (this.props.filterByWorkflow !== this.props.item.workflowId) {
      this.props.updateFilterByWorkflowOptions({
        value: this.props.item.workflowId,
        name: this.props.item.workflowName,
      });
      this.props.onSearch();
    }
  }
  ...
  renderCustomComponent() {
    ...
    if (
      !this.thumbRendered &&
      this.props.pageTypeSelectorComponents &&
      this.props.pageTypeSelectorComponents.length > 0
    ) {
      return this.props.pageTypeSelectorComponents.map(
        function (component, index) {
          const Component = component.component;
          return (
            <div
              className="search-item-thumbnail"
              key={"search-item-thumbnail-" + index}
            >
              <Component page={item} />
            </div>
          );
  ...
  render() {
    let visibleMenus = [];
    this.props.item.canViewPage &&
      visibleMenus.push(
        <li
          key={"visible-menu-item-can-view-page"}
          onClick={() => this.props.onViewPage(this.props.item)}
        >
          <div title={Localization.get("View")}>
            <SvgIcons.EyeIcon />
          </div>
        </li>,
      );
    this.props.item.canAddContentToPage &&
      visibleMenus.push(
        <li
          key={"visible-menu-item-can-add-content"}
          onClick={() => this.props.onViewEditPage(this.props.item)}
        >
          <div title={Localization.get("Edit")}>
            <SvgIcons.TreeEdit />
          </div>
        </li>,
      );
    if (
      ...
      additionalMenus &&
        additionalMenus.map((additionalMenu, index) => {
          visibleMenus.push(
            <li
              key={"visible-menu-item-additinal-" + index}
              onClick={() =>
                additionalMenu.OnClickAction &&
                typeof additionalMenu.OnClickAction === "function" &&
                this.props.CallCustomAction(additionalMenu.OnClickAction)
              }
            >
              <div title={additionalMenu.title}>
                <additionalMenu.icon />
              </div>
            </li>,
          );
    ...
    return (
      <GridCell columnSize={100}>
        <div className="search-item-card">
          ...
          <div
            className={`search-item-details${utils.isPlatform() ? " full" : ""}`}
          >
            <div className="search-item-details-left">
              <h1 onClick={() => this.onNameClick(this.props.item)}>
                <TextOverflowWrapper
                  text={this.props.item ? this.props.item.name : ""}
                />
              </h1>
            ...
            <div className="search-item-details-list">
              <ul>
                <li>
                  ...
                  <p
                    title={this.props.getPageTypeLabel(
                      this.props.item.pageType,
                    )}
                    onClick={this.handlePageType.bind(this)}
                  >
                    {this.props.getPageTypeLabel(this.props.item.pageType)}
                  </p>
                </li>
                <li>
                  <p>{Localization.get("lblPublishStatus")}:</p>
                  <p
                    title={this.props.getPublishStatusLabel(
                      this.props.item.publishStatus,
                    )}
                    onClick={this.handlePagePublishStatus.bind(this)}
                  >
                    {this.props.getPublishStatusLabel(
                      this.props.item.publishStatus,
                    )}
                  </p>
                </li>
                <li>
                  <p>
                    {Localization.get(
                      utils.isPlatform() ? "lblModifiedDate" : "lblPublishDate",
                    )}
                    :
                  </p>
                  <p
                    title={this.props.item.publishDate}
                    onClick={this.handlePublishDate.bind(this)}
                  >
                    {this.props.item.publishDate.split(" ")[0]}
                  </p>
                </li>
              </ul>
            </div>
              ...
              <ul>
                {!utils.isPlatform() && (
                  <li>
                    ...
                    <p
                      title={this.props.item.workflowName}
                      onClick={this.handleWorkflow.bind(this)}
                    >
                      {this.props.item.workflowName}
                    </p>
                  </li>
                )}
                      ...
                      return (
                        <span key={"tag-" + count}>
                          <span
                            ...
                            onClick={() => this.addToTags(tag)}
                          >
                            {tag}
                          </span>
                          {count < this.props.item.tags.length - 1 && (
