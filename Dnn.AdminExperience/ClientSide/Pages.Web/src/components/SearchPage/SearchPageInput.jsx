    ...
    this.setState({ searchTerm: e.target.value, filtersUpdated: true }, () => {
      ...
      switch (true) {
        case searchTerm.length > 3:
          ...
          return;
        case currentSearchTerm.length > 0 &&
          searchTerm.length === 0 &&
          inSearch:
          this.props.onSearch();
          return;
  ...
  render() {
    return (
      <div className="search-container">
        {this.props.inSearch ? (
          <div
            className="dnn-back-to-link"
            onClick={() => this.props.clearSearch()}
          >
            <div className="dnn-back-to-arrow">
              <SvgIcons.ArrowBack />
            </div>{" "}
            <span>{Localization.get("BackToPages").toUpperCase()}</span>
          </div>
        ) : null}
        ...
        <div className="search-box">
          ...
          {this.state.searchTerm ? (
            <div
              ...
              style={{ fill: "#444" }}
              onClick={() =>
                this.setState({ searchTerm: "", filtersUpdated: true }, () =>
                  this.props.onSearch(""),
                )
              }
            >
