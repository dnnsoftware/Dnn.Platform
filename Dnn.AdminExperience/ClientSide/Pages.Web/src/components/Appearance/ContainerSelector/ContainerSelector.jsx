...
class ContainerSelector extends Component {
  onContainerClick(containerName) {
    const container = this.props.containers.find(
  ...
  getContainerCards() {
    ...
    defaultPortalContainer =
      defaultPortalContainer === null ? "" : defaultPortalContainer;

    if (noThemeSelected) {
      return (
        <div className="no-appearance-items">
          {localization.get("NoThemeSelectedForContainers")}
        </div>
      );
    }
    ...
    return containers.map((container) => {
      return (
        <Card
          ...
          isSiteDefault={
            defaultPortalContainer.toString().toLowerCase() ===
            container.path.toString().toLowerCase()
          }
