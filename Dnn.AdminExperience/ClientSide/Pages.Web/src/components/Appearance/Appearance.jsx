...
class Appearance extends Component {
  ...
  componentDidUpdate(prevProps) {
    const { page, containers, defaultPortalThemeName, onRetrieveThemeFiles } =
      prevProps;

    if (this.props.containers && this.props.containers !== containers) {
      this.autoSelectFirstContainerIfNoOneIsSelected(
        this.props.page,
    ...
    if (
      this.props.defaultPortalThemeName &&
      this.props.defaultPortalThemeName !== defaultPortalThemeName &&
      this.props.defaultPortalThemeName &&
      this.props.defaultPortalThemeName !== null
    ) {
      onRetrieveThemeFiles(
        this.props.defaultPortalThemeName,
        this.props.defaultPortalThemeLevel,
      );
    }
  }
  ...
  autoSelectFirstContainerIfNoOneIsSelected(page, containers) {
    ...
    const selectedContainer = containers.find(
      this.findByPath(selectedContainerPath),
    );
  ...
  getPagePreviewUrl(skinSrc, containerSrc) {
    ...
    const pageUrl =
      page.tabId !== 0 ? page.absoluteUrl : window.parent.location.href;
  ...
  findByPath(componentSrc) {
    return (c) =>
      utils.areEqualInvariantCase(this.addAscxExtension(c.path), componentSrc);
  }
  ...
  render() {
    ...
    const selectedTheme = themes.find(
      (t) =>
        t.packageName === selectedThemeName && t.level === selectedThemeLevel,
    );
      ...
      <div className={style.moduleContainer}>
        ...
        <GridCell>
          <ContainerSelector
            ...
            defaultPortalContainer={this.trimAscxExtension(
              defaultPortalContainer,
            )}
        ...
        <GridCell>
          ...
          <GridCell columnSize={50}>
            <Button
              type="secondary"
              onClick={this.previewPage.bind(this)}
