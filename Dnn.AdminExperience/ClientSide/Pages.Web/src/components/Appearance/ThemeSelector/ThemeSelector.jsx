...
class ThemeSelector extends Component {
  ...
  isSelected(theme) {
    ...
    return (
      selectedTheme.packageName === theme.packageName &&
      selectedTheme.level === theme.level
    );
  }
  ...
  getThemeCards() {
    ...
    defaultPortalThemeName =
      defaultPortalThemeName === null ? "" : defaultPortalThemeName;
    defaultPortalThemeLevel =
      defaultPortalThemeLevel === null ? "" : defaultPortalThemeLevel;

    if (themes.length === 0) {
      return (
        <div className="no-appearance-items">
    ...
    return themes.map((theme) => {
      return (
        <Card
          ...
          isSiteDefault={
            defaultPortalThemeName.toString().toLowerCase() ===
              theme.packageName.toString().toLowerCase() &&
            defaultPortalThemeLevel === theme.level
          }
  ...
  render() {
    ...
    return (
      <div>
        ...
        />
        <Gallery scrollToIndex={selectedIndex}>{this.getThemeCards()}</Gallery>
      </div>
    );
  }
}
