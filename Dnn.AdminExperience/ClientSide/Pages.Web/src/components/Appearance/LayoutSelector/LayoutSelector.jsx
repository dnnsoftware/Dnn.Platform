...
class LayoutSelector extends Component {
  ...
  getLayoutCards() {
    ...
    defaultPortalLayout =
      defaultPortalLayout === null ? "" : defaultPortalLayout;

    if (noThemeSelected) {
      return (
        <div className="no-appearance-items">
          {localization.get("NoThemeSelectedForLayouts")}
        </div>
      );
    }
    if (layouts.length === 0) {
    ...
    return layouts.map((layout) => {
      return (
        <Card
          ...
          label={layout.name}
          isSiteDefault={
            defaultPortalLayout.toString().toLowerCase() ===
            layout.path.toString().toLowerCase()
          }
