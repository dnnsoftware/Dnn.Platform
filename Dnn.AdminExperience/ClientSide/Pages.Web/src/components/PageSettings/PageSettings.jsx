...
class PageSettings extends Component {
  ...
  getButtons() {
    const { selectedPage, onCancel, onSave, onDelete, selectedPageDirty } =
      this.props;
    ...
    let buttons = [
      <Button key="cancelButton" type="secondary" onClick={onCancel}>
  ...
  render() {
    ...
    const isLocalizationTabVisible =
      AllowContentLocalization && selectedPage.tabId !== 0;

    if (isEditingExistingPage && selectedPage.hasChild) {
      appearanceButtons.unshift(
        this.getCopyAppearanceToDescendantPagesButton(),
      );
      permissionsButtons.unshift(
        this.getCopyPermissionsToDescendantPagesButton(),
      );
    }
    ...
    const advancedTabs = [
      {
        ...
        component: (
          <div className="dnn-simple-tab-item" key="appearance">
            <Appearance page={selectedPage} onChangeField={onChangeField} />
            {appearanceFooter}
          </div>
        ),
      },
      {
        ...
        component: (
          <div className="dnn-simple-tab-item" key="seo">
            <Seo page={selectedPage} onChangeField={onChangeField} />
            {footer}
          </div>
        ),
      },
      {
        ...
        component: (
          <div className="dnn-simple-tab-item" key="more">
            <More
              page={selectedPage}
    ...
    if (
      (isEditingExistingPage || selectedPage.templateTabId) &&
      ...
      advancedTabs.unshift({
        ...
        component: (
          <div
            className="dnn-simple-tab-item dnn-simple-tab-item-modules"
            key="modules"
          >
            <Modules
    ...
    if (
      ...
      tabs.push(
        <div className="dnn-simple-tab-item permission-tab" key="permissions">
          <PermissionGrid
      ...
      if (isLocalizationTabVisible) {
        tabs.push(
          <div
            className="dnn-simple-tab-item dnn-simple-tab-item-localization"
            key="localization"
          >
            <PageLocalization page={selectedPage} />
          </div>,
        );
      }
    }
