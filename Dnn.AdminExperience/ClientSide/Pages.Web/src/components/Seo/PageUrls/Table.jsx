...
class Table extends Component {
  ...
  getUrlRows(pageUrls) {
    ...
    const deduplicatedUrls = uniqBy(pageUrls, (url) =>
      this.generateUrlKey(url),
    );
    ...
    return (
      <div>
        <div className={styles.addItemRow}>
          ...
          <div
            className={"AddItemBox" + (newFormOpened ? " active" : "")}
            onClick={onOpenNewForm}
          >
            <div className={"add-icon" + (newFormOpened ? " active" : "")}>
              <SvgIcons.AddIcon />
            </div>{" "}
        ...
        <div className="url-table">
          <div className="header-row">
            <GridCell columnSize={50}>{Localization.get("Url")}</GridCell>
            <GridCell columnSize={20}>{Localization.get("UrlType")}</GridCell>
          ...
          {newFormOpened && (
            <div
              className={styles.urlRow + (newFormOpened ? " row-opened" : "")}
            >
              <GridCell columnSize={50}>{"-"}</GridCell>
              <GridCell columnSize={20}>{"-"}</GridCell>
              <GridCell columnSize={20}>{"-"}</GridCell>
              <EditUrl
                url={editedUrl}
                saving={addingNewUrl}
