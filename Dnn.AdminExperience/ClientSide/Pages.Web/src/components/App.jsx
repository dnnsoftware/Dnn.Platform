...
class App extends Component {
  ...
  componentDidMount() {
    ...
    if (
      securityService.userHasPermission(
        permissionTypes.MANAGE_PAGE,
        selectedPage,
      )
    ) {
    ...
    this.props.getPageList().then(() => {
      const selectedPageId =
        utils.getSelectedPageId() || utils.getCurrentPageId();
      selectedPageId &&
        !utils.getIsAdminHostSystemPage() &&
      ...
      document.addEventListener(
        "viewPageSettings",
        this.resolveTabBeingViewed.bind(this),
        false,
      );
    });
  }
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      const errorMessage =
        ...
        Localization.get("AnErrorOccurred");
    ...
    if (
      selectedPage &&
      selectedPage.tabId > 0 &&
      selectedPage.canManagePage !== undefined &&
      !selectedPage.canManagePage
    ) {
        ...
        const execute = () => {
          if (parentId !== selectedId) {
            ...
            if (!page || !page.childListItems) {
              this.props.getChildPageList(parentId).then((data) => {
                this._traverse((item, list, update) => {
                  const left = () => {
                  ...
                  data.length > 0 && item.id === data[0].parentId
                    ? left()
                    : right();
                });
              });
  ...
  onCreatePage() {
    this.props.onCreatePage((page) => {
      if (
        (page && page.canAddContentToPage) ||
        (page && page.pageType !== "normal") ||
        utils.getIsSuperUser()
      ) {
        page.selected = true;
              ...
              switch (true) {
                case item.childCount > 0 && !item.childListItems:
                  this.props.getChildPageList(item.id).then((data) => {
                    ...
                    item.childListItems = data;
                    updateStore(list);
                    if (page.canAddContentToPage) this.onLoadPage(page.id);
                    else {
    ...
    if (parentId !== null && parentId !== -1) {
      this._traverse((item, list, updateStore) => {
        if (item.id === parentId) {
          ...
          item.hasChildren = true;
          item.childCount++;
          const newPageChildItems = item.childListItems.concat(
  ...
  onEditMode() {
    ...
    return (
      (selectedPage && selectedPage.tabId === 0) ||
      selectedPageDirty ||
  ...
  onAddPage(parentPage) {
      ...
      const onConfirm = () => {
        ...
        this.props.getNewPage(parentPage).then(() => {
          if (parentPage && parentPage.id) {
            this.props
              .getChildPageList(parentPage.id)
              .then((pageChildItems) => {
                this._traverse((item, list, update) => {
                  const updateChildItems = () => {
                    const newPageChildItems = pageChildItems.concat(
                      this.props.selectedPage,
                    );
                    item.childListItems = newPageChildItems;
                    item.isOpen = true;
    ...
    const noPermission = () =>
      this.setEmptyStateMessage(
        "You do not have permission to add a child page to this parent",
      );
    parentPage.canAddPage === undefined || parentPage.canAddPage
      ? addPage()
      : noPermission();
  }
  ...
  onCancelSettings() {
    ...
    if (props.selectedPageDirty) {
      ...
      if (
        props.selectedPage.tabId === 0 &&
        props.selectedPage.isCopy &&
        props.selectedPage.templateTabId
      ) {
        this.onCancelPage(props.selectedPage.parentId);
      } else if (
        props.selectedPage.tabId === 0 &&
        props.selectedPage.referralTabId
      ) {
        ...
        this._traverse((item, list, updateStore) => {
          if (item.id === props.selectedPage.parentId) {
            ...
            props.onDeletePage(
              props.selectedPage,
              false,
              utils.getCurrentPageId() === props.selectedPage.tabId
                ? item.url
                : null,
            );
            updateStore(list);
  ...
  showCancelWithoutSavingDialogInEditMode(input) {
    const id =
      typeof input === "object" ? this.props.selectedPage.tabId : input;
    if (this.props.selectedPageDirty) {
      const onConfirm = () => {
        this.onLoadPage(id, () => {
          this._traverse((item, list) => {
            if (item.id === id) {
              Object.keys(this.props.selectedPage).forEach(
                (key) => (item[key] = this.props.selectedPage[key]),
              );
  ...
  getPageTitle() {
    const { selectedPage } = this.props;
    return this.isNewPage() ? Localization.get("AddPage") : selectedPage.name;
  }
  ...
  getSettingsButtons() {
    ...
    const SaveAsTemplateButton =
      settingsButtonComponents.SaveAsTemplateButton || Button;

    return (
      <div className="heading-buttons">
        <Sec
          permission={permissionTypes.EXPORT_PAGE}
  ...
  getSaveAsTemplatePage() {
    return (
      <SaveAsTemplate onCancel={this.onCancelSavePageAsTemplate.bind(this)} />
    );
  }

  _traverse(comparator, pageListCopy) {
    let listItems =
      pageListCopy || JSON.parse(JSON.stringify(this.props.pageList));
    const cachedChildListItems = [];
    cachedChildListItems.push(listItems);
    const condition = cachedChildListItems.length > 0;

    const loop = () => {
      const childItem = cachedChildListItems.length
        ? cachedChildListItems.shift()
        : null;
      const left = () =>
        childItem.forEach((item) => {
          comparator(item, listItems, (pageList) =>
            this.props.updatePageListStore(pageList),
          );
          Array.isArray(item.childListItems)
            ? cachedChildListItems.push(item.childListItems)
            : null;
          condition ? loop() : exit();
        });
      const right = () => null;
  ...
  onSelection(pageId) {
    ...
    const right = () =>
      !selectedPage || pageId !== selectedPage.tabId
        ? this.showCancelWithoutSavingDialogInEditMode(pageId)
        : null;
  ...
  updatePageNameOnList(key, value) {
    this._traverse((item, list, updateStore) => {
      if (
        (item.tabId === 0 || item.id === this.props.selectedPage.tabId) &&
        key === "name"
      ) {
        item.name = value;
        item.selected = true;
  ...
  onViewEditPage(item) {
    ...
    const proceed = () =>
      selectedPageDirty ? showConfirmationDialog() : viewPage();
  ...
  onDayClick(newDay, isEndDate) {
    ...
    const right = () => {
      ...
      condition
        ? this.setState({ startDate: newDay, filtersUpdated: true })
        : this.setState({
            startDate: newDay,
            endDate: newDay,
            filtersUpdated: true,
    ...
    const left = () => {
      ...
      condition
        ? this.setState({ endDate: newDay, filtersUpdated: true })
        : null;
    };
  ...
  generateFilters() {
    const {
      ...
      filterByWorkflow,
      filterByWorkflowName,
      startDate,
      endDate,
      startAndEndDateDirty,
    } = this.state;
    ...
    filterByPageType
      ? filters.push({
          ref: "filterByPageType",
          tag: `${Localization.get("PageType")}: ${this.getPageTypeLabel(filterByPageType)}`,
        })
      : null;
    filterByPublishStatus
      ? filters.push({
          ref: "filterByPublishStatus",
          tag: `${Localization.get("lblPublishStatus")}: ${this.getPublishStatusLabel(filterByPublishStatus)}`,
        })
      : null;
    filterByWorkflow
      ? filters.push({
          ref: "filterByWorkflow",
          tag: `${Localization.get("WorkflowTitle")}: ${filterByWorkflowName}`,
        })
      : null;

    if (startAndEndDateDirty) {
      let dateRangeText = Localization.get(
        utils.isPlatform() ? "ModifiedDateRange" : "PublishedDateRange",
      );
      const fullStartDate = utils.formatDate(startDate);
      const fullEndDate = utils.formatDate(endDate);
      const dateInterval = () =>
        filters.push({
          ref: "startAndEndDateDirty",
          tag: `${dateRangeText}: ${fullStartDate} - ${fullEndDate} `,
        });
      const justOneDate = () =>
        filters.push({
          ref: "startAndEndDateDirty",
          tag: `${dateRangeText}: ${fullStartDate}`,
        });
  ...
  saveSearchFilters(searchFields) {
    return new Promise((resolve) =>
      this.setState({ searchFields }, () => resolve()),
    );
  }
  ...
  doSearch() {
    ...
    const searchDateRange = startAndEndDateDirty
      ? { publishDateStart: fullStartDate, publishDateEnd: fullEndDate }
      : {};
    ...
    this.setState(
      ...
      () => {
        if (typeof callback === "function") {
          ...
          const { selectedPage } = this.props;
          !selectedPage &&
            this.lastActivePageId &&
            this.onLoadPage(this.lastActivePageId);
        }
      },
    );
  }
  ...
  buildBreadCrumbPath(pageId) {
    ...
    const buildBreadCrumbPathInternal = () => {
      const addNode = (tabId) => {
        ...
        selectedPath.push({
          name: page.name,
          tabId: page.id !== pageId ? page.id : null,
        });
        const left = () => {
  ...
  render_PagesDetailEditor() {
    ...
    const render_pageDetails = () => {
      ...
      return (
        <PageSettings
          key={"pageDetails" + this.props.selectedPage.tabId}
          ...
          onCopyAppearanceToDescendantPages={
            props.onCopyAppearanceToDescendantPages
          }
          onCopyPermissionsToDescendantPages={
            props.onCopyPermissionsToDescendantPages
          }
    ...
    return (
      <GridCell columnSize={100} className="treeview-page-details">
        {selectedPage && selectedPage.tabId
          ? render_pageDetails()
          : render_emptyState()}
      </GridCell>
    );
  ...
  render_pageList() {
    return <PageList onPageSettings={this.onPageSettings.bind(this)} />;
  }
  ...
  getFilterByPageStatusOptions() {
    ...
    if (!utils.isPlatform()) {
      filterByPageStatusOptions = [
        { value: "", label: Localization.get("lblNone") },
      ].concat(
        filterByPageStatusOptions.concat([
          { value: "draft", label: Localization.get("lblDraft") },
        ]),
      );
    }
  ...
  getFilterByWorkflowOptions() {
    ...
    this.props.workflowList.length
      ? (workflowList = this.props.workflowList.map((item) => {
          return { value: item.workflowId, label: item.workflowName };
        }))
      : null;
    return [{ value: "", label: Localization.get("lblNone") }].concat(
      workflowList,
    );
  }
  getPageTypeLabel(pageType) {
    ...
    return (
      filterByPageTypeOptions.find((x) => x.value === pageType.toLowerCase()) &&
      filterByPageTypeOptions.find((x) => x.value === pageType.toLowerCase())
        .label
    );
  }
  getPublishStatusLabel(publishStatus) {
    ...
    return (
      (filterByPublishStatusOptions.find(
        (x) => x.value === publishStatus.toLowerCase(),
      ) &&
        filterByPublishStatusOptions.find(
          (x) => x.value === publishStatus.toLowerCase(),
        ).label) ||
  ...
  updateFilterByPageStatusOptions(data) {
    this.setState({
      filterByPublishStatus: data.value,
      filtersUpdated: true,
      pageIndex: 0,
    });
  }
  updateFilterByWorkflowOptions(data) {
    this.setState({
      filterByWorkflow: data.value,
      filterByWorkflowName: data.label,
      filtersUpdated: true,
      pageIndex: 0,
    });
  }
  ...
  render_searchResults() {
    return (
      <SearchResult
        ...
        getFilterByPageStatusOptions={this.getFilterByPageStatusOptions.bind(
          this,
        )}
        getFilterByWorkflowOptions={this.getFilterByWorkflowOptions.bind(this)}
        filterByWorkflow={this.state.filterByWorkflow}
        onApplyChangesDropdownDayPicker={this.onApplyChangesDropdownDayPicker.bind(
          this,
        )}
        updateFilterByPageTypeOptions={this.updateFilterByPageTypeOptions.bind(
          this,
        )}
        updateFilterByPageStatusOptions={this.updateFilterByPageStatusOptions.bind(
          this,
        )}
        updateFilterByWorkflowOptions={this.updateFilterByWorkflowOptions.bind(
          this,
        )}
        ...
        clearAdvancedSearchDateInterval={this.clearAdvancedSearchDateInterval.bind(
          this,
        )}
  ...
  getAdditionalPageSettings() {
    ...
    if (props.customPageSettingsComponents) {
      for (let i = 0; i < props.customPageSettingsComponents.length; i++) {
        ...
        if (
          props.selectedCustomPageSettings.pageSettingsId ===
          customPageSettings.panelId
        ) {
          const Component = customPageSettings.component;
          additionalPageSettings.push(
            <Component
              onCancel={this.onCancelSaveCustomDetail(
  ...
  render() {
    ...
    return (
      <div
        ...
        className="pages-app personaBar-mainContainer"
      >
        {isListPagesAllowed && (
          <PersonaBarPage fullWidth={true} isOpen={true}>
            <PersonaBarPageHeader
              title={Localization.get(inSearch ? "PagesSearchHeader" : "Pages")}
            >
              {securityService.canAddPages() && (
                <div>
                  <Button
                    ...
                    disabled={this.onEditMode() || this.state.inSearch}
                    size="large"
                    onClick={this.onAddPage.bind(this)}
                  >
                  ...
                  <Button
                    ...
                    disabled={this.onEditMode() || this.state.inSearch}
                    size="large"
                    onClick={this.onAddMultiplePage.bind(this)}
                  >
                    {Localization.get("AddMultiplePages")}
                  </Button>
                </div>
              )}
              ...
              {!inSearch && (
                <BreadCrumbs
                  items={this.props.selectedPagePath || []}
                  onSelectedItem={this.onSelection.bind(this)}
                />
              )}
            </PersonaBarPageHeader>
            <GridCell
              columnSize={100}
              style={{ padding: "30px 30px 16px 30px" }}
            >
              <SearchPageInput
            ...
            >
              ...
              >
                <div
                  className={
                    this.onEditMode() || this.state.inSearch
                ...
                >
                  <PersonaBarPageTreeviewInteractor
                    ...
                    showCancelDialog={this.showCancelWithoutSavingDialogInEditMode.bind(
                      this,
                    )}
                    ...
                    onNoPermissionSelection={this.onNoPermissionSelection.bind(
                      this,
                    )}
                    pageInContextComponents={props.pageInContextComponents}
                    NoPermissionSelectionPageId={
                      this.noPermissionSelectionPageId
                    }
                    CallCustomAction={this.CallCustomAction.bind(this)}
                    enabled={
                      !((selectedPage && selectedPage.tabId === 0) || inSearch)
                    }
                  />
                </div>
  ...
  return bindActionCreators(
    {
      ...
      onChangeAddMultiplePagesField:
        AddPagesActions.changeAddMultiplePagesField,
      ...
      onCopyAppearanceToDescendantPages:
        PageActions.copyAppearanceToDescendantPages,
      onCopyPermissionsToDescendantPages:
        PageActions.copyPermissionsToDescendantPages,
      ...
      getContentLocalizationEnabled:
        LanguagesActions.getContentLocalizationEnabled,
