import { portal as ActionTypes } from "../actionTypes";
import { CommonPortalService as PortalService } from "../services";
...
const portalActions = {
  getPortalLocales(portalId, callback) {
    return () => {
      PortalService.getPortalLocales(
        portalId,
        (data) => {
          if (callback) {
            callback(data);
