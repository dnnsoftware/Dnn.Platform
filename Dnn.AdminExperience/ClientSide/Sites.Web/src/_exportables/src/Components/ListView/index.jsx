...
class ListView extends Component {
  ...
  onDelete(portal, index) {
    const { props } = this;
    utilities.confirm(
      Localization.get("deletePortal").replace("{0}", portal.PortalName),
      Localization.get("ConfirmPortalDelete"),
      Localization.get("CancelPortalDelete"),
      () => {
        props.dispatch(
          CommonPortalListActions.deletePortal(portal.PortalID, index),
        );
      },
    );
  }
  ...
  getPortalMapping(portal) {
    ...
    return [
      ...
      },
      {
        ...
        value: dayjs(portal.LastModifiedOnDate)
          .locale(this.props.culture.substring(0, 2))
          .fromNow(),
      },
      {
      ...
      (this.props.getPortalMapping && this.props.getPortalMapping(portal)) ||
        [],
    );
  }
  getDetailList() {
