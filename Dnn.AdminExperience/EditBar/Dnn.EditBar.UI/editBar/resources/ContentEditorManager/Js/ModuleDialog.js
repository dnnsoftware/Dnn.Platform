...
if (
  typeof dnn.ContentEditorManager === "undefined" ||
  dnn.ContentEditorManager === null
) {
...
(function ($) {
  ...
  dnnModuleDialog.prototype = {
    ...
    addModule: function (moduleId, callback, callbackBeforeRefresh) {
      ...
      this._getService().request("AddModule", "POST", params, function (data) {
        if (callbackBeforeRefresh && typeof callback === "function") {
          callback.call(handler.getModuleManager().getPane(), [
            data.TabModuleID,
          ]);
          callback = null;
        }
    ...
    _refreshComplete: function (sender, args) {
      ...
      setTimeout(function () {
        if (
          typeof window.dnnLoadScriptsInAjaxMode === "undefined" ||
          window.dnnLoadScriptsInAjaxMode.length == 0
        ) {
    ...
    _generateLayout: function () {
      var layout = (this._dialogLayout = $(
        "" +
          ...
          '<span class="title">' +
          dnn.ContentEditorManagerResources.title +
          "</span>" +
          "</div>" +
          '<div class="dnnDialogBody dnnModuleList">' +
          "<h2>" +
          dnn.ContentEditorManagerResources.categoryRecommended +
          "</h2>" +
          '<div class="listContainer listRecommended"><ul></ul></div>' +
          "<h2>" +
          dnn.ContentEditorManagerResources.categoryAll +
          "</h2>" +
    ...
    _addSearchBox: function () {
      var searchBox = $(
        '<div class="search-container"><div class="search-input-container">' +
          '<input type="text" id="AddModule_SearchModulesInput" placeholder="' +
          dnn.ContentEditorManagerResources.searchPlaceHolder +
          '" class="search-input" aria-label="Search"/></div>' +
    ...
    _loadModuleList: function (val, isSearch) {
      if ((this._loadMore || isSearch) && !this._getService().isLoading()) {
        if (isSearch && this._startIndex == 0) {
          ...
          $(
            ".listContainer.listAll div.dnnModuleDialog_ModuleListMessage",
          ).remove();
        }
    ...
    _loadRecommendedList: function (callback) {
      var handler = this;
      this._getEditorService().request(
    ...
    _renderModuleList: function (data, list) {
      ...
      var container = this._dialogLayout.find(
        ".dnnModuleList .listContainer." + list + " ul",
      );
      ...
      if (listMode === "search" && searchWord) {
        for (var name in moduleAlias) {
          if (
            moduleAlias.hasOwnProperty(name) &&
            moduleAlias[name].toLowerCase().indexOf(searchWord) > -1
          ) {
      ...
      if (data.length > 0) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          if (
            list === "listAll" &&
            listMode === "category" &&
            this._isRecommendModule(itemData)
          ) {
            continue;
          }
        ...
        if (list === "listAll") {
          ...
          if (htmlModuleId > -1) {
            var $htmlItem = this._dialogLayout.find(
              '.dnnModuleItem[data-moduleid="' + htmlModuleId + '"]',
            );
    ...
    _switchToSimpleMode: function () {
      this._dialogLayout
        .find(".dnnModuleList .listContainer.listRecommended")
        .hide();
    ...
    _getItemTemplate: function () {
      return (
        '<li class="dnnModuleItem" data-moduleid="[$ModuleID$]">' +
    ...
    _getNoResultTemplate: function () {
      return (
        '<div class="dnnModuleDialog_ModuleListMessage"><span>' +
        dnn.ContentEditorManagerResources.nomodules +
        "</span></div>"
      );
    },
    ...
    _renderItem: function (item) {
      ...
      for (var a in renderItem) {
        if (renderItem.hasOwnProperty(a)) {
          var shortMatchRegex = new RegExp(
            "\\[\\$" + a + "\\|(\\d+)\\$\\]",
            "g",
          );
          ...
          template = this._replaceAll(
            template,
            "[$" + a + "|css]",
            this._replaceAll(renderItem[a].toString().toLowerCase(), " ", "-"),
          );
      ...
      this._getEditorService().request(
        ...
        function (returnData) {
          if (returnData.Script) {
            ...
            if (
              model &&
              model.addModuleHandler &&
              typeof model.addModuleHandler === "function"
            ) {
              model.addModuleHandler.call(
                window,
                data.ModuleID,
                data.ModuleName,
              );
          ...
          if (atBottom) {
            if ($("#AddModule_SearchModulesInput").val()) {
              $this._loadModuleList(
                $("#AddModule_SearchModulesInput").val(),
                true,
              );
            } else {
              $this._loadModuleList("", false);
    ...
    _resetModuleSearch: function () {
      ...
      $(
        '.listContainer.listAll ul li.dnnModuleItem:not([class~="dnnLayoutItem"])',
      ).remove();
      $(
        ".listContainer.listAll div.dnnModuleDialog_ModuleListMessage",
      ).remove();
    ...
    _searchKeyUp: function () {
      var val = $("#AddModule_SearchModulesInput").val();
      if (
        this._lastVal.length != val.length &&
    ...
    _attachEvents: function () {
      this._dialogLayout.on(
        "click",
        ".dnnModuleItem",
        $.proxy(this._doAddModule, this),
      );
      $(window).on("resize", "", $.proxy(this._layoutResized, this));
      this._dialogLayout.on(
        "click",
        "span.actions .button.bookmarkModule, span.actions .button.bookmarkedModule",
        $.proxy(this._toggleBookmark, this),
      );
      this._dialogLayout.on(
        "click",
        "span.btn-close",
        $.proxy(this._closeButtonHandler, this),
      );
      var $searchButton = $(
        ".dnnModuleDialog .search-container .search-button",
      );
    ...
    _doAddModule: function (e) {
      var $item = $(e.target);
      $item = $item.hasClass("dnnModuleItem")
        ? $item
        : $item.parents(".dnnModuleItem");
      var handler = this;
    ...
    _saveBookmark: function (item, removeBookmark) {
      ...
      this._getService().request(
        ...
        "POST",
        params,
        $.proxy(this._bookmarkUpdated, this),
      );
      if (removeBookmark) {
        $(item.parent())
          .parent()
          .find("span.bookmarkholder a.button.bookmarkedModule")
          .removeClass("bookmarkedModule")
          .addClass("bookmarkModule")
          .unbind("click");
        $(item.parent())
          .parent()
          .find("span.actions a.button.bookmarkedModule")
          .removeClass("bookmarkedModule")
          .addClass("bookmarkModule")
          .bind("click");
      } else {
        $(item.parent())
          .parent()
          .find("span.bookmarkholder a.button.bookmarkModule")
          .removeClass("bookmarkModule")
          .addClass("bookmarkedModule")
          .bind("click");
        $(item.parent())
          .parent()
          .find("span.actions a.button.bookmarkModule")
          .removeClass("bookmarkModule")
          .addClass("bookmarkedModule")
          .unbind("click");
      }
    ...
    _processModuleForDrag: function (module) {
      ...
      left =
        $("#personaBar-iframe").outerWidth() / 2 +
      ...
      var initFloatingContent = function () {
        ...
        var moduleItem = handler._callFromExistingModule
          ? $(
              '.dnnExistingModuleDialog li[data-moduleid="' +
                handler._addingDesktopModuleId +
                '"]',
            )
          : handler
              .getElement()
              .find(
                'li[data-moduleid="' + handler._addingDesktopModuleId + '"]',
              );
        ...
        $(
          '<a name="' +
            moduleId +
            '" href="#" class="cancel-module" data-moduleid="' +
            moduleId +
            '" />',
        )
          .appendTo($dragContent)
          .click($.proxy(handler._cancelAddModule, handler));
      };
      initFloatingContent();
    ...
    _syncCompleteFunc: function () {
      ...
      if (!this._noFloat) {
        ...
        setTimeout(function () {
          //execute after inner script executed.
          if (
            typeof window.dnnLoadScriptsInAjaxMode === "undefined" ||
            window.dnnLoadScriptsInAjaxMode.length == 0
          ) {
    ...
    _motionToNewModule: function (moduleId) {
      var $listItem = this._callFromExistingModule
        ? $(
            '.dnnExistingModuleDialog li[data-moduleid="' +
              this._addingDesktopModuleId +
              '"]',
          )
        : this.getElement().find(
            'li[data-moduleid="' + this._addingDesktopModuleId + '"]',
          );
      ...
      var $newModule = $("div.DnnModule-" + moduleId);
      var $motion = $('<div class="module-motion" />')
        .hide()
        .css({
