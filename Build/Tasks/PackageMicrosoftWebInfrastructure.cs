...
public sealed class PackageMicrosoftWebInfrastructure : FrostingTask<Context>
{
    ...
    public override void Run(Context context)
    {
        ...
        var mainAssemblyPath = binDir.CombineWithFilePath("Microsoft.Web.Infrastructure.dll");
        var packageVersion = FileVersionInfo.GetVersionInfo(context.MakeAbsolute(mainAssemblyPath).FullPath).FileVersion;

        var packageZip = context.WebsiteDir.Path.CombineWithFilePath($"Install/Library/Microsoft.Web.Infrastructure_{packageVersion}_Install.zip");
        var packageDir = context.Directory("DNN Platform/Components/Microsoft.Web.Infrastructure");
